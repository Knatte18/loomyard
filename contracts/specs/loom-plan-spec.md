...
23. `containment-unit-overlap` — the syntactic half of the cross-granularity containment check, in two lexical pairings over different cards' own target lists: a member glyph physically overlapping another card's self glyph naming the same unit, and a file self glyph physically overlapping another card's extensionless self glyph naming the directory that file sits directly in (`internal/foo/bar.go#` against `internal/foo#` — both being self glyphs, neither this check's member pairing nor the resolve-backed member→file tier ever compared them). The member→file half is `internal/planglyph`'s resolve-backed business, not this package's. Skipped entirely under `language: none`. See [Plan: handles](#plan-handles) below for the write paths this batch adds alongside it.
