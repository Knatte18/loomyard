...
4. Otherwise, an entry with no `.` and no `/` at all is a **path** too: an extensionless repository-root filename such as `Makefile`, `LICENSE`, or `Dockerfile`. Without this rule such a filename would have no legal spelling left under the `bare-symbol-target` hard rule below, since the `//` worktree-root escape does not rescue it.
   Canonicalization carries the rule the rest of the way: a slash-free path-shaped entry is canonicalized to its self glyph even without a file extension (see [Card path resolution](#card-path-resolution-root-and-)), so the spelling this rule admits is one every glyph-backed layer downstream can act on rather than a bare token `quarry` rejects before resolution.
...
14. `handle-collision` — the same `plan:` handle declared by more than one `Create` sub-bullet across the plan; one finding per colliding handle, not per declaring card.
...
17. `rename-to-not-handle` — a symbol `Rename` pair's `New` side classifies as a glyph or a bare symbol rather than a `plan:` handle. A file-rename pair (both sides self glyphs) is exempt.
18. `rename-from-not-glyph` — a symbol `Rename` pair's `Old` side classifies as a bare symbol or a `plan:` handle rather than a glyph. A file-rename pair is exempt.
