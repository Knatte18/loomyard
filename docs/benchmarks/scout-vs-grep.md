...
See also the [scout-plan-symbol-fields.md](../../manifest/designs/scout-plan-symbol-fields.md) design doc, which cites external research (ManoMano's Project Aegis benchmark, a Semble Show HN thread) reaching the same "task-dependent, not uniform" conclusion this run reaches independently.
...
- **No dramatic, universal win,
  ...
  This matches the external research already cited in [scout-plan-symbol-fields.md](../../manifest/designs/scout-plan-symbol-fields.md): LSP-backed navigation helps,
...
- Small sample, single run, three hand-picked tasks.
  A larger, repeated benchmark (matching this doc's own [board-performance.md](board-performance.md) convention of dated, repeatable measurement blocks) would be needed before treating any of these deltas as durable.
