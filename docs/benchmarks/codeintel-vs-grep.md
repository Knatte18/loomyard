...
Grep, meanwhile, got a *free, structural* advantage here that has nothing to do with text-matching precision: a `grep` invocation naturally scopes to the directory/files you point it at, so `grep -rn ... internal/builderengine/` never saw the other two packages' identical-looking interfaces in the first place. The codeintel agent had no equivalent scoping lever at the time this round ran — `lyx codeintel refs` had no `--within <dir>` flag, so it got the whole workspace's structurally-matching hits by default and had to filter after the fact.
