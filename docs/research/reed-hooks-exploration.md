...
Empirical evidence for how the **reed** module (`internal/reedengine` — see the package
...
**`claude agents --json`** registry — rather than (or alongside) the `lyx reed spawn`
replace-dispatch model. Companion to [`reed-exploration.md`](reed-exploration.md) (which proved
...
The question: can CC hooks + the agents JSON tell reed **which pane to focus when**, so pane
...
1. **Every hook payload carries `session_id` = the firing session's own id** (+ `transcript_path`,
   `cwd`). **This is the load-bearing fact**, because reed's model is *one separate `claude` process
   per pane* (see [the model](#how-pane-switching-actually-composes-the-design-takeaways)): each
   spawned process **self-identifies** on its own `SessionStart`/`Stop` → reed maps the event straight
   to the pane it launched and assigned the `--session-id` to.
2. *(For the mechanism reed replaces, not uses.)* The in-process Agent-tool subagents also carry a
   ...
   `outputFile` / `agent_transcript_path`, **and the `id` field of `claude agents --json`**. reed does
...
needs-input detection. That opens a strategic fork — reed **owning** agents vs. merely **displaying**
...
- **No control verbs.** `claude agents` only *dispatches* and *lists* — there is **no
  focus/kill/send** subcommand. reed drives tmux directly for all pane control.
...
**Join keys it exposes:** `sessionId` (↔ interactive sessions / reed's pane→session map) and `id`
...
- **Commands run under git-bash (`/usr/bin/bash`) on Windows — NOT cmd/pwsh.** First probe failed
  ...
  for the real design — a hook command of `lyx reed …` resolves fine under bash — but any path with
  backslashes in a hook command is silently destroyed. (Mirrors the send-keys slash-arg hazard in
  [`reed-exploration.md`](reed-exploration.md), different surface.)
...
the surface reed's daemon/Slack milestones (7–8) were going to build by hand.
...
The model first, because it dictates which hooks matter: **reed never uses Claude Code's in-process
Agent tool.** Every agent — the orchestrator and every descendant — is a **separate OS `claude`
process** that `lyx reed spawn` launches in its own tmux pane, with its **task injected as the launch
`[prompt]` arg** and a **reed-assigned `--session-id`**. A parent spawns a child by running
`lyx reed spawn` through its **Bash tool**, not the Agent tool. So the operative signals are each
process's **own session-scoped hooks**, keyed by the session id reed already assigned — *not* the
...
at launch, with a reed-assigned `--session-id`; its `SessionStart`/`UserPromptSubmit`/`Stop` all
...
its own `lyx reed spawn` process, so **no Agent tool appears anywhere in the tree.**

**`Stop` is the operative idle/needs-input edge — not `Notification`.** `Stop` fires the instant a
turn ends and carries `last_assistant_message` (so reed sees *whether* the pane is asking a question)
and `background_tasks[]` (so reed sees if work is still in flight). It is immediate. `Notification`
(`idle_prompt`) is a *delayed* (~60 s) redundant nudge, and `Notification` (`permission_prompt`)
cannot occur for autonomous reed children (they run with bypass). So reed keys focus on `Stop`, and
...
the tool with a reason steering the model to run `lyx reed spawn` instead. (`PreToolUse` can deny +
...
`agent_id`, the async `outputFile` — therefore documents the mechanism reed **replaces**, not one it
...
(`pid` + `sessionId`), so join on **`sessionId`** to reed's pane→session map — detect orphaned panes,
...
The supervisor (§Channel 3) overlaps so heavily with reed's daemon/monitoring goals that it reframes
what reed must build. Instead of reed owning raw process spawn + lifecycle + recovery, reed could:
...
2. **Tile** by running `claude attach <id>` in each tmux pane — proven to render — for the
   simultaneous, visible layout that is reed's actual differentiator.
...
**Why this matters:** it could shrink reed from "build a multi-agent supervisor + daemon + recovery +
...
reliability for the permission-prompt case. The own-vs-display fork — "reed owns the agents" (§A/B) vs.
"reed displays Claude Code's agents" (§D) — is now a **deliberate split**, not a single choice.
...
- **`transcript_path` is handed to every hook** → reed can tail the real JSONL for its resume
  journal without `capture-pane` scraping (relates to the `internal/reedengine` package
...
- **Async-subagent result delivery** is a synthetic `<task-notification>` `UserPromptSubmit` — a
  clean, parseable seam if reed ever needs to mirror sub-results.
...
- [ ] **`Notification` payload** (`notification_type` `permission_prompt` / `idle_prompt`) — never
  ...
  commands in `default` mode (even `curl`), and autonomous reed children run with bypass anyway, so it
  is **not a real signal for reed**. `idle_prompt` did not fire in the observed window (it appears to
...
- [ ] **Hooks reloaded mid-session?** Whether editing the orchestrator's settings after launch takes
  effect without restart (matters if reed installs orchestrator-side hooks late). Not tested.
...
- [ ] **Guardrail spike** (§B): does a `PreToolUse` matcher on `Agent` reliably **deny** the
  in-process Agent tool and inject a reason that steers the model to `lyx reed spawn` instead? The
...
reed's model is **one separate `claude` process per pane, spawned by `lyx reed spawn` (via Bash) with
...
`session_id`** — the exact key reed assigned it. The probe proved this directly (the probe session
*was* such a spawned, injected process). reed should: (1) ship each spawned `claude` callback hooks
in `--settings` keyed by its own session id (§A), keying focus on `Stop`; (2) keep `lyx reed spawn`
...
the in-process mechanism reed **replaces** — relevant only to the guardrail. Platform gotcha:
...
`claude agents --json`) that already does most of what reed milestones 6–8 planned. The own-vs-display
...
supervisor primitives (`state`/`logs`/needs-input, crash-survival) for **headless/fire-and-forget**
agents and monitoring. Settle the remaining §D risks (see Open) before reed v1 — but the interactive
