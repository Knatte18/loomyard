...
This landed on exactly the tool this session built earlier the same day specifically to remove human/agent judgment calls from a mill batch's `Deletes:`/`Moves:` safety check (see [manifest/designs/scout-plan-symbol-fields.md](../../manifest/designs/scout-plan-symbol-fields.md)). A false positive in a tool meant to replace review discipline with a deterministic gate is a worse failure mode than a false positive an agent might catch by re-reading the code — the entire point of building it was to make that re-reading unnecessary.
...
Everything in this document is about **agent-facing use**: a Claude Code agent, mid-task, choosing whether and how to call `lyx scout`. This session had already concluded (before this benchmark) that scout's more durable value is **Go-native, deterministic use** — code calling `scoutengine` directly, or a mill batch's `verify:` field calling `assert-no-callers` — where no agent is ever choosing whether to trust the result, so the "does an agent reach for the right tool and interpret its output correctly" question this benchmark tests doesn't arise the same way. That direction is not undermined by this document. If anything, the false-positive finding above is a reason to invest *more* carefully in the deterministic path (get the tool correct once, so every deterministic caller benefits) rather than in agent-facing promotion (where a skilled agent gets similar results from grep anyway, per the tables above).
