...
- **trace** — multi-language code-intelligence lookups over LSP (`internal/tracecli` + `internal/traceengine`; `lyx trace refs|definition|symbol <symbol|file:line:col>`). Generalizes the Go-only in-process `go/packages`/`go/types` approach the [trace spike](research/trace-spike.md) (#008) recommended into a uniform LSP path across five languages (Go, Python, C#, TypeScript, Rust): marker-based language detection with a pinned precedence order, a `builtins()` fallback registry with an optional operator-editable `servers.yaml` overlay (mirroring `internal/modelspec`'s registry shape), and a generalized stdio LSP client (`textDocument/references` + `textDocument/definition` + `workspace/symbol` name resolution, deadline-bounded with a hard-kill on timeout). `internal/traceengine` is a cycle-free leaf (typed results/errors, no `internal/output`); `internal/tracecli` is the sole layer that maps it to the JSON envelope, including a batch-argument mode for all three verbs. V1 ships three verbs (each with single- and batch-argument modes), an `EnsureServer` daemon lifecycle wired for Go dispatching to the `supervised` strategy (a lyx-owned, session-long daemon with a state file, spawn-race lock, and wedged-daemon recovery) with the `native` strategy (`gopls -remote=auto`) retained as its fallback, and a Go toolchain manager that pins and installs `gopls` into a machine-global cache independent of `$PATH`. ✅ Implemented (v1 scope: no call hierarchy, no `implementation` method, no in-process `go/packages` arm). Design doc deleted on landing per the documentation lifecycle; durable rationale lives in the `internal/traceengine` package documentation.
...
- `internal/traceengine` package documentation — multi-language code-intelligence lookups over LSP (`lyx trace refs|definition|symbol`) (as-built; module doc deleted per the documentation lifecycle).
