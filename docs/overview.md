...
- **batten** — drives one task worktree's whole lifecycle — create, seed the child's own inner run from the Board task's own `type`, run it to a terminal state, and tear down — as a single Shed run from the hub's prime worktree (`internal/battenshed` + `internal/battenrecipe` + `internal/battencli`; `lyx batten run|step|status|pause <run-id>`).
  ...
  A child that halts (`blocked`, `failed`, `paused`) fails `Run-Shed` rather than routing anywhere, and batten cannot restart the child's driver: `Run-Shed` spawns only while the task worktree has no status file, and resuming the outer run resumes the watch alone — the operator resumes the child from inside the task worktree (`lyx loom start`) and then resumes batten.
