...
- **the bootstrap** — `lyx loom start` (alias `lyx start`) brings up the worktree's tmux session, adds the `lyx loom status` strand (a 1-line top pane), spawns the per-hub watchdog daemon (best-effort), spawns the loom driver **detached** (via `proc`, no TTY), adds the operator's own strand, and attaches the terminal to the session. loom runs in the background;
