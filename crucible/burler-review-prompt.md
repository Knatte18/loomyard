...
- Cluster fan-out (`ClusterFan`/fork subagents) — orthogonal to the prompt-split fix. You MAY read `cluster.go`/`smoke_cluster_test.go` for context. You must NOT treat "out of scope for findings" as license to run them anyway "for extra confidence" or "if there's time" — see the hard EXECUTION BAN below, which is unconditional regardless of how much time you have left.
...
## EXECUTION BAN — never run these tests this round (BLOCKING, no exceptions)
Do not, under any circumstance, execute `TestSmokeBurlerClusterCleanFan` or `TestSmokeBurlerClusterRogueFork` (in `internal/burlerengine/smoke_cluster_test.go`) this round — not to "double-check," not "if time allows," not as a side effect of a broad `-run` pattern. Each one spawns multiple REAL, simultaneous `claude` subprocess sessions (a cluster round's handler + N real forked reviewers) — this is expensive in wall-clock, tokens, AND RAM, and is completely orthogonal to what this round is checking (the SOLO round path, `TestSmokeBurlerRoundToyFixture`, is your only live-smoke target). A prior instance of this exact round caused the host machine to exhaust ~29GB of RAM and had to be killed by the operator — treat this as a hard safety rule, not a scope suggestion. Concretely:
- **Never** run `go test -tags smoke ./internal/burlerengine/...` with a bare `-run Smoke` or any pattern broader than one exact function name — `-run TestSmokeBurlerRoundToyFixture` (the ONE test this round needs) is the only acceptable `-run` value for this package. A broad `-run Smoke` matches ALL THREE smoke tests in the package (the solo one plus both cluster ones) and would run all of them, including the banned two.
- **Never** run more than one live-substrate (`-tags smoke`) test invocation at a time, in parallel, or in a background/detached shell. One live-substrate process running at a time, in the foreground, waited on to completion, full stop.
- Before running ANY `-tags smoke` command, name the exact test function in your response and confirm it is `TestSmokeBurlerRoundToyFixture` or your own scratch CLI test (S1/S2 from the sandbox suite) — never a bare package path with no `-run` filter.
- If you ever see more than one real `claude` or `tmux` process related to a test you started, or a test invocation you did not expect to spawn multiple sessions, STOP immediately, tear everything down (`pkill -f <your dev binary>`, `tmux ls` to confirm empty), and report it as a finding rather than continuing.
...
Live smoke — REAL claude, REAL tmux (this is the decisive evidence for this round; see the EXECUTION BAN above before running anything here):
- **Prefix every `-tags smoke` invocation with `LYX_LOG_LEVEL=info LYX_LOG_FILE=.scratch/burler-review-opus-high-r1-spawns.log`** (e.g. `LYX_LOG_LEVEL=info LYX_LOG_FILE=.scratch/burler-review-opus-high-r1-spawns.log go test -tags smoke ...`). The operator is watching this file live during your round — every tmux server spawn, boot retry, and shuttle run now logs there (see `CONSTRAINTS.md`'s Live-Substrate Spawn Observability invariant). This costs you nothing and is not optional.
