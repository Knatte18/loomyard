# reed: born-as-strand for the operator's `loom run` attach
...
Running `loom run` outside reed remains a valid escape hatch for debugging/CI, mirroring `loom drive`'s relationship to `loom run` — the "escape hatch" pattern is already established.
