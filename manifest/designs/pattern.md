...
- Activation also requires `_pattern` to be listed in `fabric`'s own weft pathspec, or PATTERN content written under `_pattern/` never leaves the machine (weft never commits it). A worktree initialised before this pathspec widened keeps its narrower pathspec and must be widened by hand — the wiring does not retroactively repair an initialised worktree's weft commit scope.
