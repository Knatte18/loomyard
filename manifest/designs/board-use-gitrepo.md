...
> [gitrepo.md](gitrepo.md): rewire the currently-shipped `board`'s own git plumbing onto
> `gitrepo.Repo` instead of board's hand-rolled `gitexec.RunGit` calls. Per the
...
git-command plumbing underneath it, per [gitrepo.md](gitrepo.md#scope-boundaries--deliberately-not-a-general-purpose-git-wrapper).
...
- [gitrepo.md](gitrepo.md) — the primitive this item wires board onto.
