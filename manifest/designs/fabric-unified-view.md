...
- **Fixed `.lyx`'s own geometry — but not as one more `pathspec` entry.**
  ...
  A pre-existing real `.lyx` — every worktree in existence before this change, since the logger, reed, shuttle, and burler all write it unconditionally — is adopted into the weft target on first `reconcile` after upgrade, rather than hard-erroring; `_lyx` and `_pattern` keep their hard refusal, since `.lyx` alone is guaranteed to be lyx's own disposable scratch.
