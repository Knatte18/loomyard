...
**Keep the name `_board` — renaming was considered (2026-08-05) and dropped as unnecessary churn.** `fabricengine.BoardDir` (`<Hub>/_board`, a real `git worktree add` of weft on branch `main` — not a junction) hosts more than board's own data: `.fabric-anchor` lives there,
