...
**Rename `_board` → `_lyxharness`.** `hubgeometry.BoardDir` (`<Hub>/_board`, a real `git worktree add` of weft on branch `main` — not a junction) has grown into the general repo-wide, weft:main-backed system-metadata home: `.fabric-anchor` lives there, the repo-wide `fabric.yaml` lives there (`<BoardDir>/_lyx/config/fabric.yaml`), and even `internal/ideengine/menu.go` uses `hubgeometry.BoardDir` as its own config-path base — none of this is "board" (`internal/boardengine`)'s data. Rename to `_lyxharness` (paired naming with the existing per-worktree `_lyx`: `_lyx` is per-worktree durable system state, `_lyxharness` is the repo-wide rigging that sets everything up). `_system`/`_registry` were considered and rejected as too OS-generic. `internal/boardengine`'s own feature data moves to a subdirectory within the renamed root, on the same footing as any other module. Touches ~15 call sites (`hubgeometry.BoardDir`/`BoardDirName`, `fabricengine/{clone,junctionnames,reconcile,unwire,boardweft,add}.go`, `fabriccli/{fabric,weft_verbs}.go`, `ideengine/menu.go`).
