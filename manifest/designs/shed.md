...
The seed itself is written by a spawn-time command, not by `Shed`, and `pause_requested` living in-status rather than in a separate flag file is a deliberate divergence `contracts/specs/loom-status-spec.md` already pins.
...
**`product` carries no compatibility claim for loom's shipped schema.** `contracts/specs/loom-status-spec.md` mandates `phase`, `stage`, and `narration` as top-level fields and pins a different `history` shape, none of which a `product` sub-object satisfies; reconciling the two is loom's own later rewiring task.
