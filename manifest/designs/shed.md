...
3. Check `pause_requested` **and** `ctx.Err()`. Either set → write `state: "paused"` (`Result.Outcome = RunPaused`, nil `error`), exit cleanly; nothing more happens until the next `lyx start`. Treated identically on purpose: an operator's Ctrl-C or a parent deadline is an operational stop, not a failure, exactly as resumable as an explicit pause request — one clean-stop path, not two. Checked here, at the top of every loop iteration (not only once, and not left to producers to notice on their own) — a `Shed` that only checked `ctx` inside producers would keep launching *new* producer calls after cancellation, for however long the current one takes to notice.
   **The resume write sits at the end of this step, after the pause check passes.** When the file just read carries any state other than `"running"` — a run resumed from `paused`, `blocked`, or `failed` — `Shed` writes `state: "running"` with an empty `error` before it calls anything.
   ...
   That is precisely the three-way distinction `state` is persisted to carry — "paused, resumable" versus "blocked, needs a human" versus "crashed" — failing in the one window an operator watches hardest, right after typing `lyx loom start` to resume.
...
5. Append `{producer, outcome, output, at}` to `history` — on every branch of step 6 below **except** the two where the producer reached no verdict at all: the cancellation branch, and a producer that returned an error alongside an empty outcome — then persist.
   Both exceptions are the same rule, not two: `history[].outcome` is a closed vocabulary, and the empty string is not in it, so recording one wrote a value `loomengine`'s own coherence check then rejected, making the status file permanently un-resumable over an ordinary hard failure.
   A producer that returns a NON-empty outcome outside the vocabulary is still recorded verbatim, deliberately — that literal is the diagnosis.
...
- **Acquires `LockPath` non-blocking**, mirroring `internal/treadleengine/run.go`'s own `lock.TryAcquireWriteLock` / `ErrBlockBusy` precedent exactly (`run.go:119–128`): already held → a sentinel error (e.g. `ErrShedBusy`), `Run` returns immediately. `internal/state`'s own per-write lock does not substitute for this — it is held only for the duration of one write, not across a whole `Call()`, so two concurrent `lyx loom start` invocations could otherwise both read the same `current_producer` and both spawn it, double-spending an LLM session. Released via `defer`, OS-reclaimed on crash even if `Release` is never reached, so a killed process never bricks a later resume.
...
- Session/tmux/reed bootstrap — the product's CLI entry point (`lyx loom start`) does this *before* invoking `Shed`'s loop.
