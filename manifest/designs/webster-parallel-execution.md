...
- **Honest speedup estimate: ~2–3× wall-clock**, discounted from a naively-computed 3–5× because a wave's wall-clock is its *slowest* card, and the heaviest implementation cards tend to sit on the critical path. Two caveats push it down further: semantic edges were *inferred* from card descriptions (real edit-time dependencies only add edges and shrink waves), and 42 cards is an atypically large plan — a routine 5–10-card task has little fan-out headroom and would show a speedup near 1×, dominated by warm-context, not parallelism.
