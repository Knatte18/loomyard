...
Neither `Plan-Write` nor `Plan-Burler` carries a `commit_seam` of its own, so both gates' rewrites are captured by the same place: the `Plan-Review` segment's own `commit_seam`, fired on `Plan-Bouncer`'s approved settle, which commits the plan directory once it settles.
