...
1. **Is there a complete output file?** → *inside* an attached or started run's own wait loop — where this check lives — the step finished;
   ...
   **Inside the wait loop this check outranks every negative answer, the startup window included.** A dead pane, a strand `reed` no longer tracks, a strand whose pane binding it cleared, and an expired startup window are all answers to "has something gone wrong", never to "did this run finish" — so each consults the file contract first, and each classifies `done` when it is satisfied.
   The startup window was the exception until crucible round `opus5-high-r4` reproduced it live: a provider that wrote every declared output file and then left no parseable terminal event behind was recorded as `died` at `startup_timeout_s`, after which the next resume archived the finished files and re-ran the step — the exact rework this rule exists to prevent.
