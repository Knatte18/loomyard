...
- **Reconciliation with `contracts/specs/loom-plan-spec.md`'s existing validator checks — which of those still apply, which are now redundant, which need rewriting.**
  Closed: the disposition table this task implemented lands on one row per distinct check ID in `contracts/specs/loom-plan-spec.md`'s own Validation checks section — that table is the authoritative list, and no count of it is restated here.
