...
Mostly wiring on top of the already-built `warp` mechanics (absorbed into `fabric` once that lands — see [fabric.md](fabric.md)); worktree/branch/junction/portal teardown is explicitly **out of scope** — that's `warp cleanup`'s (future: `fabric`'s) existing, separate job, which cannot run from inside the worktree being removed, the same reason `mill-cleanup` runs from the hub, never a task worktree.
