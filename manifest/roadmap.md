...
1. **loom: redesign the Discussion format** — a companion design doc to `plan-card-format.md`. Loom's existing `_lyx/discussion/` two-file split (`decision-record.md`, the lean Plan-facing file `Plan-Write` reads exclusively; `support-log.md`, the audit trail carrying rejected alternatives and the question ledger, which `Plan-Write` never reads — see `designs/loom.md`) already keeps the Plan-facing file lean by construction, so this task is narrower than a from-scratch format redesign. Two things it must still fix, from a 2026-08-23 comparison against millhouse's `mill-start` skill: (1) `Discussion-Write`'s own exploration behavior must not spend tokens/time gathering deep architecture/interface/dependency detail regardless of which of the two files it would land in — that class of fact is now Quarry's (or manual grep's) job, computed fresh at Plan time, not pre-gathered during Discussion; millhouse's own `mill-start` bakes exactly this as a mandatory interview category, which this task must not carry over. (2) `Discussion-Review`'s rubric (a later, dependent task below) must accept "this belongs in `support-log.md`, not `decision-record.md`" or "this doesn't belong in Discussion at all" as legitimate findings, not only gap-filling — millhouse's own review/fix loop is purely additive (a finding is only ever resolved by adding content, never by removing misplaced content), which is the concrete mechanism behind `discussion.md` files that only grow across review rounds; loom's simpler Bouncer/Burler loop has no such structural bias built in, but the rubric must say so explicitly or the same growth pattern can still recur. The test for what stays in `decision-record.md` is completeness for Planner's actual needs, not brevity for its own sake — `Plan-Write` never reads `support-log.md` at all, so moving something out is a genuine loss, not a lower-visibility relocation; a "move this to support-log" finding must first check whether the content is pure deliberation narrative (safe to move) or contains a requirement/constraint Planner needs (must be extracted into `decision-record.md`'s own Decisions/Constraints before the narrative around it moves out).
   See [designs/plan-card-format.md](designs/plan-card-format.md) for the sibling design this extends, and [designs/loom.md](designs/loom.md) for the existing two-file split.

1. **loom: code-writing skills — comments, build, testing** — deploys `code-comment-conventions.md`'s rule as an actual installable skill (Go only for now), plus the two other legs millhouse's own code-writing skill set has: build and testing. A 2026-08-24 discussion revised this item's own shape from what's described below: `code-comments`/`code-quality` merged into one `code-quality` skill (comment content is one craft with the rest of the code, not a separate skill); two new skills not in the original plan, `prose` and `conversation`, split out of millhouse's `conversation` skill so its writing-style discipline applies to every piece of text an agent writes, not only chat replies; and everything ships as one plugin, `scribe` (not a millhouse-mirroring general-plugin-plus-per-language-plugin split), registered in loomyard's own marketplace (`.claude-plugin/marketplace.json`, the same mechanism `prowler` already uses) and synced via `update-plugins.sh`. `golang-build`/`golang-testing` are close to a verbatim port of millhouse's equivalents; `golang-comments` is substantially reworked, not new-from-scratch. A `golang-quality` skill (Go-idiom content: accept-interfaces/return-structs, error-as-value mechanics, package design) was considered and explicitly not built — no concrete observed problem motivates it yet, unlike every other skill here.
   All seven skills are drafted and scaffolded at `plugins/scribe/skills/`;
   discussion review is in progress.
...
1. **planparser: Card-format migration to `Edits`/`Uses`** — replace `planparser.Card`'s file-op fields (`ContextFiles`/`EditsFiles`/`CreatesFiles`/`DeletesFiles`/`Moves`) with the new `Edits`/`Uses` shape from `designs/plan-card-format.md`, and update every direct consumer in `internal/websterengine` (rendering, report parsing, deviation-checking) to read the new fields. No behavior change — webster still executes strictly in declared plan order after this lands, same as today, just against the new field shape. Bounded, format-only migration; the two packages' changes land together since websterengine's field reads would not compile against the old shape otherwise.
   See [designs/plan-card-format.md](designs/plan-card-format.md).

1. **webster: DAG-derived card sequencing** — activate `internal/websterengine`'s already-documented "dead DAG seam" (`doc.go`'s `if card.HasSymbolFields()` sketch): derive the dependency graph from `Edits`/`Uses` across the plan's cards and execute in topological order instead of blind declared order. Still one worktree, still sequential — no spawning, no concurrency. Catches a plan whose declared card order doesn't actually match its real dependencies. Depends on the item above; real, standalone value even if worktree-per-card execution (Someday, below) never lands.
   See [designs/plan-card-format.md](designs/plan-card-format.md).
...
1. **loom: Discussion-Review producer** — write `Discussion-Review`'s missing "what to check" rubric half (the "what not to flag" half already exists) as the rubric for a new `Discussion-Bouncer` instance, instantiating the shipped `Bouncer` producer with it. The rubric must also cover the Bouncer's seed-call focus-setting pass (see the shipped `Bouncer` item's own rubric-coverage note), not only post-round judgment.
   The rubric judges against the redesigned Discussion approach from the group above, not the old millhouse-derived expectations: does `decision-record.md` stay lean and Plan-facing, does anything belonging to `support-log.md` (rejected alternatives, question ledger) actually live there instead, and does either file correctly *exclude* mechanically-verifiable technical claims (exact signatures, file:line citations, existing-pattern research) — that content is Quarry's/manual grep's job now, computed fresh at Plan time. Per the group's item above, the rubric must accept "this belongs in `support-log.md`, not `decision-record.md`" or "this doesn't belong in Discussion at all" as legitimate findings, not only gap-filling — a purely-additive review loop is the concrete mechanism behind discussion files that only grow across rounds. Every such finding must also check completeness before relocation: `Plan-Write` never reads `support-log.md`, so a rubric that moves out a real requirement along with its surrounding narrative causes Planner to silently lose it, not just have to look harder for it.
