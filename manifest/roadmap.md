...
1. **loom: code-writing skills — comments, build, testing** — deploys `code-comment-conventions.md`'s rule as an actual installable skill (Go only for now), plus two other legs: build and testing conventions. A 2026-08-24 discussion settled this item's shape: `code-quality` is one skill covering both code shape and comment content (comment content is one craft with the rest of the code, not a separate skill); two further skills, `prose` and `conversation`, give the plugin its own writing-style discipline that applies to every piece of text an agent writes, not only chat replies; and all seven skills ship as one plugin, `scribe`, registered in loomyard's own marketplace (`.claude-plugin/marketplace.json`, the same mechanism `prowler` already uses) and synced via `update-plugins.sh`. A `golang-quality` skill (Go-idiom content: accept-interfaces/return-structs, error-as-value mechanics, package design) was considered and explicitly not built — no concrete observed problem motivates it yet, unlike every other skill here.
