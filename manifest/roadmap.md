...
1. **Retire perch: delete `internal/perchengine`, `internal/perchcli`, and the `lyx perch run|pause` CLI verb** — `loom` never called `internal/perchengine` (it only ever used stubs), so this was never blocked by an active code dependency; it was originally sequenced after the three `*-Review` producer tasks below as a safety margin (prove the replacement before deleting the working original), but that margin is deliberately dropped now — see the 2026-08-21 discussion. Delete perch's outer-loop module and its standalone CLI (including the `perch` line in `cmd/lyx/main.go`'s "Available modules" help text) outright, not orphan them. Pure cleanup, no rename attached: the replacement is the hand-wired `Bouncer`+`Burler` pair each review-producer task builds directly (see the `CLAUDE.md` terminology note on "perch," the folk name for that pair) — there is no module or type taking over the old module's name.
   `internal/treadleengine` is a separate question, NOT deleted by this task: it was built specifically so a second future consumer could reuse its round-loop machinery without duplicating it (see its own package doc), and the Someday `Tenter + Hardener` item now expects `Tenter`'s review-loop to land as the same flat `Shed`-segment pattern instead of a second `Treadle` consumer — so whether `treadleengine` itself gets deleted (expected, once that's confirmed) or lingers orphaned a while longer is a "final call" made once `Tenter` lands too, not tied to perch's own retirement.
