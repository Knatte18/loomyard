...
1. **fabric: unified-repo view — the single entry-portal that makes warp+weft look like one repo** — `fabric` is the sole, deliberately-simplified git portal for everything LoomYard does, giving callers the illusion of one flat repo over the two underlying warp+weft histories. **Slices 1-6 (clone-does-everything, `Fabric.Commit`/`Diff`/`Status`, commit lock + push coalescing, snapshot-as-trailer, warp-rebase/remote-reconcile detection) are shipped** — see `fabric` in Done below. **Slices 7-10, identified 2026-08-05 off GitHub issue #127, are next**: shrink `internal/hubgeometry` to the minimal cwd/root/anchor primitive and move its ~20 per-module path constructors + all `Weft*`/junction plumbing out (into owning modules and `fabricengine` respectively), rename `_board`→`_lyxharness`, close a weft-visibility leak (CLI output + 7 call sites reading weft paths directly), finish `.lyx`'s own junction hygiene (the former `dotlyx-scratch-hygiene` item), and store the warp-URL binding on `weft:main` (fold bootstrap into `fabric clone`, weft-first argument order). See [designs/fabric-unified-view.md](designs/fabric-unified-view.md).
