# Implementer Brief — Rename internal/ghissues → selfreport / code-rename
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\rename-ghissues-to-selfreport\_mill\plan\01-code-rename.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\rename-ghissues-to-selfreport\_mill\plan\01-code-rename.md`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"477bc462-901e-4b7d-aae4-7837e9342f2f"}
```
...
**`session_id` MUST be exactly `477bc462-901e-4b7d-aae4-7837e9342f2f` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"477bc462-901e-4b7d-aae4-7837e9342f2f"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
