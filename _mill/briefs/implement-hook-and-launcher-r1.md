# Implementer Brief — Introduce warp: the host↔weft-coordinated git module / hook-and-launcher
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\warp-module\_mill\plan\08-hook-and-launcher.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\warp-module\_mill\plan\08-hook-and-launcher.md`.
   - Commit the plan edit first (`plan: extend hook-and-launcher refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"79e6613c-ba08-42fa-a3db-9e70cb2169a8"}
```
...
**`session_id` MUST be exactly `79e6613c-ba08-42fa-a3db-9e70cb2169a8` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"79e6613c-ba08-42fa-a3db-9e70cb2169a8"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
