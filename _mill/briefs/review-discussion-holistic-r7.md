...
You are an independent discussion reviewer for **fabric: fold snapshot-tracking into the Warp-SHA trailer**. Round **7**. Reviewer model: **sonnetxhigh**.
...
Read the discussion at `C:\Code\loomyard\wts\fabric-snapshot-trailer\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
...
Write your full report to this file: C:\Code\loomyard\wts\fabric-snapshot-trailer\_mill\briefs\review-discussion-holistic-r7.out.md

Any format the prompt above asks for (including a `MILL_REVIEW_BEGIN` / `MILL_REVIEW_END` wrapped report) is the content of C:\Code\loomyard\wts\fabric-snapshot-trailer\_mill\briefs\review-discussion-holistic-r7.out.md -- write it there, not into chat.

Your final chat message must be exactly one line and nothing else: `WROTE C:\Code\loomyard\wts\fabric-snapshot-trailer\_mill\briefs\review-discussion-holistic-r7.out.md`
