# Implementer Brief — Sandbox suite: emit findings JSON on the shared analysis contract / emitter-and-fetch
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\sandbox-report-json\_mill\plan\01-emitter-and-fetch.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\sandbox-report-json\_mill\plan\01-emitter-and-fetch.md`.
   - Commit the plan edit first (`plan: extend emitter-and-fetch refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"6af578a8-45c7-4f3d-828b-afeeb8134b38"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"6af578a8-45c7-4f3d-828b-afeeb8134b38"}
```
...
**`session_id` MUST be exactly `6af578a8-45c7-4f3d-828b-afeeb8134b38` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6af578a8-45c7-4f3d-828b-afeeb8134b38"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
