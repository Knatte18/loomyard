...
You are an independent discussion reviewer for **ghissues module — file LoomYard bugs as GitHub issues**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\loomyard\wts\ghissues-module\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
