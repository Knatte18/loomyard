...
You are an independent discussion reviewer for **Board fixes from sandbox run — payload keys, help, rerender**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\loomyard\wts\board-sandbox-fixes\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
