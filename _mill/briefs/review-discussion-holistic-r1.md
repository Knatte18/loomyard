...
You are an independent discussion reviewer for **Move config templates home by removing the lyxtest->configreg edge**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\loomyard\wts\config-test-cleanup\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
