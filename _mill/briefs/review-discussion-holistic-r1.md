...
You are an independent discussion reviewer for **Fix lyx CLI defects + host-commit gap from the sandbox run**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\loomyard\wts\lyx-sandbox-fixes\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
