...
You are an independent discussion reviewer for **Extract internal/proc (cross-OS windowless + detached spawn)**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\loomyard\wts\extract-internal-proc\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
