...
You are an independent discussion reviewer for **Fix test-suite regression: slow Tier 1 + 2 red packages + stale benchmarks**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\loomyard\wts\test-suite-regression\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
