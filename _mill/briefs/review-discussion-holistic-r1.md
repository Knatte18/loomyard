...
You are an independent discussion reviewer for **Built-in CLI help: lyx self-documents modules & commands**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\loomyard\wts\builtin-cli-help\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
