...
You are an independent discussion reviewer for **CLI help & error ergonomics from sandbox run**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
