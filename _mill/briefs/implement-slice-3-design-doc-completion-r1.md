# Implementer Brief — fabric: warp-side commit lock + push coalescing / slice-3-design-doc-completion
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/fabric-commit-lock-coalescing/_mill/plan/05-slice-3-design-doc-completion.md`
- **Overview (for `## Shared Decisions` only):** `/home/knatte/Code/loomyard/wts/fabric-commit-lock-coalescing/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/knatte/Code/loomyard/wts/fabric-commit-lock-coalescing`
...
Before editing any file, load and follow this skill (non-optional): `code-quality`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/fabric-commit-lock-coalescing/_mill/plan/05-slice-3-design-doc-completion.md`.
   - Commit the plan edit first (`plan: extend slice-3-design-doc-completion refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"eb701b0f-4bc9-4b17-aec2-178dec2aa100"}
   ```
...
**`session_id` MUST be exactly `eb701b0f-4bc9-4b17-aec2-178dec2aa100` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"eb701b0f-4bc9-4b17-aec2-178dec2aa100","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"eb701b0f-4bc9-4b17-aec2-178dec2aa100"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C /home/knatte/Code/loomyard/wts/fabric-commit-lock-coalescing` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/knatte/Code/loomyard/wts/fabric-commit-lock-coalescing`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/loomyard/wts/fabric-commit-lock-coalescing show <parent-branch>:<path>` to read it without changing cwd.
