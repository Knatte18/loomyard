# Implementer Brief — weft producers: _lyx/config, lyx config, codeguide / paths-host-junctions
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\weft-producers\_mill\plan\02-paths-host-junctions.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\weft-producers\_mill\plan\02-paths-host-junctions.md`.
   - Commit the plan edit first (`plan: extend paths-host-junctions refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"af9468c8-de65-46b7-bcef-c81ff997b0ca"}
```
...
**`session_id` MUST be exactly `af9468c8-de65-46b7-bcef-c81ff997b0ca` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"af9468c8-de65-46b7-bcef-c81ff997b0ca"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
