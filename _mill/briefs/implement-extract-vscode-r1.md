# Implementer Brief — Extract internal/vscode; keep ide IDE-generic / extract-vscode
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\extract-internal-vscode\_mill\plan\01-extract-vscode.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\extract-internal-vscode\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\extract-internal-vscode`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\extract-internal-vscode\_mill\plan\01-extract-vscode.md`.
   - Commit the plan edit first (`plan: extend extract-vscode refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"88326b88-4d8d-4d87-a832-020f17b1ded0"}
```
...
**`session_id` MUST be exactly `88326b88-4d8d-4d87-a832-020f17b1ded0` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"88326b88-4d8d-4d87-a832-020f17b1ded0"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\extract-internal-vscode` for commits; do not `cd`.
