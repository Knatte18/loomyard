# Implementer Brief — Extend worktree module: portals and launchers / board-paths-healthcheck
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\mhgo-portals-launchers\_mill\plan\04-board-paths-healthcheck.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\mhgo-portals-launchers\_mill\plan\04-board-paths-healthcheck.md`.
   - Commit the plan edit first (`plan: extend board-paths-healthcheck refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"a674d7dd-0edc-442a-8a18-eecbdfae497f"}
```
...
**`session_id` MUST be exactly `a674d7dd-0edc-442a-8a18-eecbdfae497f` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"a674d7dd-0edc-442a-8a18-eecbdfae497f"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
