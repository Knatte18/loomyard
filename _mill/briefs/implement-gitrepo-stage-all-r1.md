# Implementer Brief — board: use gitrepo as its git operator / gitrepo-stage-all
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/board-use-gitrepo/_mill/plan/01-gitrepo-stage-all.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/board-use-gitrepo/_mill/plan/01-gitrepo-stage-all.md`.
   - Commit the plan edit first (`plan: extend gitrepo-stage-all refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"4898d553-19cc-42cd-8bae-b0906b2e9bca"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4898d553-19cc-42cd-8bae-b0906b2e9bca"}
```
...
**`session_id` MUST be exactly `4898d553-19cc-42cd-8bae-b0906b2e9bca` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4898d553-19cc-42cd-8bae-b0906b2e9bca"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
