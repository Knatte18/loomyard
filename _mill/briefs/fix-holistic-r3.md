# Holistic Fixer Brief — Introduce warp: the host↔weft-coordinated git module
...
- **Holistic review file:** `C:\Code\loomyard\wts\warp-module\_mill\reviews\20260625-170931-code-review-r3.md`
- **Plan overview:** `C:\Code\loomyard\wts\warp-module\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\warp-module`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\warp-module\_mill\reviews\20260625-170931-code-review-r3.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\warp-module` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e04361a9-db82-452d-874d-a8038053541a"}
```
...
**`session_id` MUST be exactly `e04361a9-db82-452d-874d-a8038053541a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e04361a9-db82-452d-874d-a8038053541a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\warp-module` for commits; do not `cd`.
