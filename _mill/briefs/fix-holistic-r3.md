# Holistic Fixer Brief — Replace reed's header pane with a status-line and Selvage
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/reviews/20260919-053905-code-review-r3.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/reed-header-selvage`
...
```
/home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/plan/01-render-bottom-band.md
/home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/plan/02-vocabulary-and-config.md
/home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/plan/03-selvage-pane.md
/home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/plan/04-status-line-pins.md
/home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/plan/05-watchdog-daemon.md
/home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/plan/06-standalone-watcher.md
/home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/plan/07-docs-smokes-and-residue.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/reviews/20260919-053905-code-review-r3.md`.
...
- doc.go's status-line pin bullet still describes the removed "status off" mechanism: **Location:** `internal/reedengine/doc.go:601-619`
- card 41's watchdog daemon integration suite is missing several mandated assertions: **Location:** `internal/reedcli/watchdog_integration_test.go`
...
Run each from `/home/knatte/Code/loomyard/wts/reed-header-selvage` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/reed-header-selvage rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/reed-header-selvage status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"61b45d2f-9737-4dc3-b51a-b5297cc1c06d"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `61b45d2f-9737-4dc3-b51a-b5297cc1c06d` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"61b45d2f-9737-4dc3-b51a-b5297cc1c06d"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/loomyard/wts/reed-header-selvage` for commits;
do not `cd`.
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/loomyard/wts/reed-header-selvage show <parent-branch>:<path>` to read it without changing cwd.
