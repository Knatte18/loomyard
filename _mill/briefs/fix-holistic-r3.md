# Holistic Fixer Brief — Extend worktree module: portals and launchers
...
- **Holistic review file:** `C:\Code\mhgo\wts\mhgo-portals-launchers\_mill\reviews\20260614-135540-code-review-r3.md`
- **Plan overview:** `C:\Code\mhgo\wts\mhgo-portals-launchers\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\mhgo\wts\mhgo-portals-launchers`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\mhgo\wts\mhgo-portals-launchers\_mill\reviews\20260614-135540-code-review-r3.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\mhgo\wts\mhgo-portals-launchers` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"5155e99d-9fd1-43bd-8514-8021254b9545"}
```
...
**`session_id` MUST be exactly `5155e99d-9fd1-43bd-8514-8021254b9545` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"5155e99d-9fd1-43bd-8514-8021254b9545"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
