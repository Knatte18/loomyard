# Holistic Fixer Brief — Extract scout into its own standalone repo
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/scout-extract-standalone-repo/_mill/reviews/20260820-171211-code-review-r3.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/scout-extract-standalone-repo/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/scout-extract-standalone-repo`
- **Wiki path:** `/home/knatte/Code/loomyard/wiki`
- Round: **3**
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/scout-extract-standalone-repo/_mill/reviews/20260820-171211-code-review-r3.md`.
...
- go.sum lacks a full hash for cobra's windows-only mousetrap dependency: **Location:** `/home/knatte/Code/quarry/wts/quarry/go.sum:4`, `go.mod:1-14`
- Loomyard/mill-internal residue survives in ported production comments, uncaught by the "lyx" sweep: **Location:** `quarry/errors.go:1-2`, `quarry/refs.go:9`, `quarry/registry.go:4`, `quarry/ensureserver.go:1-2`, `quarry/detect.go:2`, `quarry/refs_test.go:389`, `quarry/ensureserver_test.go:326`, `quarry/refs_integration_test.go:55`
- card 38's fix edited a Context-only file and reversed an explicit instruction: **Location:** `/home/knatte/Code/quarry/wts/quarry/internal/cli/paths_test.go:81-95, 143-156`
- duplicate card number 38 across batch 5 and batch 6: **Location:** `_mill/plan/05-quarry-live-and-equivalence.md:122` (Card 38: fix the test-isolation race) and `_mill/plan/06-lyx-removal.md:30` (Card 38: delete the two scout packages)
- quarry's slog Warn-level handler governs only 2 of 7 ported log call sites: **Location:** `/home/knatte/Code/quarry/wts/quarry/quarry/ensureserver.go:26-32` vs. `/home/knatte/Code/quarry/wts/quarry/quarry/lspclient.go:575-609`
...
Run each from `/home/knatte/Code/loomyard/wts/scout-extract-standalone-repo` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/scout-extract-standalone-repo rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/scout-extract-standalone-repo status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f443a332-2340-4a97-b561-f2d44f0edbe1"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `f443a332-2340-4a97-b561-f2d44f0edbe1` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**

**`commit_sha` MUST be a real new content commit distinct from the holistic fix housekeeping commit**, unless every finding was a legitimate --nits-only no-op requiring no code change. A fixer that made edits but did not commit must report `status: stuck` (`stuck_type: logic`) instead.
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f443a332-2340-4a97-b561-f2d44f0edbe1"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/loomyard/wts/scout-extract-standalone-repo` for commits;
do not `cd`.
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/loomyard/wts/scout-extract-standalone-repo show <parent-branch>:<path>` to read it without changing cwd.
