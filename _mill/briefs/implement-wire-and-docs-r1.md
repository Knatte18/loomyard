# Implementer Brief — ghissues module — file LoomYard bugs as GitHub issues / wire-and-docs
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\ghissues-module\_mill\plan\02-wire-and-docs.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\ghissues-module\_mill\plan\02-wire-and-docs.md`.
   - Commit the plan edit first (`plan: extend wire-and-docs refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f227b5ba-0402-40f3-9a10-41cb5d4c442c"}
```
...
**`session_id` MUST be exactly `f227b5ba-0402-40f3-9a10-41cb5d4c442c` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f227b5ba-0402-40f3-9a10-41cb5d4c442c"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
