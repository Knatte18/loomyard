# Implementer Brief — Build mhgo worktree module / subcommands
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\mhgo-worktree-module\_mill\plan\03-subcommands.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\mhgo-worktree-module\_mill\plan\03-subcommands.md`.
   - Commit the plan edit first (`plan: extend subcommands refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"7df757b8-b149-43df-90e7-19afdf68bbeb"}
```
...
**`session_id` MUST be exactly `7df757b8-b149-43df-90e7-19afdf68bbeb` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"7df757b8-b149-43df-90e7-19afdf68bbeb"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
