# Implementer Brief — Optimise and slim the rest of the test suite / board-fixtures
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\optimize-remaining-test-suites\_mill\plan\02-board-fixtures.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\optimize-remaining-test-suites\_mill\plan\02-board-fixtures.md`.
   - Commit the plan edit first (`plan: extend board-fixtures refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"bb1a63a8-1152-452a-adbd-559be89cfc6d"}
```
...
**`session_id` MUST be exactly `bb1a63a8-1152-452a-adbd-559be89cfc6d` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"bb1a63a8-1152-452a-adbd-559be89cfc6d"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
