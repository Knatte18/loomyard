# Implementer Brief — fabric: Fabric.Commit classify+dispatch + unified diff/status / commit-foundations
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/fabric-commit-api/_mill/plan/01-commit-foundations.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/fabric-commit-api/_mill/plan/01-commit-foundations.md`.
   - Commit the plan edit first (`plan: extend commit-foundations refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"dd4f2ae7-c6b8-47c7-9744-6b34d8ca3a3d"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"dd4f2ae7-c6b8-47c7-9744-6b34d8ca3a3d","cards_done":[<card numbers this commit set addresses>]}
```
...
**`session_id` MUST be exactly `dd4f2ae7-c6b8-47c7-9744-6b34d8ca3a3d` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"dd4f2ae7-c6b8-47c7-9744-6b34d8ca3a3d","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"dd4f2ae7-c6b8-47c7-9744-6b34d8ca3a3d"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
