# Implementer Brief — Fix stale .mhgo/ config-layer docs + cross-cutting stale-docs sweep / board-config-fixes
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\docs-stale-sweep\_mill\plan\01-board-config-fixes.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\mhgo\wts\docs-stale-sweep\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\mhgo\wts\docs-stale-sweep`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\docs-stale-sweep\_mill\plan\01-board-config-fixes.md`.
   - Commit the plan edit first (`plan: extend board-config-fixes refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b46cc90a-54f3-4a74-842e-02f0f3ae4b3a"}
```
...
**`session_id` MUST be exactly `b46cc90a-54f3-4a74-842e-02f0f3ae4b3a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b46cc90a-54f3-4a74-842e-02f0f3ae4b3a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
