# Implementer Brief — Prune and consolidate the test suite (board first) / ide
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\prune-board-tests\_mill\plan\04-ide.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\prune-board-tests\_mill\plan\04-ide.md`.
   - Commit the plan edit first (`plan: extend ide refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"db399861-5461-4294-8d4a-764e89c0aea6"}
```
...
**`session_id` MUST be exactly `db399861-5461-4294-8d4a-764e89c0aea6` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"db399861-5461-4294-8d4a-764e89c0aea6"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
