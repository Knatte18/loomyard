# Implementer Brief — weft engine: paths geometry, paired worktrees, lyx weft / paths-weft-geometry
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\weft-engine\_mill\plan\01-paths-weft-geometry.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\weft-engine\_mill\plan\01-paths-weft-geometry.md`.
   - Commit the plan edit first (`plan: extend paths-weft-geometry refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"fb6400eb-eed5-4abc-99d8-eea615b99cad"}
```
...
**`session_id` MUST be exactly `fb6400eb-eed5-4abc-99d8-eea615b99cad` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fb6400eb-eed5-4abc-99d8-eea615b99cad"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
