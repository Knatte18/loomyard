# Implementer Brief — Speed up internal/warp integration tests / remove-hook-misc
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\warp-test-speedup\_mill\plan\05-remove-hook-misc.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\warp-test-speedup\_mill\plan\05-remove-hook-misc.md`.
   - Commit the plan edit first (`plan: extend remove-hook-misc refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"fed9074e-748b-475a-9844-40a56a74bde7"}
```
...
**`session_id` MUST be exactly `fed9074e-748b-475a-9844-40a56a74bde7` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fed9074e-748b-475a-9844-40a56a74bde7"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
