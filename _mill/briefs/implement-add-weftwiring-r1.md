# Implementer Brief — Speed up internal/warp integration tests / add-weftwiring
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\warp-test-speedup\_mill\plan\02-add-weftwiring.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\warp-test-speedup\_mill\plan\02-add-weftwiring.md`.
   - Commit the plan edit first (`plan: extend add-weftwiring refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"dcd5e7c9-cbd2-4da0-824a-fa1f6c262543"}
```
...
**`session_id` MUST be exactly `dcd5e7c9-cbd2-4da0-824a-fa1f6c262543` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"dcd5e7c9-cbd2-4da0-824a-fa1f6c262543"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
