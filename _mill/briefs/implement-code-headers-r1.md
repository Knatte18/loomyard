# Implementer Brief — Reconcile stale design docs (stateless + weft model) / code-headers
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\reconcile-stale-docs\_mill\plan\03-code-headers.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\reconcile-stale-docs\_mill\plan\03-code-headers.md`.
   - Commit the plan edit first (`plan: extend code-headers refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"d9129140-6954-44d2-83d4-7933ed3f4b4e"}
```
...
**`session_id` MUST be exactly `d9129140-6954-44d2-83d4-7933ed3f4b4e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"d9129140-6954-44d2-83d4-7933ed3f4b4e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
