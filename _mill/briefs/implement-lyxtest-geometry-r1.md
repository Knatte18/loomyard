# Implementer Brief — Harden the Path Invariant: close enforcement hole + fix geometry leaks / lyxtest-geometry
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\harden-path-invariant\_mill\plan\04-lyxtest-geometry.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\harden-path-invariant\_mill\plan\04-lyxtest-geometry.md`.
   - Commit the plan edit first (`plan: extend lyxtest-geometry refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"6480df3b-a85e-4879-b8bd-f27f273c9a9d"}
```
...
**`session_id` MUST be exactly `6480df3b-a85e-4879-b8bd-f27f273c9a9d` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6480df3b-a85e-4879-b8bd-f27f273c9a9d"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
