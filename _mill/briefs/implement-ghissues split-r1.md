# Implementer Brief — Rename Cobra modules to <module>cli, extract kernels as <module>engine / ghissues split
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\05-ghissues-split.md`
...
2. THEN apply **surgical `Edit`** calls ONLY to the lines that change: the `package`
   ...
   seam split.
3. **Never** `Write` a whole file and then `git rm`/delete its old twin. Use full-file
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\05-ghissues-split.md`.
   - Commit the plan edit first (`plan: extend ghissues split refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"96c70934-52da-4684-b208-20c925d7b315"}
```
...
**`session_id` MUST be exactly `96c70934-52da-4684-b208-20c925d7b315` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"96c70934-52da-4684-b208-20c925d7b315"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
