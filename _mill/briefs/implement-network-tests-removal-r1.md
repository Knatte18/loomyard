# Implementer Brief — Speed up and stabilize the integration test tier / network-tests-removal
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\optimize-integration-tier\_mill\plan\01-network-tests-removal.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\optimize-integration-tier\_mill\plan\01-network-tests-removal.md`.
   - Commit the plan edit first (`plan: extend network-tests-removal refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e9b83f2c-ed6c-4ee9-8b6c-9a116fc7af69"}
```
...
**`session_id` MUST be exactly `e9b83f2c-ed6c-4ee9-8b6c-9a116fc7af69` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e9b83f2c-ed6c-4ee9-8b6c-9a116fc7af69"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
