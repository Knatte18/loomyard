# Implementer Brief — Extract internal/proc (cross-OS windowless + detached spawn) / Migrate callers
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\extract-internal-proc\_mill\plan\02-migrate-callers.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\extract-internal-proc\_mill\plan\02-migrate-callers.md`.
   - Commit the plan edit first (`plan: extend Migrate callers refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"429a76ce-4f5e-4771-a8b4-e8aaed504433"}
```
...
**`session_id` MUST be exactly `429a76ce-4f5e-4771-a8b4-e8aaed504433` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"429a76ce-4f5e-4771-a8b4-e8aaed504433"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
