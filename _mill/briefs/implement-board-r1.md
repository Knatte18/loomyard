# Implementer Brief — Prune and consolidate the test suite (board first) / board
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\prune-board-tests\_mill\plan\01-board.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\prune-board-tests\_mill\plan\01-board.md`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e61c3e1a-8cda-405e-9de5-2d0e8debbedd"}
```
...
**`session_id` MUST be exactly `e61c3e1a-8cda-405e-9de5-2d0e8debbedd` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e61c3e1a-8cda-405e-9de5-2d0e8debbedd"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
