# Implementer Brief — Fix failing TestRunCLI in internal/worktree / board
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\fix-worktree-runcli-test\_mill\plan\03-board.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\fix-worktree-runcli-test\_mill\plan\03-board.md`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"72ed0115-fddc-4965-a4ab-11c9db83f7fe"}
```
...
**`session_id` MUST be exactly `72ed0115-fddc-4965-a4ab-11c9db83f7fe` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"72ed0115-fddc-4965-a4ab-11c9db83f7fe"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
