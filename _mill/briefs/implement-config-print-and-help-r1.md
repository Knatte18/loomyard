# Implementer Brief — CLI help & error ergonomics from sandbox run / config-print-and-help
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\plan\05-config-print-and-help.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\plan\05-config-print-and-help.md`.
   - Commit the plan edit first (`plan: extend config-print-and-help refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4d682616-36bb-4d1d-83e6-9906f683d630"}
```
...
**`session_id` MUST be exactly `4d682616-36bb-4d1d-83e6-9906f683d630` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4d682616-36bb-4d1d-83e6-9906f683d630"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
