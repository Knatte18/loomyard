# Implementer Brief — Speed up and stabilize the integration test tier / lean-fixture-worktree
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\optimize-integration-tier\_mill\plan\03-lean-fixture-worktree.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\optimize-integration-tier\_mill\plan\03-lean-fixture-worktree.md`.
   - Commit the plan edit first (`plan: extend lean-fixture-worktree refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"aa7b37c1-7218-4398-b475-0106128caef1"}
```
...
**`session_id` MUST be exactly `aa7b37c1-7218-4398-b475-0106128caef1` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"aa7b37c1-7218-4398-b475-0106128caef1"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
