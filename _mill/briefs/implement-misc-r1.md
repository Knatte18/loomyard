# Implementer Brief — Fix failing TestRunCLI in internal/worktree / misc
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\fix-worktree-runcli-test\_mill\plan\04-misc.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\fix-worktree-runcli-test\_mill\plan\04-misc.md`.
   - Commit the plan edit first (`plan: extend misc refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e5c5bf7f-cec0-42dc-aa7a-df8d4832f435"}
```
...
**`session_id` MUST be exactly `e5c5bf7f-cec0-42dc-aa7a-df8d4832f435` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e5c5bf7f-cec0-42dc-aa7a-df8d4832f435"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
