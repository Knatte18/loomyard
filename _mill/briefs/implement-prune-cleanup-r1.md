# Implementer Brief — Speed up internal/warp integration tests / prune-cleanup
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\warp-test-speedup\_mill\plan\04-prune-cleanup.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\warp-test-speedup\_mill\plan\04-prune-cleanup.md`.
   - Commit the plan edit first (`plan: extend prune-cleanup refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"a519ca6a-7624-468d-a779-f1d2573630b4"}
```
...
**`session_id` MUST be exactly `a519ca6a-7624-468d-a779-f1d2573630b4` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"a519ca6a-7624-468d-a779-f1d2573630b4"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
