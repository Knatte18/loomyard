# Implementer Brief — Introduce warp: the host↔weft-coordinated git module / prune-cleanup
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\warp-module\_mill\plan\07-prune-cleanup.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\warp-module\_mill\plan\07-prune-cleanup.md`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"fa30a2e9-3281-4209-b7ea-0cbea10152ef"}
```
...
**`session_id` MUST be exactly `fa30a2e9-3281-4209-b7ea-0cbea10152ef` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fa30a2e9-3281-4209-b7ea-0cbea10152ef"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
