# Implementer Brief — Master Builder: new, parallel fork-based implementation module / recover-batch
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/master-builder/_mill/plan/06-recover-batch.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/master-builder/_mill/plan/06-recover-batch.md`.
   - Commit the plan edit first (`plan: extend recover-batch refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"c60f01b2-a4b2-45c2-ab8f-7f4ff2f41ac6"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c60f01b2-a4b2-45c2-ab8f-7f4ff2f41ac6"}
```
...
**`session_id` MUST be exactly `c60f01b2-a4b2-45c2-ab8f-7f4ff2f41ac6` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c60f01b2-a4b2-45c2-ab8f-7f4ff2f41ac6"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
