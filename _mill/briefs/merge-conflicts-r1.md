...
Stage the deletion with `git -C /home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget rm <file>`.
...
- Tests, mechanical `OnDone` re-wiring only (every scenario that relies on sequential `Done` advance needs an explicit `OnDone` chain, but no scenario changes meaning): `internal/shedengine/run_pause_test.go`, `internal/shedengine/run_persist_test.go`, `internal/shedengine/testsupport_test.go` (a linear-chain builder helper).
- Tests needing a `MaxBounces`-semantics re-read rather than an `OnDone` chain — they build through `loomshed.New` and declare no `ProducerDef` literal of their own, so the routing migration reaches them only through that constructor: `internal/loomshed/fixture_test.go` (`:118` sets `MaxBounces: 3`) and `internal/loomshed/sequence_test.go` (`TestSequence_FullRunBlocksAtPublish` drives a full run to its terminal state).
...
- Decision on the aggregate spend bound, which the old design doc made the whole case for one shared counter: **after this task there is no run-wide cap, and that is accepted.**
  `shed.md`'s current paragraph grounds the single counter in "total wasted spend before a human is pulled in" and rejects per-producer budgets precisely because an A↔B cycle then runs `2×budget`.
  The replacement position, and the sentence that paragraph is rewritten to: the aggregate is still bounded, just by a **sum rather than a single number** — within one set of episodes the total is at most the sum of the participating producers' effective `MaxBounces`, so the A↔B case really does cost `2×budget` and that is now a deliberate price, not an oversight.
  Across episodes the lifetime total is unbounded in principle, because a `Done` resets one producer's episode — but a reset is only ever *earned* by a producer succeeding, so the quantity that grows without bound is progress, not wasted spend.
  Any non-terminating `Stuck`-driven loop needs some producer that keeps failing without ever succeeding, and that producer's own episode budget caps it.
  The residual, named rather than hidden: a cycle whose every member alternates `Stuck` and `Done` forever is bounded by nothing here, exactly like the `Done`-cycle case below, and has the same stop — pause or cancellation, checked at the top of every iteration.
- Rationale: the thing worth bounding is wasted spend without progress, not spend as such, and a single run-wide counter cannot tell the two apart — which is the same conflation, at run scope, that this task removes at producer scope.
  Stating "no run-wide cap, bounded by the sum, unbounded only where progress is real" gives a plan writer an actual sentence to put in `shed.md`, which "the old argument is overridden" did not.
- Rejected: keeping a run-wide cap alongside the per-producer ones — two budgets a config author must reason about jointly, where the outer one fires for reasons the inner ones already dispositioned, and whose exhaustion message could not say which producer was at fault.
...
- Decision, the failure-path `done` entry: a producer that returns `(Done, output, err)` with a healthy context lands in `run.go`'s `case callErr != nil` arm (`run.go:177-185`), which calls the same `appendHistory` closure and records the producer's *returned* outcome verbatim — so a `done` history entry is written even though the run halts as `failed`.
  Under episode scoping that entry ends the producer's episode.
  **Accepted, and stated rather than left implicit**: the engine keeps recording what the producer actually returned, because a faithful record is worth more than a tidy one when diagnosing an adapter that violated its own contract (returning a verdict alongside an error is already forbidden by `ShedProducer`'s two documented obligations).
  The premise the no-run-wide-cap decision rests on is therefore narrowed, honestly: a reset is earned by a producer **succeeding, or granted once after a hard failure a human had to resolve**.
  That does not reopen the unbounded-spend hole — `state: "failed"` halts the run, and every continuation past it is a fresh human-initiated act, which is precisely the escalation the budget exists to force.
  `shed.md` says this in the same breath as the episode rule, so the scan's "stop at the first `Done`" is not read as "stop at the first success".
- Rationale: the alternative — making the failure arms suppress or rewrite the `done` outcome so the scan can trust its terminator — buys a cleaner invariant by destroying evidence at exactly the moment evidence matters, and would also put the engine in the business of second-guessing a producer's reported verdict, which the `default` arm's own "record the literal value received" rule deliberately refuses to do.
...
- **Runtime disposition for a `Done` cycle: accepted as unbounded and human-owned, with no iteration cap in `Run`.**
  ...
  The cost while it spins is not merely wasted iterations, and `shed.md` must say so rather than let the accepted risk read as harmless: every iteration appends one `history` entry and `persist` rewrites the entire slice, so an unattended `Done` cycle is O(n^2) total status-file bytes written and an unboundedly growing file — noisy and disk-hungry, which is also what makes it easy to spot.
...
- Rationale: preserves today's **routing** behavior exactly while removing the implicit dependency on list order.
  The claim is scoped to routing on purpose: loom's runtime bounce behavior *does* change in this same commit, because `deps.MaxBounces` becomes a per-producer, episode-scoped, cross-invocation budget with no run-wide cap — a loom run that blocks today may not block, and a resumed run may block sooner (see the episode-scoping and aggregate-bound decisions above).
...
The budget count must be taken from `st.History` (pre-append), not `nextHistory`, or the boundary shifts by one — it is what pins "a budget of three performs three bounce-backs and blocks on the fourth `Stuck`" for a *single* bouncing producer, and a post-append read would look like an off-by-one bug rather than a semantic change.

**Both existing engine budget tests change their expected numbers, and neither is merely re-pointed.**
`TestRun_BounceBudgetExhaustion` (`run_routing_test.go:203-240`) and `TestRun_MaxBouncesZeroResolvesToDefault` (`:242-267`) each wire **two** producers in a cycle (`A OnStuck: "B"`, `B OnStuck: "A"`) and assert `a.calls + b.calls` equals `MaxBounces + 1` (4) and `defaultMaxBounces + 1` (11).
Under a per-producer budget each producer carries its own count, so those totals become `2×MaxBounces + 1` (7) and `2×defaultMaxBounces + 1` (21).
The plan must either update both asserted totals *and* their explanatory comments, or re-wire each test to a single self-bouncing producer (`A OnStuck: "A"`, which stays legal) so the assertion keeps pinning the boundary directly.
Re-wiring is the better default: `2×budget + 1` is the aggregate the design deliberately accepts, not the boundary these tests exist to guard, and a two-producer total is exactly the number that silently drifts when a third producer joins the cycle.
...
- Boundary: a producer with `MaxBounces: 3` performs exactly three bounce-backs and blocks on the fourth `Stuck`, with reason `"bounce budget exhausted"` (this is `TestRun_BounceBudgetExhaustion`, whose two-producer A<->B wiring and `a.calls + b.calls` total must be re-wired or renumbered first — see "Both existing engine budget tests change their expected numbers" under Technical context).
...
- Inheritance: `ProducerDef.MaxBounces: 0` inherits `Shed.MaxBounces`; `Shed.MaxBounces: 0` in turn inherits the internal default of ten (this is `TestRun_MaxBouncesZeroResolvesToDefault`, which becomes two-level *and* needs the same A<->B re-wiring or renumbering — its current expectation of `defaultMaxBounces + 1` total calls becomes `2*defaultMaxBounces + 1` if the cycle is kept).
...
- Failure-path terminator: a producer returning `(Done, _, err)` writes a `done` history entry on the hard-failure arm, and a later run treats that entry as ending the producer's episode — pinning the accepted behavior so a future reader does not "fix" the scan into ignoring it.
...
- **Q:** (review r6 gap) A producer returning `(Done, _, err)` writes a `done` history entry on the hard-failure arm, which would end its episode without it ever succeeding. Suppress it, or accept? **A:** [auto-pick] Accept and document, narrowing the premise to "a reset is earned by succeeding, or granted once after a hard failure a human had to resolve". **Why:** suppressing the entry destroys diagnostic evidence exactly when it matters and puts the engine in the business of second-guessing a reported verdict, which the `default` arm's "record the literal value received" rule already refuses to do; `state: "failed"` halts the run, so every continuation is a fresh human act — the escalation the budget exists to force.
- **Q:** (review r5 gap) `shed.md`'s single-counter argument was grounded in bounding total spend — what replaces it, given per-producer episodes leave no run-wide cap? **A:** [auto-pick] State outright that no run-wide cap exists after this task: the aggregate is bounded by the *sum* of participating producers' budgets within one episode set (so A↔B really does cost `2×budget`, now a deliberate price), and is unbounded across episodes only because a reset is earned by a producer succeeding. **Why:** what is worth bounding is wasted spend without progress, and a single counter cannot tell that from progress — the same conflation at run scope that this task removes at producer scope. The alternating-`Stuck`/`Done` cycle is named as the residual, with pause/cancel as its stop.
...
### From _mill/plan/01-engine-fields-and-validation.md
...
### From _mill/plan/02-run-routing-and-budget.md
...
### From _mill/plan/03-loomshed-migration.md
...
### From _mill/plan/04-docs-sweep.md
...
5. Run `git -C /home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget add <file>` to stage the resolved file.
6. For modify/delete (DU) conflicts: if Task intent above lists this file under a batch's `Deletes:`, run `git -C /home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget rm <file>` instead of editing;
...
Use `git -C /home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget` for any git commands;
do not `cd`.
Worktree cwd is `/home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget`.
