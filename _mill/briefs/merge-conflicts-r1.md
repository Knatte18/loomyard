...
Stage the deletion with `git -C /home/knatte/Code/loomyard/wts/standalone-docs-and-invariants rm <file>`.
...
- **Decision:** the invariant states four things, in `CONSTRAINTS.md`'s rules-only register (no rationale, no incident narrative):
  ...
  2. **The split:** a producer requires none of the three.
     ...
     A standalone CLI invocation of a single producer **requires** none of the three: its pre-run *attempts* tier 1 through `preflight.ResolveMode`, which calls `lyxcwd.Resolve` unconditionally (`internal/preflight/predicates.go:111-112`), and treats the result as a mode question rather than a precondition.
     `ResolveMode` degrades to standalone on three outcomes and **refuses** on two, and the invariant must carry the refusal rather than claim a blanket degrade:
     degrade on a successful resolve with no board-level lyx directory beside it (`predicates.go:113-118`);
     degrade on `ErrNotAGitRepo` (`predicates.go:120-122`);
     degrade on `ErrCwdOutsideAnchor` when the re-probe finds no hub geometry (`predicates.go:124-131`);
     **refuse** — surfacing the original gated error verbatim — on `ErrCwdOutsideAnchor` inside a wired hub worktree's subdirectory (`predicates.go:126-130`), and on any other error class (`predicates.go:133`).
     `predicates.go:100-101` states the governing rule outright: a non-nil error means refuse, never a degrade to standalone.
...
- **Rationale:** these doc comments are the durable home for the rationale the deleted design doc held.
  A blanket rewrite for uniform wording would churn seven files' worth of already-correct prose for no reader gain and would make the diff unreviewable.
...
- **Decision:** implement it, as `cmd/lyx/configstrictness_test.go`, per the specification already written in `CONSTRAINTS.md`'s Config Strictness Invariant.
  ...
  **Disposition of the inherited-specification paragraph** (`CONSTRAINTS.md:520-522`, the whole **Enforced by** bullet): its first line is rewritten to name `cmd/lyx/configstrictness_test.go` and its test function, and its two specification lines (521–522 — the guard's shape and the `allowedSpawners` note) are **deleted**, not compressed and not kept.
  Once the guard exists, those lines are a specification for work already done, addressed to a task that has completed — exactly the historical narrative the file's own header (lines 3–6) bans, and the "T10 named as its home" clause becomes a dangling reference to a finished task.
  Nothing needs rescuing out of that bullet: the known-blind-spot line ("a substring scan cannot see a call reached through an alias or a function value") is already its own separate bullet at line 519 and is left untouched, since it is a live caveat about the shipped guard rather than a record of how the guard came to be.
...
- **Q:** How much `doc.go` work? **A:** [auto-pick] Additive audit — add a tier-naming sentence where absent, leave existing told-geometry prose untouched, create no new `doc.go` for the three packages that lack one. **Why:** seven of the converted packages already carry correct prose;
...
### From _mill/plan/01-constraints-invariants.md
...
### From _mill/plan/02-config-strictness-guard.md
...
### From _mill/plan/03-docgo-audit.md
...
### From _mill/plan/04-overview-docs.md
...
### From _mill/plan/05-roadmap-and-design-doc-deletion.md
...
5. Run `git -C /home/knatte/Code/loomyard/wts/standalone-docs-and-invariants add <file>` to stage the resolved file.
6. For modify/delete (DU) conflicts: if Task intent above lists this file under a batch's `Deletes:`, run `git -C /home/knatte/Code/loomyard/wts/standalone-docs-and-invariants rm <file>` instead of editing;
...
Use `git -C /home/knatte/Code/loomyard/wts/standalone-docs-and-invariants` for any git commands;
do not `cd`.
Worktree cwd is `/home/knatte/Code/loomyard/wts/standalone-docs-and-invariants`.
