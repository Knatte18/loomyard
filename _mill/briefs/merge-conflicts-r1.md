...
Stage the deletion with `git -C /home/knatte/Code/loomyard/wts/loom-discussion-write-producer rm <file>`.
...
- Rewrite `contracts/stencils/loom/loom-template-discussion.md`: fold in `loom-format-discussion.md`'s Fix 1 exploration bound (Step 2 and Step 3), replace Step 3's `Architecture` interview category with a bounded coarse-level one, add a best-effort Step 0 that loads `scribe:prose` and `scribe:conversation`, add a closing self-check step that runs `lyx loom validate-discussion`, and rewrite the leading HTML comment to describe the real call path. Everything else stays verbatim: Step 3's mode-neutral interview framing ("Interview relentlessly…", "For every question, give your **recommended answer**", the full-scope and YAGNI rules), Step 2's "before asking the operator anything", Step 1's board read, and Step 5's section shapes are untouched. All mode-specific wording stays confined to the `{{.mode_rules}}` marker at Step 4 — that confinement is what makes `keep-mode-rules-both-branches` a one-argument flip for the interactive follow-up item.
...
- **The archive-then-respawn window is accepted too.** On a `Discussion-Validate` bounce, `archiveStaleOutputs` renames the two files the previous round already committed, so between that archive and the next `Done` the weft carries two deletions of tracked files plus two untracked siblings. That window closes on the next `Done`, and it persists only when the bounce round itself blocks rather than completing — which, having no `on_stuck`, is already an operator-attention state. Narrowing it would mean committing mid-`Call`, inside a producer whose contract is one atomic action.
...
- Decision: the rewritten stencil gains a closing step, after Step 5's file writes, instructing the agent to run `lyx loom validate-discussion` and to fix what it reports before ending its turn. The verb is zero-argument, exits 0 on a clean gate and 1 otherwise, and puts findings in the failure envelope under a `findings` key.
- Rationale: this is a committed obligation, not an optional nicety. `manifest/roadmap.md:355`, in the shipped `loom: self-checkable mechanical gates` Done entry, states that "No stencil under `contracts/stencils/loom/` was touched — instructing the writer agent to call these verbs belongs to the Planned `loom: Discussion-Write producer` and `loom: Plan-Write producer` items, which this item was sequenced ahead of precisely so the verbs would exist first." `docs/overview.md:316` describes the verb's findings envelope as existing "so a writer agent can self-check before handing off". Inheriting the obligation without acting on it would leave a shipped Done entry pointing at a task that quietly declined it. The Gate Self-Check Parity Invariant guarantees the verb and the `Discussion-Validate` row call the identical `discussionparser.Validate`, so a clean self-check means the row will pass.
- Consequence for `blind-revalidate-bounce`: the self-check makes a `Discussion-Validate` bounce rarer and changes what one *means* — an agent that self-checked and still produced a rejected file failed at something other than not knowing the rule, so a blind re-write is a weaker remedy than it would otherwise be. The bounce disposition is unchanged (the budget still bounds it), but the expected frequency drops, which is the substantive improvement.
- Rejected: deferring the self-check to the interactive follow-up item (the obligation is recorded against *this* item by name); making the self-check advisory wording only, without the fix-what-it-reports instruction (an agent that runs a gate and ignores its output is not self-checking).
...
- Decision: `discussion_timeout_min` stays at `480` in `internal/loomengine/template.yaml`; only its trailing comment changes, since it currently reads "interactive interviews run long". The adjacent `plan_timeout_min` comment is corrected in the same edit: it reads "autonomous, shorter than the interview", and that contrast stops holding once the discussion run is itself autonomous. It becomes a plain statement that the plan run gets a shorter ceiling than the discussion run.
...
### From _mill/plan/01-loomengine-and-stencil.md
...
### From _mill/plan/02-loomshed-decorator.md
...
### From _mill/plan/03-registry-recipe-wiring.md
...
5. Run `git -C /home/knatte/Code/loomyard/wts/loom-discussion-write-producer add <file>` to stage the resolved file.
6. For modify/delete (DU) conflicts: if Task intent above lists this file under a batch's `Deletes:`, run `git -C /home/knatte/Code/loomyard/wts/loom-discussion-write-producer rm <file>` instead of editing;
...
Use `git -C /home/knatte/Code/loomyard/wts/loom-discussion-write-producer` for any git commands;
do not `cd`.
Worktree cwd is `/home/knatte/Code/loomyard/wts/loom-discussion-write-producer`.
