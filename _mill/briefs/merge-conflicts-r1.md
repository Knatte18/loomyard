...
4. Run `git -C C:\Code\loomyard\wts\crucible-treadle-run add <file>` to stage the resolved file.
5. For modify/delete (DU) conflicts: if Task intent above lists this file under a batch's `Deletes:`, run `git -C C:\Code\loomyard\wts\crucible-treadle-run rm <file>` instead of editing; that stages the intentional deletion.
6. For UD conflicts — files this branch **modified** that the parent branch **deleted**: do not silently keep the modification. Instead:
   ...
   c. If the deletion commit message mentions a replacement file (e.g. "replaced by", "moved to", "consolidated into"), or the commit also adds a file in the same directory with overlapping content: stage the deletion — `git -C C:\Code\loomyard\wts\crucible-treadle-run rm <file>`.
...
Available: Read, Edit, Write, Bash, Grep, Glob. Use `git -C C:\Code\loomyard\wts\crucible-treadle-run` for any git commands; do not `cd`. Worktree cwd is `C:\Code\loomyard\wts\crucible-treadle-run`.
