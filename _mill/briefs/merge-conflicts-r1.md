...
Stage the deletion with `git -C /home/knatte/Code/loomyard/wts/scout-extract-standalone-repo rm <file>`.
...
- Move scout's four research/benchmark docs to quarry, plus `template.yaml`'s content as `docs/servers.yaml.example`.
- Sweep lyx vocabulary out of the ported files (87 occurrences across 16 files), including the operator-visible `template.yaml` text and `toolchain.go`'s `lyx` cache-path segment.
- State the supported platform set: linux and windows. darwin is out — `internal/proc` has no darwin implementation.
...
### machine-global-roots-need-a-test-seam

- Decision: `internal/cli/` declares package-level function variables `var userConfigDir = os.UserConfigDir` and `var userCacheDir = os.UserCacheDir`, and all resolution goes through them. Tests redirect both to a `t.TempDir()`.
- Rationale: `os.UserConfigDir()` and `os.UserCacheDir()` are process-global and unaffected by cwd, so a test cannot reach them by changing directory. Without a seam, TDD candidate 1's default-tier case is untestable hermetically — on Linux `$XDG_CONFIG_HOME` happens to work, but on Windows and macOS the stdlib reads other sources, so an env-only approach is silently platform-specific. The pattern already exists in this codebase: `toolchain.go:30` declares exactly `var userCacheDir = os.UserCacheDir` with a comment saying it is there so tests can point the toolchain manager at a `t.TempDir()` "without ever touching the real machine-global cache". Copying an established in-repo pattern beats inventing a second one.
- **This is also a correctness risk, not only a testing one.** `internal/scoutcli/cli_test.go` isolates itself with `t.Chdir(t.TempDir())` at six sites (`:84,167,207,268,400,798`), chosen so `lyxcwd.Resolve` fails and the CLI degrades to built-ins. This task deletes that premise. Ported unchanged, those tests would resolve the *operator's real* `servers.yaml` through the new precedence chain and pass or fail depending on the developer's machine. `cli_test.go` therefore moves off the "ported unchanged" list — its isolation mechanism must be rewritten onto the seam above.
- Rejected: environment variables alone (`$XDG_CONFIG_HOME`/`$XDG_CACHE_HOME` are Linux-only in effect); `t.Chdir` (does not affect either function).
...
### toolchain-cache-is-a-third-axis-and-stays-engine-derived

- Decision: the Go toolchain cache is a **third** path axis, distinct from config and state. It stays engine-derived, but its `lyx` path segment becomes `quarry`: `toolchain.go:34-53` currently builds `os.UserCacheDir()/lyx/tools/go/<version>` and `…/install.lock`, which become `os.UserCacheDir()/quarry/tools/go/<version>`. It keeps using the existing `var userCacheDir = os.UserCacheDir` seam at `toolchain.go:30`, which is the same seam `machine-global-roots-need-a-test-seam` adopts for `internal/cli/`.
- Rationale: config and state move to the caller because both are things an operator legitimately wants to point somewhere else — a config file lives where the user put it, and a daemon's state is per-workspace. The toolchain cache is neither: it is where quarry stashes a `gopls` binary it installed itself, shared across every workspace, and no caller has a reason to override it. Threading it through `internal/cli/` would add a flag nobody would set. What is *not* acceptable is the `lyx` segment surviving — that is the same "meaningless lyx vocabulary inside `os.UserCacheDir()`" the state decision rejects, and it would leave quarry writing into a directory named after a tool the user may not have.
- Rejected: telling it from `internal/cli/` like the other two (a third flag with no use case); leaving the `lyx` segment (dead vocabulary in a user-visible cache path).
- **Consequence:** renaming the segment re-keys the cache, so an operator who already has `gopls` installed under `…/lyx/tools/go/` gets one re-download. That is acceptable and should be mentioned in the quarry README rather than mitigated.

### config-template-is-dropped-not-ported

- Decision: `template.go`, `ConfigTemplate()`, and the `//go:embed` of `template.yaml` are **not** ported. The YAML content itself moves to quarry as `docs/servers.yaml.example` — plain documentation, not embedded, with no Go accessor.
- Rationale: `scoutengine.ConfigTemplate()` has zero call sites anywhere in Loomyard. Unlike every other module's `ConfigTemplate`, scout's is absent from `internal/configreg/configreg.go`'s registry — `doc.go:283` records the reason as "no lyx config reconcile wiring for servers.yaml yet". Porting it would carry dead exported API into quarry's *public* package on day one, which is exactly what the YAGNI principle this discussion opens with forbids. The content is genuinely useful — it documents every built-in entry at its default values and explains how to add a language — so it survives as an example file the new `--config` precedence chain gives users an obvious place to copy it to.
- Rejected: port as-is (dead public API); wire it to a new `quarry config init` verb (a fifth verb, and Scope explicitly rules out redesigning the CLI surface in this task — file it as a quarry issue instead).
- **Consequence:** `template.yaml:2` says the file "is never generated or overwritten by lyx itself". As operator-visible content it is in scope for the vocabulary sweep below, and must be reworded for quarry before it lands as `docs/servers.yaml.example`.

### supported-platforms

- Decision: quarry supports **linux and windows** in this task. darwin is explicitly out, and is filed as a quarry issue rather than fixed here.
- Rationale: `internal/proc` contains exactly `proc_linux.go` and `proc_windows.go` — GOOS-suffixed files with no darwin or BSD sibling — so copying it verbatim yields a package that does not compile on macOS. That is a pre-existing property of the code being moved, not something this task introduces: Loomyard has the same gap and has never claimed otherwise. Fixing it means writing and testing a darwin `KillPID`/`IsAlive`/`DetachBreakaway`, which is new platform code, not a port, and it cannot be verified from this machine. Stating the boundary is honest; silently shipping a repo that fails `GOOS=darwin go build` is not.
- Rejected: writing a darwin implementation as part of the extraction (unverifiable here, and scope creep on a task whose whole discipline is behavioural equivalence); saying nothing (the README of a "standalone" tool must not leave the platform set to be discovered by a failed build).
- **Related pre-existing platform limit, to record rather than fix:** the supervised daemon strategy is Unix-only in practice — `ensureserver.go:205` hard-codes `-listen=unix;<path>` and `:559` dials `unix`. On Windows, `ensureSupervised` fails and `ensureServer` falls back to `ensureNative` (`gopls -remote=auto`), which is the documented fallback path and works. So windows support means "works via the native strategy", not "works identically". The README must say so.
- **Consequence for `config-and-state-paths`:** its rationale mentions `os.UserConfigDir()` returning `~/Library/Application Support` on macOS. That remains true of the stdlib but is not reachable in a quarry that does not build for darwin; the per-OS argument stands on linux and windows alone.
...
- **A quarry-side vocabulary sweep is required, and the port program does not do it.** 87 case-insensitive occurrences of `lyx` sit across 16 `scoutengine` files — `doc.go` (18), `refs_integration_test.go` (18), `scoutdaemon_test.go` (12), `daemonstate.go` (10), `ensureserver.go` (6), `lspclient_guard_test.go` (6), `supervised_test.go` (4), `toolchain.go` (4, including a dangling citation to a `_mill/discussion.md` that will not exist in quarry), `registry.go` (2), and seven files with one each. The port program is deliberately restricted to import paths and package clauses, so all of this survives verbatim. The Loomyard side gets a mandated two-sweep enumeration; the quarry side needs its own:
  ...
  ```
  grep -ric 'lyx' <ported files>
  ```

  Every hit is either rewritten or justified. Two categories need naming because they are not comments:
  - `template.yaml:2` — operator-visible content, currently reading "never generated or overwritten by lyx itself". Handled by `config-template-is-dropped-not-ported`, but the rewrite still has to happen before it lands as `docs/servers.yaml.example`.
  - `toolchain.go`'s path segment — handled by `toolchain-cache-is-a-third-axis-and-stays-engine-derived`, and it is a real path, not prose.
...
- `toolchain.go` — pins and installs `gopls` into a machine-global cache independent of `$PATH`. It derives that cache path itself (`:34-53`, `os.UserCacheDir()/lyx/tools/go/<version>`), which is the third path axis; `:30` already carries the `var userCacheDir = os.UserCacheDir` test seam this task adopts elsewhere.
- `template.go` + `template.yaml` — an embedded seed for `servers.yaml`, exposed as `ConfigTemplate()`. It has no callers anywhere and is dropped rather than ported; see `config-template-is-dropped-not-ported`.
...
**Rewritten — test isolation:** `internal/scoutcli/cli_test.go` calls `t.Chdir(t.TempDir())` at `:84,167,207,268,400,798` to make `lyxcwd.Resolve` fail so the CLI degrades to built-ins. That premise is deleted by this task, and `os.UserConfigDir()`/`os.UserCacheDir()` ignore cwd entirely, so ported unchanged these tests would read the operator's real `servers.yaml`. Rewrite the isolation onto the `userConfigDir`/`userCacheDir` seam from `machine-global-roots-need-a-test-seam`. The tests' actual subjects — flag parsing, verb wiring, exit codes — are unaffected and must be preserved.
...
4. **Behavioural equivalence.** Run `quarry refs`, `definition`, `symbol`, and `assert-no-callers` against the Loomyard checkout and compare the JSON envelopes to `lyx scout` output for the same queries, before the Loomyard deletion lands.

   **Do not take the query set from `docs/research/scout-multilang.md`.** Its five recorded Go positions (`:69-73`) are all stale: `internal/hubgeometry` no longer exists under that name (it is `internal/hubgeom`), `output.Err` is at `internal/output/output.go:27:6` not `:32:6`, and `state.ReadJSON[T]` is at `internal/state/state.go:59:6` not `:49:6`. The ground-truth file those measurements were graded against (`.scratch/codeintel/ground-truth.json`) is gitignored and was never committed. Feeding stale positions to both binaries returns `ErrSymbolNotFound` from each, and the envelopes compare equal while proving nothing.

   Instead, the port batch resolves its own query set at the port commit: pick roughly ten symbols spanning the same categories the benchmark used (high-fan-in plain function, method with many call sites, generic function, interface method, and a symbol that does not exist, for the error path), resolve each position fresh with `lyx scout symbol` on the current tree, and record them in the batch.

   **The comparison is only valid on a non-trivial result.** For every query, assert on the `lyx scout` side first that the envelope is `ok` and carries at least one result, before comparing it to quarry's. A pair of identical errors is not evidence of equivalence. The `assert-no-callers` arm additionally compares exit codes, and the deliberate not-found query is the one case where matching errors *are* the assertion — mark it as such so it is not confused with the others.

   Envelopes must otherwise match byte for byte, including error-message text (see `error-prefixes-stay-verbatim-through-the-port`); the only permitted difference is an absolute path that legitimately changed.
...
- **Q:** [auto-pick, discussion review r3 BLOCKING] Step 4 cited five benchmark symbols as having established ground truth — do they? **A:** No. All five Go positions in `scout-multilang.md:69-73` are stale (`hubgeometry` is `hubgeom` today; two others moved by 5-10 lines) and the ground-truth file was never committed. Step 4 now resolves its own query set at the port commit and must assert a non-empty `ok` result on the lyx side before comparing. **Why:** two identical `ErrSymbolNotFound` envelopes compare equal and prove nothing — the check would have looked green while testing nothing at all.
- **Q:** [auto-pick, discussion review r3 BLOCKING] How do tests reach `os.UserConfigDir()`/`os.UserCacheDir()`, which ignore cwd? **A:** Package-level `userConfigDir`/`userCacheDir` function variables in `internal/cli/`, redirected to `t.TempDir()` under test — the pattern `toolchain.go:30` already uses. `cli_test.go` moves off the ported-unchanged list. **Why:** its six `t.Chdir(t.TempDir())` calls isolate by making `lyxcwd.Resolve` fail, and that premise is deleted here; without a seam the ported tests would read the operator's real `servers.yaml`.
- **Q:** [auto-pick, discussion review r3] Does quarry support macOS? **A:** No — linux and windows only in this task; darwin filed as a quarry issue. **Why:** `internal/proc` has only `proc_linux.go`/`proc_windows.go`, so a verbatim copy does not compile on darwin. Windows works via the native strategy only: the supervised daemon hard-codes a Unix socket (`ensureserver.go:205,559`) and falls back to `ensureNative` there.
...
### From _mill/plan/01-quarry-scaffold.md
...
### From _mill/plan/02-quarry-cli-infra.md
...
### From _mill/plan/03-port-engine.md
...
### From _mill/plan/04-port-cli.md
...
### From _mill/plan/05-quarry-live-and-equivalence.md
...
### From _mill/plan/06-lyx-removal.md
...
5. Run `git -C /home/knatte/Code/loomyard/wts/scout-extract-standalone-repo add <file>` to stage the resolved file.
6. For modify/delete (DU) conflicts: if Task intent above lists this file under a batch's `Deletes:`, run `git -C /home/knatte/Code/loomyard/wts/scout-extract-standalone-repo rm <file>` instead of editing;
...
Use `git -C /home/knatte/Code/loomyard/wts/scout-extract-standalone-repo` for any git commands;
do not `cd`.
Worktree cwd is `/home/knatte/Code/loomyard/wts/scout-extract-standalone-repo`.
