...
### From _mill/plan/01-treadle-extraction.md
...
### From _mill/plan/02-judge-handoff.md
...
### From _mill/plan/03-preround-targeting.md
...
### From _mill/plan/04-modelspec-migration.md
...
### From _mill/plan/05-docs-lifecycle.md
...
4. Run `git -C /home/knatte/Code/loomyard/wts/treadle add <file>` to stage the resolved file.
5. For modify/delete (DU) conflicts: if Task intent above lists this file under a batch's `Deletes:`, run `git -C /home/knatte/Code/loomyard/wts/treadle rm <file>` instead of editing; that stages the intentional deletion.
6. For UD conflicts — files this branch **modified** that the parent branch **deleted**: do not silently keep the modification. Instead:
   ...
   c. If the deletion commit message mentions a replacement file (e.g. "replaced by", "moved to", "consolidated into"), or the commit also adds a file in the same directory with overlapping content: stage the deletion — `git -C /home/knatte/Code/loomyard/wts/treadle rm <file>`.
...
Available: Read, Edit, Write, Bash, Grep, Glob. Use `git -C /home/knatte/Code/loomyard/wts/treadle` for any git commands; do not `cd`. Worktree cwd is `/home/knatte/Code/loomyard/wts/treadle`.
