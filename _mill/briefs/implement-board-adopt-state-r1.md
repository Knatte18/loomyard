# Implementer Brief — Adopt internal/state in board and muxpoc / board-adopt-state
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\adopt-internal-state\_mill\plan\02-board-adopt-state.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\adopt-internal-state\_mill\plan\02-board-adopt-state.md`.
   - Commit the plan edit first (`plan: extend board-adopt-state refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f2b86da7-2703-4dda-99e0-09b233faf171"}
```
...
**`session_id` MUST be exactly `f2b86da7-2703-4dda-99e0-09b233faf171` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f2b86da7-2703-4dda-99e0-09b233faf171"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
