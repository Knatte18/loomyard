# Implementer Brief — Rename Cobra modules to <module>cli, extract kernels as <module>engine / warp split
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\03-warp-split.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\cobra-cli-engine-sweep`
...
## ⛔ READ FIRST — RENAMES USE `git mv`, NOT `Write`

This batch is rename/move-heavy. The batch file's cards list **"Creates:"** and
**"Deletes:"** — those describe the **END STATE, not the mechanic**. Almost every
"created" file is really the matching old-package file being **MOVED**, not authored
from scratch. The batch file's `## Rename mechanic` section is authoritative; follow it.

**Mandatory procedure for every renamed/moved file:**
1. `git -C C:\Code\loomyard\wts\cobra-cli-engine-sweep mv <old-path> <new-path>` FIRST.
   Git records a rename — history preserved, diff stays small.
2. THEN apply **surgical `Edit`** calls ONLY to the lines that change: the `package`
   declaration, import paths, identifier/type retargeting, and any `Command()`/`RunCLI`
   seam split.
3. For a file **split across two packages** (e.g. `clone.go` → `warpcli/clone.go` +
   `warpengine/clone.go`): `git mv` the file to the larger half, then extract the smaller
   half into the second file with surgical edits. It is still a move, not a rewrite.
4. **Never** `Write` a whole file and then `git rm`/delete its old twin. Use full-file
   `Write` **only** for a genuinely new file with no predecessor.

A full-rewrite diff buries the real change, loses git rename detection, and wastes
time/tokens. Prefer `git mv` + targeted `Edit` everywhere.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\03-warp-split.md`.
   - Commit the plan edit first (`plan: extend warp split refs for <short reason>`) and push via the wiki.
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C C:\Code\loomyard\wts\cobra-cli-engine-sweep status --porcelain --untracked-files=no`. If it shows ANY tracked in-scope modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change. The finalize gate now mechanically rejects a success report when in-scope files are dirty, so an uncommitted change will demote your report to stuck regardless.

Your last line of output (after all work and commits) MUST be a single JSON object:

```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c9f84ec5-699a-4862-9385-bf8c94366fd7"}
```
...
**`session_id` MUST be exactly `c9f84ec5-699a-4862-9385-bf8c94366fd7` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c9f84ec5-699a-4862-9385-bf8c94366fd7"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\cobra-cli-engine-sweep` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\loomyard\wts\cobra-cli-engine-sweep`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\cobra-cli-engine-sweep show <parent-branch>:<path>` to read it without changing cwd.
