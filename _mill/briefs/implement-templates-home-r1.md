# Implementer Brief — Move config templates home by removing the lyxtest->configreg edge / templates-home
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\config-test-cleanup\_mill\plan\02-templates-home.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\config-test-cleanup\_mill\plan\02-templates-home.md`.
   - Commit the plan edit first (`plan: extend templates-home refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"41d22672-df8c-4c2c-bc99-6d1cf52c4bcf"}
```
...
**`session_id` MUST be exactly `41d22672-df8c-4c2c-bc99-6d1cf52c4bcf` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"41d22672-df8c-4c2c-bc99-6d1cf52c4bcf"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
