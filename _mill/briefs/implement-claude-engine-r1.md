# Implementer Brief — Build internal/shuttle: one LLM agent via a swappable engine / claude-engine
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\internal-shuttle\_mill\plan\03-claude-engine.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\internal-shuttle\_mill\plan\03-claude-engine.md`.
   - Commit the plan edit first (`plan: extend claude-engine refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"1f3018fb-31e0-4c58-be1b-e7cde9a1c99e"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"1f3018fb-31e0-4c58-be1b-e7cde9a1c99e"}
```
...
**`session_id` MUST be exactly `1f3018fb-31e0-4c58-be1b-e7cde9a1c99e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"1f3018fb-31e0-4c58-be1b-e7cde9a1c99e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
