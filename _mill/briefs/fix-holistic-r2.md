# Holistic Fixer Brief — fabric: no remote/GitHub branch deletion
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete/_mill/reviews/20260919-062129-code-review-r2.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete`
...
```
/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete/_mill/plan/01-gitrepo-remote-delete-primitive.md
/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete/_mill/plan/02-fabricengine-remote-executor.md
/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete/_mill/plan/03-engine-wiring.md
/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete/_mill/plan/04-cli-surface-and-docs.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete/_mill/reviews/20260919-062129-code-review-r2.md`.
...
- doc.go's chokepoint section left un-synced with the sixth primitive: **Location:** `internal/fabricengine/doc.go:631-655` (the "# The destruction chokepoint" section) vs `internal/fabricengine/destroy.go:1-4,54,524-527`
...
Run each from `/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/fabric-remote-branch-delete rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/fabric-remote-branch-delete status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"d5b0a098-c66a-4558-9ab0-80c513259b51"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `d5b0a098-c66a-4558-9ab0-80c513259b51` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"d5b0a098-c66a-4558-9ab0-80c513259b51"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
