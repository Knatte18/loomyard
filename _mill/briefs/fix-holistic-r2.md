# Holistic Fixer Brief — Move config templates home by removing the lyxtest->configreg edge
...
- **Holistic review file:** `C:\Code\loomyard\wts\config-test-cleanup\_mill\reviews\20260625-073622-code-review-r2.md`
- **Plan overview:** `C:\Code\loomyard\wts\config-test-cleanup\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\config-test-cleanup`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\config-test-cleanup\_mill\reviews\20260625-073622-code-review-r2.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\config-test-cleanup` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"0220f4d9-cc2a-4a31-b691-73b97a9f9a28"}
```
...
**`session_id` MUST be exactly `0220f4d9-cc2a-4a31-b691-73b97a9f9a28` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"0220f4d9-cc2a-4a31-b691-73b97a9f9a28"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\config-test-cleanup` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\config-test-cleanup show <parent-branch>:<path>` to read it without changing cwd.
