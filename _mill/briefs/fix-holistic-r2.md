# Holistic Fixer Brief — Rename mhgo to Loomyard (lyx)
...
- **Holistic review file:** `C:\Code\loomyard\wts\rename-to-loomyard\_mill\reviews\20260616-124114-code-review-r2.md`
- **Plan overview:** `C:\Code\loomyard\wts\rename-to-loomyard\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\rename-to-loomyard`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\rename-to-loomyard\_mill\reviews\20260616-124114-code-review-r2.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\rename-to-loomyard` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"2a9dfc52-6302-445b-971d-d8c3efd5c224"}
```
...
**`session_id` MUST be exactly `2a9dfc52-6302-445b-971d-d8c3efd5c224` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"2a9dfc52-6302-445b-971d-d8c3efd5c224"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\rename-to-loomyard` for commits; do not `cd`.
