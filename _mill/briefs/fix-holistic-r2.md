# Holistic Fixer Brief — shedengine: per-producer bounce budget + explicit OnDone routing
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget/_mill/reviews/20260820-102913-code-review-r2.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget/_mill/reviews/20260820-102913-code-review-r2.md`.
...
- roadmap.md: stale "three" flattening-group count after the Done move: **Location:** `manifest/roadmap.md:39,48,53`
...
Run each from `/home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/shedengine-segments-bounce-budget status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"0c64ba81-f068-440c-85e0-bf90781fe296"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `0c64ba81-f068-440c-85e0-bf90781fe296` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"0c64ba81-f068-440c-85e0-bf90781fe296"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
