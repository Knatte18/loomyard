# Holistic Fixer Brief — Extract shared infrastructure (config, git, lock)
...
- **Holistic review file:** `C:\Code\mhgo\wts\extract-shared-infra\_mill\reviews\20260609-185212-code-review-r2.md`
- **Plan overview:** `C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\mhgo\wts\extract-shared-infra`
- **Wiki path:** `C:\Code\mhgo\wiki`
- Round: **2**

Batch plan files (for `verify:` commands):

```
C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\01-internal-lock.md
C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\02-internal-git.md
C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\03-internal-config.md
C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\04-board-migration.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\mhgo\wts\extract-shared-infra\_mill\reviews\20260609-185212-code-review-r2.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\mhgo\wts\extract-shared-infra` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b456b1cd-9d92-4155-b75a-25fbb1402412"}
```
...
**`session_id` MUST be exactly `b456b1cd-9d92-4155-b75a-25fbb1402412` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b456b1cd-9d92-4155-b75a-25fbb1402412"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\mhgo\wts\extract-shared-infra` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\mhgo\wts\extract-shared-infra show <parent-branch>:<path>` to read it without changing cwd.
