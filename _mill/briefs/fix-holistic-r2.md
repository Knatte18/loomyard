# Holistic Fixer Brief — Relocate producer prompt files into a stencils/ directory
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/stencils-directory-reorg/_mill/reviews/20260814-133159-code-review-r2.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/stencils-directory-reorg/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/stencils-directory-reorg`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/stencils-directory-reorg/_mill/reviews/20260814-133159-code-review-r2.md`.
...
- promote never errors on a board copy with no matching source file: **Location:** `internal/stencilcli/promote.go:51-74`
- Webster's relocated banners still name the pre-move filename: **Location:** `stencils/webster/webster-prefix-fork.md:1,7`, `stencils/webster/webster-prefix-recovery.md:1`
...
Run each from `/home/knatte/Code/loomyard/wts/stencils-directory-reorg` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/stencils-directory-reorg rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/stencils-directory-reorg status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"9f1881fe-903f-45d2-bcf9-313ab2970759"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `9f1881fe-903f-45d2-bcf9-313ab2970759` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"9f1881fe-903f-45d2-bcf9-313ab2970759"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/loomyard/wts/stencils-directory-reorg` for commits;
do not `cd`.
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/loomyard/wts/stencils-directory-reorg show <parent-branch>:<path>` to read it without changing cwd.
