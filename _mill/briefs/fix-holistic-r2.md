# Holistic Fixer Brief — Fix prowler: Reddit adapter blocked
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/prowler-fix-reddit-block/_mill/reviews/20260825-081533-code-review-r2.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/prowler-fix-reddit-block/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/prowler-fix-reddit-block`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/prowler-fix-reddit-block/_mill/reviews/20260825-081533-code-review-r2.md`.
...
- reddit-block-page.html fixture violates Card 1's own trim requirement: **Location:** `plugins/prowler/testdata/reddit-block-page.html:1-3`
...
Run each from `/home/knatte/Code/loomyard/wts/prowler-fix-reddit-block` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/prowler-fix-reddit-block rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/prowler-fix-reddit-block status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4c52afd5-6583-4226-81fc-789a60d9b28c"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `4c52afd5-6583-4226-81fc-789a60d9b28c` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4c52afd5-6583-4226-81fc-789a60d9b28c"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
