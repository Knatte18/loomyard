# Implementer Brief — Extract internal/fsx and build internal/state / board-migration
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\extract-internal-fsx\_mill\plan\02-board-migration.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\extract-internal-fsx\_mill\plan\02-board-migration.md`.
   - Commit the plan edit first (`plan: extend board-migration refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"2c940706-7310-433a-9394-ae69caa01f26"}
```
...
**`session_id` MUST be exactly `2c940706-7310-433a-9394-ae69caa01f26` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"2c940706-7310-433a-9394-ae69caa01f26"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
