# Implementer Brief — Extract yamlengine and migrate config via lyx update / templates-live-yaml
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\yamlengine\_mill\plan\04-templates-live-yaml.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\yamlengine\_mill\plan\04-templates-live-yaml.md`.
   - Commit the plan edit first (`plan: extend templates-live-yaml refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"039ae898-656a-4a59-b5e4-41051cad8ca7"}
```
...
**`session_id` MUST be exactly `039ae898-656a-4a59-b5e4-41051cad8ca7` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"039ae898-656a-4a59-b5e4-41051cad8ca7"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
