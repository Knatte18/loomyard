# Implementer Brief — Rename Cobra modules to <module>cli, extract kernels as <module>engine / board split
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\01-board-split.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\01-board-split.md`.
   - Commit the plan edit first (`plan: extend board split refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"8dd70085-5f61-4e88-a59b-744a6f07a6ff"}
```
...
**`session_id` MUST be exactly `8dd70085-5f61-4e88-a59b-744a6f07a6ff` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"8dd70085-5f61-4e88-a59b-744a6f07a6ff"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
