# Implementer Brief — Speed up internal/warp integration tests / drift-status
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\warp-test-speedup\_mill\plan\03-drift-status.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\warp-test-speedup\_mill\plan\03-drift-status.md`.
   - Commit the plan edit first (`plan: extend drift-status refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"8ca05ef7-e989-4a34-be41-af68b550c89f"}
```
...
**`session_id` MUST be exactly `8ca05ef7-e989-4a34-be41-af68b550c89f` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"8ca05ef7-e989-4a34-be41-af68b550c89f"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
