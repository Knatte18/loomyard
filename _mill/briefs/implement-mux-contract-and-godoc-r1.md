# Implementer Brief — Facilitate Linux support (Win11-side prep) / mux-contract-and-godoc
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\facilitate-linux\_mill\plan\03-mux-contract-and-godoc.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\facilitate-linux\_mill\plan\03-mux-contract-and-godoc.md`.
   - Commit the plan edit first (`plan: extend mux-contract-and-godoc refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"c047ce6f-2be5-41e1-9250-cfef5999f67d"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c047ce6f-2be5-41e1-9250-cfef5999f67d"}
```
...
**`session_id` MUST be exactly `c047ce6f-2be5-41e1-9250-cfef5999f67d` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c047ce6f-2be5-41e1-9250-cfef5999f67d"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
