# Implementer Brief — Optimise and slim the test suite / paths-tests
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\optimize-test-suite\_mill\plan\04-paths-tests.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\optimize-test-suite\_mill\plan\04-paths-tests.md`.
   - Commit the plan edit first (`plan: extend paths-tests refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"6417c0a0-4fdd-4c9b-a67c-76e515c1b0ba"}
```
...
**`session_id` MUST be exactly `6417c0a0-4fdd-4c9b-a67c-76e515c1b0ba` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6417c0a0-4fdd-4c9b-a67c-76e515c1b0ba"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
