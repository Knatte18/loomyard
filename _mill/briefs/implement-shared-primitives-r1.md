# Implementer Brief — Extract shared primitives (paths, output) / shared-primitives
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\mhgo-extract-primitives\_mill\plan\01-shared-primitives.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\mhgo-extract-primitives\_mill\plan\01-shared-primitives.md`.
   - Commit the plan edit first (`plan: extend shared-primitives refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"04ec0422-1d68-4ac7-96e4-beaf6aef0144"}
```
...
**`session_id` MUST be exactly `04ec0422-1d68-4ac7-96e4-beaf6aef0144` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"04ec0422-1d68-4ac7-96e4-beaf6aef0144"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
