# Implementer Brief — Investigate the unexplained lyx mux server crash / render-defaults
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/mux-server-crash/_mill/plan/01-render-defaults.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/mux-server-crash/_mill/plan/01-render-defaults.md`.
   - Commit the plan edit first (`plan: extend render-defaults refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"4a910ae3-2ca7-4e11-8bb4-91ed0e5ca68e"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4a910ae3-2ca7-4e11-8bb4-91ed0e5ca68e"}
```
...
**`session_id` MUST be exactly `4a910ae3-2ca7-4e11-8bb4-91ed0e5ca68e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4a910ae3-2ca7-4e11-8bb4-91ed0e5ca68e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
