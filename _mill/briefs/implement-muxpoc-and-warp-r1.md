# Implementer Brief — Built-in CLI help: lyx self-documents modules & commands / muxpoc-and-warp
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\builtin-cli-help\_mill\plan\05-muxpoc-and-warp.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\builtin-cli-help\_mill\plan\05-muxpoc-and-warp.md`.
   - Commit the plan edit first (`plan: extend muxpoc-and-warp refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"ac10d92a-849b-4636-92f6-edcef95e8577"}
```
...
**`session_id` MUST be exactly `ac10d92a-849b-4636-92f6-edcef95e8577` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"ac10d92a-849b-4636-92f6-edcef95e8577"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
