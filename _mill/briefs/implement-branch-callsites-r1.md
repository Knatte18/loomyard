# Implementer Brief — fabric: one ownership-and-dirtiness gate for all destruction (slice 12) / branch-callsites
...
- **Batch file (authoritative for this batch):** `/home/hanf/Code/loomyard/wts/fabric-destructive-chokepoint/_mill/plan/05-branch-callsites.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/hanf/Code/loomyard/wts/fabric-destructive-chokepoint/_mill/plan/05-branch-callsites.md`.
   - Commit the plan edit first (`plan: extend branch-callsites refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"fe86e9cf-b666-4bb4-97ca-769f57f02b2b"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"fe86e9cf-b666-4bb4-97ca-769f57f02b2b","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `fe86e9cf-b666-4bb4-97ca-769f57f02b2b` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"fe86e9cf-b666-4bb4-97ca-769f57f02b2b","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fe86e9cf-b666-4bb4-97ca-769f57f02b2b"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
