# Implementer Brief — Move config templates home by removing the lyxtest->configreg edge / lyxtest-tidy
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\config-test-cleanup\_mill\plan\03-lyxtest-tidy.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\config-test-cleanup\_mill\plan\03-lyxtest-tidy.md`.
   - Commit the plan edit first (`plan: extend lyxtest-tidy refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e631c253-7e6c-4b12-971c-1c262ffc1cb9"}
```
...
**`session_id` MUST be exactly `e631c253-7e6c-4b12-971c-1c262ffc1cb9` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e631c253-7e6c-4b12-971c-1c262ffc1cb9"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
