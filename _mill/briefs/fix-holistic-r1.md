# Holistic Fixer Brief — Add lyx init --undo / deinit command
...
- **Holistic review file:** `C:\Code\loomyard\wts\lyx-deinit\_mill\reviews\20260702-055329-code-review-r1.md`
- **Plan overview:** `C:\Code\loomyard\wts\lyx-deinit\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\lyx-deinit`
...
```
C:\Code\loomyard\wts\lyx-deinit\_mill\plan\01-weftengine-commit-message.md
C:\Code\loomyard\wts\lyx-deinit\_mill\plan\02-gitignore-remove.md
C:\Code\loomyard\wts\lyx-deinit\_mill\plan\03-warpengine-unwire-junctions.md
C:\Code\loomyard\wts\lyx-deinit\_mill\plan\04-initcli-undo.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\lyx-deinit\_mill\reviews\20260702-055329-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\lyx-deinit` via Bash. If a verify command fails:
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C C:\Code\loomyard\wts\lyx-deinit rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C C:\Code\loomyard\wts\lyx-deinit status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"bc71e6b7-2249-468c-aa48-84c44476cd82"}
```
...
**`session_id` MUST be exactly `bc71e6b7-2249-468c-aa48-84c44476cd82` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"bc71e6b7-2249-468c-aa48-84c44476cd82"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\lyx-deinit` for commits; do not `cd`.
