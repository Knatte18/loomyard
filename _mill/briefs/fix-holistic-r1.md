# Holistic Fixer Brief — Local lyx sandbox for manual experimentation
...
- **Holistic review file:** `C:\Code\loomyard\wts\lyx-sandbox\_mill\reviews\20260626-195758-code-review-r1.md`
- **Plan overview:** `C:\Code\loomyard\wts\lyx-sandbox\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\lyx-sandbox`
...
```
C:\Code\loomyard\wts\lyx-sandbox\_mill\plan\01-sandbox-tool-and-docs.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\lyx-sandbox\_mill\reviews\20260626-195758-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\lyx-sandbox` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c1001ed8-30d3-4f0e-a4b5-3f9414607a2c"}
```
...
**`session_id` MUST be exactly `c1001ed8-30d3-4f0e-a4b5-3f9414607a2c` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c1001ed8-30d3-4f0e-a4b5-3f9414607a2c"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\lyx-sandbox` for commits; do not `cd`.
