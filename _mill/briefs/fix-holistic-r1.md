...
- **Holistic review file:** `C:\Code\loomyard\wts\internal-builder\_mill\reviews\20260711-171632-code-review-r1.md`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\internal-builder\_mill\reviews\20260711-171632-code-review-r1.md`. This is non-negotiable.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C C:\Code\loomyard\wts\internal-builder rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C C:\Code\loomyard\wts\internal-builder status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e0e25c5d-abca-4a94-a193-8f7de8917b38"}
```
...
**`session_id` MUST be exactly `e0e25c5d-abca-4a94-a193-8f7de8917b38` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e0e25c5d-abca-4a94-a193-8f7de8917b38"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
