# Holistic Fixer Brief — fabric: accumulate the result envelope from mutations, not control flow (slice 14)
...
- **Holistic review file:** `/home/hanf/Code/loomyard/wts/fabric-mutation-record-envelope/_mill/reviews/20260812-075929-code-review-r1.md`
- **Plan overview:** `/home/hanf/Code/loomyard/wts/fabric-mutation-record-envelope/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/hanf/Code/loomyard/wts/fabric-mutation-record-envelope`
- **Wiki path:** `/home/hanf/Code/loomyard/wiki`
- Round: **1**
...
Load the **mill-receiving-review** skill before reading any finding in `/home/hanf/Code/loomyard/wts/fabric-mutation-record-envelope/_mill/reviews/20260812-075929-code-review-r1.md`.
...
Run each from `/home/hanf/Code/loomyard/wts/fabric-mutation-record-envelope` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/hanf/Code/loomyard/wts/fabric-mutation-record-envelope rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/hanf/Code/loomyard/wts/fabric-mutation-record-envelope status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"48bb4f86-298e-42c3-a237-4f8bec0183eb"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `48bb4f86-298e-42c3-a237-4f8bec0183eb` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"48bb4f86-298e-42c3-a237-4f8bec0183eb"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
