# Holistic Fixer Brief — Prune and consolidate the test suite (board first)
...
- **Holistic review file:** `C:\Code\loomyard\wts\prune-board-tests\_mill\reviews\20260622-183334-code-review-r1.md`
- **Plan overview:** `C:\Code\loomyard\wts\prune-board-tests\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\prune-board-tests`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\prune-board-tests\_mill\reviews\20260622-183334-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\prune-board-tests` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f966e41f-9041-4d76-869d-f9f189a9e383"}
```
...
**`session_id` MUST be exactly `f966e41f-9041-4d76-869d-f9f189a9e383` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f966e41f-9041-4d76-869d-f9f189a9e383"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\prune-board-tests` for commits; do not `cd`.
