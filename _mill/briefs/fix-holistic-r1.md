# Holistic Fixer Brief — Extract internal/fslink cross-OS link primitive
...
- **Holistic review file:** `C:\Code\loomyard\wts\extract-fslink\_mill\reviews\20260622-115552-code-review-r1.md`
- **Plan overview:** `C:\Code\loomyard\wts\extract-fslink\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\extract-fslink`
...
```
C:\Code\loomyard\wts\extract-fslink\_mill\plan\01-fslink-package.md
C:\Code\loomyard\wts\extract-fslink\_mill\plan\02-migrate-callsites.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\extract-fslink\_mill\reviews\20260622-115552-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\extract-fslink` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"84a14ee0-93c9-49f5-bd1a-66b3cda301d4"}
```
...
**`session_id` MUST be exactly `84a14ee0-93c9-49f5-bd1a-66b3cda301d4` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"84a14ee0-93c9-49f5-bd1a-66b3cda301d4"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\extract-fslink` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\extract-fslink show <parent-branch>:<path>` to read it without changing cwd.
