# Holistic Fixer Brief — fabric: one ownership-and-dirtiness gate for all destruction (slice 12)
...
- **Holistic review file:** `/home/hanf/Code/loomyard/wts/fabric-destructive-chokepoint/_mill/reviews/20260811-041305-code-review-r1.md`
- **Plan overview:** `/home/hanf/Code/loomyard/wts/fabric-destructive-chokepoint/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/hanf/Code/loomyard/wts/fabric-destructive-chokepoint`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/hanf/Code/loomyard/wts/fabric-destructive-chokepoint/_mill/reviews/20260811-041305-code-review-r1.md`.
...
Run each from `/home/hanf/Code/loomyard/wts/fabric-destructive-chokepoint` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/hanf/Code/loomyard/wts/fabric-destructive-chokepoint rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/hanf/Code/loomyard/wts/fabric-destructive-chokepoint status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"289e00a2-c942-4afb-877d-44f83c613c79"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `289e00a2-c942-4afb-877d-44f83c613c79` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"289e00a2-c942-4afb-877d-44f83c613c79"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
