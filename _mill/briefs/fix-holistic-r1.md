# Holistic Fixer Brief — Seeded Shed core: run addressing, seed contract, batten
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/reviews/20260920-084229-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/seeded-shed-core`
...
```
/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/plan/01-shedrun-leaf.md
/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/plan/02-board-type-field.md
/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/plan/03-batten-rename.md
/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/plan/04-loom-run-directory.md
/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/plan/05-batten-producers.md
/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/plan/06-batten-wiring.md
/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/plan/07-shed-addressing.md
/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/plan/08-docs-and-integration.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/seeded-shed-core/_mill/reviews/20260920-084229-code-review-r1.md`.
...
Run each from `/home/knatte/Code/loomyard/wts/seeded-shed-core` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/seeded-shed-core rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/seeded-shed-core status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c8531e4c-8ca7-4a69-b260-ee8463fc3e3c"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `c8531e4c-8ca7-4a69-b260-ee8463fc3e3c` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c8531e4c-8ca7-4a69-b260-ee8463fc3e3c"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/loomyard/wts/seeded-shed-core` for commits;
do not `cd`.
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/loomyard/wts/seeded-shed-core show <parent-branch>:<path>` to read it without changing cwd.
