# Holistic Fixer Brief — board: use gitrepo as its git operator
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/board-use-gitrepo/_mill/reviews/20260725-055626-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/board-use-gitrepo/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/board-use-gitrepo`
...
```
/home/knatte/Code/loomyard/wts/board-use-gitrepo/_mill/plan/01-gitrepo-stage-all.md
/home/knatte/Code/loomyard/wts/board-use-gitrepo/_mill/plan/02-boardengine-migration.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/board-use-gitrepo/_mill/reviews/20260725-055626-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `/home/knatte/Code/loomyard/wts/board-use-gitrepo` via Bash. If a verify command fails:
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/board-use-gitrepo rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C /home/knatte/Code/loomyard/wts/board-use-gitrepo status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e65e9e26-8b00-4794-a458-655bd65bdda1"}
```
...
**`session_id` MUST be exactly `e65e9e26-8b00-4794-a458-655bd65bdda1` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e65e9e26-8b00-4794-a458-655bd65bdda1"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C /home/knatte/Code/loomyard/wts/board-use-gitrepo` for commits; do not `cd`.
