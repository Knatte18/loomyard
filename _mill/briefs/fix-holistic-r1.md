# Holistic Fixer Brief — lyx loom step + external supervisor skill
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/loom-step/_mill/reviews/20260912-115354-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/loom-step/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/loom-step`
...
```
/home/knatte/Code/loomyard/wts/loom-step/_mill/plan/01-shedengine-step.md
/home/knatte/Code/loomyard/wts/loom-step/_mill/plan/02-loomshed-interrupt-policy.md
/home/knatte/Code/loomyard/wts/loom-step/_mill/plan/03-loomcli-shared-helpers.md
/home/knatte/Code/loomyard/wts/loom-step/_mill/plan/04-loom-step-verb.md
/home/knatte/Code/loomyard/wts/loom-step/_mill/plan/05-status-interrupt-policy.md
/home/knatte/Code/loomyard/wts/loom-step/_mill/plan/06-ly-plugin-supervise-skill.md
/home/knatte/Code/loomyard/wts/loom-step/_mill/plan/07-design-doc-and-roadmap.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/loom-step/_mill/reviews/20260912-115354-code-review-r1.md`.
...
Run each from `/home/knatte/Code/loomyard/wts/loom-step` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/loom-step rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/loom-step status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c518fcc1-b362-41e2-a419-80a5dddba2ad"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `c518fcc1-b362-41e2-a419-80a5dddba2ad` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c518fcc1-b362-41e2-a419-80a5dddba2ad"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
