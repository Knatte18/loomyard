# Holistic Fixer Brief — Extract internal/fsx and build internal/state
...
- **Holistic review file:** `C:\Code\loomyard\wts\extract-internal-fsx\_mill\reviews\20260616-191717-code-review-r1.md`
- **Plan overview:** `C:\Code\loomyard\wts\extract-internal-fsx\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\extract-internal-fsx`
...
```
C:\Code\loomyard\wts\extract-internal-fsx\_mill\plan\01-fsx-package.md
C:\Code\loomyard\wts\extract-internal-fsx\_mill\plan\02-board-migration.md
C:\Code\loomyard\wts\extract-internal-fsx\_mill\plan\03-muxpoc-swap.md
C:\Code\loomyard\wts\extract-internal-fsx\_mill\plan\04-state-package.md
C:\Code\loomyard\wts\extract-internal-fsx\_mill\plan\05-docs.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\extract-internal-fsx\_mill\reviews\20260616-191717-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\extract-internal-fsx` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"454c3f35-eb85-4880-94e4-13d3781329b6"}
```
...
**`session_id` MUST be exactly `454c3f35-eb85-4880-94e4-13d3781329b6` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"454c3f35-eb85-4880-94e4-13d3781329b6"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\extract-internal-fsx` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\extract-internal-fsx show <parent-branch>:<path>` to read it without changing cwd.
