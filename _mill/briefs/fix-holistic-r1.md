# Holistic Fixer Brief — Restore the Tier 1 floor: guards + perchengine
...
- **Holistic review file:** `C:\Code\loomyard\wts\restore-tier1-floor\_mill\reviews\20260713-094508-code-review-r1.md`
- **Plan overview:** `C:\Code\loomyard\wts\restore-tier1-floor\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\restore-tier1-floor`
...
```
C:\Code\loomyard\wts\restore-tier1-floor\_mill\plan\01-code-fixes.md
C:\Code\loomyard\wts\restore-tier1-floor\_mill\plan\02-benchmarks-block.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\restore-tier1-floor\_mill\reviews\20260713-094508-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\restore-tier1-floor` via Bash. If a verify command fails:
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C C:\Code\loomyard\wts\restore-tier1-floor rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C C:\Code\loomyard\wts\restore-tier1-floor status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"02fbed75-1c7b-424a-a49a-642900400093"}
```
...
**`session_id` MUST be exactly `02fbed75-1c7b-424a-a49a-642900400093` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"02fbed75-1c7b-424a-a49a-642900400093"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\restore-tier1-floor` for commits; do not `cd`.
