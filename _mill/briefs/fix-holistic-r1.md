# Holistic Fixer Brief — webster: stop re-rendering already-inherited context into fork prompts
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/webster-fork-context-hygiene/_mill/reviews/20260802-144334-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/webster-fork-context-hygiene/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/webster-fork-context-hygiene`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/webster-fork-context-hygiene/_mill/reviews/20260802-144334-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `/home/knatte/Code/loomyard/wts/webster-fork-context-hygiene` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"9d27ed9b-84b9-4f7e-b666-12129fb2d372"}
```
...
**`session_id` MUST be exactly `9d27ed9b-84b9-4f7e-b666-12129fb2d372` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"9d27ed9b-84b9-4f7e-b666-12129fb2d372"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C /home/knatte/Code/loomyard/wts/webster-fork-context-hygiene` for commits; do not `cd`.
