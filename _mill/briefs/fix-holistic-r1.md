# Holistic Fixer Brief — Rename the fabric host vocabulary to warp, and name the composite repo Fabric
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/fabric-host-to-warp-rename/_mill/reviews/20260809-072331-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/fabric-host-to-warp-rename/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/fabric-host-to-warp-rename`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/fabric-host-to-warp-rename/_mill/reviews/20260809-072331-code-review-r1.md`.
...
Run each from `/home/knatte/Code/loomyard/wts/fabric-host-to-warp-rename` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/fabric-host-to-warp-rename rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/fabric-host-to-warp-rename status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b6200f19-b735-40b8-927e-62923b7e6b7d"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `b6200f19-b735-40b8-927e-62923b7e6b7d` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b6200f19-b735-40b8-927e-62923b7e6b7d"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/loomyard/wts/fabric-host-to-warp-rename` for commits;
do not `cd`.
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/loomyard/wts/fabric-host-to-warp-rename show <parent-branch>:<path>` to read it without changing cwd.
