# Holistic Fixer Brief — Extract internal/vscode; keep ide IDE-generic
...
- **Holistic review file:** `C:\Code\loomyard\wts\extract-internal-vscode\_mill\reviews\20260622-180819-code-review-r1.md`
- **Plan overview:** `C:\Code\loomyard\wts\extract-internal-vscode\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\extract-internal-vscode`
...
```
C:\Code\loomyard\wts\extract-internal-vscode\_mill\plan\01-extract-vscode.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\extract-internal-vscode\_mill\reviews\20260622-180819-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\extract-internal-vscode` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"71b91aaa-fd57-4e6c-be23-0fcd0d10aafa"}
```
...
**`session_id` MUST be exactly `71b91aaa-fd57-4e6c-be23-0fcd0d10aafa` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"71b91aaa-fd57-4e6c-be23-0fcd0d10aafa"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\extract-internal-vscode` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\extract-internal-vscode show <parent-branch>:<path>` to read it without changing cwd.
