# Holistic Fixer Brief — internal/paths: subpath init + mirrored system dirs
...
- **Holistic review file:** `C:\Code\mhgo\wts\paths-subpath-mirroring\_mill\reviews\20260615-111347-code-review-r1.md`
- **Plan overview:** `C:\Code\mhgo\wts\paths-subpath-mirroring\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\mhgo\wts\paths-subpath-mirroring`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\mhgo\wts\paths-subpath-mirroring\_mill\reviews\20260615-111347-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\mhgo\wts\paths-subpath-mirroring` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"0255abf7-044a-4922-b654-3842e085c487"}
```
...
**`session_id` MUST be exactly `0255abf7-044a-4922-b654-3842e085c487` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"0255abf7-044a-4922-b654-3842e085c487"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\mhgo\wts\paths-subpath-mirroring` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\mhgo\wts\paths-subpath-mirroring show <parent-branch>:<path>` to read it without changing cwd.
