# Holistic Fixer Brief — Build mhgo worktree module
...
- **Holistic review file:** `C:\Code\mhgo\wts\mhgo-worktree-module\_mill\reviews\20260613-140416-code-review-r1.md`
- **Plan overview:** `C:\Code\mhgo\wts\mhgo-worktree-module\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\mhgo\wts\mhgo-worktree-module`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\mhgo\wts\mhgo-worktree-module\_mill\reviews\20260613-140416-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\mhgo\wts\mhgo-worktree-module` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"ff7d3463-75b5-412a-b78f-41c410344b50"}
```
...
**`session_id` MUST be exactly `ff7d3463-75b5-412a-b78f-41c410344b50` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"ff7d3463-75b5-412a-b78f-41c410344b50"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
