# Holistic Fixer Brief — Extend codeintel lookup to non-Go languages via LSP
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/codeintel-multilang/_mill/reviews/20260717-184755-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/codeintel-multilang/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/codeintel-multilang`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/codeintel-multilang/_mill/reviews/20260717-184755-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `/home/knatte/Code/loomyard/wts/codeintel-multilang` via Bash. If a verify command fails:
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/codeintel-multilang rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/codeintel-multilang status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"770c3b25-6aee-4d5a-9bbf-95e9c7a2428b"}
```
...
**`session_id` MUST be exactly `770c3b25-6aee-4d5a-9bbf-95e9c7a2428b` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"770c3b25-6aee-4d5a-9bbf-95e9c7a2428b"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C /home/knatte/Code/loomyard/wts/codeintel-multilang` for commits; do not `cd`.
