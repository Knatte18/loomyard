...
- **Holistic review file:** `C:\Code\loomyard\wts\yamlengine\_mill\reviews\20260624-092834-code-review-r1.md`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\yamlengine\_mill\reviews\20260624-092834-code-review-r1.md`. This is non-negotiable.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c41de289-0ebd-4c78-8a00-e8bfbbf7da75"}
```
...
**`session_id` MUST be exactly `c41de289-0ebd-4c78-8a00-e8bfbbf7da75` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c41de289-0ebd-4c78-8a00-e8bfbbf7da75"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
