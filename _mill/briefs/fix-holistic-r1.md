# Holistic Fixer Brief — CLI help & error ergonomics from sandbox run
...
- **Holistic review file:** `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\reviews\20260628-165200-code-review-r1.md`
- **Plan overview:** `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\cli-help-ergonomics`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\reviews\20260628-165200-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\cli-help-ergonomics` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"31c08e36-9d53-4407-8863-b21da22c8bdb"}
```
...
**`session_id` MUST be exactly `31c08e36-9d53-4407-8863-b21da22c8bdb` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"31c08e36-9d53-4407-8863-b21da22c8bdb"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\cli-help-ergonomics` for commits; do not `cd`.
