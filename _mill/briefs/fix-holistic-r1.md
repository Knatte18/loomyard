# Holistic Fixer Brief — shed: the LLM driver as a generic stepper and mender
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/shed-llm-driver/_mill/reviews/20260926-121510-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/shed-llm-driver/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/shed-llm-driver`
...
```
/home/knatte/Code/loomyard/wts/shed-llm-driver/_mill/plan/01-logger-trace-accessors.md
/home/knatte/Code/loomyard/wts/shed-llm-driver/_mill/plan/02-fabric-mutation-trace.md
/home/knatte/Code/loomyard/wts/shed-llm-driver/_mill/plan/03-shed-envelope-trace.md
/home/knatte/Code/loomyard/wts/shed-llm-driver/_mill/plan/04-step-entry-point-integration.md
/home/knatte/Code/loomyard/wts/shed-llm-driver/_mill/plan/05-ly-drive-recipe-blind.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/shed-llm-driver/_mill/reviews/20260926-121510-code-review-r1.md`.
...
Run each from `/home/knatte/Code/loomyard/wts/shed-llm-driver` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/shed-llm-driver rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/shed-llm-driver status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b0d1148b-fbc7-40fe-b42b-bb1f1674f24b"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `b0d1148b-fbc7-40fe-b42b-bb1f1674f24b` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b0d1148b-fbc7-40fe-b42b-bb1f1674f24b"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
