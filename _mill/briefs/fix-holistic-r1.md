# Holistic Fixer Brief — Sandbox suite: emit findings JSON on the shared analysis contract
...
- **Holistic review file:** `C:\Code\loomyard\wts\sandbox-report-json\_mill\reviews\20260630-191658-code-review-r1.md`
- **Plan overview:** `C:\Code\loomyard\wts\sandbox-report-json\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\sandbox-report-json`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\sandbox-report-json\_mill\reviews\20260630-191658-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\sandbox-report-json` via Bash. If a verify command fails:
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C C:\Code\loomyard\wts\sandbox-report-json rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C C:\Code\loomyard\wts\sandbox-report-json status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"fcb97e1e-da2c-464c-96b8-18ff38fb262e"}
```
...
**`session_id` MUST be exactly `fcb97e1e-da2c-464c-96b8-18ff38fb262e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fcb97e1e-da2c-464c-96b8-18ff38fb262e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\sandbox-report-json` for commits; do not `cd`.
