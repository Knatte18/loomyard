# Holistic Fixer Brief — gitexec: decide whether RunGit should return a typed error carrying stderr
...
- **Holistic review file:** `/home/hanf/Code/loomyard/wts/gitexec-error-shape-decision/_mill/reviews/20260811-041220-code-review-r1.md`
- **Plan overview:** `/home/hanf/Code/loomyard/wts/gitexec-error-shape-decision/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/hanf/Code/loomyard/wts/gitexec-error-shape-decision`
...
```
/home/hanf/Code/loomyard/wts/gitexec-error-shape-decision/_mill/plan/01-verdict-record.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/hanf/Code/loomyard/wts/gitexec-error-shape-decision/_mill/reviews/20260811-041220-code-review-r1.md`.
...
Run each from `/home/hanf/Code/loomyard/wts/gitexec-error-shape-decision` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/hanf/Code/loomyard/wts/gitexec-error-shape-decision rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/hanf/Code/loomyard/wts/gitexec-error-shape-decision status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c7932e75-a51f-4b7e-9b47-a1cf660cc1c3"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `c7932e75-a51f-4b7e-9b47-a1cf660cc1c3` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c7932e75-a51f-4b7e-9b47-a1cf660cc1c3"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
