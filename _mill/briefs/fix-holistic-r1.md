# Holistic Fixer Brief — Extract shared primitives (paths, output)
...
- **Holistic review file:** `C:\Code\mhgo\wts\mhgo-extract-primitives\_mill\reviews\20260611-094808-code-review-r1.md`
- **Plan overview:** `C:\Code\mhgo\wts\mhgo-extract-primitives\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\mhgo\wts\mhgo-extract-primitives`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\mhgo\wts\mhgo-extract-primitives\_mill\reviews\20260611-094808-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\mhgo\wts\mhgo-extract-primitives` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"fa4ad424-af92-48a5-8a6a-30ee2fe17ae6"}
```
...
**`session_id` MUST be exactly `fa4ad424-af92-48a5-8a6a-30ee2fe17ae6` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fa4ad424-af92-48a5-8a6a-30ee2fe17ae6"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
