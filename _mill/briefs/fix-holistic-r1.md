# Holistic Fixer Brief — shed: land the producer-typology decision (atomicity carve-out) and sweep remaining doc contradictions
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/shed-producer-typology-sweep/_mill/reviews/20260811-154543-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/shed-producer-typology-sweep/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/shed-producer-typology-sweep`
...
```
/home/knatte/Code/loomyard/wts/shed-producer-typology-sweep/_mill/plan/01-shed-md-and-constraints.md
/home/knatte/Code/loomyard/wts/shed-producer-typology-sweep/_mill/plan/02-loom-md-pointers-and-kind-column.md
/home/knatte/Code/loomyard/wts/shed-producer-typology-sweep/_mill/plan/03-roadmap-overview-hardener-followups.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/shed-producer-typology-sweep/_mill/reviews/20260811-154543-code-review-r1.md`.
...
Run each from `/home/knatte/Code/loomyard/wts/shed-producer-typology-sweep` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/shed-producer-typology-sweep rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/shed-producer-typology-sweep status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"3ba157e1-6fa7-4af8-a0ed-b09e53b615c4"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `3ba157e1-6fa7-4af8-a0ed-b09e53b615c4` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"3ba157e1-6fa7-4af8-a0ed-b09e53b615c4"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
