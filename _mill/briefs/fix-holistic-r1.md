...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete/_mill/reviews/20260919-061247-code-review-r1.md`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/fabric-remote-branch-delete/_mill/reviews/20260919-061247-code-review-r1.md`.
...
never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C /home/knatte/Code/loomyard/wts/fabric-remote-branch-delete status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b4d48db0-9d2f-40fc-9b9d-f8be1a0a97ec"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `b4d48db0-9d2f-40fc-9b9d-f8be1a0a97ec` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b4d48db0-9d2f-40fc-9b9d-f8be1a0a97ec"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
