# Implementer Brief — fabric: unify warp + weft into one git-coordination module / fabric-topology-mechanics
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/fabric/_mill/plan/03-fabric-topology-mechanics.md`
- **Overview (for `## Shared Decisions` only):** `/home/knatte/Code/loomyard/wts/fabric/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/knatte/Code/loomyard/wts/fabric`
...
**Resume-after-incomplete:** When `` is non-empty, you are being re-dispatched to finish a partially-completed batch. Before implementing any cards, identify which cards are already committed: run `git -C /home/knatte/Code/loomyard/wts/fabric log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages. When `` is empty, derive the range start via `git -C /home/knatte/Code/loomyard/wts/fabric log --grep="^mill-go: start batch" -n 1 --format=%H`. Implement only the remaining cards — do not re-edit or re-commit cards whose `Commit:` message already appears in the log.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/fabric/_mill/plan/03-fabric-topology-mechanics.md`.
   - Commit the plan edit first (`plan: extend fabric-topology-mechanics refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"722842f1-5d6c-45f9-9563-ca39a099b44c"}
   ```
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C /home/knatte/Code/loomyard/wts/fabric status --porcelain --untracked-files=no`. If it shows ANY tracked in-scope modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change. The finalize gate now mechanically rejects a success report when in-scope files are dirty, so an uncommitted change will demote your report to stuck regardless.

**Card-count self-check (mandatory before writing your free-text turn summary):** Before stating anything about completion in your prose summary to the Builder/operator, count how many cards you actually committed versus how many the batch file declares. Determine the range start exactly as in "Resume-after-incomplete" above: use `` when non-empty, else `git -C /home/knatte/Code/loomyard/wts/fabric log --grep="^mill-go: start batch" -n 1 --format=%H`. Run `git -C /home/knatte/Code/loomyard/wts/fabric log <range-start>..HEAD --oneline` and match commit subjects against the batch file's `## Cards` `Commit:` messages to get an exact count. Your free-text summary MUST state the real count honestly (e.g. "4 of 9 cards committed") — never write an unqualified "all complete"/"all done" claim without having actually verified the count this way. This applies regardless of which model is running this session: this check is what protects an operator who is only reading your chat summary from a false completion claim, independent of whatever the machine-readable JSON status line below says.

Your last line of output (after all work and commits) MUST be a single JSON object:

```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"722842f1-5d6c-45f9-9563-ca39a099b44c"}
```
...
**`session_id` MUST be exactly `722842f1-5d6c-45f9-9563-ca39a099b44c` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"722842f1-5d6c-45f9-9563-ca39a099b44c"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C /home/knatte/Code/loomyard/wts/fabric` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/knatte/Code/loomyard/wts/fabric`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/loomyard/wts/fabric show <parent-branch>:<path>` to read it without changing cwd.
