# Implementer Brief — board-modul (rename fra wiki) + _mhgo-konfigurasjon / cwd-activation-and-board-path
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\config-layer\_mill\plan\04-cwd-activation-and-board-path.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\mhgo\wts\config-layer\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\mhgo\wts\config-layer`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\config-layer\_mill\plan\04-cwd-activation-and-board-path.md`.
   - Commit the plan edit first (`plan: extend cwd-activation-and-board-path refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"9f1d5c12-85b0-407c-815e-030e4acadc7f"}
```
...
**`session_id` MUST be exactly `9f1d5c12-85b0-407c-815e-030e4acadc7f` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"9f1d5c12-85b0-407c-815e-030e4acadc7f"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\mhgo\wts\config-layer` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\mhgo\wts\config-layer`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\mhgo\wts\config-layer show <parent-branch>:<path>` to read it without changing cwd.
