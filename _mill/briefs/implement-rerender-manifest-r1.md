# Implementer Brief — Board fixes from sandbox run — payload keys, help, rerender / rerender-manifest
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\board-sandbox-fixes\_mill\plan\03-rerender-manifest.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\board-sandbox-fixes\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\board-sandbox-fixes`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\board-sandbox-fixes\_mill\plan\03-rerender-manifest.md`.
   - Commit the plan edit first (`plan: extend rerender-manifest refs for <short reason>`) and push via the wiki.
...
**`session_id` MUST be exactly `720ffdff-bcbc-47a3-a23b-f8c83fea8b97` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"720ffdff-bcbc-47a3-a23b-f8c83fea8b97"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\board-sandbox-fixes` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\loomyard\wts\board-sandbox-fixes`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\board-sandbox-fixes show <parent-branch>:<path>` to read it without changing cwd.
