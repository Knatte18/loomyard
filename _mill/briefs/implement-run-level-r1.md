# Implementer Brief — Master Builder: new, parallel fork-based implementation module / run-level
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/master-builder/_mill/plan/07-run-level.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/master-builder/_mill/plan/07-run-level.md`.
   - Commit the plan edit first (`plan: extend run-level refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"0828bb13-6d9c-4ac9-8436-981e795a920b"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"0828bb13-6d9c-4ac9-8436-981e795a920b"}
```
...
**`session_id` MUST be exactly `0828bb13-6d9c-4ac9-8436-981e795a920b` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"0828bb13-6d9c-4ac9-8436-981e795a920b"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
