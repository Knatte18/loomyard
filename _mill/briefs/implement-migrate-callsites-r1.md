# Implementer Brief — Extract internal/fslink cross-OS link primitive / migrate-callsites
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\extract-fslink\_mill\plan\02-migrate-callsites.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\extract-fslink\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\extract-fslink`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\extract-fslink\_mill\plan\02-migrate-callsites.md`.
   - Commit the plan edit first (`plan: extend migrate-callsites refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"0b6529db-74c7-4467-84cb-28da2eb19383"}
```
...
**`session_id` MUST be exactly `0b6529db-74c7-4467-84cb-28da2eb19383` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"0b6529db-74c7-4467-84cb-28da2eb19383"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
