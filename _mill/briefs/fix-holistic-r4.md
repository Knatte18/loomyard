# Holistic Fixer Brief — fabric: live-state integration harness (slice 13)
...
- **Holistic review file:** `/home/knatte/Code/loomyard/wts/fabric-live-state-harness/_mill/reviews/20260811-134443-code-review-r4.md`
- **Plan overview:** `/home/knatte/Code/loomyard/wts/fabric-live-state-harness/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/loomyard/wts/fabric-live-state-harness`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/loomyard/wts/fabric-live-state-harness/_mill/reviews/20260811-134443-code-review-r4.md`.
...
Run each from `/home/knatte/Code/loomyard/wts/fabric-live-state-harness` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/loomyard/wts/fabric-live-state-harness rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/loomyard/wts/fabric-live-state-harness status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"d7cdc8cb-e825-484b-9f26-17accc85f891"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `d7cdc8cb-e825-484b-9f26-17accc85f891` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"d7cdc8cb-e825-484b-9f26-17accc85f891"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/loomyard/wts/fabric-live-state-harness` for commits;
do not `cd`.
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/loomyard/wts/fabric-live-state-harness show <parent-branch>:<path>` to read it without changing cwd.
