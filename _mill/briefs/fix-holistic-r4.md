# Holistic Fixer Brief — weft engine: paths geometry, paired worktrees, lyx weft
...
- **Holistic review file:** `C:\Code\loomyard\wts\weft-engine\_mill\reviews\20260619-181555-code-review-r4.md`
- **Plan overview:** `C:\Code\loomyard\wts\weft-engine\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\loomyard\wts\weft-engine`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\loomyard\wts\weft-engine\_mill\reviews\20260619-181555-code-review-r4.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\loomyard\wts\weft-engine` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"7ab8c90f-536d-483e-9341-e60b4553917d"}
```
...
**`session_id` MUST be exactly `7ab8c90f-536d-483e-9341-e60b4553917d` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"7ab8c90f-536d-483e-9341-e60b4553917d"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\weft-engine` for commits; do not `cd`.
