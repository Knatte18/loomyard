...
You are an independent plan reviewer for **Crucible review spawn as effort-selectable Agent profiles**. You evaluate the complete plan (all batches) and produce a structured review.
...
Write your full report to this file: C:\Code\loomyard\wts\crucible-effort-agent-profiles\_mill\briefs\review-plan-holistic-r1.out.md

Any format the prompt above asks for (including a `MILL_REVIEW_BEGIN` / `MILL_REVIEW_END` wrapped report) is the content of C:\Code\loomyard\wts\crucible-effort-agent-profiles\_mill\briefs\review-plan-holistic-r1.out.md -- write it there, not into chat.

Your final chat message must be exactly one line and nothing else: `WROTE C:\Code\loomyard\wts\crucible-effort-agent-profiles\_mill\briefs\review-plan-holistic-r1.out.md`
