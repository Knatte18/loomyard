...
You are an independent plan reviewer for **Shuttle guarantees a started run is past its startup gates**.
...
- Enforcement is a **tripwire, not a completeness proof**, in the same sense the Completion Signal Invariant's own scan is: `internal/loomcli/bootstrap_test.go` asserts that the only production readers of the seed's driver **field** outside the `shedrun` package are `internal/loomcli` and the files named in its own `driverFieldReadCarveOuts`, scanning the field selector rather than the driver constants (the constants are legitimately named elsewhere, per the bullet above). Adding a reader fails it and forces a human to confirm, and each carve-out carries its justification beside the entry.
...
Every unprefixed path below is relative to `/home/hanf/Code/loomyard/wts/shuttle-start-guarantees-readiness`.
- `wiki/` paths are relative to `/home/hanf/Code/loomyard/wiki`

## Files included (N=55)

- _mill/plan/00-overview.md
- _mill/plan/01-shuttle-blocking-start.md
- _mill/plan/02-webster-startup-window-docs.md
...
Write your full report to this file: /home/hanf/Code/loomyard/wts/shuttle-start-guarantees-readiness/_mill/briefs/review-plan-holistic-r1.out.md

Any format the prompt above asks for (including a `MILL_REVIEW_BEGIN` / `MILL_REVIEW_END` wrapped report) is the content of /home/hanf/Code/loomyard/wts/shuttle-start-guarantees-readiness/_mill/briefs/review-plan-holistic-r1.out.md -- write it there, not into chat.

Your final chat message must be exactly one line and nothing else: `WROTE /home/hanf/Code/loomyard/wts/shuttle-start-guarantees-readiness/_mill/briefs/review-plan-holistic-r1.out.md`
