...
You are an independent plan reviewer for **CLI ergonomics from the sandbox run: config editor + warp error wrapping**. You evaluate the complete plan (all batches) and produce a structured review.
...
- C:\Code\loomyard\wts\sandbox-cli-ergonomics\internal\warpengine\clone_integration_test.go
