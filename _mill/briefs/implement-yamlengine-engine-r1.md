# Implementer Brief — Extract yamlengine and migrate config via lyx update / yamlengine-engine
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\yamlengine\_mill\plan\01-yamlengine-engine.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\yamlengine\_mill\plan\01-yamlengine-engine.md`.
   - Commit the plan edit first (`plan: extend yamlengine-engine refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"82764e2a-84aa-46db-a5b0-430e6b18cdc2"}
```
...
**`session_id` MUST be exactly `82764e2a-84aa-46db-a5b0-430e6b18cdc2` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"82764e2a-84aa-46db-a5b0-430e6b18cdc2"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
