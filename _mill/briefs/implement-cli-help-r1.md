# Implementer Brief — Board fixes from sandbox run — payload keys, help, rerender / cli-help
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\board-sandbox-fixes\_mill\plan\02-cli-help.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\board-sandbox-fixes\_mill\plan\02-cli-help.md`.
   - Commit the plan edit first (`plan: extend cli-help refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"ff252d7f-2c62-4b16-bdd9-93c95cbb1db6"}
```
...
**`session_id` MUST be exactly `ff252d7f-2c62-4b16-bdd9-93c95cbb1db6` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"ff252d7f-2c62-4b16-bdd9-93c95cbb1db6"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
