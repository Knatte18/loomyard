# Implementer Brief — Build internal/mux: the window to the world (overlay + strands + render) / muxcli
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\internal-mux\_mill\plan\06-muxcli.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\internal-mux\_mill\plan\06-muxcli.md`.
   - Commit the plan edit first (`plan: extend muxcli refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"fded9050-8f4e-46ef-93d9-64a8e0dae6a4"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"fded9050-8f4e-46ef-93d9-64a8e0dae6a4"}
```
...
**`session_id` MUST be exactly `fded9050-8f4e-46ef-93d9-64a8e0dae6a4` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fded9050-8f4e-46ef-93d9-64a8e0dae6a4"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
