# Implementer Brief — Introduce warp: the host↔weft-coordinated git module / warp-worktree-absorb
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\warp-module\_mill\plan\03-warp-worktree-absorb.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\warp-module\_mill\plan\03-warp-worktree-absorb.md`.
   - Commit the plan edit first (`plan: extend warp-worktree-absorb refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"055b488f-d82d-4e61-b008-4dbbd3a72c88"}
```
...
**`session_id` MUST be exactly `055b488f-d82d-4e61-b008-4dbbd3a72c88` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"055b488f-d82d-4e61-b008-4dbbd3a72c88"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
