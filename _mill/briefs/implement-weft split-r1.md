# Implementer Brief — Rename Cobra modules to <module>cli, extract kernels as <module>engine / weft split
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\02-weft-split.md`
...
1. `git -C C:\Code\loomyard\wts\cobra-cli-engine-sweep mv <old-path> <new-path>` FIRST
   (e.g. `git mv internal/weft/<file>.go internal/weftengine/<file>.go` or
   `…/weftcli/…`). Git records a rename — history preserved, diff stays small.
2. THEN apply **surgical `Edit`** calls ONLY to the lines that change: the `package`
   declaration (`package weft` → `package weftengine`/`weftcli`), import paths,
   identifier/type retargeting to `weftengine.*`, and the `Command()`/`RunCLI` seam split.
3. **Never** `Write` a whole file and then `git rm`/delete its old twin.
4. Use full-file `Write` **only** for a genuinely new file with no predecessor in the old
   package.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\02-weft-split.md`.
   - Commit the plan edit first (`plan: extend weft split refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"167f5b0b-34e4-47fa-84aa-03a2623a5dac"}
```
...
**`session_id` MUST be exactly `167f5b0b-34e4-47fa-84aa-03a2623a5dac` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"167f5b0b-34e4-47fa-84aa-03a2623a5dac"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
