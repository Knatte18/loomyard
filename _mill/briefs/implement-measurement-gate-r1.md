# Implementer Brief — Reed attach dot-fill render artifact on resize and cross-client mouse move / measurement-gate
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/reed-attach-dotfill-artifact/_mill/plan/02-measurement-gate.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/reed-attach-dotfill-artifact/_mill/plan/02-measurement-gate.md`.
   - Commit the plan edit first (`plan: extend measurement-gate refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"5ba1587c-ec4f-4bbf-8ee2-42041c346a2c"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"5ba1587c-ec4f-4bbf-8ee2-42041c346a2c","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `5ba1587c-ec4f-4bbf-8ee2-42041c346a2c` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"5ba1587c-ec4f-4bbf-8ee2-42041c346a2c","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"5ba1587c-ec4f-4bbf-8ee2-42041c346a2c"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
