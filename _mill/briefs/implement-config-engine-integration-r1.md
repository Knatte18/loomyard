# Implementer Brief — Extract yamlengine and migrate config via lyx update / config-engine-integration
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\yamlengine\_mill\plan\05-config-engine-integration.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\yamlengine\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\yamlengine`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\yamlengine\_mill\plan\05-config-engine-integration.md`.
   - Commit the plan edit first (`plan: extend config-engine-integration refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"a796454b-a127-4218-b832-0fea9988817d"}
```
...
**`session_id` MUST be exactly `a796454b-a127-4218-b832-0fea9988817d` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"a796454b-a127-4218-b832-0fea9988817d"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\yamlengine` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\loomyard\wts\yamlengine`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
