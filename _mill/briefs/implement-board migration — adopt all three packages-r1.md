# Implementer Brief — Extract shared infrastructure (config, git, lock) / board migration — adopt all three packages
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\04-board-migration.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\mhgo\wts\extract-shared-infra`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\04-board-migration.md`.
   - Commit the plan edit first (`plan: extend board migration — adopt all three packages refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b10cdaef-aa13-4a29-93b5-03afc40ab344"}
```
...
**`session_id` MUST be exactly `b10cdaef-aa13-4a29-93b5-03afc40ab344` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b10cdaef-aa13-4a29-93b5-03afc40ab344"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
