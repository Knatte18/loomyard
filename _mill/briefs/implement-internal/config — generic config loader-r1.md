# Implementer Brief — Extract shared infrastructure (config, git, lock) / internal/config — generic config loader
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\03-internal-config.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\03-internal-config.md`.
   - Commit the plan edit first (`plan: extend internal/config — generic config loader refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"86a2683f-da76-409e-a34e-7d26dbdb16e9"}
```
...
**`session_id` MUST be exactly `86a2683f-da76-409e-a34e-7d26dbdb16e9` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"86a2683f-da76-409e-a34e-7d26dbdb16e9"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
