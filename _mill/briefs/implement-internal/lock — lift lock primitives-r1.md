# Implementer Brief — Extract shared infrastructure (config, git, lock) / internal/lock — lift lock primitives
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\01-internal-lock.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\extract-shared-infra\_mill\plan\01-internal-lock.md`.
   - Commit the plan edit first (`plan: extend internal/lock — lift lock primitives refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"40613790-1c94-4b85-bfce-3c3c6d8b4932"}
```
...
**`session_id` MUST be exactly `40613790-1c94-4b85-bfce-3c3c6d8b4932` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"40613790-1c94-4b85-bfce-3c3c6d8b4932"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
