# Implementer Brief — Reconcile stale design docs (stateless + weft model) / docs-sweep-and-moves
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\reconcile-stale-docs\_mill\plan\02-docs-sweep-and-moves.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\reconcile-stale-docs\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\reconcile-stale-docs`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\reconcile-stale-docs\_mill\plan\02-docs-sweep-and-moves.md`.
   - Commit the plan edit first (`plan: extend docs-sweep-and-moves refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"7f8f01df-aadf-49c1-ae6a-94e34e27c3ad"}
```
...
**`session_id` MUST be exactly `7f8f01df-aadf-49c1-ae6a-94e34e27c3ad` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"7f8f01df-aadf-49c1-ae6a-94e34e27c3ad"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\reconcile-stale-docs` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\loomyard\wts\reconcile-stale-docs`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
