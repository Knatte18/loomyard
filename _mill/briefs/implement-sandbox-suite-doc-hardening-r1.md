# Implementer Brief — Fix lyx CLI defects + host-commit gap from the sandbox run / sandbox-suite-doc-hardening
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\lyx-sandbox-fixes\_mill\plan\03-sandbox-suite-doc-hardening.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\lyx-sandbox-fixes\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\lyx-sandbox-fixes`
...
**Resume-after-incomplete:** When `` is non-empty, you are being re-dispatched to finish a partially-completed batch. Before implementing any cards, identify which cards are already committed: run `git -C C:\Code\loomyard\wts\lyx-sandbox-fixes log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages. When `` is empty, derive the range start via `git -C C:\Code\loomyard\wts\lyx-sandbox-fixes log --grep="^mill-go: start batch" -n 1 --format=%H`. Implement only the remaining cards — do not re-edit or re-commit cards whose `Commit:` message already appears in the log.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\lyx-sandbox-fixes\_mill\plan\03-sandbox-suite-doc-hardening.md`.
   - Commit the plan edit first (`plan: extend sandbox-suite-doc-hardening refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"fa2841da-31f3-483d-baf4-b1c119b4b7cd"}
   ```
...
**`session_id` MUST be exactly `fa2841da-31f3-483d-baf4-b1c119b4b7cd` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fa2841da-31f3-483d-baf4-b1c119b4b7cd"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\lyx-sandbox-fixes` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\loomyard\wts\lyx-sandbox-fixes`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\lyx-sandbox-fixes show <parent-branch>:<path>` to read it without changing cwd.
