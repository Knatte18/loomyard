# Implementer Brief — Extract internal/proc (cross-OS windowless + detached spawn) / Create internal/proc
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\extract-internal-proc\_mill\plan\01-create-internal-proc.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\extract-internal-proc\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\extract-internal-proc`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\extract-internal-proc\_mill\plan\01-create-internal-proc.md`.
   - Commit the plan edit first (`plan: extend Create internal/proc refs for <short reason>`) and push via the wiki.
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C C:\Code\loomyard\wts\extract-internal-proc status --porcelain --untracked-files=no`. If it shows ANY tracked in-scope modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change. The finalize gate now mechanically rejects a success report when in-scope files are dirty, so an uncommitted change will demote your report to stuck regardless.

Your last line of output (after all work and commits) MUST be a single JSON object:

```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4f0dc0aa-da3f-467b-99e5-523dce7bad1c"}
```
...
**`session_id` MUST be exactly `4f0dc0aa-da3f-467b-99e5-523dce7bad1c` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4f0dc0aa-da3f-467b-99e5-523dce7bad1c"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\extract-internal-proc` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\loomyard\wts\extract-internal-proc`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\extract-internal-proc show <parent-branch>:<path>` to read it without changing cwd.
