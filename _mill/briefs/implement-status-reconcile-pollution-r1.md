# Implementer Brief — Introduce warp: the host↔weft-coordinated git module / status-reconcile-pollution
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\warp-module\_mill\plan\06-status-reconcile-pollution.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\warp-module\_mill\plan\06-status-reconcile-pollution.md`.
   - Commit the plan edit first (`plan: extend status-reconcile-pollution refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"30102e00-3651-4f5f-a330-e9944be75f4b"}
```
...
**`session_id` MUST be exactly `30102e00-3651-4f5f-a330-e9944be75f4b` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"30102e00-3651-4f5f-a330-e9944be75f4b"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
