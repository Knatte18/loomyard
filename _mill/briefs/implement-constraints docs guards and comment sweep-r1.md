# Implementer Brief — Rename Cobra modules to <module>cli, extract kernels as <module>engine / constraints docs guards and comment sweep
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\08-constraints-docs-guards.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\08-constraints-docs-guards.md`.
   - Commit the plan edit first (`plan: extend constraints docs guards and comment sweep refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"0669c08f-1b70-4dee-a60a-3462a675ef87"}
```
...
**`session_id` MUST be exactly `0669c08f-1b70-4dee-a60a-3462a675ef87` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"0669c08f-1b70-4dee-a60a-3462a675ef87"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
