# Implementer Brief — Spike: structured Go reference/call-graph lookup (go/packages / gopls) / revert-and-verify
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/codeintel-spike/_mill/plan/04-revert-and-verify.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/codeintel-spike/_mill/plan/04-revert-and-verify.md`.
   - Commit the plan edit first (`plan: extend revert-and-verify refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"619a4beb-9d0c-4d9f-84b8-5f6290357b7a"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"619a4beb-9d0c-4d9f-84b8-5f6290357b7a"}
```
...
**`session_id` MUST be exactly `619a4beb-9d0c-4d9f-84b8-5f6290357b7a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"619a4beb-9d0c-4d9f-84b8-5f6290357b7a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
