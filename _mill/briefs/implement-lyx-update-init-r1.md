# Implementer Brief — Extract yamlengine and migrate config via lyx update / lyx-update-init
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\yamlengine\_mill\plan\06-lyx-update-init.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\yamlengine\_mill\plan\06-lyx-update-init.md`.
   - Commit the plan edit first (`plan: extend lyx-update-init refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"35c1f07a-f244-4332-93ca-3c1a8c16e360"}
```
...
**`session_id` MUST be exactly `35c1f07a-f244-4332-93ca-3c1a8c16e360` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"35c1f07a-f244-4332-93ca-3c1a8c16e360"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
