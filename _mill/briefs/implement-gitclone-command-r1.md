# Implementer Brief — ly-git-clone hub-creator (host, weft, board) / gitclone-command
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\ly-git-clone\_mill\plan\01-gitclone-command.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\ly-git-clone\_mill\plan\01-gitclone-command.md`.
   - Commit the plan edit first (`plan: extend gitclone-command refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"efa75c0d-dc25-4a19-ac20-a38874fa2bd4"}
```
...
**`session_id` MUST be exactly `efa75c0d-dc25-4a19-ac20-a38874fa2bd4` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"efa75c0d-dc25-4a19-ac20-a38874fa2bd4"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
