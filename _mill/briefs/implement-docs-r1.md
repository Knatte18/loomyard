# Implementer Brief — Optimise and slim the rest of the test suite / docs
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\optimize-remaining-test-suites\_mill\plan\05-docs.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\optimize-remaining-test-suites\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\optimize-remaining-test-suites`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\optimize-remaining-test-suites\_mill\plan\05-docs.md`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"8ebf5e78-5560-44d5-8a16-4b5a6e1deebf"}
```
...
**`session_id` MUST be exactly `8ebf5e78-5560-44d5-8a16-4b5a6e1deebf` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"8ebf5e78-5560-44d5-8a16-4b5a6e1deebf"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\optimize-remaining-test-suites` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\loomyard\wts\optimize-remaining-test-suites`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
