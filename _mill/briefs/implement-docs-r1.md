# Implementer Brief — Extract yamlengine and migrate config via lyx update / docs
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\yamlengine\_mill\plan\07-docs.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\yamlengine\_mill\plan\07-docs.md`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b64970f9-e1e3-4687-8de2-18b04e96d8e3"}
```
...
**`session_id` MUST be exactly `b64970f9-e1e3-4687-8de2-18b04e96d8e3` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b64970f9-e1e3-4687-8de2-18b04e96d8e3"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
