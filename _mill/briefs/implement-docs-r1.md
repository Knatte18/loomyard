# Implementer Brief — internal/paths: subpath init + mirrored system dirs / docs
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\paths-subpath-mirroring\_mill\plan\03-docs.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\paths-subpath-mirroring\_mill\plan\03-docs.md`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"79f07215-2382-4299-bb6c-4d4bd268dc37"}
```
...
**`session_id` MUST be exactly `79f07215-2382-4299-bb6c-4d4bd268dc37` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"79f07215-2382-4299-bb6c-4d4bd268dc37"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
