# Implementer Brief — Dedicated sandbox suite for mux / docs
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\mux-sandbox-suite\_mill\plan\03-docs.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\mux-sandbox-suite\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\mux-sandbox-suite`
...
**Resume-after-incomplete:** When `` is non-empty, you are being re-dispatched to finish a partially-completed batch. Before implementing any cards, identify which cards are already committed: run `git -C C:\Code\loomyard\wts\mux-sandbox-suite log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages. When `` is empty, derive the range start via `git -C C:\Code\loomyard\wts\mux-sandbox-suite log --grep="^mill-go: start batch" -n 1 --format=%H`. Implement only the remaining cards — do not re-edit or re-commit cards whose `Commit:` message already appears in the log.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\mux-sandbox-suite\_mill\plan\03-docs.md`.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"658b94af-515e-40e2-b366-8ece2fedfe41"}
   ```
...
- Before reporting any failure as "pre-existing" or "unrelated to my changes", confirm the failure reproduces on the parent branch `internal-mux`:
  - Run `git log internal-mux..HEAD -- <files in the failure's import/dependency chain>`. If a same-task commit touches those files, the failure is NOT pre-existing -- fix it.
  - Or run `git show internal-mux:<path>` to inspect the parent's version of the failing file. If the failure does not exist on the parent, it is in-scope: fix it, or escalate `logic` -- never label it "pre-existing verify".
  - If `internal-mux` is empty (the token renders as an empty string), skip the parent-reproduction check entirely and treat the failure as in-scope.
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C C:\Code\loomyard\wts\mux-sandbox-suite status --porcelain --untracked-files=no`. If it shows ANY tracked in-scope modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change. The finalize gate now mechanically rejects a success report when in-scope files are dirty, so an uncommitted change will demote your report to stuck regardless.

Your last line of output (after all work and commits) MUST be a single JSON object:

```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"658b94af-515e-40e2-b366-8ece2fedfe41"}
```
...
**`session_id` MUST be exactly `658b94af-515e-40e2-b366-8ece2fedfe41` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"658b94af-515e-40e2-b366-8ece2fedfe41"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
- `verify` — `verify:` still failing after 2 self-fix attempts. Before using this type, you MUST verify the failure is NOT pre-existing by checking `internal-mux` (see `## Verify` above). Only use `verify` when you have confirmed the failure is not pre-existing OR when `internal-mux` is empty.
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\mux-sandbox-suite` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\loomyard\wts\mux-sandbox-suite`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\mux-sandbox-suite show <parent-branch>:<path>` to read it without changing cwd.
