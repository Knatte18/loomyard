...
### [BLOCKING:scope] Watch path has a second hardcoded `loom` literal
...
### [BLOCKING:decision] loom `status`'s third error path has no stated home
...
### [BLOCKING:decision] `pause`'s absent-file refusal is undecided for lifecycle
**Section:** `pause-and-watch-generalize`
