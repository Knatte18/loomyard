# Implementer Brief — CLI help & error ergonomics from sandbox run / warp-features
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\plan\03-warp-features.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\plan\03-warp-features.md`.
   - Commit the plan edit first (`plan: extend warp-features refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"ea7e3da5-26c1-4844-9281-ee4bbb7da83c"}
```
...
**`session_id` MUST be exactly `ea7e3da5-26c1-4844-9281-ee4bbb7da83c` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"ea7e3da5-26c1-4844-9281-ee4bbb7da83c"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
