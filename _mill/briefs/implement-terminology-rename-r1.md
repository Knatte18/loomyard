# Implementer Brief — Weft repo — companion-repo overlay for lyx / terminology-rename
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\weft-repo\_mill\plan\01-terminology-rename.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\weft-repo\_mill\plan\01-terminology-rename.md`.
   - Commit the plan edit first (`plan: extend terminology-rename refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"080e8c1e-61aa-4948-95f9-2f7d07aae7b7"}
```
...
**`session_id` MUST be exactly `080e8c1e-61aa-4948-95f9-2f7d07aae7b7` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"080e8c1e-61aa-4948-95f9-2f7d07aae7b7"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
