# Implementer Brief — Rename Cobra modules to <module>cli, extract kernels as <module>engine / muxpoc rename
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\06-muxpoc-rename.md`
...
## ⛔ READ FIRST — THIS IS A PURE DIRECTORY RENAME, USE `git mv`

This batch renames `internal/muxpoc` → `internal/muxpoccli` with **no** cli/engine split
and **no** file content changes beyond the `package` clause. The card's `Creates:`/
`Deletes:` lists describe the END STATE, not the mechanic.

**Mandatory procedure:**
1. `git -C C:\Code\loomyard\wts\cobra-cli-engine-sweep mv internal/muxpoc internal/muxpoccli`
   to move the **whole directory at once** — git records every file as a rename, history
   preserved, diff stays minimal.
2. THEN apply **surgical `Edit`** calls ONLY for the `package muxpoc` → `package muxpoccli`
   line in each file (production and test), plus the sole importer in `cmd/lyx/main.go`
   (`internal/muxpoc` → `internal/muxpoccli`, `muxpoc.Command()` → `muxpoccli.Command()`).
3. Preserve platform build constraints verbatim. **Never** `Write` a file from scratch and
   delete its old twin. No full-file rewrites — content is byte-identical except the package
   clause.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\06-muxpoc-rename.md`.
   - Commit the plan edit first (`plan: extend muxpoc rename refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"aa9e6430-5b6f-42bf-94a8-88a0e3c24baf"}
```
...
**`session_id` MUST be exactly `aa9e6430-5b6f-42bf-94a8-88a0e3c24baf` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"aa9e6430-5b6f-42bf-94a8-88a0e3c24baf"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
