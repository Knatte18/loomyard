# Implementer Brief — Fix failing TestRunCLI in internal/worktree / worktree-fix
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\fix-worktree-runcli-test\_mill\plan\01-worktree-fix.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\fix-worktree-runcli-test\_mill\plan\01-worktree-fix.md`.
   - Commit the plan edit first (`plan: extend worktree-fix refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"9ed07ad7-ad8b-47f3-9150-205be9ac9f07"}
```
...
**`session_id` MUST be exactly `9ed07ad7-ad8b-47f3-9150-205be9ac9f07` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"9ed07ad7-ad8b-47f3-9150-205be9ac9f07"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
