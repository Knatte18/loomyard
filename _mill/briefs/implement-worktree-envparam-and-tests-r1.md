# Implementer Brief — Optimise and slim the test suite / worktree-envparam-and-tests
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\optimize-test-suite\_mill\plan\03-worktree-envparam-and-tests.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\optimize-test-suite\_mill\plan\03-worktree-envparam-and-tests.md`.
   - Commit the plan edit first (`plan: extend worktree-envparam-and-tests refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"ba8830cd-c6ba-45e3-8bd4-c0951f986ec8"}
```
...
**`session_id` MUST be exactly `ba8830cd-c6ba-45e3-8bd4-c0951f986ec8` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"ba8830cd-c6ba-45e3-8bd4-c0951f986ec8"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
