# Implementer Brief — Harden the Path Invariant: close enforcement hole + fix geometry leaks / paths-foundation
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\harden-path-invariant\_mill\plan\01-paths-foundation.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\harden-path-invariant\_mill\plan\01-paths-foundation.md`.
   - Commit the plan edit first (`plan: extend paths-foundation refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"1921b511-fcb0-4105-bcc9-d8e05f308845"}
```
...
**`session_id` MUST be exactly `1921b511-fcb0-4105-bcc9-d8e05f308845` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"1921b511-fcb0-4105-bcc9-d8e05f308845"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
