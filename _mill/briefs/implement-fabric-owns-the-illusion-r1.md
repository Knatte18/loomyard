# Implementer Brief — fabric: shrink hubgeometry to the minimal illusion primitive (slice 7) / fabric-owns-the-illusion
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/fabric-illusion-core/_mill/plan/06-fabric-owns-the-illusion.md`
...
**Resume-after-incomplete:** When `9cf2e32e6bdbad8577c2404673f69a87ff61352c` is non-empty, you are being re-dispatched to finish a partially-completed batch.
Before implementing any cards, identify which cards are already committed: run `git -C /home/knatte/Code/loomyard/wts/fabric-illusion-core log 9cf2e32e6bdbad8577c2404673f69a87ff61352c..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages.
When `9cf2e32e6bdbad8577c2404673f69a87ff61352c` is empty, derive the range start via `git -C /home/knatte/Code/loomyard/wts/fabric-illusion-core log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/fabric-illusion-core/_mill/plan/06-fabric-owns-the-illusion.md`.
   - Commit the plan edit first (`plan: extend fabric-owns-the-illusion refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"0ac50a79-6c67-4a32-9707-7ecec36e8f09"}
   ```
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C /home/knatte/Code/loomyard/wts/fabric-illusion-core status --porcelain --untracked-files=no`.
...
Determine the range start exactly as in "Resume-after-incomplete" above: use `9cf2e32e6bdbad8577c2404673f69a87ff61352c` when non-empty, else `git -C /home/knatte/Code/loomyard/wts/fabric-illusion-core log --grep="^mill-go: start batch" -n 1 --format=%H`.
Run `git -C /home/knatte/Code/loomyard/wts/fabric-illusion-core log <range-start>..HEAD --oneline` and match commit subjects against the batch file's `## Cards` `Commit:` messages to get an exact count -- Commit: none cards are never expected to appear in this log;
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"0ac50a79-6c67-4a32-9707-7ecec36e8f09","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `0ac50a79-6c67-4a32-9707-7ecec36e8f09` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
**On a `--resume-incomplete` re-dispatch specifically:** if you independently re-verify that every card's requirements are already satisfied by the existing commit(s) since `9cf2e32e6bdbad8577c2404673f69a87ff61352c` and you make no new commit this turn, report `status: success` with `"already_complete": true` in the envelope, **in addition to** (not instead of) a `cards_done` array covering every card declared in this batch:

```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"0ac50a79-6c67-4a32-9707-7ecec36e8f09","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"0ac50a79-6c67-4a32-9707-7ecec36e8f09"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/loomyard/wts/fabric-illusion-core` for commits;
do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/knatte/Code/loomyard/wts/fabric-illusion-core`.**
