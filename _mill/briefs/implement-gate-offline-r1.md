# Implementer Brief — Optimise and slim the rest of the test suite / gate-offline
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\optimize-remaining-test-suites\_mill\plan\01-gate-offline.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\optimize-remaining-test-suites\_mill\plan\01-gate-offline.md`.
   - Commit the plan edit first (`plan: extend gate-offline refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"54bcf947-57a0-41ac-9994-9cd994d4ae96"}
```
...
**`session_id` MUST be exactly `54bcf947-57a0-41ac-9994-9cd994d4ae96` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"54bcf947-57a0-41ac-9994-9cd994d4ae96"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
