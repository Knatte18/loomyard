# Implementer Brief — CLI help & error ergonomics from sandbox run / weft-commit-doc
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\plan\04-weft-commit-doc.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\plan\04-weft-commit-doc.md`.
   - Commit the plan edit first (`plan: extend weft-commit-doc refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b3b58274-0cae-42bd-8431-77f64fe2049c"}
```
...
**`session_id` MUST be exactly `b3b58274-0cae-42bd-8431-77f64fe2049c` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b3b58274-0cae-42bd-8431-77f64fe2049c"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
