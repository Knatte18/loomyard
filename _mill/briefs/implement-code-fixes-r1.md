# Implementer Brief — Restore the Tier 1 floor: guards + perchengine / code-fixes
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\restore-tier1-floor\_mill\plan\01-code-fixes.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\restore-tier1-floor\_mill\plan\01-code-fixes.md`.
   - Commit the plan edit first (`plan: extend code-fixes refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"f73bf23e-deb9-4497-a53f-8deb63e6c6fe"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f73bf23e-deb9-4497-a53f-8deb63e6c6fe"}
```
...
**`session_id` MUST be exactly `f73bf23e-deb9-4497-a53f-8deb63e6c6fe` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f73bf23e-deb9-4497-a53f-8deb63e6c6fe"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
