# Implementer Brief — Speed up and stabilize the integration test tier / board-skip-seam-parallelize
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\optimize-integration-tier\_mill\plan\02-board-skip-seam-parallelize.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\optimize-integration-tier\_mill\plan\02-board-skip-seam-parallelize.md`.
   - Commit the plan edit first (`plan: extend board-skip-seam-parallelize refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"1df5cc1f-5339-40a2-aa18-73a49c48885e"}
```
...
**`session_id` MUST be exactly `1df5cc1f-5339-40a2-aa18-73a49c48885e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"1df5cc1f-5339-40a2-aa18-73a49c48885e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
