# Implementer Brief — Harden the Path Invariant: close enforcement hole + fix geometry leaks / warp-geometry
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\harden-path-invariant\_mill\plan\02-warp-geometry.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\harden-path-invariant\_mill\plan\02-warp-geometry.md`.
   - Commit the plan edit first (`plan: extend warp-geometry refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"8ca83ef7-b4ea-4f59-9134-2eb99bb3722e"}
```
...
**`session_id` MUST be exactly `8ca83ef7-b4ea-4f59-9134-2eb99bb3722e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"8ca83ef7-b4ea-4f59-9134-2eb99bb3722e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
