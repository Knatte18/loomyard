# Implementer Brief — Extract internal/fsx and build internal/state / state-package
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\extract-internal-fsx\_mill\plan\04-state-package.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\extract-internal-fsx\_mill\plan\04-state-package.md`.
   - Commit the plan edit first (`plan: extend state-package refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"a87c3c2b-163e-42e3-a51c-d0d4c1c9b3e3"}
```
...
**`session_id` MUST be exactly `a87c3c2b-163e-42e3-a51c-d0d4c1c9b3e3` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"a87c3c2b-163e-42e3-a51c-d0d4c1c9b3e3"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
