# Implementer Brief — Harden the Path Invariant: close enforcement hole + fix geometry leaks / board-paths-owned
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\harden-path-invariant\_mill\plan\03-board-paths-owned.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\harden-path-invariant\_mill\plan\03-board-paths-owned.md`.
   - Commit the plan edit first (`plan: extend board-paths-owned refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"a4092a5f-298e-4784-aa66-63d1ce8d138a"}
```
...
**`session_id` MUST be exactly `a4092a5f-298e-4784-aa66-63d1ce8d138a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"a4092a5f-298e-4784-aa66-63d1ce8d138a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
