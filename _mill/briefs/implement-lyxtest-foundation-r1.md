# Implementer Brief — Optimise and slim the test suite / lyxtest-foundation
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\optimize-test-suite\_mill\plan\01-lyxtest-foundation.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\optimize-test-suite\_mill\plan\01-lyxtest-foundation.md`.
   - Commit the plan edit first (`plan: extend lyxtest-foundation refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"66560b4a-0b1d-4c46-9c84-e55a39a822a9"}
```
...
**`session_id` MUST be exactly `66560b4a-0b1d-4c46-9c84-e55a39a822a9` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"66560b4a-0b1d-4c46-9c84-e55a39a822a9"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
