...
You are an independent discussion reviewer for **Build burler - the review+fix round worker**. Round **3**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\loomyard\wts\internal-burler\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
