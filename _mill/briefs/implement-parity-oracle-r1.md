# Implementer Brief — native clients: migrate gitrepo to go-git + selfreport gh-CLI to go-github / parity-oracle
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\native-clients-migration\_mill\plan\02-parity-oracle.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\native-clients-migration\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\native-clients-migration`
- **Wiki path (for plan-edit commits if needed):** `C:\Code\loomyard\wiki`
- Round: **1**
...
**Resume-after-incomplete:** When `` is non-empty, you are being re-dispatched to finish a partially-completed batch. Before implementing any cards, identify which cards are already committed: run `git -C C:\Code\loomyard\wts\native-clients-migration log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages. When `` is empty, derive the range start via `git -C C:\Code\loomyard\wts\native-clients-migration log --grep="^mill-go: start batch" -n 1 --format=%H`. Implement only the remaining cards — do not re-edit or re-commit cards whose `Commit:` message already appears in the log. A card whose Commit: is "none" never appears in this log by definition -- exclude it from this matching scan entirely. Treat a Commit: none card as complete once you have (re-)performed its Requirements: verification step this turn (or a prior turn, per your own judgment from the batch's current state); it needs no log entry to be considered done.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\native-clients-migration\_mill\plan\02-parity-oracle.md`.
   - Commit the plan edit first (`plan: extend parity-oracle refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"0c8bd158-ad12-41b6-912b-e7d937547b91"}
   ```
...
**`session_id` MUST be exactly `0c8bd158-ad12-41b6-912b-e7d937547b91` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"0c8bd158-ad12-41b6-912b-e7d937547b91","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"0c8bd158-ad12-41b6-912b-e7d937547b91"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\native-clients-migration` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\loomyard\wts\native-clients-migration`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\native-clients-migration show <parent-branch>:<path>` to read it without changing cwd.
