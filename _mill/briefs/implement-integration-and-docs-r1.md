# Implementer Brief — Build mhgo worktree module / integration-and-docs
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\mhgo-worktree-module\_mill\plan\05-integration-and-docs.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\mhgo-worktree-module\_mill\plan\05-integration-and-docs.md`.
   - Commit the plan edit first (`plan: extend integration-and-docs refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4773e24d-45f1-4c1c-83f9-8c3f81b0e680"}
```
...
**`session_id` MUST be exactly `4773e24d-45f1-4c1c-83f9-8c3f81b0e680` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4773e24d-45f1-4c1c-83f9-8c3f81b0e680"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
