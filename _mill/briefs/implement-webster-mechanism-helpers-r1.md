# Implementer Brief — webster: rewrite for flat card list / webster-mechanism-helpers
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/webster-rewrite/_mill/plan/05-webster-mechanism-helpers.md`
- **Overview (for `## Shared Decisions` only):** `/home/knatte/Code/loomyard/wts/webster-rewrite/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/knatte/Code/loomyard/wts/webster-rewrite`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/webster-rewrite/_mill/plan/05-webster-mechanism-helpers.md`.
   - Commit the plan edit first (`plan: extend webster-mechanism-helpers refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"6b4d5dc8-4f87-45a7-81dc-89fb60dc717b"}
   ```
...
**`session_id` MUST be exactly `6b4d5dc8-4f87-45a7-81dc-89fb60dc717b` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"6b4d5dc8-4f87-45a7-81dc-89fb60dc717b","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6b4d5dc8-4f87-45a7-81dc-89fb60dc717b"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/loomyard/wts/webster-rewrite show <parent-branch>:<path>` to read it without changing cwd.
