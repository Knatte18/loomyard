# Implementer Brief — Facilitate Linux support (Win11-side prep) / config-version-probe
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\facilitate-linux\_mill\plan\02-config-version-probe.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\facilitate-linux\_mill\plan\02-config-version-probe.md`.
   - Commit the plan edit first (`plan: extend config-version-probe refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"9262a5a1-f7e1-491c-a1f3-66b7aac33d9a"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"9262a5a1-f7e1-491c-a1f3-66b7aac33d9a"}
```
...
**`session_id` MUST be exactly `9262a5a1-f7e1-491c-a1f3-66b7aac33d9a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"9262a5a1-f7e1-491c-a1f3-66b7aac33d9a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
