# Implementer Brief — weft producers: _lyx/config, lyx config, codeguide / config-edit-machinery
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\weft-producers\_mill\plan\03-config-edit-machinery.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\weft-producers\_mill\plan\03-config-edit-machinery.md`.
   - Commit the plan edit first (`plan: extend config-edit-machinery refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"7cddad31-3417-4f1f-9807-048182388b1b"}
```
...
**`session_id` MUST be exactly `7cddad31-3417-4f1f-9807-048182388b1b` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"7cddad31-3417-4f1f-9807-048182388b1b"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
