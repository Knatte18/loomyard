# Implementer Brief — Fix lyx config --set dropping unrecognized keys + reconcile not detecting drift / configcli-json-envelope
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\config-set-key-loss\_mill\plan\03-configcli-json-envelope.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\config-set-key-loss\_mill\plan\03-configcli-json-envelope.md`.
   - Commit the plan edit first (`plan: extend configcli-json-envelope refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"f7bed8de-6e4a-4f50-8170-68ea2f954f4e"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f7bed8de-6e4a-4f50-8170-68ea2f954f4e"}
```
...
**`session_id` MUST be exactly `f7bed8de-6e4a-4f50-8170-68ea2f954f4e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f7bed8de-6e4a-4f50-8170-68ea2f954f4e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
