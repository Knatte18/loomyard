...
You are an independent code reviewer for **Replace reed's header pane with a status-line and Selvage**.
...
- overlay.go's execHook doc still names the deleted ensureHeaderPaneLocked: **Location:** `internal/reedengine/overlay.go:28`
- attach.go's AttachArgv doc still says "a lone header pane": **Location:** `internal/reedengine/attach.go:71-73`
- render/rules_test.go points at a test that was renamed: **Location:** `internal/reedengine/render/rules_test.go:320-322`
- cli_test.go's no-arg listing comment undercounts the verb list: **Location:** `internal/reedcli/cli_test.go:17-19`
- docs/overview.md's tokenvocab directory-tree entry omits the worktree token: **Location:** `docs/overview.md:261`
- reed-header-selvage.md's token-count sentence still says "the header's ... two tokens": **Location:** `manifest/designs/reed-header-selvage.md:25`
...
Every unprefixed path below is relative to `/home/knatte/Code/loomyard/wts/reed-header-selvage`.
- `wiki/` paths are relative to `/home/knatte/Code/loomyard/wiki`

## Files included (N=127)

- _mill/plan/00-overview.md
- _mill/plan/01-render-bottom-band.md
- _mill/plan/02-vocabulary-and-config.md
- _mill/plan/03-selvage-pane.md
- _mill/plan/04-status-line-pins.md
- _mill/plan/05-watchdog-daemon.md
- _mill/plan/06-standalone-watcher.md
...
Write your full report to this file: /home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/briefs/review-code-holistic-r2.out.md

Any format the prompt above asks for (including a `MILL_REVIEW_BEGIN` / `MILL_REVIEW_END` wrapped report) is the content of /home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/briefs/review-code-holistic-r2.out.md -- write it there, not into chat.

Your final chat message must be exactly one line and nothing else: `WROTE /home/knatte/Code/loomyard/wts/reed-header-selvage/_mill/briefs/review-code-holistic-r2.out.md`
