# Implementer Brief — Prune and consolidate the test suite (board first) / weft
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\prune-board-tests\_mill\plan\03-weft.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\prune-board-tests\_mill\plan\03-weft.md`.
   - Commit the plan edit first (`plan: extend weft refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"da58073c-f27e-4fad-a6c7-567708a1b78d"}
```
...
**`session_id` MUST be exactly `da58073c-f27e-4fad-a6c7-567708a1b78d` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"da58073c-f27e-4fad-a6c7-567708a1b78d"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
