# Implementer Brief — CLI help & error ergonomics from sandbox run / w16-reject-unknown-subcommand
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\plan\02-w16-reject-unknown-subcommand.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cli-help-ergonomics\_mill\plan\02-w16-reject-unknown-subcommand.md`.
   - Commit the plan edit first (`plan: extend w16-reject-unknown-subcommand refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"de0e97bc-71a4-4771-ad09-fe19c667a88c"}
```
...
**`session_id` MUST be exactly `de0e97bc-71a4-4771-ad09-fe19c667a88c` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"de0e97bc-71a4-4771-ad09-fe19c667a88c"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
