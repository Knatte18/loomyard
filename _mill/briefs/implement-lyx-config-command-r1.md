# Implementer Brief — weft producers: _lyx/config, lyx config, codeguide / lyx-config-command
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\weft-producers\_mill\plan\04-lyx-config-command.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\weft-producers\_mill\plan\04-lyx-config-command.md`.
   - Commit the plan edit first (`plan: extend lyx-config-command refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"20d94d9f-82b3-4606-a0f0-5bc961b0d92f"}
```
...
**`session_id` MUST be exactly `20d94d9f-82b3-4606-a0f0-5bc961b0d92f` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"20d94d9f-82b3-4606-a0f0-5bc961b0d92f"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
