# Implementer Brief — Optimise and slim the rest of the test suite / ide-fixtures
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\optimize-remaining-test-suites\_mill\plan\03-ide-fixtures.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\optimize-remaining-test-suites\_mill\plan\03-ide-fixtures.md`.
   - Commit the plan edit first (`plan: extend ide-fixtures refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"ce6ab8bc-8b73-4984-be56-89085378c9ed"}
```
...
**`session_id` MUST be exactly `ce6ab8bc-8b73-4984-be56-89085378c9ed` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"ce6ab8bc-8b73-4984-be56-89085378c9ed"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
