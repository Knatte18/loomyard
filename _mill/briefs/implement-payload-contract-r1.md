# Implementer Brief — Board fixes from sandbox run — payload keys, help, rerender / payload-contract
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\board-sandbox-fixes\_mill\plan\01-payload-contract.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\board-sandbox-fixes\_mill\plan\01-payload-contract.md`.
   - Commit the plan edit first (`plan: extend payload-contract refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"50dad741-3773-4f93-b471-b32bdf1d17e7"}
```
...
**`session_id` MUST be exactly `50dad741-3773-4f93-b471-b32bdf1d17e7` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"50dad741-3773-4f93-b471-b32bdf1d17e7"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
