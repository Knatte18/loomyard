# Implementer Brief — Spike: structured Go reference/call-graph lookup (go/packages / gopls) / poc-gopls-callgraph
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/codeintel-spike/_mill/plan/02-poc-gopls-callgraph.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/codeintel-spike/_mill/plan/02-poc-gopls-callgraph.md`.
   - Commit the plan edit first (`plan: extend poc-gopls-callgraph refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"5fc2ce0f-438f-4406-9774-7e1f2e5f93dd"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"5fc2ce0f-438f-4406-9774-7e1f2e5f93dd"}
```
...
**`session_id` MUST be exactly `5fc2ce0f-438f-4406-9774-7e1f2e5f93dd` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"5fc2ce0f-438f-4406-9774-7e1f2e5f93dd"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
