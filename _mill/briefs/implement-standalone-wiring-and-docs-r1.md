# Implementer Brief — webster standalone mode: run refuses to start Master; logs write untracked into target repo / standalone-wiring-and-docs
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene/_mill/plan/03-standalone-wiring-and-docs.md`
- **Overview (for `## Shared Decisions` only):** `/home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene`
...
Before implementing any cards, identify which cards are already committed: run `git -C /home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages.
When `` is empty, derive the range start via `git -C /home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene/_mill/plan/03-standalone-wiring-and-docs.md`.
   - Commit the plan edit first (`plan: extend standalone-wiring-and-docs refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"58de4b23-4f7d-42a0-a5dc-84dd1315e3ae"}
   ```
...
- Before reporting any failure as "pre-existing" or "unrelated to my changes", confirm the failure reproduces on the parent branch `crucible-loom-glyph-hardening`:
  - Run `git log crucible-loom-glyph-hardening..HEAD -- <files in the failure's import/dependency chain>`.
    If a same-task commit touches those files, the failure is NOT pre-existing -- fix it.
  - Or run `git show crucible-loom-glyph-hardening:<path>` to inspect the parent's version of the failing file.
    If the failure does not exist on the parent, it is in-scope: fix it, or escalate `logic` -- never label it "pre-existing verify".
  - If `crucible-loom-glyph-hardening` is empty (the token renders as an empty string), skip the parent-reproduction check entirely and treat the failure as in-scope.
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C /home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene status --porcelain --untracked-files=no`.
...
Determine the range start exactly as in "Resume-after-incomplete" above: use `` when non-empty, else `git -C /home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene log --grep="^mill-go: start batch" -n 1 --format=%H`.
Run `git -C /home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene log <range-start>..HEAD --oneline` and match commit subjects against the batch file's `## Cards` `Commit:` messages to get an exact count -- Commit: none cards are never expected to appear in this log;
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"58de4b23-4f7d-42a0-a5dc-84dd1315e3ae","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `58de4b23-4f7d-42a0-a5dc-84dd1315e3ae` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"58de4b23-4f7d-42a0-a5dc-84dd1315e3ae","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"58de4b23-4f7d-42a0-a5dc-84dd1315e3ae"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
- `verify` — `verify:` still failing after 2 self-fix attempts.
  Before using this type, you MUST verify the failure is NOT pre-existing by checking `crucible-loom-glyph-hardening` (see `## Verify` above).
  Only use `verify` when you have confirmed the failure is not pre-existing OR when `crucible-loom-glyph-hardening` is empty.
...
Use `git -C /home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene` for commits;
do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene`.**
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/loomyard/wts/standalonegeom-webster-run-and-log-hygiene show <parent-branch>:<path>` to read it without changing cwd.
