# Implementer Brief — Ensure weft branches are orphan branches / weft-mirror-branching
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\weft-orphan-branches\_mill\plan\01-weft-mirror-branching.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\loomyard\wts\weft-orphan-branches\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\loomyard\wts\weft-orphan-branches`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\weft-orphan-branches\_mill\plan\01-weft-mirror-branching.md`.
   - Commit the plan edit first (`plan: extend weft-mirror-branching refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"295c2eb3-8901-4679-9459-51f479ed9f30"}
```
...
**`session_id` MUST be exactly `295c2eb3-8901-4679-9459-51f479ed9f30` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"295c2eb3-8901-4679-9459-51f479ed9f30"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\loomyard\wts\weft-orphan-branches` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\loomyard\wts\weft-orphan-branches`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\loomyard\wts\weft-orphan-branches show <parent-branch>:<path>` to read it without changing cwd.
