# Implementer Brief — internal/paths: subpath init + mirrored system dirs / paths-geometry
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\paths-subpath-mirroring\_mill\plan\01-paths-geometry.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\paths-subpath-mirroring\_mill\plan\01-paths-geometry.md`.
   - Commit the plan edit first (`plan: extend paths-geometry refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"fa8145c8-6543-4ae5-850f-49e25f79301b"}
```
...
**`session_id` MUST be exactly `fa8145c8-6543-4ae5-850f-49e25f79301b` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fa8145c8-6543-4ae5-850f-49e25f79301b"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
