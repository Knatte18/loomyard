# Implementer Brief — Extract internal/fslink cross-OS link primitive / fslink-package
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\extract-fslink\_mill\plan\01-fslink-package.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\extract-fslink\_mill\plan\01-fslink-package.md`.
   - Commit the plan edit first (`plan: extend fslink-package refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4466162c-2f24-493d-b093-650d5e789055"}
```
...
**`session_id` MUST be exactly `4466162c-2f24-493d-b093-650d5e789055` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4466162c-2f24-493d-b093-650d5e789055"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
