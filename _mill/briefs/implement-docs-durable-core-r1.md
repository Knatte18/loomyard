# Implementer Brief — Reconcile stale design docs (stateless + weft model) / docs-durable-core
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\reconcile-stale-docs\_mill\plan\01-docs-durable-core.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\reconcile-stale-docs\_mill\plan\01-docs-durable-core.md`.
   - Commit the plan edit first (`plan: extend docs-durable-core refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"cb4758c9-a2f2-4349-8141-1a82d19dbf52"}
```
...
**`session_id` MUST be exactly `cb4758c9-a2f2-4349-8141-1a82d19dbf52` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"cb4758c9-a2f2-4349-8141-1a82d19dbf52"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
