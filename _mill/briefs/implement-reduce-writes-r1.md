# Implementer Brief — Cut boardtest concurrency test run time / reduce-writes
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\boardtest-concurrency-speed\_mill\plan\01-reduce-writes.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\mhgo\wts\boardtest-concurrency-speed\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\mhgo\wts\boardtest-concurrency-speed`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\boardtest-concurrency-speed\_mill\plan\01-reduce-writes.md`.
   - Commit the plan edit first (`plan: extend reduce-writes refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"db191d1a-c68e-4691-a4ce-6fcb3879f9c0"}
```
...
**`session_id` MUST be exactly `db191d1a-c68e-4691-a4ce-6fcb3879f9c0` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"db191d1a-c68e-4691-a4ce-6fcb3879f9c0"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
- **If you need a file from the parent:** use `git -C C:\Code\mhgo\wts\boardtest-concurrency-speed show <parent-branch>:<path>` to read it without changing cwd.
