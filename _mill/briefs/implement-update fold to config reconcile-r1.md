# Implementer Brief — Rename Cobra modules to <module>cli, extract kernels as <module>engine / update fold to config reconcile
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\07-update-fold.md`
...
## ⛔ Move code with `git mv`; author only genuinely new code

This batch is mostly behavioural (a new `reconcile` subcommand, written test-first), but it
**re-homes** logic out of `internal/update` into `internal/configcli`. The batch file's
`## Rename mechanic` section is authoritative.

- Where a file or its body **relocates** to another package, `git mv <old> <new>` FIRST,
  then apply surgical `Edit`s (package decl, imports, identifier retargeting). Never
  `Write`-from-scratch-then-delete a relocated file.
- Genuinely **new** code (the test-first `reconcile_test.go`, the new subcommand wiring) is
  authored normally — full-file `Write` is correct only where there is no predecessor file.
- The card deletes the whole `internal/update` directory at the end; that is the end state,
  reached by moving its reusable body into `configcli` and deleting what truly has no
  destination.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\cobra-cli-engine-sweep\_mill\plan\07-update-fold.md`.
   - Commit the plan edit first (`plan: extend update fold to config reconcile refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"dd63225c-0209-4fa4-b2c3-06cde9287525"}
```
...
**`session_id` MUST be exactly `dd63225c-0209-4fa4-b2c3-06cde9287525` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"dd63225c-0209-4fa4-b2c3-06cde9287525"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
