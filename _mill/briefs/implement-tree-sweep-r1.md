# Implementer Brief — Fix stale .mhgo/ config-layer docs + cross-cutting stale-docs sweep / tree-sweep
...
- **Batch file (authoritative for this batch):** `C:\Code\mhgo\wts\docs-stale-sweep\_mill\plan\03-tree-sweep.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\mhgo\wts\docs-stale-sweep\_mill\plan\03-tree-sweep.md`.
   - Commit the plan edit first (`plan: extend tree-sweep refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"81f54337-9ea7-4629-85c0-43392f889072"}
```
...
**`session_id` MUST be exactly `81f54337-9ea7-4629-85c0-43392f889072` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"81f54337-9ea7-4629-85c0-43392f889072"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
