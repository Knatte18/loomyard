# Implementer Brief — Reconsider whether lyx mux needs anchor:top at all / dereference-consumers
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/mux-anchor-top-redesign/_mill/plan/01-dereference-consumers.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/mux-anchor-top-redesign/_mill/plan/01-dereference-consumers.md`.
   - Commit the plan edit first (`plan: extend dereference-consumers refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"2a9f6e76-2379-46f5-97b5-2ee0186acddc"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"2a9f6e76-2379-46f5-97b5-2ee0186acddc"}
```
...
**`session_id` MUST be exactly `2a9f6e76-2379-46f5-97b5-2ee0186acddc` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"2a9f6e76-2379-46f5-97b5-2ee0186acddc"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
