# Implementer Brief — Worktree spawn/teardown as Shed producers / loom run --no-attach
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/loomyard/wts/worktree-lifecycle-shed-producers/_mill/plan/04-loom-run-no-attach.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/loomyard/wts/worktree-lifecycle-shed-producers/_mill/plan/04-loom-run-no-attach.md`.
   - Commit the plan edit first (`plan: extend loom run --no-attach refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"67cdaade-34a7-4b42-b4c4-a937fea72b5e"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"67cdaade-34a7-4b42-b4c4-a937fea72b5e","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `67cdaade-34a7-4b42-b4c4-a937fea72b5e` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"67cdaade-34a7-4b42-b4c4-a937fea72b5e","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"67cdaade-34a7-4b42-b4c4-a937fea72b5e"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
