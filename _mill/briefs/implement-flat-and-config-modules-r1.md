# Implementer Brief — Built-in CLI help: lyx self-documents modules & commands / flat-and-config-modules
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\builtin-cli-help\_mill\plan\02-flat-and-config-modules.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\builtin-cli-help\_mill\plan\02-flat-and-config-modules.md`.
   - Commit the plan edit first (`plan: extend flat-and-config-modules refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c8fd9816-cacd-4f63-969e-ba9f663fad19"}
```
...
**`session_id` MUST be exactly `c8fd9816-cacd-4f63-969e-ba9f663fad19` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c8fd9816-cacd-4f63-969e-ba9f663fad19"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
