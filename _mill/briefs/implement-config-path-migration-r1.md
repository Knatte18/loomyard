# Implementer Brief — Weft repo — companion-repo overlay for lyx / config-path-migration
...
- **Batch file (authoritative for this batch):** `C:\Code\loomyard\wts\weft-repo\_mill\plan\02-config-path-migration.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\loomyard\wts\weft-repo\_mill\plan\02-config-path-migration.md`.
   - Commit the plan edit first (`plan: extend config-path-migration refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"a81ccdc1-f5f1-4427-9029-8511e17c0125"}
```
...
**`session_id` MUST be exactly `a81ccdc1-f5f1-4427-9029-8511e17c0125` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"a81ccdc1-f5f1-4427-9029-8511e17c0125"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
