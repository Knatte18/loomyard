...
4. **F16/F22 mill-wiki task: NOT YET OPENED.** These are real, confirmed-live, shipped defects in `webster`/`standalonegeom`, out of loom's own scope. Per Hard Rule 5 this orchestrator must open a proper mill-wiki task for them (never by hand-editing wiki files) — deliberately deferred pending the operator's go-ahead on which mill flow to use (raised to the operator in the same turn this handoff was last refreshed; check the conversation, not just this file, for their answer). Do not let this fall through — if several rounds pass with no task opened, that's a process failure to fix immediately, not something to keep deferring.
