...
## R3 outcome against the pre-count

Both R3 pre-count items were found by R3 itself: the typed-error/call-site wiring gap (F-WIRING-1) and the create-side SIGKILL mirror (F-SIGKILL-ADD, confirmed live at two crash points).

## R4 pre-count (at the HANDOFF commit that records R3, before R4 spawned)

- **Orchestrator-found by reading (not driven), expected in focus 2:** the incomplete-pair remedy (`incompletePairRemedy` in `internal/battencli/wire.go`) tells the operator `git branch -D <slug>`, but `Topology.Add` names the warp branch `cfg.BranchPrefix + slug`. With a non-empty `branch_prefix` the remedy names a branch that does not exist; the next create then hits `createRefusal`, which names the right branch (`ErrBranchExists.Branch`, prefix included) — recoverable, not destructive. Expected severity LOW. The remedy also never names the weft branch; `Add` adopts an existing weft branch (`weftBranchAdopted`), so that is likely correct — check the round's reasoning.
