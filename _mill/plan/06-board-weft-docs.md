...
- **Deletes:** none
- **Moves:** none
- **Requirements:** In the `## Requirements` section, change `- Git 2.35+ (for \`git worktree\`)` to `- Git 2.42+ (for \`git worktree add --orphan\`)` — the new minimum needed by `_board`'s fresh-bootstrap orphan-branch path (batch 2, `internal/fabricengine/boardweft.go`). README.md also carries its own copy of the topology diagram — line 45 currently reads `  └── _board/                       (board repo; the task store)`, the identical stale line Card 29 corrects in `docs/overview.md`; change it here too, to the same replacement: `  └── _board/                       (weft:main worktree; the task store)`.
- **Commit:** `docs(readme): bump minimum git version to 2.42+, fix stale _board topology-diagram line`
...
- **Deletes:**
  - `manifest/designs/board-weft-storage.md`
- **Moves:** none
- **Requirements:** Per the Documentation Lifecycle (`docs/overview.md#documentation-lifecycle`): module-design docs are deleted when their module lands, with durable rationale folded into the Go package doc comment next to the code — the exact pattern `internal/fabricengine/doc.go` already follows, and the pattern `board-weft-storage.md`'s own header line already commits to. Extend `board.go`'s package doc comment (already touched by batch 3's Card 14 for the git-routing rewrite) with new content folding in the durable parts of `board-weft-storage.md`: (a) the branch-reservation rationale from its "Branch naming convention" section — no task's weft branch can ever be named exactly the host's default branch (every paired weft branch carries the `-weft` suffix), which is what makes that unsuffixed name permanently unclaimed by the pairing convention and reserved exclusively for board; (b) the why-not-alternatives rationale from "Why the earlier approaches were rejected" — a separate repo is unneeded git-identity overhead, and GitHub wiki rendering requires whichever repo hosts the wiki to be public (in the old model, board's own separate repo — never the host/warp repo, a distinct, specific term in this codebase's own vocabulary, which `board-weft-storage.md`'s source text never claims needs to be public), disqualifying for private consulting work; (c) prime's asymmetric relationship to weft from "The 'prime' worktree's asymmetric relationship to weft" — prime is the only worktree with a reason to check out two weft branches simultaneously (its own ordinary `<name>-weft` companion, plus `weft:main` for board access, never paired with any warp branch); (d) the `fabric` consequence from "`fabric` consequence" — `weft:main` has no corresponding warp branch, so the `Warp-SHA` trailer/correspondence-index machinery does not apply to it; (e) one sentence preserving the design doc's forward-looking "Consequence" note (this repo's own `manifest/roadmap.md` and the mill wiki's task list are candidates to eventually fold into board, once loomyard's own development moves off mill onto lyx/loom) — not a change to make now, just recorded so it is not silently lost. Do NOT fold in the now-superseded "Board's data model" (the four-category Proposals/Manifest/Tasks/Done model) or "Two JSON stores, not one" sections' `curated`-flag design — `_mill/discussion.md`'s Decisions section replaced both with the plain two-file `tasks.json`/`notes.json` split with no curated flag; the package doc should describe the CURRENT (shipped) two-store design, not the superseded one. Do NOT fold in "How other worktrees reach board" (portal-based access) — superseded by the finding that no junction was ever needed (`hubgeometry.BoardDir(hub)` is a pure function of the shared `Hub` path). Do NOT fold in "Curation flow" — that content moves to the new stub instead (next). Create `manifest/designs/curation-triage.md`, a short Someday-tier stub (mirroring `hardener.md`'s "DRAFT doc, do not implement from it yet" framing per `manifest/roadmap.md`'s own description of it) with a status header stating it is deferred out of this task and carries forward `board-weft-storage.md`'s now-deleted "Curation flow" section (about to be deleted by this same card) verbatim in substance: raw notes can be added by anyone with no intake gatekeeping (including via a GitHub issue on the `weft` repo); only the orchestrating thread (running in prime) curates them; task extraction from the manifest is a deliberate, explicit, human-triggered command (a skill), not an autonomous background loop; and a section stating explicitly what already shipped and is OUT of this stub's remaining scope — `notes.json`/`tasks.json`'s shared schema and `promote-note`'s pure mechanical cross-store move, both delivered by `board: move storage to weft:main` — this item is the automation layer (GitHub-issue ingestion, periodic/triggered triage) on top of that already-shipped primitive, not the primitive itself. Sweep every remaining `board-weft-storage` reference before deleting the file, so nothing dangles: `manifest/designs/fabric-unified-view.md` lines 94 and 123 (the first names it inline as "board-weft-storage removes `board-url`..." — reword to name the shipped `board: move storage to weft:main` item and point at `internal/boardengine`'s package doc instead of the deleted file; the second is a `[board-weft-storage.md](board-weft-storage.md)` link in a "## Related"-style list — replace with a non-link bullet pointing at the package doc); `manifest/designs/raddle.md` line 52 (same link pattern, same fix — point at the package doc instead); `manifest/designs/host-visibility.md` lines 13 and 27 (both link to `board-weft-storage.md`; replace both with the package doc pointer, preserving each line's surrounding rationale prose unchanged); `manifest/designs/pattern.md` lines 21 and 83 (same treatment). In `internal/fabricengine/cleanup.go`: line 33's comment ("...and delete the very branch board-weft-storage requires to stay permanent") loses its dead doc-name reference — reword to name the actual mechanism (board's reserved `weft:main` branch) instead of the file. Additionally reword the adjacent lines 37-38 ("The board repo is excluded entirely — Cleanup only enumerates weft branches and compares them against the set of known host worktree slugs.") — this is now stale in a substantive way, not just a dead link: board no longer has its own separate repo, and `Cleanup` needs no explicit board exclusion at all — board's `_board` worktree branch (the host's own unsuffixed default branch, e.g. `main`) never matches the `-weft`-suffixed pattern `WeftHostSlug` looks for, so it is never a `Cleanup` candidate in the first place, not because of any explicit repo-level exclusion; reword the comment to state this mechanism precisely rather than describing a now-nonexistent "board repo" exclusion. Delete `manifest/designs/board-weft-storage.md` last, after every reference above is updated, in the same commit.
- **Commit:** `docs(boardengine): fold board-weft-storage.md's durable rationale into board.go, stub curation-triage.md, sweep dead references, delete superseded design doc`
