...
### Card 1: write manifest/designs/discussion-format.md
...
- **Requirements:** Write a new standalone design doc modeled on `manifest/designs/plan-card-format.md`'s house style — terse, decision-oriented prose, no meta-commentary, an H1 title, a `> **Status: …**` blockquote line directly under it, and an `## Open, not decided here` closing section.
  ...
  It must name the exactly two inbound pointers that go stale at that deletion and state that resolving them is the deleting task's job, not this doc's: `manifest/designs/loom.md`'s producer-table pointer (retarget to the stencil, where Fix 1's content then lives, or remove it) and `manifest/roadmap.md`'s `## Done` entry link for this item (retarget to the stencil, or keep it as a historical reference and drop it from the Markdown Link Integrity scope — that task's call).
  Note that Fix 2 is unaffected by this deletion because its durable copy is in `loom.md`.
...
- **Requirements:** Two edits to `manifest/designs/loom.md`, both additive.
  ...
  Append a link-only pointer to the new doc inside that same table cell, for Fix 1's exploration-scope content — a short trailing clause carrying `[discussion-format.md](discussion-format.md)`, no restated content.
  ...
  Link `[discussion-format.md](discussion-format.md)` once from this subsection as the companion design doc stating the writer-side half, and note that this subsection, not that doc, is the durable copy.
...
- **Requirements:** The status blockquote is the file's third line and currently reads, in part, `Supersedes `contracts/specs/loom-plan-spec.md`'s Card fields, `contracts/stencils/loom/loom-template-discussion.md`, and `contracts/stencils/loom/loom-template-plan.md` — none of these are rewritten yet.`
  ...
  In the same blockquote, add one short sentence pointing at the new doc as the holder of the discussion stencil's own scoped supersession claim, linked as `[discussion-format.md](discussion-format.md)`.
...
- **Requirements:** Three edits to `manifest/roadmap.md`, all inside or under `## Planned`.
  ...
  Write the Done entry per `manifest/roadmap.md`'s own Maintenance rule — a bold item name plus one or two sentences of what and why, then a `See [designs/discussion-format.md](designs/discussion-format.md).` line.
...
- **Requirements:** A zero-diff verification gate over cards 1 through 5.
  ...
  Then run `grep -rn "loom-template-discussion" --include="*.md" manifest/ docs/ CONSTRAINTS.md` and confirm three things about its output: no line in `manifest/designs/plan-card-format.md` or `manifest/roadmap.md` still claims outright supersession of the discussion stencil, both reworded sentences read as grammatical prose rather than clause-dropped fragments, and `manifest/designs/discussion-format.md`'s own scoped Step 2 / Step 3 supersession claim appears.
