...
- **Requirements:**
  ...
  - `lifecycle.go:565-567` (the raw `e.tmux.output("split-window", ...)` call's own error return), `:576-578` (the separate `validateSplitCreatedNewPane` failure branch immediately after it), and `:599-604` — header-pane split/send-keys errors: wrap and return with no log call. Add `logger.Warn` at each, naming the pane ID/target and the failing tmux operation.
