...
- **Every pre-existing fixture declares `language: none`.** `internal/loomcli/parity_test.go` and `internal/loomshed/planvalidate_test.go` both build their fixtures with `worktreeRoot := t.TempDir()`, which is not a git repository — so the moment those roots reach `planglyph`, `openRepo` would wrap `ErrQuarryUnavailable` and every existing cell would flip to an error verdict. Writing `language: none` into those fixture overviews makes the resolve pass a documented no-op that opens nothing, so the existing cells keep their existing expectations and go on testing exactly what they were written to test: the gate wiring. The fixtures that must reach quarry say so by carrying `language: go` and seeding a real repository.
...
All four packages' untagged tests stay tier1-pure — the two new parity fixtures build a small repository under `t.TempDir()` and reach `quarry.Resolve`, which reads files rather than spawning, and the non-repository fixture never opens anything at all.
