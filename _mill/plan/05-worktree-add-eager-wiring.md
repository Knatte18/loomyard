...
Depends on batch 1 (the wired junction endpoints are `WorktreeRoot`+`RelPath`-anchored; a subpath-anchored hub needs the correct `RelPath` from the recorded anchor). Reads the repo-wide name-set via `fabricengine.WiredNames(hubgeometry.BoardDir(l.Hub))` (available today; `BoardDir` base is explicit).
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Extend `internal/fabricengine/add_rollback_adopt_test.go` (`//go:build integration`) to assert:
