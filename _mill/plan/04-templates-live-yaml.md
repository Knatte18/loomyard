...
- **Requirements:**
  ...
  - In `internal/board/init_test.go`: REMOVE the two loops that assert every non-blank line of `board.yaml`/`worktree.yaml` starts with `#` (the template is no longer commented). KEEP the byte-equality assertions (`content == board.ConfigTemplate()`, `worktreeContent == worktree.ConfigTemplate()`) and all other assertions unchanged. Also update the file's top doc comment that says the templates are "fully commented" so it no longer claims that (the file is deleted in batch 6, but keep it accurate while it lives).
