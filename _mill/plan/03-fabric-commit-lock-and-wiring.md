...
- **Moves:** none
- **Requirements:**
  - `TestCommit_SkipGit_TwoSided` (pre-existing) asserts no `.weft` lock dir exists after a two-sided `Commit()` call under `SkipGit` — an assertion true only for the pre-batch weft-only lock scope. Under `combined-commit-lock`, this call's `warpFiles` is non-empty (the "README" input), so `committing` is true and the lock dir is now legitimately created. Update the test's final assertion to expect the lock dir to exist (the warp side still committed and still takes the combined lock), keeping every other assertion in the test (warp lands, weft no-ops, weft HEAD unchanged) unchanged.
...
- **Requirements:**
  ...
  - Behavior note for the warp-only bypass edge: `CoalescePushBothAt` rejects an empty `weftPath` with an error (batch 2 card 4's guard), so a manual `lyx fabric --warp-path X push` (no `--weft-path`) now surfaces that error via `output.Err` instead of the old handler's silent single warp push. This is acceptable and intended: the detached push child (`SpawnDetachedPush`, the only production caller of this bypass) always injects both paths, so the error path is unreachable in production. Do not attempt to preserve the old warp-only single-push behavior.
