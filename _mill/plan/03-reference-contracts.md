...
- **Requirements:** Delete `docs/reference/builder-contract.md` and `docs/reference/plan-format.md`.
  ...
  `manifest/designs/finalize.md` carries a third builder-era site the link repair does not reach: the merge-conflict paragraph describing an LLM "escalating **the same way Builder escalates a stuck batch to a fresh higher-capability model**".
  Rewrite that clause so it describes the escalation on its own terms — a fresh, higher-capability model in a clean session rather than a `/model` switch inside a polluted one — keeping the existing pointer to `internal/websterengine`'s package documentation and dropping the Builder comparison.
  `manifest/designs/shed-followups.md` assigns this site to this task.
