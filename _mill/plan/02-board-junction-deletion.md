...
- **Requirements:** In `internal/fabricengine/clone.go`, delete the `wireBoardLink(rec, l, filepath.Base(warpWorktreePath))` call, its `teardownHub` error branch, and the four-line comment above it beginning "Wire the operator-convenience `_board` junction as a named special case".
  ...
  Correct `repairPairWiring`'s own doc comment in the same edit: its summary sentence reads "it re-wires whatever `checkJunctionHealth` reports broken, always re-wires the operator-convenience `_board` link, and applies declarative stale-removal" — drop the `_board` clause — and the paragraph below it beginning "The `_board` re-wire is unconditional with respect to junction health" documents behaviour this card deletes, so remove that paragraph rather than leaving it describing code that is gone.
