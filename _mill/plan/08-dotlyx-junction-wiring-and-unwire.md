...
- **Requirements:** add `lyxdirs.DotLyxDirName + "/"` to `seedWeftArtifactExcludes`'s entries slice, keeping it the **sole owner** of that file's content and keeping the line-exact idempotency loop unchanged.
  ...
  In `junction.go`'s `WireJunctions`, call the weft-side seeding after `seedLyxJunction` succeeds and before `seedGitExclude`, resolving the weft worktree path as `WeftWorktreePath(l, slug)` — the same base `HostJunctions` computes its targets from.
  Do not derive it from a junction record's `Target` parent: `Target` is `filepath.Join(WeftWorktreePath(l, slug), l.AnchorRel, name)`, so its parent is the worktree root only when `AnchorRel == "."` and a subdirectory otherwise.
