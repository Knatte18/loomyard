...
- **Requirements:** In `structuraldirs_test.go`, rename `TestHubReservedNames_StillReturnsExactlyTheFourHubStructuralTokens` to name **three** tokens, and change `want` at line 103 to `[]string{BoardDirName, portalsDirName, launchersDirName}`.
  ...
  Rewrite the comment at lines 137-141 on **three** counts, not one.
  Its claim that "`_lyx`/`_pattern` are rejected only via this injected pathspec" stops being true for `_pattern` with this batch — and it was **already** inaccurate for `_lyx` before this task, because `IsReservedHubName` folds `structuralCommittedDirs` in directly, so `_lyx` is reserved regardless of what the injected pathspec names.
  Correct both halves rather than only the newly-false one; leaving the `_lyx` half would preserve a pre-existing error in a comment this card is already rewriting.
  It must also drop `_raddle` from its list of names rejected by `HubReservedNames()`.
