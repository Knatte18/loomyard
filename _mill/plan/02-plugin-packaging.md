...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Create an executable (`chmod +x`, committed exec bit) bash script that: (1) self-locates `SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"` and sets `PLUGIN_ROOT="$SCRIPT_DIR/.."` (the nested Go module root and parent of `bin/`); (2) sets `BIN="$PLUGIN_ROOT/bin/prowler"`, appending `.exe` when `uname -s` matches `MINGW*|MSYS*|CYGWIN*`; (3) if `BIN` exists and is non-empty and `PROWLER_BUILD_ONLY` is unset, `exec "$BIN" "$@"`; (4) otherwise acquire an atomic lock at `LOCK="$PLUGIN_ROOT/bin/.build.lock"` (after `mkdir -p "$PLUGIN_ROOT/bin"`): loop `mkdir "$LOCK"` with `sleep 1`, up to a ~120s deadline; inside the wait loop, if `BIN` becomes present+non-empty another builder finished → break to run it, and if `LOCK` exists with an mtime older than ~120s treat it as orphaned (SIGKILL case), `rm -rf "$LOCK"`, and retry — read mtime portably via `stat -c %Y "$LOCK" 2>/dev/null || stat -f %m "$LOCK"`; on deadline exceeded, print a lock-timeout message to stderr and exit non-zero; (5) `trap 'rm -rf "$LOCK"' EXIT` once the lock is held; (6) verify the toolchain with `command -v go` — if absent, print `prowler: Go toolchain not found — install Go or add it to PATH` to stderr, release the lock, exit non-zero; (7) build with `go build -o "$BIN" "$PLUGIN_ROOT"` sending build stdout+stderr to stderr (`>&2`); on failure print a build-failed message to stderr, `rm -f "$BIN"`, release the lock, exit non-zero; (8) release the lock; (9) if `PROWLER_BUILD_ONLY` is non-empty, exit 0 without running; else `exec "$BIN" "$@"`. Only the executed binary's stdout may reach the wrapper's stdout — every wrapper diagnostic and all `go build` output goes to stderr.
...
### Card 14: README + gitignore build artifacts
