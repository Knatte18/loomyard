...
- **Requirements:**
  ...
  Build the `*lyxcwd.Location` the same way this file's existing tests do (e.g. `TestLoomStatusFile_EqualsAnchorPathJoinedWithLoomStatusRel`): a bare `&lyxcwd.Location{...}` over a fixed synthetic path literal (`filepath.Join("home", "user", "repo-HUB")` for `HubPath`, `"repo"` for `WorktreeName`), never a real `t.TempDir()` — no test in this file builds a `Location` over a temp directory, no fixture, no git spawn, this file stays untagged Tier 1.
