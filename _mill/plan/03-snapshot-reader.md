...
`verify: go test -tags integration -count=1 -skip 'TestDiff_MergesWarpAndWeftSides|TestStatus_MergesUncommittedChangesBothSides_ExcludesWeftArtifacts' ./internal/fabricengine/...` — scoped to the one package tree this batch touches. The `-tags integration` build also compiles and runs `internal/fabricengine`'s untagged tests, including the new `index_test.go`, so one command covers both tiers.
