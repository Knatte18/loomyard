...
- **Requirements:** Create `internal/shedengine/run.go` with a file-level comment naming it as the six-step loop, and declare `func (s *Shed) Run(ctx context.Context) (Result, error)` plus an unexported helper `func findProducer(producers []ProducerDef, name string) (ProducerDef, int, bool)` returning the matching definition, its index in the list, and whether it was found.
  ...
  - **Step 3, the pause and cancellation check.** When the read `pause_requested` is true **or** `ctx.Err()` is non-nil, take the pause exit: persist `StatePaused` with an empty error text, the history unchanged, `current_producer` unchanged, and `pause_requested` cleared to false, then return `Result{Outcome: RunPaused, HaltedProducer: <the unchanged current_producer>, History: <the unchanged history>}, nil`. Card 8 supplies the persist helper; write this branch against its signature. Comment that the two conditions are treated identically on purpose — an operator's Ctrl-C or a parent deadline is an operational stop, not a failure, exactly as resumable as an explicit pause request — and that clearing the flag in the same persist is what stops the next `Run` re-pausing forever on the flag it is resuming from; the durable record of "this run is paused" is `state`, not the flag.

  Leave steps 4 through 6 to card 9; end card 7's loop body with a placeholder that card 9 replaces, or stage the file so it compiles — either way `go build ./internal/shedengine/` must succeed at the end of this card.
...
- **Requirements:** Add one unexported method to `internal/shedengine/run.go` that is the single write path for the whole loop.
  ...
  Then rewrite card 7's pause exit to call this method, and confirm the pause persist passes the pause-consuming bool as true and leaves the history and `current_producer` unchanged.
