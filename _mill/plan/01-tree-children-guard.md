...
- **Requirements:** In `plugins/prowler/scripts/testdata/github-tree/bin/gh`, change the single `body_path` assignment so that a mapped fixture name beginning with a slash is used verbatim as an absolute path, while every other value keeps resolving under `GH_STUB_BODIES` exactly as today.
  ...
  Extend the harness's header comment to note the absolute-fixture mechanism and the generator, and append two lines to that header's existing "NOT covered here" note recording the two live checks this batch adds and does not assert offline: one live children-mode run against a real repository, and one live guard trip against a large public repository.
...
- **Requirements:** Append new numbered test sections to `plugins/prowler/scripts/github-tree-selftest.sh` covering seven guard scenarios, using `gen_tree_body` for the two that need large listings and pointing their map's fixture column at the generated file's absolute path.
  ...
  Fourth, the boundary: run the three-blob `small-root-rec.json` scenario twice, once with a ceiling of exactly 3 and once with a ceiling of 2, and assert the first succeeds printing all three paths while the second aborts — the same fixture on both sides, so the two runs are provably one entry apart.
