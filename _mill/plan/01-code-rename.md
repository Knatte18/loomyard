...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Rewrite all `github.com/Knatte18/mhgo/` imports → `github.com/Knatte18/loomyard/`. In `internal/ide/menu.go`: change the literal `filepath.Join(entry.Path, l.RelPath, "_mhgo")` → `"_lyx"` and local `mhgoPath`→`lyxPath`. In `internal/ide/spawn.go`: rename any `_mhgo` literal. In `internal/ide/vscode.go`: rename the `mhgo-managed` reference → `lyx-managed`. In `internal/ide/cli.go`: rename the CLI usage/help string `usage: mhgo ide ...` (and any other `mhgo ide` invocation literal) → `lyx ide ...`. In `internal/ide/menu_test.go`: rename the test function `func TestMenuRequiresMhgoDir` → `func TestMenuRequiresLyxDir`. These are non-comment runtime strings/identifiers — rename them explicitly (not just under the comment sweep); the matching assertions move with them. Apply prose-voice to all remaining `mhgo` comment mentions across the package.
...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Rewrite all `github.com/Knatte18/mhgo/` imports → `github.com/Knatte18/loomyard/`. Rename every `_mhgo` literal and `mhgoDir`/`mhgoPath` local identifier in `add.go`, `cli.go`, `config.go`, `portals.go` and their tests → `_lyx`/`lyxDir`/`lyxPath`. Rename non-comment CLI-invocation literals explicitly: in `internal/worktree/launchers.go` the generated launcher commands `mhgo ide spawn` / `mhgo ide menu` (and any `mhgo ide`/`mhgo worktree` string) → `lyx ide ...` (assertions in `launchers_test.go` move with them); in `internal/worktree/config.go` the `run "mhgo init"` guidance string → `run "lyx init"` (assertion in `config_test.go` moves with it). If any worktree code calls `paths.Layout.MhgoDir()`, update the call site to `LyxDir()` (Card 2 renames the definition; the whole batch lands together). Apply prose-voice to all remaining `mhgo` comment mentions across the package.
