...
- **Requirements:**
  ...
  - `internal/perchengine/identity.go:100-103` — `TerminalOutcome` is currently a single unconditional `return Outcome(outcome), ok, err` with no existing `if err != nil` branch to attach a log call to. Introduce one: `outcome, ok, err := treadleengine.TerminalOutcome(runDir); if err != nil { logger.Warn(...) }` naming `runDir` and the error, then `return Outcome(outcome), ok, err` unchanged — this is a new branch, not an edit to an existing one.
