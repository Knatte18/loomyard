...
### Card 32: Seed repo-wide fabric config in the loomengine preflight fixture (fix RepoWiredNames regression)
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Batch 2's `RepoWiredNames` migration (card 7) moved `checkJunctionHealth`/`PairInSync`'s wired-junction name-set read to the repo-wide `fabric.yaml` at `hubgeometry.BoardDir(hub)`. `setupPreflightFixture` in `internal/loomengine/preflight_integration_test.go` seeds a per-worktree weft config via `lyxtest.SeedConfig(t, f.WeftPrime, ...)` only — it was never updated for the repo-wide location, so every scenario now fails its junction check with `cannot load fabric.yaml: not initialized here; run "lyx fabric reconcile"` (8 subtests: `TestPreflight_HealthyPairAndSeed`, `TestPreflight_HostDirty`, `TestPreflight_SeedMissing`, `TestPreflight_SeedUnknownField`, `TestPreflight_SeedHalfFinished`, `TestPreflight_LegacyWorktreeUpgrade`, plus the two others surfaced by batch 6's `verify:` run). Add a package-local helper mirroring fabricengine's `seedRepoWideFabricConfig(t testing.TB, hub string)` (`internal/fabricengine/reconcile_stale_registration_test.go`): `boardDir := hubgeometry.BoardDir(hub)`; `os.MkdirAll(hubgeometry.ConfigDir(boardDir), 0o755)`; write `fabricengine.ConfigTemplate()` to `hubgeometry.ConfigFile(boardDir, "fabric")` via `os.WriteFile` (no git add/commit — `_board` is not a git repo, same as the fabricengine original). Call it from `setupPreflightFixture` right after the existing `lyxtest.SeedConfig(t, f.WeftPrime, ...)` call, passing `f.Layout.Hub`. Update `setupPreflightFixture`'s doc comment (lines 23-35) to describe the added repo-wide seed step alongside the existing per-worktree one.
- **Commit:** `test(loomengine): seed repo-wide fabric config in preflight fixture (fix RepoWiredNames regression)`

## Batch Tests

`verify: go test -tags integration ./internal/fabricengine/... ./internal/fabriccli/... ./cmd/lyx/... ./internal/loomengine/...` spans the four areas this cross-cutting batch touches: fabricengine (new `unwire.go`/`unwire_test.go`, the reference-string sweep, the retargeted `config.go`), fabriccli (new `unwire.go`, `fabric.go` registration, exercised by `cli_test.go`'s verb-surface test which now expects `unwire`), cmd/lyx (the help-tree/registration/longlist/drift guards that must stay green after `init` deletion — this is where a dangling `initcli` reference or a missing help-tree entry fails — plus `sandbox_coverage_test.go`, whose Assert-2 requires card 31's S6 `**Covers:** init` removal), and loomengine (its `config_test.go` string assertion, plus card 32's preflight-fixture repo-wide config fix — a batch-2-introduced regression this batch's `verify:` is the first to catch, since it is the first batch whose `verify:` scope includes `./internal/loomengine/...`). The broad scope is inherent to init dissolution, which removes a registered module and touches eight engines; it is justified and named here per the per-batch-scoping exception. The repo-wide `done_gate` (`go test ./...`) is the final backstop for the other seven engines' retargeted messages and any missed `initcli`/`initengine` reference.
