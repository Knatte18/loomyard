...
- **Requirements:** Make two distinct edits to `manifest/roadmap.md`.
  ...
  Amend the immediately following sentence too, the judge-call else-branch that currently reads "If the artifact exists, this is the **judge call**": it is the seed clause's logical complement, so it must state the same pair predicate — the judge call is the case where **both** artifacts exist — or the paragraph's two halves test different predicates and a review-only orphan would still read as a judge call, which is exactly the ambiguity this task designed out.
  Keep the rest of that judge-call sentence, including its `Done`/`Stuck` routing, unchanged.
