...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add a `//go:build integration` file (package `fabricengine`) reusing the existing fixtures. For `Fabric.Diff`: after two `SyncWeft` rounds on a warp/weft pair (which record correspondence), assert `Diff(earlierWarpSHA)` returns a merged `[]ChangeEntry` with warp-side paths labelled `SideWarp` and weft-side paths labelled `SideWeft`, anchored via the nearest-older weft SHA. Cover the **nearest-older** path with a warp-only anchor that has no exact correspondence (advance warp with an un-synced commit, à la `TestRevertWithWeft_Gap_ResetsToNearestOlderAndReportsRange`) and assert the weft side resolves to the nearest older synced weft baseline rather than empty. Cover the **no-correspondence** path with a `sinceWarpSHA` older than the first weft commit and assert warp entries present, weft entries empty, `NoWeftCorrespondence == true`, and no error. For `Fabric.Status`: run one `CommitWeft`/`SyncWeft` first (so `ensureWeftLockDir` seeds the weft `.git/info/exclude` with **both** `.weft/` and `gitrepo.PushLockFileName`, per `seedWeftArtifactExcludes`), then create uncommitted changes in both the warp and weft worktrees and assert the merged `[]ChangeEntry` reports both sides' uncommitted paths with correct side labels. Then verify the exclude mechanism is genuinely exercised (not asserted vacuously): the `.weft/` lock dir is already created by `ensureWeftLockDir`, but `.gitrepo-push.lock` is **not** created by this flow — `SyncWeft` pushes via `f.Weft.Push()` (`Repo.Push`), which never writes the push lock; only `PushCoalesced` does — so the test must **explicitly create a `.gitrepo-push.lock` file in the weft worktree root** (e.g. `os.WriteFile(filepath.Join(weftPath, gitrepo.PushLockFileName), []byte(""), 0o644)`) before calling `Status`. Assert that **neither** `.weft/` **nor** `.gitrepo-push.lock` appears on the weft side of the result (both are git-excluded), verifying the go-git primitive honors `.git/info/exclude`. Scope this artifact-exclusion assertion to the **weft** side only: the warp/host side deliberately does not seed these excludes (the documented known limitation — see Card 14), so do not create or assert on `.gitrepo-push.lock` on the warp side. (Card 11's Requirements originally asserted this exclude-file honoring was inherent to `wt.Status()`; that claim was superseded — see the "Superseded" note on Card 11 above — by `worktree.go`'s own finding that `Status()` alone reads no exclude file, and that `readGitDirExcludePatterns` was added specifically to restore this behavior. This test is therefore a regression guard over `readGitDirExcludePatterns`, not over unmodified `Status()`.)
