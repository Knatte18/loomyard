...
- **Requirements:** In `docs/overview.md`:
  ...
  - Sweep the one cross-reference that ties another module's config reconcile to `lyx init`: overview.md:181 (`loom`: "…reconciles via `lyx init` / `lyx config reconcile`.") — drop the `lyx init` mention, keeping `lyx config reconcile`. (The `config` bullet at overview.md:170 contains no `lyx init` substring — verified in round-2 review — so it needs no edit; the only `lyx init` occurrences in the file are lines 168, 181, 188, of which 168 and 188 are the removed `init` entries above.)
