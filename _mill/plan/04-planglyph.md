...
- **Requirements:** Create the package and its two exported entry points.
  ...
  `resolvePass` is also the boundary card 19's `CanonicalizeHandles` is invoked from, so it needs the plan directory as well as the worktree root: take it from `plan.Dir`, the field `planparser.ParsePlan` already stamps with the directory it was given, rather than adding a parameter to either exported entry point — which would break the `planparser.Validate(plan, worktreeRoot)` shape the parity pair mirrors.
...
- **Requirements:** Turn each draft handle into its canonical `plan:<expected-glyph>` form, mechanically, at the validation boundary and never inside the planning loop.
  ...
  Collect declarations from **two** sources, not one, and issue **one** `quarry.Name` call covering both.
  Source one is every `planparser.CardDeclaration` across every card, taking `Unit` from `planparser`'s own handle-unit accessor and `Decl` from the declaration head verbatim. No file read needed.
  Source two is every `Rename` pair whose `New` is a handle: card 11 requires that shape precisely so the to-side can be **computed** here rather than trusted, and without this source the planner's untrusted draft spelling would survive to the end of the pipeline, which is the one thing the rename rule exists to prevent.
  For each such pair, read the resolved `Old` glyph's own declaration from the batched `Resolve` answer — a `found` result's single `Symbols` entry carries the declaration text verbatim — swap the declared identifier for the member name the draft to-side handle carries, and add the result as a second `quarry.Declaration` with the same unit.
  Take **only** the identifier from the draft handle, never its glyph spelling: the spelling is what `Name` computes.
  A pair whose `Old` did not resolve `found` contributes no declaration and instead produces a blocking finding, check ID `rename-old-unresolved`, since there is nothing to derive the new name from.
  ...
  Emit a blocking finding, check ID `handle-name-failed`, for each result carrying a non-empty `Error`, with `Reason` in the detail.
  Then build the substitution map from each successful result's draft handle to `planparser.HandlePrefix + result.ID` — covering both sources identically, so a `Rename` to-side handle is rewritten across the whole plan exactly as a `Create` handle is — and hand it to `planparser.RewriteRefs(planDir, subs)`, which rewrites every occurrence in one pass, the draft spelling on the left and the computed spelling on the right.
  ...
  **Name the caller, so the function does not ship dead:** `resolvePass` invokes `CanonicalizeHandles(plan, plan.Dir)` **before** cards 17, 18 and 20's passes, and merges its findings into the same returned set.
  The ordering is required rather than stylistic: canonicalization rewrites `plan:` handles across the plan on disk, and the later passes must see the canonical spellings rather than the draft ones.
  This placement is what puts the batched `quarry.Name` call at the validation boundary and never inside a planning loop, which is the whole point of batching it.
  A `CanonicalizeHandles` infrastructure error propagates out of `resolvePass` as card 21's wrapped `ErrQuarryUnavailable`, exactly like a failed `Resolve`.
  Cover in tests: one batched call covering three `Create` declarations plus one `Rename`-derived declaration, proving both sources ride the same call; positional matching verified by an out-of-order fixture; a `Rename` to-side handle whose draft spelling is deliberately wrong still being rewritten to the computed glyph; a `Rename` whose `Old` does not resolve producing `rename-old-unresolved` and contributing no declaration; one failing declaration producing exactly one `handle-name-failed` and leaving the others rewritten; a canonical collision producing `handle-canonical-collision` and rewriting neither; and the rewrite landing on every card that referenced the draft handle, not only the declaring card.
...
- **Requirements:** Add the half of the containment check that string prefixes cannot see, reporting its own finding ID distinct from the syntactic tier's.
  ...
  Wire the call into `resolvePass` after card 17's and card 18's passes — and therefore after card 19's `CanonicalizeHandles` — so it sees the same single batched `Resolve` result set rather than issuing a second call.
