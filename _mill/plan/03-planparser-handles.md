...
- **Requirements:** Extend a `**Create:**` group's sub-bullet grammar to the two-field form `` `plan:<draft-handle>` -> `<declaration head>` ``, reusing the arrow grammar `**Rename:**` already parses.
  ...
  In `plan.go`, add `Declarations []CardDeclaration` to `Card` and declare `type CardDeclaration struct{ Handle, Decl string }`, documented as one `Create` sub-bullet's handle and declaration head, both verbatim.
  In `parse.go`, teach `parseTypeLabelCase`'s `createLabel` path to try `splitHandleDeclaration` on each sub-bullet first: on a match, append a `CardDeclaration` and append the handle alone to the group's `Refs` and the card's `Targets`, so a handle participates in `Targets` exactly as any other ref does; on no match, keep today's single-ref behaviour unchanged.
  A `**Create:**` bullet that looks like the arrow form but is malformed is captured rather than dropped, mirroring how `RenameRaw` preserves a malformed `**Rename:**` bullet so a check has something to report.
  Cover in tests: a well-formed handle declaration parsed into both `Declarations` and `Targets`; a plain non-arrow `Create` ref still parsing as today; a malformed arrow bullet preserved rather than silently dropped; and a handle surviving `normalizeCard` without picking up a `root:` prefix, which `classifyRef` already guarantees by classifying it `refKindHandle`.
...
- **Requirements:** Add the plan-time checks that keep handles internally consistent, all of them pure string work over the parsed model.
  ...
  Add `checkHandleMalformed` emitting check ID `handle-malformed` for a `**Create:**` sub-bullet card 9 captured as malformed, and for a handle whose text after `HandlePrefix` carries no `#` and therefore names no unit.
...
- **Requirements:** Add one new primitive, `RewriteRefs(planDir string, subs map[string]string) error`, as the single write path all three later rewrite occasions call: handle canonicalization, handle binding at card completion, and drift auto-repair.
  ...
  Bridge the surface↔model gap here rather than in the callers: parse the plan with `ParsePlan` to obtain `Plan.SurfaceRefs`, map each key through it to the lexeme the file actually carries, and substitute that lexeme.
  A key with no `SurfaceRefs` entry substitutes itself, which is correct for a ref that was already canonical on disk.
  ...
  Cover in tests: substitution across `Targets`, `Uses` and both endpoints of a `Pairs` entry; substitution finding a surface lexeme that differs from the canonical key; idempotence when the same map is applied twice; a no-op map leaving every file's bytes untouched; and prose containing the key string left unmodified.
...
- **Requirements:** Record both cross-cutting obligations this batch creates, in the same commit that finishes creating them.
  ...
  Create `cmd/lyx/constraintchokepoint_test.go` enforcing the new invariant the way this repository's sibling guards do: walk every non-test `.go` file under `internal/` and `cmd/`, skipping the same directories `tierPuritySkipDirs` names, and fail any file that contains a `#`-trimming suffix call over a glyph-typed value, or that reads a `glyph.Glyph` value's `Unit` field in a `filepath`/`os` path-construction context. No file read needed.
