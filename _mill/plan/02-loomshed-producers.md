...
- `./internal/loomshed/...` is the batch's own new untagged suite: `ctx_test.go`, `stub_test.go`, `discussionvalidate_test.go`, `planvalidate_test.go`, `batchifier_test.go`, `webster_test.go`, `loomshed_test.go`, `seed_test.go`, and the `seam_enforcement_test.go` import guard. Every one of these is offline — temp dirs and fakes only, no git spawn — so the whole package stays Tier 1.
