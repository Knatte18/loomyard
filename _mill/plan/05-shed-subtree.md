...
- **Requirements:** declare `Command() *cobra.Command` returning a `shed` parent carrying a non-empty `Short`, a `Long` describing the four verbs and the `--recipe` flag, `RunE: clihelp.GroupRunE` so a bare `lyx shed` lists subcommands and `lyx shed bogus` emits a JSON error envelope, and a persistent `--recipe` string flag defaulting to `loom`.
  ...
  Record every envelope this subtree's own pre-run writes through `clihelp.SetExit(cmd.Context(), …)` wrapping the `output` helper's return, exactly as both existing modules' pre-runs do — `internal/clihelp/exec.go`'s `RunRootCtx` returns `es.code`, which only `SetExit` and `Abort` ever write, so an unwrapped `output.Err` exits 0 on a refusal.
  The pre-run's own failures use `clihelp.Abort(ctx, 1)`, which writes the same field and additionally short-circuits the leaf body, matching both modules' existing pre-runs.
