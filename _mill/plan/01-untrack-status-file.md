...
- **Requirements:**
  ...
  Rewrite `TestSmokeBootstrap_OriginRecordSelfHealsAfterCrashBetweenWriteAndCommit`'s doc comment too, which carries the same stale claim in its own words: it explains that the overall pair is not asserted clean because a running driver's persists rewrite the status file's working-tree content on every phase transition, "which legitimately dirties the weft again for a reason that has nothing to do with the origin-record healing this case exists to pin".
  Those persists now land under the never-committed scratch tree and cannot dirty the pair at all, so the stated reason is gone; state instead that the case scopes its cleanliness assertion to the origin record because that record is the only tracked path this scenario is about.
  The same comment cross-references the renamed test by its old name — update that reference to the new name, or drop it if the rewritten sentence no longer needs it.
