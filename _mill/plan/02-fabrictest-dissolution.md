...
- **Requirements:**
  ...
  Rewrite the moved file's own strings and comments that name the old package: the `os.MkdirTemp` pattern `"fabrictest-bare-*"` becomes `"hubforge-bare-*"`, the two seed-commit messages `"fabrictest: seed warp template"` and the `README` bodies naming `fabrictest` become their `hubforge` equivalents, and every comment referring to `lyxtest` names `gitkit`.
