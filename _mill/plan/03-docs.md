...
- **Requirements:**
  ...
  Also update the file-header comment on the first line and the package doc's opening paragraph so neither describes `Directive` as returning constants, and add a short subsection or paragraph stating the read path this package now has: `Directive` is told a `stencilsDir`, reads the role's stencil through `stencilstore.Read`, strips the leading banner with `stencil.StripLeadingComment` because its return value is injected as a producer template's marker value and so never passes through `stencil.Fill`, and returns an error rather than an empty string when an active PATTERN's stencil cannot be read.
