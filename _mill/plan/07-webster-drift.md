...
- **Requirements:** Turn a handle into the real glyph the card actually created, from the delta rather than from anyone's spelling.
  ...
  In `internal/websterengine/recordbatch.go`, add the batch's **single** delta call — `planglyph.Delta(deps.Geom.WorktreeRoot, bs.StartSHA, report.HeadSHA)` — immediately after card 33's done-checks and before this card's own binding, and thread the one returned `quarry.GitDeltaAnswer` into `BindHandles` here, `ScopeGuard` in card 35 and `DetectDrift` in card 36.
  This card owns that call: cards 35 and 36 consume its result and must not add a second one.
  `bs.StartSHA` is the begin-batch record's captured start SHA and `report.HeadSHA` is the fork's self-reported head, which this file has already cross-checked against the worktree's actual HEAD by this point — that cross-check is why the delta can be trusted here and nowhere earlier.
  A `DeltaGit` infrastructure error does not abort the call sequence: card 35 degrades the scope guard to an informational notice, while card 33's done-checks have already run on their own `Resolve` and are unaffected.
  Binding must run **after** card 33's done-checks, so a card that already failed `create-not-done` is never bound, and the substitution must be applied in one `RewriteRefs` call per record-batch rather than one per card, so the plan bytes are rewritten once.
...
The untagged half, `go test ./internal/planglyph/ ./internal/websterengine/ ./internal/webstercli/ ./cmd/lyx/ ./internal/lyxcwd/`, covers the pure logic — `scope_test.go` and `handle_test.go`, driven through fake deltas rather than real ones — plus the two extended guards in `cmd/lyx`, whose own sub-tests prove the new detections fire, and the markdown link guard.
The tagged half, `go test -tags integration ./internal/planglyph/ ./internal/websterengine/`, runs `delta_integration_test.go`, `donecheck_integration_test.go` and `drift_integration_test.go`, which build real fixture repositories and spawn git through `DeltaGit`, **and** `beginbatch_test.go`/`recordbatch_test.go`, which carry `//go:build integration` on their own first line and therefore run only here — the wiring assertions cards 32 through 36 add to those two files belong to this half, not the untagged one.
All of it runs under `internal/planglyph`'s `TestMain` hermetic git environment from card 16.
...
The scope is five named packages rather than the module because those are the ones this batch edits or depends on a guard from, and the hub's own `pipeline.done_gate` — `go test ./... && go test -tags integration ./...` — runs the whole repository in both modes before the task is marked done, which is what catches a regression in a package no batch verify covers.
