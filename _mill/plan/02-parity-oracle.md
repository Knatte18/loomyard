...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add parity cases for `remoteName`, `hasUnpushed`, and `isStrictDescendant` to the internal `package gitrepo` test file, since they are unexported and unreachable from the external test package. Cases: `remoteName`'s `origin` fallback and its configured `branch.<name>.remote` path; `isStrictDescendant` across an ancestor, self (must be false — strict), and an unrelated orphan-branch commit; `hasUnpushed` across **five** states, two of which the spike never covered at all — HEAD ahead of upstream; HEAD **strictly behind** upstream, which the naive single-hash exclusion gets wrong; **no upstream configured at all**, which must be `true` because that is what makes the first push of a branch happen instead of being skipped as nothing-to-do; **configured but never fetched**; and the **failure-swallowing** path. The spike's `TestHasUnpushed` already has `AheadOfUpstream`, `Behind` and `NoUpstreamConfigured` subtests, so those three are carried over rather than invented — but none of the five has ever been compared against a live CLI oracle, which is what changes here. In the last state the CLI returns `(true, nil)` on any non-zero `rev-list` exit while the poc returns `(false, err)` on a `Head()`/`CommitObject` failure. That divergence would turn `PushCoalesced` — called from `boardengine/sync.go` and `fabricengine/weftgit.go` — from "attempt the push anyway" into a hard error on an unreadable or unborn-HEAD repo, so the case pins the swallow-into-`true` contract before the implementation moves.
