...
- **Moves:** none
- **Requirements:**
  (1) `internal/fabricengine/doc.go`: add `Pull` to the cross-repo operations the `Fabric` handle exposes — the sentence currently naming "(`SyncWeft`, `RevertWithWeft`)" becomes "(`SyncWeft`, `RevertWithWeft`, `Pull`)" — and add a short paragraph describing `Fabric.Pull` as the unified read path: weft ff-pull first, then warp fetch + fast-forward-vs-rewrite classification, safe auto-reconcile (warp-only reset + weft correspondence re-anchor to the nearest surviving `Warp-SHA`, via the empty-commit mechanism) when local warp is clean, the double-conflict and no-surviving-anchor loud aborts, and the `PullResult` PATTERN-residue report — all detection/reachability via ancestry, never `SHAExists`. Also update `internal/fabricengine/fabric.go`'s own package-doc sentence, which independently names the cross-repo operations as "(`SyncWeft`, `RevertWithWeft`)": bring it in line with `doc.go` by naming the current set — add `Pull` and the pre-existing-but-omitted `Commit` so it reads "(`Commit`, `SyncWeft`, `RevertWithWeft`, `Pull`)" (or equivalent), leaving no stale cross-repo-methods list in either file.
  (2) `manifest/designs/fabric-unified-view.md`: correct BOTH stale `SHAExists`-detection claims in this file, since both are load-bearing and both are edited in this same commit — (a) in the "## Warp-rebase and remote-reconcile" section, the "**Detection is already honest and shipped.** … `SHAExists` catches this rather than trusting a dead reference" paragraph is flatly false and directly contradicts this slice's core "reachability, never object-existence" decision (`git fetch` never prunes objects, so `SHAExists` reports `true` post-fetch on a rebased-away commit and detection never fires) — rewrite (or strike) it to state detection uses ancestry/reachability (`merge-base --is-ancestor` via `gitrepo.IsAncestor`), never `SHAExists`; and (b) in the "Build order — slices" section's slice-6 entry, correct the stale "Detection (`SHAExists`)" phrasing the same way. Mark the fabric-layer half of slice 6 as landed (detection + safe re-anchor + PATTERN document via `Fabric.Pull`), and in the "still-open question" about exact rebase/remote-reconcile orchestration, record that the fabric-layer half is now resolved by `Fabric.Pull` while the orchestration-layer half (who consumes the PATTERN document and drives raddle-regen) stays open until `loom`/`Shed` exist. Do not overclaim: raddle regeneration and the LLM conflict-resolver remain out of scope/unbuilt.
