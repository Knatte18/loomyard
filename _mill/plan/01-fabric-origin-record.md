...
- **Requirements:** Create the new file in `package fabricengine`.
  ...
  Add `func CommitWeftPaths(rec *Mutations, weftPath, anchorRel string, relPaths []string, msg string, opts SyncOptions) (sha string, committed bool, err error)` with this body order, which the doc comment must state is load-bearing:
  (1) `opts.SkipGit` returns `("", false, nil)` with no lock taken, nothing staged, and nothing recorded, exactly as `commitWeftAt` does, so an existing `SkipGit` test path can thread its opts straight through;
  (2) an empty `relPaths` returns `("", false, nil)`, likewise recording nothing — nothing staged is never reported as a commit;
  ...
  (4) call `gitrepo.New(weftPath).StageAndCommit(msg, weftCommitPathspec(anchorRel, relPaths))`, and when it returns a nil error with `committed` true — never on a no-op, never on an error — append `KindCommitCreated` to `rec` at `weftPath` with the returned sha as the detail, matching the shape the package's existing commit site already uses, then return its three values.
  The doc comment must state that the recorder is what keeps a real weft commit from being invisible in the calling verb's own mutation record, that the recording is conditional on the commit having observably landed per the Mutation Record Invariant, and that a caller with no verb outcome of its own passes a throwaway recorder and discards it.
...
- **Requirements:** Both files are untagged and must spawn nothing, per the Test Tier Purity Invariant — build `*lyxcwd.Location` values by hand exactly as `newPortalLauncherTestLocation` already does rather than resolving one.
  ...
  Both of those two cases must additionally pass a fresh recorder and assert its snapshot is empty afterwards — the guard that neither early return claims a commit it did not make.
