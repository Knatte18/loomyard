...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Change `internal/fabricengine/template.yaml`'s `pathspec:` default from `_lyx` to `_lyx _pattern`, and correct its inline comment, which currently ends `_lyx is the default`. Add the batch's single most important regression assertion to `internal/fabricengine/weftgit_pathspec_integration_test.go` (created by card 13, in this same package): with the widened default pathspec (`Config{}.Dirs()`'s two-entry split of `_lyx _pattern`, not a hand-written literal — the whole point is proving the real config default survives the filter) and **no files under `_pattern`**, a real `_lyx` change still commits. Cover both shapes — an existing-but-empty `_pattern` directory and a wholly absent one — since `git add`'s unmatched-pathspec guard may treat the two differently; derive each test's expected pre-filter git behavior empirically rather than assuming. Without this assertion `CommitWeft`'s `("", false, nil)` no-error return makes the batch's core regression invisible — only asserting the commit actually happened catches it. This is the single place the default weft-staging pathspec is declared; without it nothing ever stages `_pattern`, so a `PATTERN.md` written through the junction would never be committed or pushed and would never reach another machine or another worktree's weft pull — the mechanism would be inert in exactly the way that matters. **A pre-existing test goes stale on this exact change and must be updated, never deleted:** `internal/fabricengine/template_test.go`'s `TestConfigTemplate_PathspecResolvesToLyx` resolves the template and asserts exact string equality, `if pathspec != "_lyx"`, so it fails the moment the default widens — this batch's own `verify:` would go red on it. Rewrite that assertion to split the resolved value on whitespace and assert it yields exactly `_lyx` and `_pattern`, in that order. Rename the test and rewrite its doc comment as well: both encode the old single-value expectation in their text (`…PathspecResolvesToLyx`, "asserts that the template's pathspec default resolves to `_lyx` regardless of environment"), so leaving either in place would misdescribe what the test now checks. Asserting the whitespace split rather than the whole string is the point: the value is whitespace-split at the consumer, so a splitting bug here is silent and would simply drop `_pattern`. Update `internal/fabriccli/weft_verbs.go`, whose `Long` for `lyx fabric commit`/`push`/`sync` reads `Staging is scoped to the directories listed in the fabric config (default: _lyx)` — help accuracy is a review-blocking obligation under the CLI/Cobra Invariant. Then document two consequences in `internal/fabricengine/doc.go`, both confirmed rather than speculative. First, **existing worktrees never pick this up**: `configsync.ReconcileAll` → `yamlengine.Reconcile` keeps a `pathspec:` key that is already present and adds no key when one exists, so every already-initialised worktree stays on `pathspec: _lyx` forever and never persists PATTERN content, no matter how many times `lyx init` is re-run — operators must widen it by hand. Second, **no detection or warning surface is in scope**: nothing, not `lyx fabric status` and not `lyx init`, reports a narrow pathspec, so an existing worktree stays silently inert. That is accepted here rather than papered over — a "your pathspec predates PATTERN" warning means a new diagnostic class in `fabric status`, and PATTERN has no content to persist in this repo yet — and documenting it is what makes the next operator meet it in writing rather than by surprise. Note the deliberate asymmetry with the junction side, which batch 5 documents: junctions self-heal on the next `init`/`reconcile` and report loudly until they do, because `WireJunctions` owns junction state outright, whereas `pathspec` is an operator-editable config value `configsync` must not overwrite. Write both markdown-facing prose edits as one unwrapped line per paragraph.
