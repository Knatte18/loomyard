...
- Round 2: implemented this batch's own "resize reed's own window directly on reed's own socket" correction exactly as specified (`resize-window -t <reedSession>` against `h.reedSocket`, shrink then grow) — `TestSmokeDotFillFloorIsCleanOnASettledAttach` and `TestSmokeDotFillCrossClientControl` both pass, but `TestSmokeDotFillResizeControl` still misses every time in this environment. Diagnostics performed live during this round: (1) `display-message -p '#{window_width}x#{window_height}'` confirms the resize-window calls DO take effect on reed's session (140x42 -> 80x24 -> 160x50), so the trigger is real, not a no-op; (2) a manual capture of the harness pane right after the trigger shows the newly-exposed region as plain blank cells, never a dot-fill glyph; (3) a tight capture-pane loop with no sleep (927 samples across 2s, far finer than the production 100ms poll) found zero dot-fill hits at any point after the trigger; (4) widening the shrink/grow extremes to 10x5 -> 300x100 (still within Card 5's "distinctly smaller"/"grow back past the original" wording, no plan deviation) also produced zero hits. Conclusion: in this container's tmux 3.6 build, a `resize-window` issued directly against reed's own session — with no accompanying real client-terminal resize (no SIGWINCH to the harness pane's own pty) — does not produce any dot-fill artifact, transient or standing, no matter the magnitude or sampling granularity. This round's evidence narrows the open question for the next round: the artifact may require an actual client-terminal size change (a real SIGWINCH reaching the attached client), not merely a server-side `resize-window` on the session, which is a mechanism round 1 already tried in isolation (cascading the *harness* window's resize) and also failed to reproduce — so neither mechanism alone reproduces it in this environment, and a combined/different trigger technique needs new design thought, not another implementer-side retry of the same two mechanisms.

- Round 3: reproduced, all three tests pass reliably (four consecutive full-suite runs).
  The mechanism both prior rounds missed is ordering, not trigger choice: round 2's `resize-window` shrink against reed's own socket DOES paint the artifact — measured live, the dot padding appears within ~10ms of the shrink and stands for as long as the window stays smaller than the attached client (`resize-window` flips the window's `window-size` option to `manual`, so the shrunken window stands against the 140x42 client instead of snapping back under `latest`) — but the immediately-following grow clears it instantly and permanently (zero hits across ~390 tight no-sleep capture samples over 2.5s, three runs), so both prior rounds' polls, which all started after the back-to-back shrink+grow pair, sampled only the clean fully-covered post-grow regime and could never hit.
  `TestSmokeDotFillResizeControl` now asserts `pollPaneHasDotRun` between the shrink and the grow, then grows past the original and asserts the padding clears via a new `pollPaneDotRunClears` helper (first-clean-capture inverse of `pollPaneHasDotRun`), which is what proves the dots track the window-versus-client mismatch rather than reed's content — a deviation from Card 5's fire-both-then-poll ordering, forced by the measured behaviour above.
  The SIGWINCH-cascade question round 2 left open is also now answered negatively: resizing the outer harness window so the attached client's own pty changes size and `window-size latest` follows it was measured live in both directions and in 1-column drag steps, each with a concurrent tight sampling loop, and produced zero hits — in this container's tmux 3.6 build the client repaint after a followed resize is atomic from `capture-pane`'s viewpoint, so the transient stale-paint smear the field report describes is not observable headlessly, and the standing window-smaller-than-client mismatch is this environment's reliable reproduction of the same client-side padding paint.
