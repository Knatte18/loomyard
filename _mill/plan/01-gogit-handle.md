...
- **Round 2 fix note:** batch 2's card 9 (`internal/gitrepo/gogit_test.go`) ended up building its own equivalent, package-`gitrepo`-local fixture (`newLinkedParityFixture`) rather than consuming `linkedWorktreeFixture`/`newLinkedWorktreeFixture`/`newLinkedWorktreeFixtureViaJunction` from this card — no test anywhere ever called the three symbols this card added. A round-2 holistic review flagged them as dead code; they were deleted (the whole file, since nothing else in it remained) rather than wired into `parity_test.go`, because `gogit_test.go`'s own linked-worktree suite already covers every exported and unexported migrating method (direct and junction-reached) against the real CLI oracle, so wiring a second, parallel fixture into `parity_test.go` would only duplicate that coverage. `internal/gitrepo/fixtures_test.go` is deleted as of the round-2 fix pass; do not recreate it.
