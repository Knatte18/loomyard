...
- **Requirements:** Cover `NewDiscussionGate` with a pass case, a findings case, and an error case proving a validator error is returned rather than reported as a failed gate — a decision record that is a directory is the error fixture the existing validate tests already use.
  ...
  Reuse the package's three existing fixture helpers rather than writing new ones — `validDecisionRecord` and `writeDiscussionFixture`, both declared in the discussion validator's own test file, and `seedPlanValidateFixture`, declared in the plan validator's — since all three are package-level and already build exactly the on-disk shapes these cases need.
  Batch 5 relocates all three into a surviving fixture file when it deletes their current hosts, so this reuse is deliberate rather than a dependency on files that are about to disappear.
