...
- **Requirements:**
  ...
  - **`resetTraceState` must not copy `traceOnce` by value.** `sync.Once` contains a `noCopy` marker; assigning it to/from a local (`savedOnce := traceOnce` / `traceOnce = savedOnce`) fails `go vet`'s copylocks check even though nothing here runs concurrently. There is no need to restore the prior `traceOnce` value at test end — every test that needs a fresh resolution already calls `resetTraceState` itself before running, so a forward-only reset is sufficient: `traceOnce = sync.Once{}` and `traceID = ""`, with no `t.Cleanup` restore of `traceOnce`. `traceID` (a plain `string`, no lock) may still be saved/restored via `t.Cleanup` if convenient, but is not required either.
