...
- **Requirements:** Add three functions to `internal/reedengine/overlay.go`, placed after the existing `listSessionsVia`.
  ...
  2. Build a throwaway engine value carrying **only** the one field the closure walk reads — `eng := &Engine{cfg: Config{Shell: shellPath}}` — and compute `pids := eng.descendantClosurePIDs(sessionReapRoots(live))`. This runs **before** the kill, while the panes are still alive: `descendantClosurePIDs` derives the closure from live parent links, so a closure computed after `kill-session` reparents the pane children collapses to the root pids alone and silently loses the detached agent descendants the reap exists to kill. `paneProcessTreePIDsLocked` carries this same rule in its own doc comment.

  Populate no other field, and in particular construct **no** `Geometry` value here, not even a zero one. `descendantClosurePIDs` is a method only because its Windows body spawns `e.cfg.Shell`; its Linux body reads no `Engine` field whatsoever, and neither body reads geometry. CONSTRAINTS.md's Told-Geometry Invariant names `internal/hubgeom` and `internal/standalonegeom` as the only `Geometry`-struct constructors, and no production file outside those two packages builds a `Geometry` literal today — so the reap must not become the first, and it has no need to be. Say so in the doc comment: the throwaway exists solely to reach one method, and it is deliberately not a usable `Engine`.

  The struct literal is used rather than `New` because `New` builds a `TmuxCmd` from `cfg.Tmux` and `geom.SocketKey` and would therefore require exactly the `Geometry` value this card declines to construct. The tmux side is already covered by `cmd`, which steps 1 and 3 take directly.
