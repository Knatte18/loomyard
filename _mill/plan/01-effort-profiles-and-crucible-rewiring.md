...
4. **Round-tag and tier-enumeration consistency (implementer).** Confirm the renamed "Model + effort selection" section lists all five tier names, and that every example round tag in both edited files uses the `<model>-<effort>-r<N>` form with no leftover `<model>-r<N>`. Then grep for `rotate` and `next model` as a second expected-hit inventory, **scoped to the two edited files only** — `crucible/orchestrator-prompt.md` and `crucible/README.md`. Do not grep `crucible/` as a whole: the directory also holds seven per-module instance prompts (`board-`, `builder-`, `fabric-`, `gitrepo-`, `loom-planner-`, `reed-`, `webster-review-prompt.md`) plus `review-prompt-template.md`, all of which describe model-only rotation for their own historical campaigns, are outside this batch's `Edits:` list, and must not be touched. Within the two edited files, the legitimate remaining hits are: `README.md`'s `### Why rotate the model` heading and its paragraph (~48, deliberately kept — it explains model diversity specifically, and the new adjacent paragraph covers the effort axis); the historical reed-campaign prose (~110 and ~120), which describes a campaign that predates effort tiers and must not be retroactively reworded; and the model-rotation rationale carried into `orchestrator-prompt.md`'s renamed "Model + effort selection" section (~56), which edit 5 of card 2 deliberately preserves. Any other line in those two files still framing round-to-round rotation as model-only is stale and must be updated.
