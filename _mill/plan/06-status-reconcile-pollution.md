...
- **Creates:** none
- **Deletes:** none
- **Requirements:** In `internal/weft/status.go`, remove the junction-integrity reporting now owned by warp: drop `checkJunction` and the `junction_ok`/`junction_reason` keys from the `Status` map (and the `hostLink`/`weftLyxDir` params that only served the junction check, if they become unused — adjust the `weft/cli.go` caller accordingly). Keep the content fields (`weft_worktree`, `branch`, `dirty`, `ahead`, `behind`). In `weft/status_test.go`, **remove** the two junction-dedicated tests entirely — `TestStatus_Junction` and `TestStatus_JunctionOk_Windows` (their only assertions are `junction_ok`/`junction_reason`, and they pass the to-be-removed `hostLink`/`weftLyxDir` args) — rather than just stripping their assertions (which would leave empty tests and uncompilable `Status(weftRepo, hostLink, weftLyxDir, …)` calls). Update the surviving content test `TestStatus` to call the trimmed signature (`Status(weftRepo, []string{"_lyx"})`, dropping the two `""` args). Also remove the now-unused `internal/fslink` import from `weft/status_test.go` — it was used only by the deleted `TestStatus_JunctionOk_Windows`, and an unused import is a Go compile error that would fail batch-6 verify. In `weft/cli_test.go`, update `TestRunCLI_StatusWithMinimalFixture` to remove the `junction_ok` field assertion that belonged to the removed junction reporting (replace with an assertion that the content fields `branch` and `weft_worktree` are present instead). `weft` remains the content-sync owner; topology/junction reporting is warp's.
