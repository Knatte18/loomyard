...
- **Requirements:** In `Topology.Add`, immediately after the `WireJunctionsWith` call that ends step 10b and before step 11's warp push, add one new step that records the pair's provenance.
  ...
  It then commits the record on the new pair's weft branch with `CommitWeftPaths(rec, weftPath, l.AnchorRel, []string{OriginRecordRel()}, "fabric: record parent branch for "+slug, opts)`, using the `weftPath` local already bound earlier in the function and threading `Add`'s own recorder through so the landed commit appears in `AddResult`'s mutation record alongside the record's own write entry, and rolls back the same way on error.
  The commit's `sha` and `committed` returns are not read at the call site — the helper records the commit itself, at its own success site — so discard them, and state in a comment that the recording happens inside the helper rather than here, per the overview's `origin-record-records-both-its-write-and-its-commit` decision.
...
- **Requirements:** The new file carries the `integration` build constraint as its first non-empty line and a `TestMain` is already provided by the package, so add none.
  ...
  (g) the `AddResult` mutation snapshot contains exactly one `KindFileWritten` entry whose target is the record, and exactly one `KindCommitCreated` entry whose target is the new pair's weft worktree and whose detail is the sha the record commit landed at — this assertion is the only guard that entry has, because the live-state oracle classifies that kind as a git-state kind and exempts it from its commission direction entirely;
  (g2) an add run with the sync options' git-skipping flag set still writes the record and still carries its `KindFileWritten` entry, but lands no commit and carries no `KindCommitCreated` entry — the guard that the commit is never claimed on a run that deliberately performed none;
...
- **Requirements:** Run the live-state matrix cells for the `add` verb at both anchors and confirm the mutation oracle still passes in both directions with no assertion loosened.
  The expectation is that it passes unchanged for the record's write entry: the omission direction is covered because the pair's own worktree-creation entries already cover every path beneath both worktree roots, including the second path the warp junction exposes for the same file, and the commission direction is covered because the record's own entry names a path outside the git metadata directory that does produce a manifest change.
  State explicitly, as part of this card's output, that the oracle covers the write entry only: the commit entry's kind is classified as a git-state kind in that oracle's own observable-kind table and is therefore exempt from the commission direction, so no matrix cell can catch a missing or spurious commit entry and card 8's integration assertion is its only guard.
  Confirm too that the new kind of entry does not trip the oracle's unclassified-kind fatal path — that path fires for a kind absent from the table, and the commit kind is already on it, so this is a confirmation rather than an expected failure.
