...
- **Deletes:** none
- **Requirements:** Add `type Config struct { Pathspec string \`yaml:"pathspec"\` }`, `func DefaultConfig() Config { return Config{Pathspec: "_lyx"} }`, and `func (c Config) Dirs() []string { return strings.Fields(c.Pathspec) }`. Add `func LoadConfig(weftBaseDir string) (Config, error)` that calls `config.Load(weftBaseDir, "weft", map[string]string{"pathspec": DefaultConfig().Pathspec})` and maps `raw["pathspec"]` into `Config`. `config.Load` reads `<weftBaseDir>/_lyx/config/weft.yaml` and, via `FindBaseDir`, returns the generic error `not initialized: _lyx/ directory not found in <weftBaseDir>` when `<weftBaseDir>/_lyx` is absent. Wrap that case into a clearer weft message: when the underlying error contains "not initialized", return `fmt.Errorf("weft worktree or its _lyx is missing at %s", weftBaseDir)` (mirrors the board/worktree LoadConfig wrap pattern); otherwise return the error unchanged. The caller passes `weftBaseDir = filepath.Join(layout.WeftWorktree(), layout.RelPath)` — junction-independent (reads the real `_lyx/config/weft.yaml` in the weft worktree, never via the host junction). Tests (`config_test.go`, black-box `package weft` or white-box — match the package style of sibling weft tests): default pathspec when no `weft.yaml`; override from a written `<base>/_lyx/config/weft.yaml` containing `pathspec: "_lyx _codeguide"`; `Dirs()` splits on whitespace into `["_lyx","_codeguide"]`; `LoadConfig` returns the wrapped "weft worktree or its _lyx is missing" error when `<base>/_lyx` is absent (the missing-weft-worktree case).
...
- **Deletes:** none
- **Requirements:** Mirror board's `spawnSync` as `func spawnPush(weftPath string) error`. **First line of `spawnPush` (in BOTH `spawn_windows.go` and `spawn_other.go`): return `nil` immediately if `os.Getenv("WEFT_SKIP_GIT") == "1" || os.Getenv("WEFT_SKIP_PUSH") == "1"`** — so no child process is forked under the skip guards (the `env-skip-guards` decision requires SKIP_PUSH to suppress the detached spawn, and tests setting `WEFT_SKIP_PUSH=1` must not fork real `lyx` processes). Otherwise build `exec.Command(exe, "weft", "--weft-path", abs, "push")` where `exe, _ = os.Executable()` and `abs, _ = filepath.Abs(weftPath)`. `spawn_windows.go` (`//go:build windows` implicit via filename) uses `SysProcAttr{HideWindow:true, CreationFlags: createNoWindow | createNewProcessGroup}` with the same two const values as board; `spawn_other.go` (`//go:build !windows`) uses `SysProcAttr{Setsid:true}`. Leave stdio nil; `cmd.Start()` without `Wait()`. No test file (the spawn path is exercised end-to-end in the integration test, card 12; the skip-guard no-op is covered indirectly by `cli_test` sync under `WEFT_SKIP_PUSH`).
...
- **Deletes:** none
- **Requirements:** Add `func Status(weftWorktree, hostLink, weftLyxDir string, pathspec []string) (map[string]any, error)` returning a map with keys: `weft_worktree` (the path), `branch` (`git rev-parse --abbrev-ref HEAD` in `weftWorktree`), `dirty` (bool — `git status --porcelain --` + pathspec is non-empty), `ahead`/`behind` (ints from `rev-list --count @{u}..HEAD` and `rev-list --count HEAD..@{u}`; both null when no upstream), and junction integrity `junction_ok` (bool) + `junction_reason` (string, empty when ok). Junction check (Windows-junction-correct — do NOT use `os.Readlink`, which on an NTFS junction errors or returns a `\??\C:\…` device path; the existing `links.go` deliberately avoids Readlink): `os.Lstat(hostLink)` — if missing (`os.IsNotExist`) → `junction_ok=false, reason="host _lyx junction missing"`. If present but the mode bit `info.Mode()&os.ModeSymlink == 0` (a plain dir, not a junction/symlink) → `junction_ok=false, reason="host _lyx is not a junction"`. Otherwise resolve BOTH ends with `filepath.EvalSymlinks` (which resolves junctions to a clean absolute path on Windows): compare `EvalSymlinks(hostLink)` to `EvalSymlinks(weftLyxDir)` (after `filepath.Clean`) → ok when equal, else `junction_ok=false, reason="host _lyx junction points elsewhere"`. If either `EvalSymlinks` errors → `junction_ok=false` with the error in `reason`. Status must complete and return even when `junction_ok=false` (config + git already resolved from the weft worktree). Tests (`status_test.go`): `junction_ok=false` with a missing `hostLink` and with a plain-dir `hostLink`; `dirty` true/false; `branch` reported; and a positive junction case — on Windows create the junction via `cmd /c mklink /J` (a `mustRunMklink` helper; junctions need no privilege), elsewhere `os.Symlink`, with `t.Skip` only if neither is creatable — asserting `junction_ok=true`. Build the weft worktree as a temp git repo.
...
- **Requirements:** Add `func RunCLI(out io.Writer, args []string) int`. Parse a `--weft-path` string flag (internal, for the detached push child), mirroring board's `--board-path`. Then:
  ...
  - **Else (cwd path):** `cwd, _ := paths.Getwd()`; `l, err := paths.Resolve(cwd)` (Err on failure); `weftWorktree := l.WeftWorktree()`; `weftBaseDir := filepath.Join(l.WeftWorktree(), l.RelPath)`; `cfg, err := LoadConfig(weftBaseDir)` (Err on failure); `pathspec := scopedPathspec(l.RelPath, cfg.Dirs())`. Dispatch on subcommand:
    ...
    - `sync` → `Commit(weftWorktree, pathspec)` then `spawnPush(weftWorktree)` → `output.Ok`. (`spawnPush` is itself a no-op under `WEFT_SKIP_GIT`/`WEFT_SKIP_PUSH` per card 8, so no child is forked in skip mode.)
...
- **Creates:** none
- **Deletes:** none
- **Requirements:** In `docs/overview.md`: (1) in the "## Weft overlay model" → "### Status" subsection, update the bullets to record that the Go core (paths geometry, `lyx weft` command) has landed for task 006 and that paired spawn lands with it. Specifically replace the exact sentence "**Weft has no Go code yet** — portals are still the live mechanism." with one stating the weft engine (geometry, paired spawn, `lyx weft`) now exists in Go, that paired `lyx worktree add` hard-requires a weft repo built by the downstream hub-creator, and that portals remain on hold (not yet removed). Keep the surrounding task-006/008 framing intact. (2) In the "## Modules" list, add a `weft` entry: "**weft** — owns all git into the paired weft repo (`lyx weft status|commit|push|pull|sync`)." Do not touch the geometry method list (batch 1 owns it).
