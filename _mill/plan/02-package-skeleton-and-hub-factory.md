...
- **Requirements:** Add an exported `GitStatusPorcelain(tb testing.TB, repoPath string) string` to `internal/fabricengine/fabrictest/hub.go`, returning the raw output of `git status --porcelain` run in `repoPath` and calling `tb.Fatalf` on failure — the same body as the existing `gitStatusPorcelain` in `internal/fabricengine/weftgit_exclude_test.go`, with `*testing.T` widened to `testing.TB` so states and cells can use it too.
  ...
  `internal/fabricengine/dotlyxjunction_integration_test.go` shares that same external `package fabricengine_test` compilation unit and also calls the now-deleted `gitStatusPorcelain`, discovered only once the delete broke `go vet`'s compile — its one call site is retargeted to `fabrictest.GitStatusPorcelain` too, with its own `internal/fabricengine/fabrictest` import added (Go requires each file to import what it uses, even within the same package).
