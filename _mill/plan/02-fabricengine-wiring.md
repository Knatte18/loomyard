...
Batch-local decisions (all elaborated in the overview's HYBRID Shared Decisions): the mutating primitives `WireJunctions`/`UnwireJunctions`/`removeHostJunction` take an explicit `names []string` and load NO config; their callers supply names by loading each operation's TARGET-pair config from that pair's own weft base (`junctionNames(...)` in-package; `WiredNames(baseDir)` for `init`/`undo`) — never from the acting worktree's `t.cfg`, so multi-pair `Reconcile`/`Remove` use the right pathspec. Read-only health checks (`PairInSync`/`checkJunctionHealth`/`junctionRepointedDetail`) keep loading internally from the same weft base. The signature change is compiler-checked: every call site across `fabricengine`, `initengine`, `configcli`, and `loomengine` (prod + test) must be migrated for the tree to build — this batch's `verify:` widens to cover all four packages.
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Change the mutating primitives to take the wired name-set explicitly and load NO config: `WireJunctions(l *hubgeometry.Layout, slug string, names []string) error` and `UnwireJunctions(l *hubgeometry.Layout, slug string, names []string) (UnwireResult, error)`. Inside them, replace every `l.HostJunctions(slug)` with `l.HostJunctions(slug, names)` and pass `names` down to the unexported helpers (`seedLyxJunction`, `seedGitExclude`, `unseedLyxJunction`, `unseedGitExclude`) as a new `names []string` parameter (each helper receives the slice, never loads). `unseedLyxJunction` delegates to `unseedJunctionRecords(l.HostJunctions(slug, names))`; `unseedJunctionRecords` (already takes `[]hubgeometry.HostJunction`) is unchanged. (`removeJunctionRecords` lives in `weftwiring.go` and is card 7's domain — not touched here.) Because `WireJunctions` no longer reads config, its documented missing-target self-heal is preserved verbatim (`seedLyxJunction` still `os.MkdirAll`s a missing weft target first). Update the godoc to state the wired name-set is now supplied by the caller (sourced from `fabric.yaml` `pathspec`, hub-reserved names filtered). **Migrate every call site (compiler-forced):** (a) fabricengine PROD — source `names` from each operation's TARGET-pair weft base via the in-package `junctionNames(...)`, NOT from `t.cfg` (see overview hybrid Decision: `Reconcile` iterates many pairs and each may have its own `pathspec`, so `t.cfg` would use the wrong worktree's name-set and could loop the re-wire against the per-pair health check). `checkout.go:152` (`Checkout` re-wires the current worktree's slug): `names, err := junctionNames(filepath.Join(l.WeftWorktreePath(slug), l.RelPath))`; on error roll back and return as the existing wiring-failure path does, then `WireJunctions(l, slug, names)`. `reconcile.go:155` (`Reconcile` re-wires a per-pair `hostLayout`): `names, err := junctionNames(filepath.Join(hostLayout.WeftWorktree(), hostLayout.RelPath))`; on error set `pr.Error`/`pr.Action = ReconcileActionJunctionRepointed` mirroring the existing `wireErr` branch, else `WireJunctions(hostLayout, slug, names)` — this uses the SAME per-pair weft base as `checkJunctionHealth(hostLayout)` (card 9) so re-wire and health never diverge. (b) All test callers pass the literal default set `[]string{"_lyx", "_pattern"}` (they wire the default pair), EXCEPT any test needing a different set: `junction_pattern_integration_test.go` and `remove_junctions_integration_test.go` — pass `[]string{"_lyx", "_pattern"}` at their existing `WireJunctions`/`UnwireJunctions` call sites. The `MaterialisesMissingWeftTarget` test (`junction_pattern_integration_test.go:86`) needs NO config seeding — `WireJunctions` no longer reads config, so passing names directly fixes the R6/R7 wire breakage and self-heal of the deleted weft target still works. REMOVE its now-dead `lyxtest.SeedConfig(t, fixture.WeftPrime, …)` call (`junction_pattern_integration_test.go:70-73`): after this change nothing in that test reads the seeded config, so leaving it in would falsely imply config is still consulted on the wire path. BUT the nested `TestRemove_TearsDownNestedJunction` (`remove_junctions_integration_test.go`) DOES still exercise `Remove` (line 74), and `Remove` loads the removed slug's config to know which nested junctions to tear down (card 7); its fixture (`Add`+`WireJunctions` at `RelPath == "sub"`, no `lyx init`) has no config at `<slug>-weft/sub/_lyx/config/`. So in this test, after the nested wiring and before `topology.Remove(nestedLayout, slug, true)`, seed the nested weft config: `os.MkdirAll(hubgeometry.ConfigDir(filepath.Join(l.WeftWorktreePath(slug), "sub")), 0o755)` then `os.WriteFile(hubgeometry.ConfigFile(filepath.Join(l.WeftWorktreePath(slug), "sub"), "fabric"), []byte(fabricengine.ConfigTemplate()), 0o644)` — so `Remove`'s name-load finds `_lyx _pattern` and the nested junctions are actually removed (the happy path the test asserts). `Remove`'s best-effort load (card 7) means an unseeded run would not hard-fail but would leak the nested junction, which is exactly why this test seeds it — to verify the full nested teardown, not the degraded path. Test files to migrate: `junction_pattern_integration_test.go`, `junction_repoint_test.go`, `remove_junctions_integration_test.go`, `checkout_rollback_test.go`, `checkout_index_refresh_test.go`, `reconcile_stale_registration_test.go` (fabricengine); `configcli/configcli_integration_test.go:59` and `loomengine/preflight_integration_test.go:41` (other packages — pass `[]string{"_lyx", "_pattern"}`). `initengine`'s `init.go`/`undo.go` callers are migrated in card 11.
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Change `removeHostJunction(l, slug)` (`internal/fabricengine/weftwiring.go:131`) to `removeHostJunction(l *hubgeometry.Layout, slug string, names []string) error`, passing `l.HostJunctions(slug, names)` into `removeJunctionRecords`; it loads no config. Update its caller `remove.go:91` (`Topology.Remove`, which tears down the NAMED `slug`'s pair): source names from that slug's own weft base, not `t.cfg` (the removed slug may differ from the acting worktree and carry its own `pathspec`). Before step 5, add `names, err := junctionNames(filepath.Join(l.WeftWorktreePath(slug), l.RelPath))` — but make this BEST-EFFORT, matching `Remove`'s existing `_ = removeHostJunction(...)` discard posture: `Remove` is a teardown and must NOT hard-fail when the target pair's config is absent/unreadable (a pair is often removed *because* it is broken, and `TestRemove_TearsDownNestedJunction` at `remove_junctions_integration_test.go:74` sets up a nested pair via `Add`+`WireJunctions` with NO `lyx init`, so `<slug>-weft/sub/_lyx/config/fabric.yaml` does not exist). On load error, do NOT return — set `names = nil` (optionally log) and continue. Then `_ = removeHostJunction(l, slug, names)`. Residual-risk note in the godoc: when config is unreadable, `names` is nil, so `removeHostJunction` removes nothing and a nested junction (`RelPath != "."`) may leak past `fslink.RemoveLinksIn`'s root-level-only safety net (step 6); this is the accepted teardown-robustness trade-off (never hard-fail a teardown; never guess a wiring set). This best-effort posture is why the nested test seeds the nested config in card 6 — so the happy-path nested teardown is still verified. `remove.go` must import `"path/filepath"` if not already present. `weftwiring.go` itself needs no `"path/filepath"` import. Update `removeHostJunction`'s godoc to note the name-set is caller-supplied.
- **Note on no-fallback:** the discussion's no-fallback rule targets the WIRING path (never wire a guessed set — dangerous). Teardown best-effort (remove what you can, safety-net the rest) is a distinct, safe semantic and does not violate it; `Remove` already discards `removeHostJunction`'s error, so non-hard-failing here is consistent with the existing contract.
- **Commit:** `feat(fabricengine): thread names param into removeHostJunction; caller sources from slug weft base`
