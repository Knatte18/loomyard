...
`verify: go test -tags integration ./internal/fabricengine/ ./internal/initengine/` compiles and runs both packages against the batch-1 hubgeometry signatures — the point at which the repo compiles again after batch 1's signature change. Most existing `fabricengine` integration tests (`junction_pattern_integration_test.go`, `reconcile_stale_registration_test.go`, etc.) exercise the threaded call sites through real host+weft fixtures whose weft ROOT already carries `fabric.yaml`, so they validate that config-sourced names reproduce today's `_lyx`+`_pattern` wiring with no behavior change. The ONE exception is `remove_junctions_integration_test.go`'s `TestRemove_TearsDownNestedJunction`, which wires at `RelPath == "sub"`: because config is now loaded from the RelPath-adjusted weft base (`<slug>-weft/sub`), that test must additionally seed a nested `fabric.yaml` under `sub/_lyx/config/` before wiring — done in card 6 (the fixture's root-only seed predates config-driven wiring). No other existing test changes behavior. `initengine`'s own init tests validate the reorder. `-tags integration` is required for the git-spawning fixtures. New proof tests (extensibility, wiring-guard, no-fallback error) are added in batch 3; this batch relies on the existing suite staying green to confirm the threading is behavior-preserving.
