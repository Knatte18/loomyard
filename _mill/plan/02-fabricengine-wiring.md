...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Change `removeHostJunction(l, slug)` (`internal/fabricengine/weftwiring.go:131`) to `removeHostJunction(l *hubgeometry.Layout, slug string, names []string) error`, passing `l.HostJunctions(slug, names)` into `removeJunctionRecords`; it loads no config. Update its caller `remove.go:91` (`Topology.Remove`, which tears down the NAMED `slug`'s pair): source names from that slug's own weft base, not `t.cfg` (the removed slug may differ from the acting worktree and carry its own `pathspec`). Before step 5, add `names, err := junctionNames(filepath.Join(l.WeftWorktreePath(slug), l.RelPath))` (the removed slug's weft config is still present — teardown of the weft worktree is step 9, after step 5); on error, surface it (return the load error from `Remove` — a slug whose config cannot be read is a genuine broken state, and this is before any destructive step). Then `_ = removeHostJunction(l, slug, names)` (the `_ =` best-effort discard of the *removal* result is unchanged). `remove.go` must import `"path/filepath"` if not already present. `weftwiring.go` itself needs no `"path/filepath"` import (the function no longer computes a base). Update `removeHostJunction`'s godoc to note the name-set is caller-supplied; because it no longer loads config, the earlier internal-load residual-junction-leak risk is dissolved.
