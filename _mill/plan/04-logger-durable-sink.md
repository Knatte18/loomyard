...
- **Requirements:**
  ...
  1. `sinkOnce.Do(func() { ... })` body:
     - **Check the `SetDurableSinkDir` (Card 12) override FIRST, before the `testing.Testing()`/`LYX_TRACE` gate.** If a non-empty override directory has been set, treat that call itself as the opt-in signal and proceed directly to opening the sink in that directory (leaving `header.WorktreeRoot` empty, test-seam path) — **regardless of `testing.Testing()`**. Only when no override directory is set does the function fall through to: if `testing.Testing()` is true and `os.Getenv("LYX_TRACE") != "1"`, leave `sinkOK = false` and return (no filesystem or geometry access at all — this is the `LYX_TRACE=1` opt-in gate from `test-entry-activation`, for entry points that never call `SetDurableSinkDir` at all). **Getting this order backwards breaks every unit test that calls `SetDurableSinkDir(t.TempDir())`** (batch 4's Cards 15/16, batch 5's Cards 21/22, batch 6's Card 26) — under `go test`, `testing.Testing()` is always true and none of those tests set `LYX_TRACE=1`, so a `testing.Testing()`-first check would keep `sinkOK` false regardless of the override, silently defeating the seam every later test card depends on.
     - Call `armHeader()` (idempotent — a no-op if `Arm()` already ran).
     - Resolve the target directory: if the override from the step above was set, use it. Otherwise call `hubgeometry.Resolve(...)` (via `hubgeometry.Getwd()` for the starting cwd) and `layout.WorktreeLogsDir()` (batch 1); on any resolution error, set `sinkOK = false` and return — **never** fail the calling operation (per `lazy-sink-open`'s "failure to open a diagnostic sink must never break the operation being diagnosed"). On success, set `header.WorktreeRoot = layout.WorktreeRoot`.
