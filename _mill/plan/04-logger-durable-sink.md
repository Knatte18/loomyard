...
- **Moves:** none
- **Requirements:**
  Untagged unit tests in `internal/logger/sink_test.go`, using `SetDurableSinkDir(t.TempDir())` (Card 12) so no `hubgeometry.Resolve` runs (Test Tier Purity):
...
- **Moves:** none
- **Requirements:**
  Add to `internal/logger/sink_test.go` (created by Card 15):
