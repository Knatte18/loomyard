...
- **Requirements:**
  ...
  - Update the "Known instrumented call sites today" list (currently naming `internal/reedengine/lifecycle.go`, `internal/shuttleengine/run.go`, `internal/burlerengine/engine.go`) to add `internal/scoutengine/ensureserver.go` (its supervised-daemon spawn/teardown, added by batch 13) and `internal/perchengine/engine.go` (its first-pass calls, added by batch 12) — both are now real instrumented call sites this entry should name.
