...
- **Requirements:** Delete the directories `internal/builderengine` and `internal/buildercli` in full.
  ...
  In `tools/sandbox/main.go`: delete the entire `case "builder-suite":` block, drop `builder-suite` from the two usage/doc comment suite lists, and change the header comment's leading count from "It supports ten subcommands" to nine — that count is not reachable by any acceptance-grep pattern, so it will not self-surface if left stale.
