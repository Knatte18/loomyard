...
**Build-unit note (intended non-atomicity across cards):** intermediate per-card commits do not all compile, by design. Non-test compilation (`go build`) is only expected to pass after Card 4 (`hackernews.go`), because `defaultAdapters()` (Card 1) forward-references both concrete adapter types (`redditAdapter` from Card 3, `hackerNewsAdapter` from Card 4). Test compilation (`go test`, the batch `verify`) is only expected to pass after Card 6: Card 3 deletes `isRedditUrl`/`toRedditJsonUrl`/`fetchReddit`/`formatRedditPost`/`formatRedditSubreddit`/`redditThing`/`redditData`, but `reddit_test.go` still references them until Card 5 and `fetch_test.go`'s Reddit routing test until Card 6. mill-go's `verify` runs once at the end of the implementer round (after all 8 cards, not per card), so these intermediate non-compiling commits across Cards 1–6 are expected and not a defect. Do not attempt to make each intermediate card independently compilable.
