...
- **Requirements:** Run the live captures first, before writing any other card in this batch, and build every failure fixture from what actually came back rather than from a written description.
  ...
  `plugins/prowler/scripts/testdata/github-read/bodies/probe-file.json` is a real contents-API object response for a file, `plugins/prowler/scripts/testdata/github-read/bodies/probe-dir.json` a real array response for a directory, and `plugins/prowler/scripts/testdata/github-read/bodies/probe-symlink.json` and `plugins/prowler/scripts/testdata/github-read/bodies/probe-submodule.json` object responses whose `type` field carries those two values.
  `plugins/prowler/scripts/testdata/github-read/bodies/error-401.json`, `plugins/prowler/scripts/testdata/github-read/bodies/error-403.json`, and `plugins/prowler/scripts/testdata/github-read/bodies/error-404.json` are error bodies carrying a `status` field, in the shape the capture observed.
...
- **Requirements:** In `plugins/prowler/scripts/github-read.sh`, add the two-call `gh api` fallback, reached whenever the raw attempt did not succeed.
  ...
  Record in a comment that the probe's second call is paid only on the fallback path, which is already the rare and far slower one, and that its base64 content is downloaded and discarded, which is accepted for the same reason.
