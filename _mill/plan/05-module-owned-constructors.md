...
- **Requirements:** Delete `PatternDir`, `PatternFile` and `(*Location).PatternFileHere` from `internal/lyxcwd/lyxcwd.go`, but **keep the `PatternDirName` const, transitionally**: `fabricengine/pull.go:299` reads it as a git pathspec and five fabric test files still read it too (`fabriccli/cli_test.go:416`, `pull_integration_test.go:258,292`, `unwire_test.go:65,71`, `junction_pattern_integration_test.go` at nine sites), none of which is in this batch's card set — deleting the const here would fail this batch's own `go vet ./...` and `go test ./cmd/lyx/...` gates a full batch before card 35 retargets those readers. Card 35 deletes it and cuts every reader over in the same card. Declare in `internal/pattern`: `const DirName = "_pattern"`, `func Dir(baseDir string) string` = `filepath.Join(baseDir, DirName)`, `func File(baseDir string) string` = `filepath.Join(Dir(baseDir), "PATTERN.md")` and `func FileHere(l *lyxcwd.Location) string` = `File(l.AnchorPath())`. `internal/pattern` becomes a declarer of `_pattern`, so **this card moves that token's ownership row in the same breath** — `const DirName = "_pattern"` is a production const declaration of a policed token, and this batch's `verify` runs `go test ./internal/lyxcwd/...`, which executes the guard. In `internal/lyxcwd/enforcement_test.go`, change `_pattern`'s row to `internal/pattern` **plus, transitionally, `internal/lyxcwd`** for the retained `PatternDirName` const — the same two-owner shape the `_lyx` row already uses for its own transitional declarer. Card 35 removes the transitional entry when it deletes the const and swaps in `internal/fabricengine`, which declares its own pathspec const there; `fabricengine` is not added here, because nothing in this card puts a `_pattern` literal in `fabricengine`. The Pattern Leaf Invariant needs no widening: `internal/pattern` imports stdlib plus `internal/lyxcwd` only — which is exactly what `leaf_enforcement_test.go:23-24` already allowlists — never `configengine`, never `fabricengine` and never `weftname`. None of the four declared bodies needs a config path. `pattern/doc.go` is in this card's `Edits:` for its stale godoc: `:13` names `hubgeometry.Layout.PatternFileHere()` — now `pattern.FileHere(l)`, declared by this card — and `:49` says "built from a `Layout` field", which becomes `Location`; correct both. The moved test file drops the sub-tests covering `WeftPatternDir`/`WeftPatternDirFor`/`HostPatternLink`/`HostPatternLinkHere` (`pattern_test.go:88-118`) — those four accessors are deleted in batch 6 and the coverage they stood in for is re-asserted there against the generic junction path — and keeps everything else. Rename the file to `patternpath_test.go` so it does not collide with the existing `internal/pattern/pattern_test.go`.
