...
- **Requirements:**
  ...
  `Parse` is filesystem-free but `Build` is not, because four registry constructors reach disk at construction time (two create their resolved run directory, two eagerly probe a stencil), and this package neither suppresses nor wraps those effects;
