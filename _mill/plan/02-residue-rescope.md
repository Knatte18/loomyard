...
- **Requirements:** Delete the private const `patternDirName` and its doc comment from `internal/fabricengine/pull.go`.
  ...
  Keep the "Separator placement" paragraph and the "RelPath-blind scope (documented limitation)" paragraph — both limitations survive the re-scope unchanged — but adjust their contents.
  The RelPath-blind paragraph names the deleted identifier as well as the old path ("the pathspec is the bare `patternDirName` at the weft worktree root", "a subpath-anchored hub whose `_pattern` lives at `RelPath/_pattern`"): the identifier mention must become `pattern.PathspecFile`/`PathspecDir`, not only the `_pattern` path text.
  A doc comment naming a const that no longer exists is a dangling reference, not a wording preference.
