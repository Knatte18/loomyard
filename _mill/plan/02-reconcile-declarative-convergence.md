...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Migrate **every** per-pair `junctionNames(filepath.Join(<pair's own weft base>, RelPath))` call site to the repo-wide `BoardDir` base, so no site reads a per-worktree `fabric.yaml` that batches 3-4 stop materializing. There are six such sites — migrate all of them to `RepoWiredNames(<layout>)` (each of which loads from `hubgeometry.BoardDir(l.Hub)`):
  - `checkJunctionHealth` (reconcile.go:341) → `RepoWiredNames(hostLayout)`;
  - `Reconcile`'s inline wiring load (reconcile.go:161) → `RepoWiredNames(hostLayout)`;
  - `junctionRepointedDetail` (reconcile.go:390) → `RepoWiredNames(hostLayout)`;
  - `PairInSync` (drift.go:92, uses layout `l`) → `RepoWiredNames(l)` — this one is wired into loom's preflight per drift.go's own doc comment, so leaving it unmigrated would permanently report "junction check unavailable" and block loom preflight on every worktree;
  - `Topology.Checkout` (checkout.go:156, uses `l.WeftWorktreePath(slug)`) → `RepoWiredNames(l)` — otherwise `lyx fabric checkout` hard-fails and rolls back on every worktree;
  - `Topology.Remove` (remove.go:104, best-effort) → `RepoWiredNames(l)` — otherwise `lyx fabric remove` silently stops removing nested junctions.
  Preserve the load-bearing behavior documented at reconcile.go:156-160 (names must be a real config load, not `t.cfg`) — `RepoWiredNames` is still a fresh config load, just from `BoardDir` instead of the per-pair weft base. Verify `status.go`'s use of `checkJunctionHealth` (status.go:149) still compiles unchanged; if `status.go` independently loads a name-set for health, retarget it to `RepoWiredNames` too. Update the load-bearing comment text at reconcile.go:156-160 to say the name-set comes from the repo-wide `BoardDir` base. **Also, in `internal/fabricengine/hostlayout.go`, change `hostLayoutFor`'s non-sibling fallback (hostlayout.go:25) from `hubgeometry.Resolve(worktreeRoot)` to `hubgeometry.ResolveWorktree(worktreeRoot)` (batch 1's gate-free resolver):** the fallback resolves *another* worktree from its root (above a subpath anchor), so the gated `Resolve` would now hard-error with `ErrCwdOutsideAnchor` for a subpath-anchored hub — the gate-free `ResolveWorktree` reads the recorded anchor for `RelPath` without the cwd-legitimacy check, which is exactly the geometry-derivation contract this fallback needs. Update hostlayout.go's doc comment (hostlayout.go:14-20) accordingly (both paths stay byte-equivalent to `ResolveWorktree(worktreeRoot)`, not the gated `Resolve`).
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** In `Reconcile` (reconcile.go:95), add a stale-removal step that converges the pair to the repo-wide `pathspec`. **Placement:** run it **only inside the weft-worktree-exists branch** (reconcile.go:149 onward — the same branch that calls `checkJunctionHealth`/`WireJunctions`), after the add-missing wiring, for every pair reaching that branch (both the already-healthy and the just-repaired outcomes). Do **not** run it in the weft-missing branches (`reconcileMissingWeft` — weft-recreated, raw-adopted-dormant, or unmanaged-reported): those pairs have no established host↔weft junctions to converge yet (a raw-adopted pair's `_lyx` is dormant until a later reconcile once the weft exists), and scanning/stripping there would be meaningless or premature. For the pair's `hostLayout`:
  ...
  **`pr.Detail` composition:** stale-removal runs after add-missing/re-point in the same iteration, where the just-repaired case has already set `pr.Detail` via `junctionRepointedDetail(hostLayout)`. Do NOT overwrite it — **append** the stale-removal outcome (removed-names or abort text) to any existing `pr.Detail` (e.g. join with `"; "` when `pr.Detail` is non-empty, else set it directly), so a pair that was both re-pointed and had a stale junction removed reports both facts rather than one silently clobbering the other.
  **`pr.Action` when only a stale junction was removed:** when a pair is otherwise `ReconcileActionAlreadyHealthy` (nothing added/repointed) but stale-removal actually deletes an on-disk junction, the pair was NOT truly healthy — do not leave `pr.Action == ReconcileActionAlreadyHealthy`, which would misreport the JSON `action` field as unchanged. Set `pr.Action` to the same non-healthy "converged/changed" action value the just-repaired/repoint path uses (so consumers keying off `Action`, not just `Detail`, see that convergence altered the pair). If reusing the repoint action reads semantically wrong, introduce a dedicated `ReconcileActionStaleRemoved` value in the same enum and register it wherever reconcile action values are enumerated/asserted (e.g. the reconcile tests + any action-string mapping). When stale-removal removed nothing, leave `pr.Action` unchanged.
...
- **Requirements:** **First, migrate the shared fabricengine test fixtures to seed fabric config at the repo-wide `BoardDir`** — this is mandatory because card 7 migrates `checkJunctionHealth`/`PairInSync`/`Reconcile`/`Checkout`/`Remove`/`junctionRepointedDetail` to read from `hubgeometry.BoardDir(l.Hub)`, so any fixture seeding fabric config only at the per-pair weft base now yields "cannot load fabric.yaml" and fails this batch's whole-package verify. Update the shared `newFabricFixture` helper (`reconcile_stale_registration_test.go:102-108`, currently `lyxtest.SeedConfig(t, fixture.WeftPrime, {"fabric": …})`) to ALSO materialize the repo-wide config at `hubgeometry.ConfigFile(hubgeometry.BoardDir(fixture.Hub), "fabric")` (i.e. `<BoardDir>/_lyx/config/fabric.yaml`); `lyxtest.CopyPaired`/`CopyPairedLocal` do not create a `_board` dir, so create it and its `_lyx/config/` first. Then sweep every other fabricengine `*_test.go` that seeds fabric config at a per-pair weft base and exercises a migrated read (checkout, reconcile-repoint, remove, pattern-junction health) and seed the same config at `BoardDir` — the listed Edits files (`junction_pattern_integration_test.go`, `remove_junctions_integration_test.go`, `checkout_rollback_test.go`, `checkout_index_refresh_test.go`, `junction_repoint_test.go`, `config_driven_junctions_integration_test.go`) are the known sites; grep the package for the per-pair `SeedConfig(…, "fabric": …)` pattern to confirm none is missed (the whole-package `verify` surfaces any miss as a `cannot load fabric.yaml` failure). Add-path fixtures (`add_*_test.go`) are migrated separately in batch 5 (card 22), which depends on this batch. Then, create `internal/fabricengine/reconcile_stale_removal_test.go` (first line `//go:build integration`) covering the new convergence:
  ...
  **Repo-wide-base regression coverage:** add assertions (in this file or the relevant existing test) that the migrated `RepoWiredNames`-based sites resolve the name-set from the repo-wide `BoardDir` when NO per-pair `fabric.yaml` exists — seed only `<BoardDir>/_lyx/config/fabric.yaml` (not the pair's own weft base) and assert `PairInSync` returns a real health verdict (not "junction check unavailable"), `Topology.Checkout` succeeds (does not hard-fail/rollback on a name-set load error), and `Topology.Remove` still tears down junctions. This exercises the four newly-migrated sites against a hub that mirrors the new clone flow (repo-wide-only config), not a per-pair `SeedConfig` fixture.
