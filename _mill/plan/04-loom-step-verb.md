...
- **Requirements:**
  ...
  In `manifest/designs/loom.md`, add `step` to the module's verb-list prose wherever the verbs are enumerated, with a one-sentence description matching the above. Do not touch the `/ly-*` skills table row in this card — batch 6 owns that edit, and both cards editing it would collide.
