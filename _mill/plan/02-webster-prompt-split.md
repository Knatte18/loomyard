...
- **Requirements:**
  ...
  - In `internal/websterengine/recoverbatch.go`, change the `RenderForkPrompt(deps.Plan, batch, prevDigest, reportPath, deps.Layout, deps.Config.SelfFixCap)` call (near line 216) to `RenderRecoveryPrompt(...)` with the identical argument list. Also sweep this file's own stale doc comments: the package-level header comment (lines 1–13) and `recoverSpawn`'s function doc comment (near lines 172–173) both assert the recovery strand renders "the SAME fork prompt a fork would have gotten" via `RenderForkPrompt` — rewrite both to name `RenderRecoveryPrompt` and drop the "SAME fork prompt" claim (the recovery strand now gets a deliberately distinct, fuller cold-start prompt), so no comment in this file contradicts the `fork-context-hygiene` decision.
