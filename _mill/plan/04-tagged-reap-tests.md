...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add the never-entered case: the worktree directory is already gone before `runWatchdogLoop`'s first cycle runs, so the daemon never entered the session and it was never in `known`. Assert it is still reaped.
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add the empty-shell case: call `runWatchdogLoop` directly with an empty `shellPath` against a live hub socket and an orphaned worktree, and assert the session is killed and its pane root pids have exited.
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add two cases that only this tier can make.
