...
Six rounds across two campaign instalments establish the shape: the obvious behavioral defects are gone, and what remains concentrates in **proof quality** (tests that pass for the wrong reason) PLUS occasional genuinely new behavioral defects at narrowing arity/edge — round 6 found two of the latter (an octopus-shaped adoption bug, a Windows path-separator bug), the first new behavioral defects since round 4. Recurring high-yield shapes:
- A test that stays green when the mechanism it claims to guard is sabotaged, INCLUDING a round's own freshly-landed tests (round 5 found this in round 4's work; round 6 found it in round 5's work, twice — F2, F3). Re-sabotaging the immediately preceding round's own new tests is now a proven, repeatable high-yield move — do it again against round 6's.
- A correctness predicate that is close but not exact (round 6's F1: "≥2 parents, source somewhere among them" instead of "exactly 2, source second"). Look for other places in the merge surface using a bound (≥, ≤, "contains") where the SPEC or doc.go's own prose promises an exact value.
- A raw-value assumption that holds on Linux but not cross-platform (round 6's F6: forward-slash vs OS-separator). Are there other OS-dependent assumptions in the merge surface besides path separators (line endings, case-sensitivity, path length limits)?
- A mutation/guard-read running before its serializing mechanism exists (round 4's R4-F3, round 5's F2 — four sites, all now fixed and re-verified).

Concrete invariants to actively drive:
- **Re-sabotage round 6's own new tests** — the octopus regression test (`TestMergeContinue_OctopusMergeCarryingTheSource_IsNeverAdopted`) and the `mergepaths_test.go` backslash/multi-segment rows are the newest, least-battle-tested code. Does either test fail for the RIGHT reason if a nearby-but-different clause breaks instead of the one it targets?
- `sideConcludeAlreadyLanded`'s now-exact `len(parents) != 2 || parents[0] != start || parents[1] != sourceSHA` check — any other arity assumption elsewhere in the adoption/guard machinery that should be "exact N" but is coded as a bound?
- Any other git-output site assuming forward-slash without `filepath.ToSlash`/equivalent, beyond what round 6's audit covered.
- `MergeStageResolved`'s new foreign-state guard (round 6's F7) and `MergeResult.Conflicts`'s new nil-safety (round 6's F8) — both freshly landed, unaudited by anyone but their own round.
...
**This is a continuation of a two-instalment campaign, now six rounds deep.** The first instalment (mill task 85) ran three rounds and stopped with two open findings. This task's round 4 closed both plus five more (one BLOCKING). Round 5 found and fixed 7 more (3 MEDIUM, mostly proof-quality gaps). Round 6 (`opus-high-r6`) found and fixed 9 more (3 MEDIUM) — including two genuinely NEW behavioral defects at narrow arity/platform edges: an octopus-merge adoption bug (round 4's parentage check used a loose bound instead of exact equality) and a Windows path-separator bug (traced and fixed, still never executed on real Windows — no Windows host exists in this campaign). The orchestrator independently verified all three rounds from cold, sabotage-proving every new test itself and live-redriving both of round 6's behavioral fixes (the octopus fix end-to-end; the Windows fix by the strongest verification available without a Windows host). **Zero residual found by the orchestrator after any of the three.**

**No known residual is currently seeded.** This is round 7 of the operator's fixed 4-round plan (Opus medium → Fable high → Opus high → **Opus medium, this round** — the last scheduled round). Three consecutive rounds have each found and fixed real material, so do NOT read the lack of a seeded residual as near-done. Do a genuinely independent clean-room pass across the WHOLE merge surface, same as every prior round.

**Re-examination target:** round 6's own new mechanisms are the least battle-tested code in the surface — the octopus regression test and the Windows path-separator fix's test rows (see "High-yield focus" above). Sabotage them yourself, the same way round 6 sabotaged round 5's.
...
- This task's round 4: R4-F1–F7, all fixed and independently verified.
- This task's round 5: F1–F7, all fixed and independently verified.
- This task's round 6: F1–F9, all fixed and independently verified — the octopus fix live-redriven end-to-end, the Windows fix verified by sabotage of the correct-vs-naive implementation choice. Do not re-litigate these as mechanisms; only round 6's own re-examination target above is open.
