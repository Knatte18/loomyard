...
Seven rounds across two campaign instalments establish the shape: obvious behavioral defects are gone, and what remains concentrates in **proof quality** (tests that pass for the wrong reason) PLUS occasional new behavioral defects at narrowing edges — round 6 found two (octopus adoption, Windows path-separator), round 7 found two more (F5: a guard evaluated on stale pre-fetch knowledge; F1: a shipped operability gap — an engine verb with no CLI surface, blocking the documented lifecycle). Recurring high-yield shapes:
- A test that stays green when the mechanism it claims to guard is sabotaged, INCLUDING the immediately preceding round's own freshly-landed tests. Every round from 5 onward found this in the round before it. Re-sabotage round 7's new tests the same way.
- A guard/predicate computed from information that is stale or not-yet-fetched at the point it is read, with fresher information arriving later in the same call but never re-consulted (round 7's F5 — the campaign's fourth instance of the broader "mutation/read before its supporting mechanism is ready" family, after R4-F3 and round 5's F2 four-site fix).
- A documented operator-facing lifecycle step that the shipped surface cannot actually complete (round 7's F1). Walk the CLI help text and doc.go's operator-facing prose yourself, end to end, as a naive operator would, rather than as a source reader.
- A correctness predicate coded as a bound where the spec promises exact (round 6's F1); a raw-value assumption holding on one platform but not another (round 6's F6).

Concrete invariants to actively drive:
- **Re-sabotage round 7's own new tests** — the four `TestMerge_*Diverged*`/`*Behind*` tests (F5) and the three `merge-stage` CLI integration tests (F1) are the newest, least-battle-tested code in the surface.
- `syncSideBeforeMerge`'s now-exhaustive equal/behind/ahead/diverged classification — any other guard in the merge surface that reads pre-fetch state and never re-decides post-fetch?
- Walk the ENTIRE operator-facing CLI lifecycle yourself as a naive operator: `merge-in`/`merge`'s help text, `merge-stage`'s help text, doc.go's conflict-reporting prose — does every documented step actually work, for both a warp-side and a weft-side path, including edge cases (a path resolved on one side but conflicted on both, an already-resolved path passed to `merge-stage` again)?
- `MergeStageResolved`'s foreign-state guard (round 6's F7) and the new `merge-stage` CLI wiring (round 7's F1) together — is there a gap between what the engine verb guards and what the CLI layer exposes/validates before calling it (argument parsing, empty path list, a path outside either checkout)?
...
**This is a continuation of a two-instalment campaign, now seven rounds deep.** The first instalment (mill task 85) ran three rounds. This task's rounds 4-7 each found and fixed real material: round 4 (1 BLOCKING + 6), round 5 (7, mostly proof-quality), round 6 (9, incl. 2 new behavioral defects — octopus adoption, Windows path-separator), round 7 (8, incl. 2 more new — F5 a stale-guard bypass, F1 a shipped operability gap: an engine verb existed but had no CLI surface, so an operator following the documented lifecycle could not complete a weft-side conflict). The orchestrator independently verified all seven rounds from cold, sabotage-proving every new test itself and live-redriving every behavioral fix. **Zero residual found by the orchestrator after any of the seven.**

**The operator's original fixed 4-round plan (r4-r7) is complete without convergence.** Per the operator's standing directive, the campaign continues autonomously — this round (`fable-max-r8`) is the orchestrator's own pick (new model+effort combination), not a pre-planned one. No known residual is currently seeded. Do a genuinely independent clean-room pass across the WHOLE merge surface, same as every prior round — this could be the safety pass that shows convergence, or it could find more, and either outcome is a valid, useful result.

**Re-examination target:** round 7's own new mechanisms are the least battle-tested code in the surface — see "High-yield focus" above.
...
- This task's rounds 4-7: every finding (R4-F1–F7, F1–F7, F1–F9, F1–F8 respectively) fixed and independently verified by the orchestrator — every behavioral fix live-redriven, every new test sabotage-proven. Do not re-litigate these as mechanisms; only round 7's own re-examination target above is open.
