...
- Status: **CONFIRMED — reproduced LIVE** (scenario F3, "What was tested"): with the pair's status file overwritten to `THIS IS NOT JSON AT ALL {{{`, `lyx loom run` returned `{"error":"unmarshal state: invalid character 'T' looking for beginning of value","ok":false}` and exit 1 — the driver was never spawned, no tmux handover, the status file left untouched. This is the "poisoned status file looks like bootstrap's own gate" state loom.md:380 and 69886823e claim is closed for malformed JSON. By contrast, `lyx loom drive` on the same file passes ownership and reaches `Shed.Run`, which reports `{"error":"shedengine: read status file ...: state: decode failed: invalid character 'T'...","ok":false}` from its OWN step-1 read gate — the correct, designed behavior, and simultaneously the live confirmation of F2's attribution point (the decode failure is surfaced by Shed.Run's step-1 gate, NOT by the Loom-Preflight producer/CheckSeed). A first, ambiguous run against a truncated-but-partially-valid poison proceeded because a still-live driver concurrently rewrote status.json to valid JSON before the run's own Seed read it — a race, not a counterexample; the definitively-non-JSON repro is clean.
