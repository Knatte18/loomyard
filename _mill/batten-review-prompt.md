...
**Round 2 of 4 — a fixed schedule the operator set up front (all four rounds run regardless of what any round self-reports, including this one).** Round 1 (`opus-high-r1`) found and fixed 15 findings (2 BLOCKING, 6 MEDIUM, 5 LOW, 2 NIT, 1 verified non-defect), independently verified by the orchestrator — build/vet/test-x5/integration all reproduced green from cold state, and the two BLOCKING fixes plus one MEDIUM fix were sabotage-proven by hand (reverted, confirmed the regression test fails at the right assertion, restored to an empty diff). R1's own "MERGEABLE" self-verdict was NOT the basis for any of this — see `_mill/batten-review-HANDOFF.md` for the full independent-verification record. This is NOT a safety pass: keep reviewing adversarially, do not treat R1's clean bill as license to go easy.

**Residual to press — the loom-success path has never been driven live.** R1's real end-to-end drive (a real Board `type: loom` task, `--child-driver llm`, a real spawned `claude` provider) reached loom's own `Preflight` row and blocked there (out of batten's scope to fix). R1 confirmed the FAILURE/blocked arm of the `Run-Shed` → terminal-state transition live (hard error, not `Stuck`; no destructive teardown reachable) but the SUCCESS arm — a real child reaching `StateDone`, followed by `Run-Shed`'s own `Done` outcome and the automatic hand-off into `Worktree-Teardown` — has only ever been exercised by the stubbed integration test (`TestBattenIntegration_FourRowRun_*`), never by a real child. Drive a real child all the way to a genuine `StateDone` this round (clear whatever precondition blocked R1's `Preflight` row, or pick a fixture task shaped to actually finish) and confirm the whole chain — real `Done` status → `Run-Shed` returns `Done` → `Worktree-Teardown` fires and actually tears down — for real, not just via the stub.

**Do NOT re-open the CLOSED-AND-VERIFIED work** (see `_mill/batten-review-HANDOFF.md` for the full list with commit shas — do not re-litigate any of it): F0–F8, F10–F15 all fixed and independently verified. Two items are consciously NOT closed, re-evaluate rather than re-litigate:
- **F9's recreate-from-branch half** — batten still does not self-heal a cold-machine-absent task worktree by recreating it from its branch (only the honest-reporting half is fixed). Blocked on a fabric capability (`Topology.Add` refuses a pre-existing branch by design) that doesn't exist — an operator decision, not a batten bug. Re-confirm this is still the shape of the gap; do not attempt to build the fabric capability yourself (out of scope — a cross-cutting change to a different module).
- **F6's `step`-mode pacing cost** — the 30s poll sleep still elapses once per `step` call (now cancellable mid-wait, which was the actual defect fixed). Re-confirm this is still true and still judged acceptable (moving pacing out of the producer body would be a `shedengine` change, out of this round's scope) rather than assuming it without checking.

State the **merge bar** so you calibrate: correctness in the NORMAL single-instance flow (one slug, driven start to finish, including now the SUCCESS terminal state) is the gate. The generic N×-concurrent-suite gate from `crucible/README.md`/`orchestrator-prompt.md` does NOT apply to batten's real end-to-end live-driving scenario at all (see the cost declaration below) — never attempt it. Item 6 above (two DIFFERENT slugs interleaved) is a real, in-scope concurrency check; it is not the N×-concurrent amplifier gate.
...
- **F9's recreate-from-branch half** (see "Round context" above) — re-confirm the fabric-capability gap still holds; do not build the missing fabric capability yourself.
- **F6's `step`-mode pacing cost** (see "Round context" above) — re-confirm the once-per-step elapse is still the accepted shape, not a new defect.
- The design doc's own two consciously-shipped residuals (dead-strand detection, auto-teardown of a finished driver's strand/run-dir) — R1 re-verified both still hold and wrote the dead-strand one into `battenshed`'s package doc. Re-check they're still accurately described after any of your own changes.
