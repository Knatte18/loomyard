...
**Round 4 of 4 — the last round of a fixed schedule the operator set up front (all four rounds run regardless of what any round self-reports, including this one).** After this round's independent verification, the normal convergence judgment (README's "Decide" step) applies — this is likely a genuine safety pass, not a residual-chasing round, but confirm that from your own findings rather than assuming it.

Round 1 (`opus-high-r1`, Opus/high): 15 findings, all fixed and independently verified — two BLOCKING fixes plus one MEDIUM fix sabotage-proven by hand.

Round 2 (`fable-high-r2`, Fable/high): 13 findings + F6[R2] (cross-module, deferred) + F7[R2] (spun off to task #017, now fixed and merged in — see below) + 6 verified non-defects, all in-scope findings fixed and independently verified. Headline: a real Board `type: loom` task walked the WHOLE chain via a `step`-loop, `Worktree-Create → Seed-Child → Run-Shed` through 80 real self-bounces, to a real child `StateDone`, `Run-Shed → Done`, `Worktree-Teardown` tearing down for real.

Round 3 (`sonnet-xhigh-r3`, Sonnet/xhigh): 2 findings (F1 LOW, F2 NIT), both fixed and independently verified — F1 sabotage-proven by hand. Headline: this round's own named residual (a SINGLE blocking `lyx batten run <slug>` call, not a step-loop) is now directly proven — one backgrounded `lyx batten run` process self-bounced internally for ~13.5 real minutes with zero external `step` calls, walking a real nested loom campaign to a genuine failure at `Publish` (a fixture environment limitation — no real GitHub host — not a batten/loom defect), with the task worktree correctly left intact. Also independently re-drove and re-confirmed the FULL High-yield focus list live (items 1, 2, 6, 8, 9, 10, 11) as non-defects. F6[R2] (the llm-driver trust-dialog hang) did NOT reproduce this round on a genuinely fresh worktree — recorded as "not reproduced," not "fixed"; still not filed as its own task.

**F7[R2] / task #017 (`lyx-bin-pane-path`) is FIXED and MERGED IN from `main`** (commit `949dffef6`). `CONSTRAINTS.md` gained a "Pane Binary Resolution" invariant (`internal/reedengine/panebin.go`): a strand pane resolves `lyx` to the binary that spawned it, never whatever `lyx` is on the pane shell's `PATH`. The fix lives entirely outside batten's three packages — NOT a re-openable batten finding. Do not re-report the underlying symptom.

**Operational fix, applies to YOUR OWN live driving this round: the "GitHub self-report hazard" note in this file's own cost declaration below is now BLOCKING and MANDATORY — read it before building your fixture hub.** Prior rounds (R2, plus an unrelated killed resume-agent) drove real loom campaigns without overriding `selfreport: false` before every slug's `Worktree-Create`, and loom's own Tier-1 self-report primitive genuinely filed 6 real GitHub issues against this repo's own upstream tracker before it was caught. 5 were pure test debris and have been closed; 1 (a genuine `burler` observation, unrelated to batten) was left open. Follow the "Fixture hub" section's step 3 exactly — commit the override before ANYTHING else touches a Board task or `Worktree-Create`.

**F6[R2] — the llm-driven child can park forever on Claude Code's own workspace-trust dialog on a fresh worktree it has never seen — is STILL NOT FILED as its own task, and is still NOT batten's own bug** (root cause is `internal/loomcli`'s llm arm / `internal/shuttleengine`). R3 could not reproduce it this round on a genuinely fresh worktree. If you drive an `llm`-driven child and it recurs, or you find a different trigger condition, that is worth noting precisely (what differed from R3's non-reproduction) — but do not attempt to fix the trust dialog itself, a different module's own crucible lineage. What IS in scope: batten's own boundary behavior (honest "still running" reporting) around it.

**Do NOT re-open the CLOSED-AND-VERIFIED work** (see `_mill/batten-review-HANDOFF.md` for the full list with commit shas — do not re-litigate any of it): R1's F0–F8, F10–F15; R2's F1, F2, F3, F5, F8, N1–N5, F9[R2]+F4[docs]; R3's F1, F2. Three items are consciously NOT closed, re-evaluate rather than re-litigate:
...
- **R1-F6's `step`-mode pacing cost** — the 30s poll sleep still elapses once per `step` call (R3 measured 30.042s-30.269s across several live calls, matching R2's 30.0-30.4s). Moving pacing out of the producer body would be a `shedengine` change, out of scope. Re-confirm this is still accepted.
- **The design doc's two consciously-shipped residuals** (dead driver strand not detected/recovered; a finished driver's strand/run-dir not torn down except at whole-worktree teardown) — R1 wrote the dead-strand one into `battenshed`'s package doc; R3 live-demonstrated it this round (killed a real child driver process, confirmed zero detection). Re-check both are still accurately described.
...
**This is likely a safety pass — treat it as one, but earn that conclusion.** Three consecutive rounds (R1, R2, R3) each found real findings and R3's count dropped to 2 minor ones with zero BLOCKING/MEDIUM. If your own independent, adversarial pass finds nothing new either, say so plainly ("no new defects, ship it" is a valid, valuable outcome) — do not invent work to justify the round. If you DO find something, three clean-ish prior rounds missing it makes it more interesting, not less credible.
...
**GitHub self-report hazard (BLOCKING — this bit anyone who skipped it before you).** loom's Tier-1 self-report primitive defaults to `selfreport: true` with the target repo hardcoded to this repo's own real upstream (`Knatte18/loomyard`). A real anomaly in ANY nested loom campaign your fixture drives — bounce-budget-exhausted, crash-resume, escalation-to-human, a recurring finding — files a REAL GitHub issue there, fixture or not. **Before creating ANY Board task or your fixture's FIRST `Worktree-Create`** (step 3 below, materializing `_board` — not only before the one slug you intend to drive end-to-end), commit a `loom.yaml` override with `selfreport: false` onto the fixture hub's own prime weft (`main-weft`). Every later slug's own weft branch forks from whatever prime's `main-weft` carries AT THAT SLUG'S OWN CREATE TIME — an override committed after some slugs already exist does not protect them. Six real issues were filed against the production tracker by earlier rounds' fixture hubs before this was caught (see `_mill/batten-review-HANDOFF.md`'s "Operational incident" entry) — do not repeat it.
...
3. **BEFORE anything else** — write and commit a `loom.yaml` override with `selfreport: false` onto this hub's prime weft (`main-weft`). See the "GitHub self-report hazard" note above; this step is not optional and must land before step 4.
