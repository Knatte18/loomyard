...
**Round 3 of 4 — a fixed schedule the operator set up front (all four rounds run regardless of what any round self-reports, including this one).**

Round 1 (`opus-high-r1`, Opus/high): 15 findings (2 BLOCKING, 6 MEDIUM, 5 LOW, 2 NIT, 1 verified non-defect), all in-scope findings fixed and independently verified — two BLOCKING fixes plus one MEDIUM fix sabotage-proven by hand.

Round 2 (`fable-high-r2`, Fable/high): 13 findings (5 MEDIUM, 3 LOW, 5 NIT) + F6[R2] (cross-module, deferred) + F7[R2] (spun off to its own task, see below) + 6 verified non-defects, all in-scope findings fixed and independently verified — two MEDIUM fixes (F3, F1) sabotage-proven by hand. Headline result: a real Board `type: loom` task walked the WHOLE chain — `Worktree-Create → Seed-Child → Run-Shed` through 80 real 30s self-bounces over a genuine `--child-driver go` loom campaign — all the way to a real child `StateDone`, then `Run-Shed → Done` and `Worktree-Teardown` firing for real and tearing the pair down. This closes R1's own residual (the success arm had only ever been exercised by the stubbed integration test); the SUCCESS terminal state is now proven live, at least once. R2's own "MERGEABLE for batten's own scope" self-verdict was NOT the basis for any of this — see `_mill/batten-review-HANDOFF.md` for the full independent-verification record. This is NOT a safety pass: keep reviewing adversarially.

**F7[R2] / task #017 (`lyx-bin-pane-path`) is now FIXED and MERGED IN from `main`** (commit `949dffef6` on this branch). `CONSTRAINTS.md` gained a new "Pane Binary Resolution" invariant (`internal/reedengine/panebin.go`): a strand pane now resolves `lyx` to the binary that spawned it, never whatever `lyx` happens to be on the pane shell's `PATH`. This was found live during R2's own driving (agents inside a child worktree ran the PATH-resolved production binary instead of the dev binary under test, silently rewriting the child's `reed.yaml`/`loom.yaml` to a different build's template) but the fix itself lives entirely in `internal/reedengine`/`internal/shell`, outside batten's three packages — it is NOT a re-openable batten finding. Do not re-report the underlying symptom as a new finding.

**Residual explicitly named for you, R3 — drive the SUCCESS path via a SINGLE blocking `lyx batten run <slug>` call, not a step-loop.** Both R1 and R2 only ever drove `Run-Shed`'s self-bounce via an external loop of individual `lyx batten step` calls (a `step`-loop shell script). `shedverbs/run.go`'s own doc comment says `run` calls `shed.Run(ctx)`, which runs the whole phase machine — including every `Run-Shed` self-bounce, up to 1440 at 30s each — inside ONE blocking call. Nobody has directly observed `lyx batten run <slug>` behave this way end to end; R2 judged it "almost certainly mechanically equivalent" from reading the shared `Call` logic, but that is an inference, not a live observation. Drive it for real this round: background/`nohup` a single `lyx batten run <slug>` invocation (redirect output to a file, per `ly-drive`'s own pattern for exactly this problem, so the round's own tool-call timeout doesn't get in the way) against a real child, and confirm it reaches the same terminal state the step-loop equivalent did — including tearing down cleanly on `Done`.

**F6[R2] — the llm-driven child can park forever on Claude Code's own workspace-trust dialog on a fresh worktree it has never seen — is STILL NOT FILED as its own task, and is still NOT batten's own bug** (root cause is `internal/loomcli`'s llm arm / `internal/shuttleengine`, which never plays the trust-dismissal sequence outside `Run.Wait`). Re-confirm it is still true if you drive an `llm`-driven child on a never-trusted worktree path, but do not attempt to fix the trust dialog itself — that is a different module's own crucible lineage. What IS in scope: re-check that batten's own boundary behavior (reporting "still running" honestly, never silently) is unaffected by this.

**Do NOT re-open the CLOSED-AND-VERIFIED work** (see `_mill/batten-review-HANDOFF.md` for the full list with commit shas — do not re-litigate any of it): R1's F0–F8, F10–F15; R2's F1, F2, F3, F5, F8, N1, N2, N3, N4, N5, and the F9[R2]+F4[docs] pairing. Three items are consciously NOT closed, re-evaluate rather than re-litigate:
- **R1-F9's recreate-from-branch half** — batten still does not self-heal a cold-machine-absent task worktree by recreating it from its branch (only the honest-reporting half is fixed). Blocked on a fabric capability (`Topology.Add` refuses a pre-existing branch by design) that doesn't exist — an operator decision, not a batten bug. Re-confirm this is still the shape of the gap; do not attempt to build the fabric capability yourself (out of scope — a cross-cutting change to a different module).
- **R1-F6's `step`-mode pacing cost** — the 30s poll sleep still elapses once per `step` call (R2 measured 30.0–30.4s across 80 real steps). Moving pacing out of the producer body would be a `shedengine` change, out of this round's scope. Re-confirm this is still true and still accepted.
- **The design doc's two consciously-shipped residuals** (dead driver strand not detected/recovered; a finished driver's strand/run-dir not torn down except at whole-worktree teardown) — R1 wrote the dead-strand one into `battenshed`'s package doc; R2 additionally noted a LIVE-but-parked strand (the F6[R2] trust-dialog case) is equally invisible to batten. Re-check both are still accurately described after any of your own changes.

State the **merge bar** so you calibrate: correctness in the NORMAL single-instance flow (one slug, driven start to finish, including the SUCCESS terminal state via a real `run` call this round) is the gate. The generic N×-concurrent-suite gate from `crucible/README.md`/`orchestrator-prompt.md` does NOT apply to batten's real end-to-end live-driving scenario at all (see the cost declaration below) — never attempt it. Item 6 (two DIFFERENT slugs interleaved) is a real, in-scope concurrency check; it is not the N×-concurrent amplifier gate.
...
- **F6[R2]'s llm-driver trust-dialog hang** (see "Round context" above) — cross-module, still not filed as its own task, still not batten's own bug to fix; re-confirm it still reproduces and that batten's own status reporting stays honest around it.
- The design doc's own two consciously-shipped residuals (dead-strand detection, auto-teardown of a finished driver's strand/run-dir) — R1 wrote the dead-strand one into `battenshed`'s package doc; R2 extended it to cover a live-but-parked strand too. Re-check both are still accurately described after any of your own changes.
