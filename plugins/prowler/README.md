...
An entry-count guard aborts a listing, incrementally, once it exceeds a ceiling that defaults to 1000 entries and is overridable with `--max-entries N` (`0` disables it): the default exists to stop an unscoped listing against a very large repository from silently returning tens of thousands of paths, and the abort is an ordinary one-stderr-line, non-zero-exit failure like every other, leaving scoping, `--children`, or raising the ceiling as the caller's deliberate next step.
...
That order is measured, not assumed: a raw fetch is a flat cost per file, while the authenticated contents-API path grows with file size, and the two are roughly an order of magnitude apart on the operation the skill performs most often — reading one small-to-medium file.
Its hard prerequisite is `gh` alone, the same prerequisite the skill already requires; `curl` is optional, and its absence costs speed rather than capability by skipping straight to the `gh api` fallback.
