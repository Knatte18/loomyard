...
**When to wrap:** a general-purpose or long-lived thread, or any expensive tier, dispatches a cheap-tier subagent (Agent tool, the cheap distillation tier, currently Haiku) to do the read and return only a distilled answer, keeping the raw content out of the caller's context.
