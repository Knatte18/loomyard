...
- **Otherwise** (a general-purpose or long-lived thread, any expensive tier) — wrap each fetch in a Haiku subagent (step 3). Dispatch overhead is roughly 5-10s per call, so batch multiple fetches as parallel Agent calls, not a sequential loop.
...
3. If wrapping: dispatch one Haiku-tier subagent (Agent tool, `model: haiku`) per fetch — all in one message when there's more than one — each running step 2 inside its own context and returning only its distilled answer, never the raw content.
