...
- **Module ownership.** Weft-internal git (`commit`/`push`/`pull`/`sync`) and coordinated host↔weft topology (checkout that moves both and re-points junctions, dual-worktree add/remove/clone) go through `internal/fabricengine`. No other package runs raw git against weft or warp, regardless of purpose. Read-only verbs (current SHA, `git status --porcelain`) are exempt — only *mutating* warp git must dispatch through fabric.
- **Orchestration, not agent.** Weft commits are Go calling the engine in-process (`SyncWeft`/`CommitWeft`) at a round/phase boundary the loop owner controls. Agents write overlay files (reviews, fixer-reports, status, raddle docs) into `_lyx`/`_raddle` via the junction; Go reads and commits them. Agents may commit their own code to the host repo, never the weft. **Board carve-out:** any LLM session may trigger a board weft write via `lyx board <verb>` at any time; module ownership still holds (routes through `fabricengine.CommitWeftAt`/`PushWeftAt`, never raw git) — only timing control is scoped away from board. `Fabric.Commit` (`internal/fabricengine/commit.go`) is the Go API the orchestration layer calls; never invoked by an agent, by policy.
- **Anchored exclusions.** A `CommitWeft` pathspec with `:(exclude)` entries must anchor every exclusion under the same scoped `_lyx` base the positive entry names, forward-slash spelled — `":(exclude)" + base + "/*.lock"`, never `":(exclude)*.lock"`. A leading-`*`, no-further-wildcard pathspec false-positive-matches intermediate directories and silently no-ops the commit. Live callers: `internal/buildercli`'s and `internal/webstercli`'s `weftCommit` (anchored); `internal/perchcli`'s block-exit commit is still unanchored (known bug, owned by perch).
- **Cross-module exclusions.** A weft commit must exclude every module's machine-local artifacts, not just its own, since `_lyx` is shared. Fixed at the git-exclude layer: `fabricengine.seedWeftArtifactExcludes` (called from `CommitWeft`'s `ensureWeftLockDir`) seeds `crossModuleMachineLocalExcludes` gitignore-syntax patterns into the weft repo's `.git/info/exclude`, so every committer is correct by construction regardless of its own pathspec. Known limitation: does not untrack an artifact a pre-fix `lyx fabric sync` already committed — `git rm --cached <path>` is the manual remedy.
- **Enforced by** review obligation generally. `cmd/lyx/boardguard_test.go` machine-checks `internal/boardengine` specifically (no raw `gitrepo`/`gitexec` import or shell-out). `websterengine`'s `weftReferencePattern` partially machine-checks the agent half for webster runs (a fork or Master Bash command matching `lyx fabric`, or a weft worktree path, is a hard `weft-reference` violation).
