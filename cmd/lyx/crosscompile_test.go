...
// TestCrossCompileLinux cross-compiles the entire module for GOOS=linux and fails on any non-zero exit, surfacing the compiler's combined output.
// It is the whole-module analogue of the per-batch `GOOS=linux go build ./<pkg>/...` gates run during development: those check one package as it lands, this one is the durable guard that every seamed package (proc, fslink, vscode, configengine, tools/deploy) plus every Linux-tagged file added across the task still compiles together, indefinitely.
