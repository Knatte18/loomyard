// state.go defines the persisted strand record and ReedState container, plus the .lyx/reed.json load/save wrappers and the mapper that projects the persisted record down to render.Strand.
// This is the module's dumb-carrier contract in concrete form: Strand stores every field a caller writes (cmd, resumeCmd, sessionId, worktree, name) and reedengine itself reads none of them semantically — only Display feeds the layout decision, via toRenderStrands.
...
// Strand is the persisted record for one tmux pane reedengine owns, reusing render.Display for the display vocabulary.
type Strand struct {
...
// ReedState is the persisted record for one hub's tmux server: the socket name, the session, stripped env keys, and every strand as a flat list.
