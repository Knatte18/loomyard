// config_test.go verifies reed.yaml's template parses, defaults resolve through LoadConfig, and environment overrides + the not-initialized error path behave the way fabricengine's config tests establish the pattern.
