// rules.go composes the policy layer (policy.go, height.go, focus.go) and the mechanics layer (layout.go, checksum.go) into Rules, the package's single public entry point: a pure, total function from a strand set and a window Box to a tmux window_layout string and a focus target.
...
// When p.Header.PaneID is non-empty, Rules carves a fixed-height top band for the header before laying out the stack below.
