...
// Config represents the resolved reed.yaml configuration: the machine tool
...
type Config struct {
	...
	// on a non-numeric ${env:LYX_REED_DEBUG} override — validating and
	...
	// plan's Shared Decisions); a hub whose reed.yaml predates this field
	...
	// on a non-boolean ${env:LYX_REED_MOUSE} override — validating and mapping
	...
	// whose reed.yaml predates this field needs "lyx config reconcile" to
	// adopt it.
...
// embedded default" (see internal/reedengine.HeaderTemplate); HeightRows
// defaults to 1.
type HeaderConfig struct {
	...
	// deliberately leaves the running pane's text unchanged; "lyx reed
...
//
// Calls configengine.Load with reed's ConfigTemplate() to strictly validate
