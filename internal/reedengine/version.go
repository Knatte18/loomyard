...
// Pinned minimum multiplexer versions. minTmuxVersionWindows is set one
// patch below the version the on-box dev Windows tmux port (psmux) binary
// reports, so the pin acts as a drift canary — it fails loud on a real
// regression without bricking `reed up` on the current dev box.
// minTmuxVersion is pinned per the batch discussion at 3.3, the first
// native tmux release this engine's overlay was verified against.
var (
...
// tmuxVersionLinePatternWindows extracts the major.minor.patch triple from
// the Windows tmux port's (psmux's) `-V` output ("psmux X.Y.Z"), which —
// unlike native tmux — always reports a full three-component version.
...
// parseTmuxVersionWindows parses the Windows tmux port's (psmux's) `-V`
// output shape ("psmux X.Y.Z") into [major, minor, patch]. It returns an
// error when no major.minor.patch triple can be found anywhere in out.
...
// pinned floors (minTmuxVersion for POSIX, minTmuxVersionWindows for
...
// version: minTmuxVersionWindows on Windows (tmux port / psmux,
// 3-component), minTmuxVersion everywhere else (native tmux, 2-component),
// mirroring the engine's own Windows-tmux-port / POSIX-tmux binary split
...
// multiplexer-specific parser: parseTmuxVersionWindows on Windows,
