...
// Every test here is untagged Tier 1: it uses only os.Stat (via the package's statFile seam) and t.TempDir,
// and spawns nothing.
...
// TestDirective_ActiveWithFile covers the common active case — PATTERN.md present as a regular file — for all three roles.
...
// TestDirective_InactiveWithoutFile covers the two ordinary inactive cases: the _pattern directory present without PATTERN.md (the normal state lyx init leaves behind),
...
// TestDirective_EmptyPatternFileIsActive pins the "empty file still counts as active" edge rule: a degenerate but harmless state, preferable to a content-inspecting check that would turn a benign empty file into a runtime error.
...
// TestDirective_PatternFileAsDirectoryIsInactive pins the "PATTERN.md as a directory counts as inactive" edge rule: a directory in that place is not a readable index.
...
// TestDirective_NilLayout pins the nil-Layout guard: several Deps structs are assembled field-by-field by CLI callers that could leave Layout unset,
...
// TestDirective_UnknownRole pins the documented unknown/zero Role behaviour: no directive text, even when PATTERN is active.
...
// TestDirective_VariantsArePairwiseDistinct pins that the three role variants never collapse into the same text,
// and that each carries the literal relative pointer "_pattern/PATTERN.md" — never an interpolated absolute path, which would make the value vary per worktree.
...
// TestDirective_VariantsBeginWithOwnHeading pins that each variant carries its own "##" heading inline, so an inactive render leaves no orphan heading behind in the surrounding prompt template.
...
// TestDirective_RelPathNestedSubdirectory is the regression guard for the worst failure mode in this task: a Layout whose RelPath is a nested subdirectory must resolve PATTERN.md at <WorktreeRoot>/<RelPath>/_pattern/PATTERN.md and must NOT be satisfied by one planted at the worktree root instead.
// Without this guard, a root-anchored resolution would render PATTERN silently inactive in every agent invoked from a subdirectory, with no error anywhere.
...
// TestDirective_NonNotExistStatErrorIsActive pins the third edge rule: a stat error that is not os.IsNotExist (a permission or I/O failure) is treated as active, not inactive.
// This is simulated through the package-level statFile seam rather than a real unreadable-directory trick, because an actual permission-denied stat error is not portable — it depends on the OS and on whether the test process runs elevated (e.g.
// as root in a container, where POSIX permission bits are not enforced), and Windows has no equivalent lever at all.
