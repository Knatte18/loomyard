...
// AttemptInput contains everything Engine assembles for one RoundRunner.RunAttempt
// call: identity, paths, prior hydration, and per-round tuning. SeedPath is
// optional pre-round-targeting input, round-scoped not attempt-scoped.
...
// shuttle-style Outcome and, when done, the verdict, blocking count, and
// artifact paths for Engine's round history.
...
// RoundRunner is the seam Engine drives once per attempt: RunAttempt runs one
// attempt of one round and reports its result or a hard error. Engine owns
// retry policy, triage, and stale-artifact move-aside; a RoundRunner
// implementation only adapts "spawn one attempt, report its result."
