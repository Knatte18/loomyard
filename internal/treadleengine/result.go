...
// state.json's per-round history. Mirrors perchengine.RoundSummary
// field-for-field except Verdict, which uses treadle's own Verdict
// vocabulary instead of burlerengine.Verdict. Every field describes
// something that may or may not have happened that round: an empty or nil
// field means the corresponding sub-step did not occur this round (e.g.
// JudgePath is empty on round 1, where the judge never runs; GatePassed is
// nil for a round whose gate mode is GateLLMVerdict, which never runs a
// command).
type RoundSummary struct {
	// Round is the round number (1-based); Attempts is how many RoundRunner
	...
	Round    int
	Attempts int
	// Verdict and BlockingCount are the fresh round's runner result — the
	// review the round loop reasons about, independent of gate mode.
	Verdict       Verdict
	BlockingCount int
	...
	JudgePath       string
	GatePath        string
	// TriagePath is set when this round's attempt(s) included an
	...
	// appended, so a persisted TriagePath always implies the triage verdict
	// was RETRY.
	TriagePath string
