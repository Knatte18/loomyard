// template_test.go pins the four embedded judge/triage/targeting prompt templates' load-bearing statements as substring assertions,
// and separately proves each template actually fills through stencil with its required markers — mirroring burlerengine's TestTemplate_StatesRoundDiscipline / TestTemplate_FillsWithAllMarkers style.
...
// TestJudgeCirclingTemplate_StatesLoadBearingRules asserts the template's load-bearing phrases are present.
...
// TestJudgeMilestoneTemplate_StatesLoadBearingRules is the milestone continuation gate's analogue of the circling-check test above.
...
// TestTriageTemplate_StatesLoadBearingRules is the asking-triage template's analogue: its vocabulary, the one-line-restate-the-blocker rule, and the single-output-file instruction.
...
// TestTargetingTemplate_StatesLoadBearingRules is the pre-round targeting judge template's analogue of the other templates' load-bearing-statement pins: the read-the-handoff instruction, the exactly-one-output-file rule, and the free-form (no frontmatter) output rule — unlike every other template in this package, targeting produces no verdict and so has no rationale-quoting rule to pin.
...
// TestJudgeCirclingTemplate_FillsWithAllMarkers asserts stencil.Fill succeeds when every required marker is supplied,
...
// TestJudgeMilestoneTemplate_FillsWithAllMarkers is the milestone template's analogue of the circling-check fill test above.
...
// TestTriageTemplate_FillsWithAllMarkers is the triage template's analogue of the two judge fill tests above.
...
// TestTargetingTemplate_FillsWithAllMarkers is the pre-round targeting template's analogue of the three fill tests above.
