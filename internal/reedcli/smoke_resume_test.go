...
func TestSmokeCrashRecovery(t *testing.T) {
	...
	// Simulate a tmux crash: kill the whole server out from under reed.
	...
	// we called up while the dying server still answered has-session, reed
...
// TestSmokeClaudeResumeRecallsCodeword is the end-to-end proof of reed's one Claude-adjacent responsibility: env hygiene on the server spawn (without it, a claude launched from inside a Claude Code session treats itself as a nested child and silently stops persisting its transcript) plus the opaque resumeCmd replay.
// It launches a real claude in a strand with a codeword prompt, kills the whole tmux server out from under it, resumes via the stored `claude --continue`, and asserts the codeword comes back — which is only possible if the transcript was persisted and found again.
...
func TestSmokeClaudeResumeRecallsCodeword(t *testing.T) {
	...
	// found" resume looks like — a test artifact, not a reed defect). Waiting
