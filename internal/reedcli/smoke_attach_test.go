...
// `lyx reed attach`. A pane inside a separate harness tmux server has a
// real ConPTY terminal, so running lyx reed attach THERE (with TMUX_SESSION
...
// harness pane's capture must show the reed session's strand content and
...
func TestSmokeAttachRendersInsideHarnessPane(t *testing.T) {
	...
	// all keep the fixture hub directory busy; reed's own down reap never covers
	...
	// worktree-dir-in-use error — a test-harness artifact, not a reed defect.
	t.Cleanup(func() {
	...
	// The handover under test: attach to the reed session from inside the
	...
	// only ever existed inside the reed session, so seeing it here proves
	...
	// The reed session itself must have survived the client detaching.
