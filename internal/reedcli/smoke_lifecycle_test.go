...
// "adopt or not" decision at all on tmux; see reedengine.RemoveStrand's
...
// the session (an operator's raw split-window — 2+ panes, none reed's), the
...
// definition not a git repo, so its "lyx reed header --blocking" command
...
// os.Executable() + " reed header --blocking", and an in-process RunCLI's
...
// tmux-driven verification style (list-panes via the real binary, not reed's
