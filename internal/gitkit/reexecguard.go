// reexecguard.go implements the CLI-reexec refusal guard HermeticGitEnv runs before anything else:
// a Go test binary that receives a positional (non-flag) leading argument is being mistaken for a
// CLI by whatever spawned it,
// and must refuse to run rather than silently execute its entire test suite.
//
// The concrete failure this kills: reedengine's header pane launches "<os.Executable()> reed header
// --blocking".
...
// this guard is the defense-in-depth backstop for any FUTURE spawn point that hands os.Executable()
// to a child while running under go test.
//
// Only the FIRST argument is inspected: a leading non-flag argument can never be a flag's value, so
// it is unambiguously positional.
