...
// by name.
//
// A driver strand that dies mid-run is not detected here, by design: the InnerRun row watches the
// child's persisted status file, never the driver's own liveness.
// A long-quiet Run-Shed therefore means "possibly dead", not "working", until the row's bounce
// budget runs out; an operator tells the two apart by attaching to the child's session.
//
