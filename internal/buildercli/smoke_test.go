...
// TestSmoke_PollDoneReleasesStrand proves against a real tmux server that a done classification
// releases the batch's strand: the pane is gone from the live reed Status after poll returns (the
// F1 leak, observed live as panes accumulating across runs).
...
// TestSmoke_SpawnRefusedWhileStrandLive proves against a real tmux server that spawn-batch's
// in-flight guard refuses while a non-terminal batch's strand is genuinely live (the F4
// double-spawn, observed live as two implementer panes for the same batch).
...
// TestSmoke_RunEntryReclaimsOrphanedOrchestrator proves against a real tmux server that Run's
// entry-time reclaim stops a recorded orchestrator strand the reed still reports live (the fable-r4
// double-orchestrator: a killed `run` process, or a timed-out orchestrator whose kept pane kept
// working) before the fresh orchestrator spawn ever starts.
