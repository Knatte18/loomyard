// run_test.go covers the run verb's envelope shapes and weft-boundary behavior through a fake builderengine.OrchestratorStarter injected directly on a *builderCLI literal (bypassing Command()'s PersistentPreRunE, the same package-local injection pattern as spawnbatch_test.go): ErrRunBusy skips the weft sync;
