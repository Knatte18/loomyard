...
// pollFakeEngine and pollFakeReed (both git-free doubles) live in
// testdata_test.go, untagged, since smoke_test.go and run_test.go
...
// TestPollCmd_TerminalCleanupMatrix proves poll's terminal branch releases the batch's substrate exactly per the doc's discipline: done removes the strand AND the run dir (shuttle-finalize parity);
...
// Without the done/stuck cleanup every finished batch leaks a live pane hosting an idle agent process forever, since nobody else ever holds the shuttle Run handle (found live in round fable-r2: four leaked panes after two runs).
...
// TestPollCmd_ReportLandingDuringGatherBeatsStopEvent proves the report-present branch wins FOR REAL, not just in decision order: a report written after gather's first stat but before its Stop-event read must still classify done — never dead/asking, which would wedge the orchestrator's next respawn on "batch report already exists" (found in round fable-r2; the implementer always writes its report before its turn ends, so this interleave is reachable on every stuck/done batch).
...
// TestPollCmd_DeadRecheckStatErrorPropagates proves the dead-classification re-check's report-existence stat gets the same fail-loud treatment as gather's primary stat (round opus-r3's R2): the primary stat already propagated a non-ENOENT error as a poll-tick failure,
// but the re-check silently ignored one and let a dead classification stand -- exactly the false positive this re-check exists to prevent.
// A real filesystem race between the two stat calls cannot be scripted deterministically, so this test drives it via statReportPath, the package seam both calls go through.
...
// TestPollCmd_TerminalPersistMergesConcurrentSpawn proves the terminal persist writes onto a FRESH state loaded under the state-mutation lease, never the copy loaded at poll entry: a spawn-batch landing inside the long-poll's window (here scripted via the statReportPath seam, firing between poll's entry-time LoadState and its terminal write) records a new batch and moves CurrentBatch, and saving poll's stale entry-time copy would erase both — a live implementer with no state record.
...
// TestPollCmd_ReportBatchFieldMismatchFailsLoud proves a report whose batch: field names a different batch than the one being polled is a fail-loud error, never a silently mislabeled digest: Distill passes the field verbatim into the digest's batch identifier — the one field the orchestrator navigates by — so a typo'd or copy-pasted stem must surface as the same malformed-report error class every other field gets.
...
// TestPollCmd_HalfWrittenReportGetsOneTickGrace proves the half-write grace: the implementer's report write is not atomic, so poll's 1s tick can catch the file created-but-unfinished;
// the first failed parse of a just-seen report must be treated as inconclusive (keep polling),
// and the next tick's successful parse classifies normally — never a hard error for a report one flush away from done.
// Scripted via the statReportPath seam: the first stat sees a truncated report, the second sees it completed.
...
// TestPollCmd_PersistentlyMalformedReportFailsAfterGrace proves the grace is exactly one tick: a report that is still unparseable on the second consecutive tick is a genuinely malformed report and fails loud — the grace must never let a broken report wedge the poll into polling forever.
