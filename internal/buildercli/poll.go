// poll.go implements the `poll` builder verb: assembles Classify's inputs for the current in-flight
// batch (report parse; whether the implementer's turn has ended, via the run dir's events.jsonl and
// the claude engine PersistentPreRunE already constructed; whether its reed strand is still live,
