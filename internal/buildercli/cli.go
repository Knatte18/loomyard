// cli.go builds the cobra command tree for the builder module and the RunCLI seam that wires it
// into the standard io.Writer-based call contract.
// The parent "builder" command carries a PersistentPreRunE that resolves cwd -> layout -> shuttle
// config -> reed config -> builder config -> model registry -> resolved roles -> reed engine ->
...
// underlying engines directly: poll's terminal classification needs to call the claude engine's
// ParseEvents and the reed engine's Status directly, and Runner's own engine/reed fields are
// unexported, so builderCLI keeps its own handles to both rather than re-deriving them.
