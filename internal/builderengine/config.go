...
//
// Defines the Config type mirroring builder.yaml's keys and LoadConfig, which uses internal/configengine.Load with ConfigTemplate() to strictly validate and resolve builder's config file, then validates each role model-spec's grammar via modelspec.Parse so a typo'd spec fails loud at load time rather than hours into a run when that role first spawns.
...
// Config represents the resolved builder.yaml configuration: the four role model-specs (see docs/reference/model-spec.md's "Roles that use this notation" section) and the numeric knobs the batch loop and its validation gate consult.
type Config struct {
...
// LoadConfig loads and unmarshals builder configuration, validating role model-spec grammar and wrapping errors with the offending config key.
