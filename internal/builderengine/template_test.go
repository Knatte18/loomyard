...
// resolves it into the documented defaults — the same expectation config_test.go's
// TestLoadConfig_TemplateDefaultsResolve already exercises via LoadConfig directly;
// this test instead pins the template's raw content against the same defaults so a template edit
// that silently changes a default value is caught here too.
...
// TestImplementerTemplate_StatesBatchDiscipline asserts the embedded implementer template's bytes
...
// TestOrchestratorTemplate_FillsWithAllMarkers asserts stencil.Fill succeeds when every one of
// OrchestratorTemplate's five required markers is supplied,
...
// TestOrchestratorTemplate_NamesTheThreeVerbsItDrives asserts the embedded orchestrator template
// names every one of the three `lyx builder` verbs its loop touches: spawn-batch and poll drive the
// blocking loop itself,
// and status is named even though the template explicitly forbids using it as a substitute for that
// loop.
...
// TestOrchestratorTemplate_QuotesDigestFieldsAndNoOthers asserts the template's digest-field bullet
// list names exactly the ten pinned digest field names (docs/reference/plan-format.md's poll digest
// contract) — no fewer, no extras — the mechanical half of the discussion's "the orchestrator reads
// only distilled digests" decision.
...
// TestOrchestratorTemplate_QuotesOutcomeSchemaKeys asserts the template's outcome-file bullet list
// names exactly the three outcome.yaml schema keys the discussion's outcome-contract decision pins,
...
// TestOrchestratorTemplate_StatesBatchOrderAndRecoveryLadder asserts the embedded template's bytes
// carry the strict-ordering rule and every rung of the discussion's recovery ladder (dead -> fresh
// respawn; stuck -> recovery role; stuck chain member -> whole-chain restart) in prose.
...
// TestImplementerTemplate_FillsWithAllMarkers asserts stencil.FillOptional succeeds when every one
// of ImplementerTemplate's five required markers plus the optional pattern_directive marker is
// supplied,
...
// TestImplementerTemplate_PatternDirectiveOptional asserts pattern_directive behaves as an optional
...
// the directive block ahead of the first work instruction ("## Your batch and the overview").
