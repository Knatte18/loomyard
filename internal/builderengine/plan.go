// plan.go implements the plan-format v2 parser: ParsePlan reads a plan directory's 00-overview.md
// (frontmatter + Batch Index + framing) and, for each batch the index lists, its own
// NN-<batch-slug>.md file, producing the in-memory Plan the rest of builderengine drives from.
// Every parse failure is a distinct, "builder:"-prefixed wrapped error — plan-format.md's fail-loud
// discipline admits no silent-default reading of a malformed plan.
...
// Plan is the in-memory form of a parsed plan-format v2 plan: the overview's frontmatter and task
// framing, plus every batch the Batch Index lists, each itself parsed from its own
// NN-<batch-slug>.md file (see PlanBatch).
type Plan struct {
...
// PlanBatch is one plan batch: the Batch Index entry's fields plus everything parsed from its own
// NN-<batch-slug>.md file.
...
// (both normalized worktree-relative paths, per normalizeCardPath).
// The repo's own rename convention — "git mv" first, then only surgical edits — is what a card's
// Moves: entry declares;
...
// Card-level defects (a missing field, a malformed Moves: bullet) are never parse errors —
// plan-format.md's lenient-card-parse discipline records them leniently here so Validate can turn
// them into enumerable findings instead of a single fail-loud error.
...
// ParsePlan reads the plan directory planDir and returns the fully parsed Plan: 00-overview.md's
// frontmatter, framing, and Batch Index, plus every listed batch's own per-batch file.
// Every distinct failure mode — a missing overview file, missing or duplicate frontmatter keys, an
// unparseable Batch Index line, or a per-batch file failure — is returned as its own wrapped error,
// all prefixed "builder:" per the fail-loud, never-misread discipline plan-format.md pins for every
// machine-read plan artifact.
