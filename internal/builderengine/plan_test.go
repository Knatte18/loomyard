// plan_test.go covers ParsePlan's overview-parsing behavior (frontmatter decoding, Batch Index
// parsing including the mandatory "(C cards)" segment, framing extraction), its per-batch
// file-parsing behavior (frontmatter incl.
// root:, Scope, the typed per-card model, verify: sections, and the "one or the other, never both"
// verify rule), and a full round-trip over the hand-written testdata/plan-valid fixture.
...
// an empty field: an empty-but-present field passes card-missing-field while its paths vanish from
// every other check — exactly the silent degradation the none-sentinel grammar exists to prevent.
...
// TestParsePlan_Card_FiveFieldsNoneSentinel covers the three-way distinction plan-format v2 pins
// for each of the five typed file-op fields: absent entirely (nil slice, HasX == false), present
// with the literal "none" (empty non-nil slice, HasX == true), and present with bullets (populated
...
// TestParsePlan_RootNormalization covers the per-batch root: frontmatter shorthand and the "//"
// worktree-root-relative escape, across every case the per-batch-root-path-shorthand decision pins:
// root set, root absent, a "//"-escaped path under a set root, and a Moves: pair whose two sides
...
// TestParsePlan_HasRenameMechanic covers PlanBatch.HasRenameMechanic: it is true exactly when the
// batch body contains a "## Rename mechanic" heading at all (presence only, never the section's own
// prose), and the testdata/plan-valid fixture's own Moves batch (02-list-tests.md) has it set,
// since it is hand-written with the section already present.
...
// TestParsePlan_CardHeading covers the "### Card NN.C — <title>" heading grammar: both em-dash and
// ASCII separators populate BatchPrefix/Number/ Title identically,
// and a "### " heading that does not match the shape at all is a fail-loud parse error (document
// structure, not a card-level defect).
...
// TestParsePlan_CardCommitAndVerify covers the optional per-card "**Commit:**" field
// (backtick-stripped) and the optional per-card "**verify:**" field (taken verbatim, v1 semantics
// unchanged).
...
// TestParsePlan_IndexCardCount covers the Batch Index's mandatory "(C cards)" segment: it parses
// into IndexCardCount (singular "(1 card)" accepted),
// and a missing segment is the pre-existing "unparseable batch index line" fail-loud error
// (batch-index-card-counts decision).
...
// TestParsePlan_PlanValidFixture round-trips the hand-written testdata/plan-valid fixture exactly:
// every batch's number, slug, flags, chain-end, root, index card count, scope list, and every
// card's typed fields must match the fixture's own byte-consistent content.
...
// TestParsePlan_OtherFixturesParseCleanly confirms the plan-unapproved and plan-broken-chain
// fixtures are each well-formed at the ParsePlan level — they are designed to trip Validate's
// checks (a design-level gate), never ParsePlan's fail-loud parse errors.
