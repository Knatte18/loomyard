// validate_test.go covers Validate's checks against plan-format v2: the plan-valid fixture must
// yield zero findings, plan-unapproved and plan-broken-chain must trip their designed checks, and
// synthetic in-memory plans exercise the checks that need disk state or cap values the hand-written
// fixtures do not exercise (index-file-mismatch, verify-missing, batch-oversized, scope-malformed)
// plus the five move-* checks (move-format, move-redundant, move-source-missing,
// move-target-collision, move-mechanic-missing).
// batch-oversized sums Scope plus every card's typed file-op paths and compares
// len(PlanBatch.Cards) against the card cap.
...
// TestValidate_ChainEndWithoutDeferred proves a batch declaring chain-end: next to a real verify:
// command trips chain-end-dangling: chain membership keys on chain-end: alone, so such a batch
// silently joins the chain's destructive rollback set while never deferring anything.
...
// TestValidate_MoveFormat covers move-format: a malformed Moves: bullet (retained verbatim in
// PlanCard.MovesRaw by the parser) yields one finding citing the card and quoting the raw entry;
...
// TestValidate_MoveRedundant covers move-redundant: an endpoint duplicated into the same batch's
// Creates: is flagged,
...
// TestValidate_MoveSourceMissing covers move-source-missing: a source path with no on-disk file and
// no plan-wide Creates:/Moves-target suppression is flagged;
// a source satisfied by another batch's Creates: is suppressed;
// and a chained rename across two batches (A: x -> y, B: y -> z) is suppressed regardless of batch
// order.
...
// TestValidate_MoveTargetCollision covers move-target-collision's three OR'd conditions: an
// existing on-disk target, two batches targeting the same path, and a cross-batch Creates:
// collision (same-batch overlap is move-redundant's job, deliberately not re-flagged here).
...
// TestValidate_MoveMechanicMissing covers move-mechanic-missing: a batch with a Moves: pair but no
// "## Rename mechanic" section is flagged;
...
// TestValidate_CardMissingField covers card-missing-field: each of the six required fields
// (What:/Context:/Edits:/Creates:/Deletes:/Moves:) is flagged individually when absent,
...
// TestValidate_CardFieldOverlap covers card-field-overlap: the same path in one card's Edits: and
// Creates: is flagged,
...
// TestValidate_ScopeMalformed_CardPaths covers checkScopeMalformed's extension to card paths: a
// ".."-escaping or absolute card path is flagged as scope-malformed citing the offending card.
...
// TestValidate_CardNumbering covers card-numbering's two independent violations: a card heading
// whose batch prefix disagrees with the batch's own number,
...
// TestValidate_CardCountMismatch covers card-count-mismatch: the Batch Index's IndexCardCount must
// equal len(PlanBatch.Cards).
...
// TestValidate_PathMissing covers path-missing: a card's Edits: path that does not exist on disk is
// flagged,
// but the same defect is suppressed when another batch's Creates: satisfies it (the plan-wide
// suppression set move-source-missing also consults).
...
// TestValidate_CardOutsideScope covers card-outside-scope: an Edits: path not covered by any Scope
// entry is flagged,
...
// TestValidate_CommitSubjectMismatch covers commit-subject-mismatch: a Commit: value matching its
// own card's "NN.C: " prefix is clean;
...
// TestValidate_FindingsOrderedByCheckThenBatch pins the deterministic ordering Validate promises:
// findings are grouped by check (in check-1..6 order) and, within a check, by ascending batch
// number.
...
// TestValidate_ValidationErrorImplementsError pins ValidationError's Error() formatting, which lets
// a finding be used directly wherever a single error value is expected (e.g.
