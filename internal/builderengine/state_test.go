// state_test.go covers LoadState/SaveState's three documented cases: round-tripping a populated State through disk, an absent state.json returning (nil, nil), and a corrupt state.json returning a wrapped error rather than a guessed value.
