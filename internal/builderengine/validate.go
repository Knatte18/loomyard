// validate.go implements Validate, plan-format v2's complete machine check set
// (docs/reference/plan-format.md's "Validation checks" section), run in this fixed order:
// format/approval (format-unrecognized, plan-unapproved), Batch Index <-> file consistency
// (index-file-mismatch), verify: presence (verify-missing), chain-end soundness
// (chain-end-dangling), the oversized-batch context/card-count cap (batch-oversized), scope
// well-formedness for both "## Scope" entries and every card's typed file-op paths
// (scope-malformed), the five move-* checks that police a card's Moves: field and its "## Rename
// mechanic" companion section (move-format, move-redundant, move-source-missing,
// move-target-collision, move-mechanic-missing), the per-card structural checks
// (card-missing-field, card-field-overlap), the numbering checks (card-numbering,
// card-count-mismatch), and the cross-referencing checks (path-missing, card-outside-scope,
// commit-subject-mismatch).
// Validate runs both as the standalone `lyx builder validate` verb and as builder's hard automatic
// gate inside `run` and `spawn-batch` — the fail-loud-refusal half of plan-format.md's contract.
...
// ValidateCaps carries the two operator-configured cap values Validate's batch-oversized check
// compares each batch's estimate against.
...
// ValidationError is one finding from Validate: which check tripped (Check, a stable kebab-case
// name matching plan-format.md's check names), which batch it concerns (Batch, empty for a
// plan-level finding), and a human-readable Detail naming the specific problem.
...
// Error implements the error interface so a ValidationError can be used anywhere a single error is
// expected (e.g.
...
// Validate runs every plan-format v2 machine check against plan and returns all findings, ordered
// by check and batch number.
