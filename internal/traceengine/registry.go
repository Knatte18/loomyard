...
// resolvable — e.g. a command run outside any worktree — so trace
...
func validateEntry(name string, e Entry) error {
	if len(e.Markers) == 0 {
		return fmt.Errorf("traceengine: entry %q has no markers", name)
	}
	if e.Match != "all" && e.Match != "any" {
		return fmt.Errorf("traceengine: entry %q has invalid match %q; want \"all\" or \"any\"", name, e.Match)
	}
	if len(e.Command) == 0 {
		return fmt.Errorf("traceengine: entry %q has no command", name)
	}
	if e.InstallHint == "" {
		return fmt.Errorf("traceengine: entry %q has no install hint", name)
	}
	return nil
}
