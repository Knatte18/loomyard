...
func DetectLanguage(targetDir string, reg Registry, langOverride string) (string, Entry, error) {
	if langOverride != "" {
		...
		if !ok {
			return "", Entry{}, fmt.Errorf("traceengine: unknown language %q; known languages: %v", langOverride, sortedLanguages(reg))
		}
	...
	return "", Entry{}, fmt.Errorf("traceengine: %w: searched markers %v under %s", ErrNoLanguage, searched, targetDir)
}
