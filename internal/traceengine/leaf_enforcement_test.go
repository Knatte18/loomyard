// leaf_enforcement_test.go enforces the Traceengine Leaf Invariant:
// production code in internal/traceengine imports ONLY the standard
...
func TestLeafInvariant_AllowlistOnly(t *testing.T) {
	...
	if !ok {
		t.Fatal("could not determine traceengine source directory location")
	}
	...
	if err != nil {
		t.Fatalf("failed to walk traceengine directory: %v", err)
	}

	if len(failures) > 0 {
		t.Errorf("Traceengine Leaf Invariant violated; imports outside the allowlist (stdlib + hubgeometry + lock + yaml.v3) found: %v", failures)
	}
}
