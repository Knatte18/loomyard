...
// TraceDaemonStateFile/TraceDaemonLock resolution to batch 6's
...
func readDaemonState(path string) (daemonState, bool, error) {
	...
	if err != nil {
		...
		return daemonState{}, false, fmt.Errorf("traceengine: read daemon state file %s: %w", path, err)
	}

	var s daemonState
	if err := json.Unmarshal(data, &s); err != nil {
		return daemonState{}, false, fmt.Errorf("traceengine: unmarshal daemon state file %s: %w", path, err)
	}
	return s, true, nil
}
...
func writeDaemonState(path string, s daemonState) error {
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		return fmt.Errorf("traceengine: create daemon state dir %s: %w", filepath.Dir(path), err)
	}

	data, err := json.Marshal(s)
	if err != nil {
		return fmt.Errorf("traceengine: marshal daemon state: %w", err)
	}

	tmpPath := path + ".tmp"
	if err := os.WriteFile(tmpPath, data, 0o644); err != nil {
		return fmt.Errorf("traceengine: write daemon state temp file %s: %w", tmpPath, err)
	}
	if err := os.Rename(tmpPath, path); err != nil {
		return fmt.Errorf("traceengine: rename daemon state temp file %s to %s: %w", tmpPath, path, err)
	}
	return nil
}
