...
func TestEnsureSupervised_RetryExhaustionReturnsErrServerSpawnTimeout(t *testing.T) {
	...
	_, err = ensureSupervised(context.Background(), []string{"lyx-trace-should-never-spawn"}, lang, worktreeRoot, worktreeRoot, timeout)
...
func TestEnsureSupervised_WedgedEscalationReuseReleasesLock(t *testing.T) {
	...
	client, err := ensureSupervised(ctx, []string{"lyx-trace-should-never-spawn"}, lang, worktreeRoot, worktreeRoot, 5*time.Second)
	<-done
	if err != nil {
