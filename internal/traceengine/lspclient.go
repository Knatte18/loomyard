...
func newLSPClient(command []string) (*lspClient, error) {
	if len(command) == 0 {
		return nil, fmt.Errorf("traceengine: empty launch command")
	}

	bin, err := exec.LookPath(command[0])
	if err != nil {
		return nil, fmt.Errorf("traceengine: resolve %q on $PATH: %w", command[0], err)
	}
...
func newLSPClientDial(ctx context.Context, network, address string) (*lspClient, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("traceengine: dial lsp server at %s %s: %w", network, address, err)
	}
...
func (c *lspClient) close() {
	...
	if _, err := c.call(ctx, "shutdown", "shutdown", nil); err != nil {
		fmt.Fprintf(os.Stderr, "traceengine: lsp shutdown request: %v\n", err)
	}
	if err := c.notify("exit", nil); err != nil {
		fmt.Fprintf(os.Stderr, "traceengine: lsp exit notification: %v\n", err)
	}
	c.closer.Close()
	if c.cmd != nil {
		if err := c.cmd.Wait(); err != nil {
			fmt.Fprintf(os.Stderr, "traceengine: lsp process exit: %v\n", err)
		}
	}
}
...
func (c *lspClient) kill() {
	...
	if err := c.cmd.Process.Kill(); err != nil {
		fmt.Fprintf(os.Stderr, "traceengine: kill lsp process: %v\n", err)
	}
	if err := c.cmd.Wait(); err != nil {
		fmt.Fprintf(os.Stderr, "traceengine: lsp process exit after kill: %v\n", err)
	}
}
