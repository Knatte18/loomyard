...
// seam from manifest/designs/trace-redesign.md: given a registry entry
...
func ensureServer(ctx context.Context, lang string, entry Entry, targetDir, worktreeRoot string, timeout time.Duration) (*lspClient, connKind, error) {
	...
	if err != nil {
		return nil, connKindNative, fmt.Errorf("traceengine: resolve go toolchain for %q: %w", lang, err)
	}
...
func finalizeConnection(ctx context.Context, client *lspClient, rootURI string, timeout time.Duration) error {
	...
	if err := probe(ctx, client, timeout); err != nil {
		...
		return fmt.Errorf("traceengine: probe failed: %w", err)
	}
...
func rootURIFor(targetDir string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("traceengine: resolve absolute path for %s: %w", targetDir, err)
	}
...
// time between trace calls routinely exceeds a minute (a benchmark run
...
func ensureNative(ctx context.Context, lang string, entry Entry, targetDir string, timeout time.Duration) (*lspClient, error) {
	binPath, err := resolveGoToolchain(ctx, entry.PinnedVersion)
	if err != nil {
		return nil, fmt.Errorf("traceengine: resolve go toolchain for %q: %w", lang, err)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("traceengine: start language server for %q: %w", lang, err)
	}
...
func ensureSupervised(ctx context.Context, command []string, lang, targetDir, worktreeRoot string, timeout time.Duration) (*lspClient, error) {
	...
	statePath := layout.TraceDaemonStateFile(lang)
	lockPath := layout.TraceDaemonLock(lang)
	...
	for {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("traceengine: ensureSupervised read daemon state for %q: %w", lang, err)
		}
		...
		// .lyx/trace/<lang>/ exists at all) must create it here first,
		// matching this package's own MkdirAll-before-lock precedent
		// (goToolchainInstallLock in toolchain.go).
		if err := os.MkdirAll(filepath.Dir(lockPath), 0o755); err != nil {
			return nil, fmt.Errorf("traceengine: ensureSupervised create spawn lock dir %s: %w", filepath.Dir(lockPath), err)
		}
		fileLock, acquired, err := lock.TryAcquireWriteLock(lockPath)
		if err != nil {
			return nil, fmt.Errorf("traceengine: ensureSupervised acquire spawn lock for %q: %w", lang, err)
		}
		if !acquired {
		...
		if escalating {
			...
			if err != nil {
				fileLock.Release()
				return nil, fmt.Errorf("traceengine: ensureSupervised re-read daemon state during wedged-daemon escalation for %q: %w", lang, err)
			}
			...
			if escalationFound {
				if network, address, ok := strings.Cut(escalationState.Address, ";"); ok {
					...
					if err != nil {
						fileLock.Release()
						return nil, fmt.Errorf("traceengine: ensureSupervised re-dial under lock for %q: %w", lang, err)
					}
				}
			}
			if healthy {
			...
			if escalationFound {
				if err := proc.KillPID(escalationState.PID); err != nil {
					fmt.Fprintf(os.Stderr, "traceengine: kill wedged supervised daemon pid %d for %q: %v\n", escalationState.PID, lang, err)
				}
			}
			...
			if err != nil {
				fileLock.Release()
				return nil, fmt.Errorf("traceengine: ensureSupervised re-read daemon state for %q: %w", lang, err)
			}
			if found && !daemonStale(state) {
				fileLock.Release()
		...
		if err := os.Remove(socketPath); err != nil && !os.IsNotExist(err) {
			fileLock.Release()
			return nil, fmt.Errorf("traceengine: ensureSupervised remove stale socket %s: %w", socketPath, err)
		}
		...
		if err != nil {
			fileLock.Release()
			return nil, fmt.Errorf("traceengine: ensureSupervised open daemon log %s: %w", logPath, err)
		}
		cmd.Stderr = logFile
		if err := cmd.Start(); err != nil {
			...
			fileLock.Release()
			return nil, fmt.Errorf("traceengine: ensureSupervised spawn daemon for %q: %w", lang, err)
		}
		...
		if err := writeDaemonState(statePath, daemonState{
			...
			fileLock.Release()
			return nil, fmt.Errorf("traceengine: ensureSupervised write daemon state for %q: %w", lang, err)
		}
		fileLock.Release()
		...
		if dialErr != nil {
			return nil, fmt.Errorf("traceengine: ensureSupervised dial newly spawned daemon for %q: %w", lang, dialErr)
		}
