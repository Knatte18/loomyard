// Package traceengine finds every reference to a symbol name or an
// explicit source position, shows a symbol's definition, and searches
// workspace symbols by name (`lyx trace refs|definition|symbol
...
//
// traceengine is a leaf package: it returns typed Go results and typed
...
// codes, no internal/output. internal/tracecli is the sole consumer
...
// directly). This keeps traceengine cycle-free and importable by any
...
//     works — trace does not wire that flow itself (the accessor exists
...
// via hubgeometry.Layout.TraceDaemonStateFile/TraceDaemonLock at
// .lyx/trace/<lang>/ — never _lyx/. That distinction matters: .lyx/ is
...
// internal/tracecli can tell "confirmed absent" apart from "found, but
// ambiguous" without parsing error strings — exit codes and the rest of
// that CLI-level contract are internal/tracecli's concern, not this
...
//     trace always goes through LSP, including for Go (gopls), trading
...
//     exists and mirrors models.yaml's shape, but trace is not yet added
