// errors.go defines the typed error vocabulary the traceengine package
// returns to its sole caller, internal/tracecli. Each failure mode gets
...
var ErrNoLanguage = errors.New("traceengine: no language detected")
...
// errors.Is(err, traceengine.ErrServerNotFoundSentinel) without needing
// to know the concrete field values of the *ErrServerNotFound the engine
// actually returned.
var ErrServerNotFoundSentinel = errors.New("traceengine: language server not found")
...
func (e *ErrServerNotFound) Error() string {
	return fmt.Sprintf("traceengine: no language server found for %q; install with: %s", e.Language, e.InstallHint)
}
...
var ErrSymbolNotFoundSentinel = errors.New("traceengine: symbol not found")
...
func (e *ErrSymbolNotFound) Error() string {
	return fmt.Sprintf("traceengine: symbol %q not found under %s", e.Symbol, e.TargetDir)
}
...
var ErrAmbiguousSymbolSentinel = errors.New("traceengine: ambiguous symbol")
...
func (e *ErrAmbiguousSymbol) Error() string {
	return fmt.Sprintf("traceengine: symbol %q is ambiguous; candidates: %s", e.Symbol, strings.Join(e.Candidates, ", "))
}
...
var ErrResolverUnsupportedSentinel = errors.New("traceengine: resolver unsupported")
...
func (e *ErrResolverUnsupported) Error() string {
	return fmt.Sprintf("traceengine: language server %q for %q does not support this resolver", e.Server, e.Language)
}
...
// compares against, mirroring ErrServerNotFoundSentinel.
var ErrServerTimeoutSentinel = errors.New("traceengine: language server timed out")
...
func (e *ErrServerTimeout) Error() string {
	return fmt.Sprintf("traceengine: language server timed out during %q after %s", e.Phase, e.Timeout)
}
...
// ErrServerNotFoundSentinel.
var ErrServerSpawnTimeoutSentinel = errors.New("traceengine: gave up waiting for supervised daemon spawn")
...
func (e *ErrServerSpawnTimeout) Error() string {
	return fmt.Sprintf("trace: gave up waiting for the supervised daemon for %q to become ready", e.Lang)
}
