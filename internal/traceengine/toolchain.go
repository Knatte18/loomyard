...
// a cold cache, installing) a pinned gopls binary into a trace-owned,
...
func runGoInstall(ctx context.Context, version, destDir string) error {
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("traceengine: go install %s: %w: %s", target, err, out)
	}
	return nil
}
...
func resolveGoToolchain(ctx context.Context, pinnedVersion string) (string, error) {
	...
	if err := os.MkdirAll(filepath.Dir(lockPath), 0o755); err != nil {
		return "", fmt.Errorf("traceengine: create go toolchain install lock dir %s: %w", filepath.Dir(lockPath), err)
	}
	fileLock, err := lock.AcquireWriteLock(lockPath)
	if err != nil {
		return "", fmt.Errorf("traceengine: acquire go toolchain install lock: %w", err)
	}
	...
	if err := os.MkdirAll(cacheDir, 0o755); err != nil {
		return "", fmt.Errorf("traceengine: create go toolchain cache dir %s: %w", cacheDir, err)
	}
	if err := installGoToolchain(ctx, pinnedVersion, cacheDir); err != nil {
		return "", fmt.Errorf("traceengine: install go toolchain %s: %w", pinnedVersion, err)
	}
	return binPath, nil
}
