...
// proves the template actually fills through stencil with all eight
// required markers.
...
// eight required top-level markers set to a non-empty placeholder, so tests
...
// one of the eight required markers is supplied, and fails — naming the
...
func TestTemplate_FillsWithAllMarkers(t *testing.T) {
	...
	for _, marker := range []string{
		...
		"prior_rounds", "review_path", "fixer_report_path",
	} {
