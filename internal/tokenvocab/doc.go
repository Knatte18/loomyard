...
// It owns the token registry (currently "repo" and "hub", both resolved from lyxcwd.Location) and
// Render, the reusable compose over internal/stencil that every consumer calls to fill a template
// with the vocabulary.
//
// Leaf invariant: tokenvocab imports only stdlib, internal/lyxcwd, and internal/stencil — never a
