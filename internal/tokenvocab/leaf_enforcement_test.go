...
func TestLeafInvariant_AllowlistOnly(t *testing.T) {
	...
	if !ok {
		t.Fatal("could not determine tokenvocab source directory location")
	}
	...
	if err != nil {
		t.Fatalf("failed to walk tokenvocab directory: %v", err)
	}

	if len(failures) > 0 {
		t.Errorf("Tokenvocab Leaf Invariant violated; imports outside the allowlist (stdlib + stencil) found: %v", failures)
	}
}
