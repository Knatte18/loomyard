...
// Like tokenvocab's leaf_enforcement_test.go, this check is an ALLOWLIST: any import outside the
...
// It is empty: internal/standalonestate has no permitted non-stdlib import.
...
func TestLeafInvariant_AllowlistOnly(t *testing.T) {
	...
	if !ok {
		t.Fatal("could not determine standalonestate source directory location")
	}
	...
	if err != nil {
		t.Fatalf("failed to walk standalonestate directory: %v", err)
	}

	if len(failures) > 0 {
		t.Errorf("Standalonestate Leaf Invariant violated; imports outside the allowlist (empty -- no non-stdlib import permitted) found: %v", failures)
	}
}
