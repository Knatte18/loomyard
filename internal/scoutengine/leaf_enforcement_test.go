...
// library, internal/hubgeometry, internal/configengine, internal/lock,
...
func TestLeafInvariant_AllowlistOnly(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to walk scoutengine directory: %v", err)
	}

	if len(failures) > 0 {
		t.Errorf("Scoutengine Leaf Invariant violated; imports outside the allowlist (stdlib + hubgeometry + configengine + lock + proc + logger + yaml.v3) found: %v", failures)
	}
}
