// refs.go implements References, the public orchestration entry point that ties detection (detect.go), the language-server registry (registry.go), and the generalized LSP client (lspclient.go) together: given a target directory and a query (a symbol name or an explicit file:line:col position), it launches the right language server, resolves the query to a position if needed, and returns the reference list.
// It also defines the shared lookup pipeline (acquireConnection, teardownConnection, lookup) that References wraps and that a later batch's Definition wraps too — both differ only in which single LSP call they make once a position is resolved.
...
// Query selects a symbol or position to look up: one of InFile (file-scoped name), Pos (explicit position), or Symbol (project-wide name search).
type Query struct {
	InFile *InFileQuery
	Symbol string
	Pos    *Position
