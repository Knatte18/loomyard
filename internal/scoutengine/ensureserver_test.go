...
// think-time gaps between scout calls (see daemonIdleTimeout's own doc
...
func TestEnsureServer_SupervisedFailsForNonToolchainReasonFallsBackToNative(t *testing.T) {
	...
	lockPath := (&lyxcwd.Location{HubPath: filepath.Dir(worktreeRoot), WorktreeName: filepath.Base(worktreeRoot)}).ScoutDaemonLock("go")
