...
// TestResolveToken_Chain covers the full resolution order end to end: env vars always win over the cache, GH_TOKEN before GITHUB_TOKEN, a fresh cache entry is used when both env vars are empty, a stale one is not, and an unresolvable token surfaces as ErrTokenUnresolvable.
...
// TestReadCachedToken_MalformedFileIsMiss covers every way the on-disk file can fail to match the exact two-field schema: each is a cache miss, never a fatal error, so a foreign or worn-out file never blocks resolution.
...
// TestWriteCachedToken_UnwritableDirDegradesToInProcessResolution covers the requirement that a cache directory which cannot be created degrades to in-process resolution rather than failing the command: writeCachedToken must not panic or error out loud,
...
// TestAuthRT_401InvalidatesAndReplaysExactlyOnce covers the transport's core contract: a 401 invalidates the rejected (cache-sourced) token, resolves a fresh one exactly once, and replays with it -- never in a loop.
...
// TestAuthRT_SecondConsecutive401Propagates asserts the replay never loops: once the replay itself is rejected, the second 401 goes back to the caller unchanged.
...
// TestAuthRT_EnvSourcedTokenSkipsReplayOn401 asserts that an environment-sourced token is never invalidated and replayed: replaying it is guaranteed to reproduce the identical value, so the 401 is surfaced as an error naming the rejected variable, with no second request at all.
...
// TestAuthRT_ReplayRewindsRequestBodyByteIdentical is the case the design flags as silently misbehaving without it: a 401 then success, asserting the replayed request's body is byte-identical to the first.
// This is what proves the req.GetBody rewind actually happened, rather than the replay sending an already-drained (empty) body.
...
// TestRunGHAuthTokenSeam_HonoursGhAuthTokenTimeout encodes the operator requirement directly rather than mere behaviour: a fake that hangs like a stuck `gh` process must still cause resolveToken to return once ghAuthTokenTimeout elapses.
...
// TestResolveToken_UnresolvableReturnsTypedErrorWithoutBlocking encodes the second operator requirement: when the gh CLI itself fails fast (no session), resolution must return ErrTokenUnresolvable immediately, never wait or prompt.
