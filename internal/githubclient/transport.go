...
// RoundTrip implements http.RoundTripper. It sends req with a resolved token,
// and on a 401, it invalidates the cache and replays exactly once with a fresh
// token (unless the token is environment-sourced, in which case it returns an
// error instead).
