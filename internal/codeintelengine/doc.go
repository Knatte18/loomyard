...
// workspace symbols by name (`lyx codeintel refs|definition|symbol
...
// codeintel spike (wiki task codeintel-spike, #008) recommended for Go alone
...
// scope trade the codeintel multilang research (wiki task
...
// codes, no internal/output. internal/codeintelcli is the sole consumer
...
// directly). This keeps codeintelengine cycle-free and importable by any
...
// via hubgeometry.Layout.CodeintelDaemonStateFile/CodeintelDaemonLock at
// .lyx/codeintel/<lang>/ — never _lyx/. That distinction matters: .lyx/ is
...
// internal/codeintelcli can tell "confirmed absent" apart from "found, but
// ambiguous" without parsing error strings — exit codes and the rest of
// that CLI-level contract are internal/codeintelcli's concern, not this
