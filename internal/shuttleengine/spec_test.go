// spec_test.go verifies Spec.validate's normalization and error paths: mandatory Prompt/OutputFiles, relative-to-absolute resolution for output files, the Timeout defaulting/negative-rejection rules, and the Display.Anchor default.
...
// TestSpec_Validate_EffortUntouched proves validate neither defaults nor rejects Effort in any way — it is provider vocabulary the engine alone validates (see claudeengine's validateEffort), so Spec.validate must leave an empty Effort empty and a non-empty Effort (even a nonsense value) unchanged and error-free.
...
// TestSpec_Validate_VersionUntouched proves validate neither defaults nor rejects Version in any way — it is provider vocabulary the engine alone validates (see claudeengine's resolveModelID), so Spec.validate must leave an empty Version empty and a non-empty Version (even a nonsense value) unchanged and error-free.
