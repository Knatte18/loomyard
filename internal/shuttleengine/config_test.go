// config_test.go verifies shuttle.yaml's template parses, defaults resolve through LoadConfig, and environment overrides + the not-initialized error path behave the way reedengine's config tests establish the pattern.
