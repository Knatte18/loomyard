// audit_test.go covers AuditForks end to end against a fake ~/.claude/projects layout built under t.TempDir(): the parent-transcript spawn tally, one ForkReport per fork transcript with every field asserted, the missing-subagents-dir zero-fork finding, the missing-parent-transcript error, and the path-encoding derivation claudeProjectDirFor implements.
...
// TestAuditForks_FullLayout builds a complete fake project dir — a parent transcript plus three fork transcripts under subagents/ — and asserts every ForkAudit/ForkReport field the audit populates.
...
// TestAuditForks_MissingSubagentsDirYieldsZeroForks proves a fork-authorized session that never actually spawned a fork (no subagents/ directory at all) is a legitimate zero-fork finding, not an error — the parent transcript's own SpawnCalls/NamedSpawns are still populated from what it attempted.
...
// TestAuditForks_MissingParentTranscriptErrors proves a missing parent transcript IS an error — unlike the missing-subagents-dir case, there is no way to know what the session spawned without it.
...
// TestClaudeProjectDirFor_EncodesNonAlnumBytes pins the exact cwd-encoding derivation: every non-alphanumeric byte becomes '-', mirroring claudeProjectDir in internal/reedcli/smoke_test.go.
