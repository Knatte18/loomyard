// command_test.go table-tests the pane-shell command composition helpers: quoting of paths with spaces and embedded single quotes, model/flag presence per the interactive toggle, the exact resume-command shape, the resolveModelID bare-word-plus-version composition rule, the forkSubagents env-prefix wrapping on both launch and resume lines, and a no-newline invariant every produced command must hold (they are typed into a pane via a single send-keys call).
// The pwsh-quote cases formerly asserted here now live in internal/shell's shell_test.go, since quoting itself moved there.
...
// TestResolveModelID covers resolveModelID's full input space: an empty version passes the model through unchanged (including an empty model), a dotted and a dotless version both compose against a bare model, an empty model with a non-empty version errors (nothing to compose against), and a dashed (full id) model with a non-empty version errors (the id already pins its own version — combining it with version is a contradiction).
