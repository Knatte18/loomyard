// audit.go implements Claude's AuditForks/AuditForksIncremental: reading the on-disk session transcript layout Claude Code itself maintains under ~/.claude/projects/<encoded-cwd>/ to recover mechanical facts about a fork-authorized run's fork subagents, optionally filtered to only the fork transcripts a long-lived caller has not yet seen.
// All of this file's knowledge — the project directory's cwd-encoding scheme, the parent/fork transcript paths, and the JSONL message shape — is Claude-specific and stays contained here, per the Shuttle Provider-Seam Invariant;
// shuttleengine itself only ever sees the provider-invariant ForkAudit/ForkReport value types this file populates.
...
// AuditForks implements shuttleengine.Engine.AuditForks for Claude, reporting every fork transcript by calling AuditForksIncremental with nil seenTranscripts.
...
// AuditForksIncremental implements shuttleengine.Engine.AuditForksIncremental for Claude, deriving the session's project directory from workdir and reading transcripts.
