...
func Pull(boardPath string) (updated bool, err error) {
	stdout, stderr, exitCode, err := gitexec.RunGit([]string{"pull", "--ff-only"}, boardPath)
	if err != nil {
...
func CommitPush(boardPath string, relPaths []string, message string, skipPush bool) error {
	...
	_, _, exitCode, err := gitexec.RunGit(args, boardPath)
	if err != nil {
	...
	_, _, exitCode, err = gitexec.RunGit([]string{"diff", "--cached", "--quiet"}, boardPath)
	if err != nil {
	...
	// Commit
	_, _, exitCode, err = gitexec.RunGit([]string{"commit", "-m", message}, boardPath)
	if err != nil {
	...
	for attempt := 0; attempt < 2; attempt++ {
		_, stderr, exitCode, err := gitexec.RunGit([]string{"push"}, boardPath)
		if err != nil {
		...
		if strings.Contains(stderr, "non-fast-forward") || strings.Contains(stderr, "rejected") {
			// Try rebase
			_, _, exitCode, err := gitexec.RunGit([]string{"pull", "--rebase"}, boardPath)
			if err != nil {
