...
// The "create" subcommand's RunE closure reads flag state via the named createCmd variable to distinguish "flag not set" from "flag set to empty string" using Changed("body"), following the warp module pattern for local-flag access inside a cobra RunE.
...
// The module is reachable as `lyx selfreport create` and delegates the actual GitHub API call to the selfreportengine domain package.
...
// RunCLI is the public seam for the selfreport module, delegating to clihelp.Execute for in-process test capture.
