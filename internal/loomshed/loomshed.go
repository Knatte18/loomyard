...
// bouncing to Webster); 12 Publish (real, ""); 13 Finalize (real, ""). Every gate and validator
...
// to and a human is the only thing that can fix either. Publish and Finalize share that same
...
func New(deps Deps) (*shedengine.Shed, error) {
	...
	producers := []shedengine.ProducerDef{
		...
		{Name: NamePlanSweep, Producer: newStub(NamePlanSweep), OnStuck: ""},
