...
//
// The twelve rows, with their backing, OnStuck target, and OnDone target: 1 Preflight
// (deps.Preflight, "", -> Discussion-Write); 2 Discussion-Write (stub, "", -> Discussion-Validate);
// 3 Discussion-Validate (real, bouncing to Discussion-Write, -> Discussion-Review); 4
// Discussion-Review (stub, bouncing to Discussion-Write, -> Plan-Write); 5 Plan-Write (stub, "",
// -> Plan-Validate); 6 Plan-Validate (real, bouncing to Plan-Write, -> Plan-Review); 7 Plan-Review
// (stub, bouncing to Plan-Write, -> Batchifier); 8 Batchifier (real, "", -> Webster); 9 Webster (the
// lazy wrapper, "", -> Webster-Review); 10 Webster-Review (stub, bouncing to Webster, -> Publish);
// 11 Publish (real, "", -> Finalize); 12 Finalize (real, "", -> "", the empty OnDone that finishes
// the whole run). Every gate and validator bounces back to the producer whose artifact it guards,
// and a gate whose guarded artifact is produced by no row in the list escalates instead --
// Preflight gates git and filesystem state and Batchifier gates the batch config, neither of which
// any row writes, so there is nothing to bounce to and a human is the only thing that can fix
// either. Publish and Finalize share that same escalate-only posture: neither bounces, because
// nothing in the list produces what these two gate -- an unresolvable conflict against the parent's
// current state, an unreachable remote service, a drifting parent, and a pull request awaiting
// human review are all things only a human fixes.
//
// The list's physical order above no longer carries routing meaning -- it is preserved only as the
// display and enumeration order this comment and the table below walk in, and each row's own
// OnDone is what actually routes a Done verdict to its successor, forward or backward, regardless
// of list position.
//
...
func New(deps Deps) (*shedengine.Shed, error) {
	...
	producers := []shedengine.ProducerDef{
		{Name: NamePreflight, Producer: deps.Preflight, OnStuck: "", OnDone: NameDiscussionWrite},
