...
func TestPlanWrite_Call(t *testing.T) {
	...
	// CommitErrorSurfacesAsErrorNamingProducerWithEmptyOutcome: a commit failure maps to a returned
	// error on the non-empty-pointer Stuck path exactly as it already does on the Done path.
...
// Shed run is exactly the failure this codebase guards against elsewhere. It runs against both
// branches that now reach the commit seam -- Done and a gate-failed Stuck -- each carrying a
// non-empty pointer, since the nil-commit-seam guard sits downstream of the pointer.Path == "" check
// and must still fire on either shape.
func TestPlanWrite_NilCommitSeamIsANamedError(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			inner := &planInnerProducer{outcome: tt.outcome, pointer: shedengine.OutputPointer{Path: "00-overview.md"}}
