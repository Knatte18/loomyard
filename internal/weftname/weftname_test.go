// weftname_test.go exercises SiblingPath and BareSiblingPath over a range of
// container/base combinations, and locks in the relationship between them
// that lyxtest's fixture builders rely on, so the two on-disk shapes can
...
// TestSiblingPath covers SiblingPath's container/base join over a range of
...
// TestBareSiblingPath covers BareSiblingPath's container/base join, the
// bare-remote fixture directory paired with a weft sibling.
...
// TestBareSiblingPath_AgreesWithSiblingPath locks in the relationship
// lyxtest's fixture builders depend on: a weft sibling's bare-remote fixture
// name is always SiblingPath's own result with "-bare" appended, never an
// independently-derived literal. This is the drift BareSiblingPath exists to
// prevent between production geometry and the on-disk shape test fixtures
// must reproduce for the same input.
