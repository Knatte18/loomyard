...
// environment: lyxtest.HermeticGitEnv() runs once before any test, so gitexec's
// git-spawning fixtures never inherit the operator's global gitconfig (see
// CONSTRAINTS.md's Hermetic Git Test Environment Invariant). This file lives in
// the external package gitexec_test, matching the directory's existing
