...
// ensureWeftLockDir), not by any ":(exclude)" pathspec websterCLI builds
// itself -- weftCommit passes only the positive scoped _lyx pathspec. The
// nested case is the regression guard for the pre-fix, unanchored
// ":(exclude)*.lock" spelling this migration retired: that spelling's
// one-star pathspec false-positive-matched the intermediate directories
// leading to a multi-segment positive pathspec and pruned the entire
// subtree, so `git add` staged nothing and weftCommit returned (false, nil)
// -- a completely silent no-op. It is also the cross-module regression
// guard: a webster commit must hold back BUILDER's pause flag too, since
// both round-loop modules share one _lyx -- committing it pins it in weft
// HEAD, where builder can never stage its deletion (its own exclusion hides
// it from `git add`), so every other machine's weft pull materializes a
// pause request nobody made. Each excluded artifact is asserted both absent
// from the commit AND still untracked via `git ls-files` -- proving the
// exclude file, not merely an already-tracked file happening to be omitted
// from this one commit. WEFT_SKIP_PUSH is set because the scratch weft repo
// has no remote; the commit half is what is under test.
