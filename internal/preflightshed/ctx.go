// ctx.go implements the two context checks this package's producer shares: entryErr, consulted
// before Call starts anything, and cancelErr, consulted by every non-success exit path. This is
// preflightshed's own copy of loomshed's identically-shaped helpers -- see doc.go for why the
...
func entryErr(ctx context.Context, name string) error {
	...
	return fmt.Errorf("preflightshed: %s: context cancelled before run started: %w", name, ctx.Err())
}
...
func cancelErr(ctx context.Context, name string) error {
	...
	return fmt.Errorf("preflightshed: %s: context cancelled during run: %w", name, ctx.Err())
}
