// logger.go provides a thin log/slog wrapper for lyx: a package-level level threshold, an injectable io.Writer stderr sink (defaulting to os.Stderr), and Debug/Info/Warn helpers that fan every call out to that stderr sink and, independently, to the durable trace-file sink (sink.go) via the composite dualHandler defined below.
...
// Handle implements slog.Handler by independently re-checking each half's own Enabled and delegating only to the halves that pass -- see the dualHandler doc comment for why this independent re-check, rather than a single shared gate, is load-bearing.
// record.Clone() is used for all but the last delegate because slog.Record documents that a Record passed to more than one Handler must be cloned for each use beyond the first, since its internal small-attrs storage may otherwise be shared.
...
// Write opens the durable sink on first use (a no-op on every call after the first, via sinkOnce) and writes p to it.
// A closed/unarmed sink (sinkOK false) is not an error here -- per sink.go's lazy-sink-open rule, a diagnostic sink that never opens must never surface as a write failure to the logging call site, so Write reports success and discards p.
...
// Debug logs msg at debug level with the given key/value args, stamping a trace key with TraceID()'s current value (batch 2) on every line as every level does.
// Debug never reaches the durable sink -- durableHandler.Enabled only accepts Info and above -- so a Debug record is a no-op sink-wise regardless of trace stamping;
// it reaches the stderr half only once SetVerbosity(2) or higher has been called.
// Calling TraceID() here is also what triggers the trace-ID's own first resolution if nothing has resolved it yet.
...
// Warn is the default threshold, so a Warn call reaches both halves -- stderr and the durable sink -- even without SetVerbosity.
func Warn(msg string, args ...any) {
	log.With("trace", TraceID()).Warn(msg, args...)
}

// SetVerbosity maps a -v repeat count to a log level: count<=0 keeps the default Warn threshold (silent normal run), count==1 lowers it to Info, and count>=2 lowers it to Debug.
...
// SetOutput rebinds the stderr half of the log sink to w and rebuilds the composite handler's stderr-half inner handler in place;
// the durable half (sink.go's ensureDurableSink/writeDurable) is untouched, per discussion.md's dual-handler-fan-out decision -- redirecting stderr must never silently detach the durable sink.
// It is a seam both tests (withCapturedOutput in logger_test.go, so tests can assert on captured output without writing to the real os.Stderr) and production code use: configureFromEnv already calls SetOutput for LYX_LOG_FILE, independent of anything this package's trace/sink work adds.
// LYX_LOG_FILE redirects only the stderr half to an operator-chosen, unmanaged, whole-verbosity file;
// the durable sink independently opens its own Info+-only, lyx-managed, retention-swept trace file under this package's own WorktreeLogsDir(l).
// An Info+ line therefore lands in both files when LYX_LOG_FILE is set -- that duplication is intended, not a bug to reconcile: the two are different artifacts with different lifetimes, and neither should suppress the other.
//
// The write to out and the rebuild of currentStderr happen under sinkMu (sink.go), the same mutex guarding the durable sink's state, per discussion.md's concurrency-contract decision: SetOutput can race with an in-flight Handle call reading currentStderr via stderrHandlerSnapshot.
