...
// and every `lyx config` surface (help text, unknown-module error, --print sections, reconcile output, menu numbering) renders it in exactly this order, so an out-of-sort entry is user-visible.
...
// TestModules_SeedOnly pins the seed-only flag: "models" and "burler" are the two modules carrying an open-ended, operator-owned key set (model aliases; lenses/fans respectively), so they are the only entries with SeedOnly == true.
