...
// It is the sole consumer of internal/output within the scout surface: internal/scoutengine returns typed Go errors and results with no io.Writer/exit-code machinery (per the plan's engine/CLI layering Shared Decision), so this file is where every engine result and typed error gets mapped to the internal/output JSON envelope.
