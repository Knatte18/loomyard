// stencil_test.go is the black-box, table-driven contract test for stencil.Fill and stencil.FillOptional: the happy path, the unfilled-top-level-marker guard (including sorting/dedup), the incremental branch-internal guard, conditional sections, the leading-comment strip, the no-HTML-escaping / idempotence guarantees, and FillOptional's optional-marker exemption from both guards.
...
// TestFill_HappyPath covers several top-level {{.X}} markers, all present and non-empty, rendering the correct substituted output with a nil error.
...
// TestFill_MissingTopLevelMarker covers a referenced top-level marker absent from values, which must error naming that marker.
...
// TestFill_EmptyValue covers a top-level marker present as "" and as whitespace-only (" "): both must error, the empty-fasit guard.
...
// TestFill_MultipleOffendersSortedAndDeduped covers two or more unfilled top-level markers collapsing into a single error listing all of them in sorted order, with a repeated reference to the same marker deduplicated to one entry.
...
// TestFill_BranchInternalMissCaughtIncrementally covers a taken branch referencing an absent marker (error naming it),
// and a mix of an absent top-level marker plus an absent in-branch marker, where the top-level check reports first and the in-branch name never appears (Fill returns before execution reaches the branch).
...
// TestFill_MalformedTemplate covers an unparseable template (an unclosed {{if}}), which must return a non-nil error wrapping the parse failure, never panic.
...
// TestFill_ConditionalTaken covers {{if eq .Type "Cluster"}}...{{end}} with a matching Type, where the section is present and its inner markers are substituted.
...
// TestFill_ConditionalNotTaken covers the same template with a non-matching Type: the section is absent,
// and markers living only inside that branch are not required (no error even though the branch-only marker is absent from values).
...
// TestFill_ForgottenDiscriminator covers a template that references {{if eq .Type ...}} while values has no Type key at all: the condition is always evaluated, so this must error rather than silently treat Type as false/empty.
...
// TestFill_UnusedValuesIgnored covers values carrying keys the template never references: no error, output unaffected.
...
// TestFill_LeadingCommentStrip covers a leading <!-- ... --> being dropped (a marker inside it is neither substituted nor checked, so no error even though it references an undefined value), a mid-template comment preserved verbatim, and a comment-only template rendering empty output.
...
// TestFill_EmptyOrWhitespaceOnlyTemplate covers an empty template and a whitespace-only template, both of which must render with a nil error and no substantive content.
...
// TestFill_IdempotenceAndDeterminism covers repeated calls with the same template and values producing byte-identical output,
...
// TestFill_NoHTMLEscaping covers a value containing <, >, &, and quotes passing through verbatim, confirming Fill uses text/template rather than html/template.
...
// TestFillOptional_AbsentRendersNothing covers a marker listed as optional and absent from values rendering as nothing with no error, instead of tripping the unfilled-top-level-marker guard.
...
// TestFillOptional_PresentButEmptyRendersNothing covers a marker listed as optional and present in values as "" rendering as nothing with no error.
...
// TestFillOptional_WhitespaceOnlyNormalisesToEmpty covers a marker listed as optional and present in values as whitespace-only rendering as nothing, not as its whitespace verbatim — the same TrimSpace-based "empty" definition unfilledTopLevelMarkers uses must also govern what FillOptional seeds before execution.
...
// TestFillOptional_PresentAndNonEmptyRendersValue covers a marker listed as optional but present and non-empty rendering its actual value, confirming the optional exemption only changes behaviour for absent/empty values, not for a value that is genuinely set.
...
// TestFillOptional_NonOptionalEmptyMarkerStillErrors covers a template with only a non-optional empty marker: the existing unfilled-top-level-marker error must still fire, confirming FillOptional(t, v, nil-equivalent-for-that-name) behaves exactly like Fill for names not listed as optional.
...
// TestFillOptional_MixOfOptionalAndRequiredEmptyReportsOnlyRequired covers a template with one optional-and-empty marker plus one required-and-empty marker: the error must name only the required one, confirming the optional exemption removes a name from the offenders list rather than merely suppressing the whole error.
...
// TestFillOptional_OptionalNameAbsentFromTemplateIsNoOp covers an optional name listed but never referenced anywhere in the template: rendering succeeds unaffected, since there is no marker for the exemption to apply to.
...
// TestFillOptional_CallerValuesMapNotMutated covers the caller's values map being left untouched after a call whose optional-seeding step would otherwise need to add or overwrite an entry, confirming FillOptional operates on a private copy.
...
// TestFillOptional_RepeatedCallsProduceIdenticalOutput covers repeated FillOptional calls with the same inputs producing byte-identical output and identical error text, mirroring Fill's own idempotence guarantee.
