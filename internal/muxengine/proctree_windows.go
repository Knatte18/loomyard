...
// (descendantClosurePIDs, serverProcessesOnSocket) via the configured pwsh's
// Get-CimInstance Win32_Process table — the only reliable psmux-server
// liveness and parent-walk signal on this platform, since every psmux CLI
// probe (list-sessions, kill-server, has-session) exits identically with and
// without a server on the socket. These bodies are moved here verbatim from
// lifecycle.go so the pure helpers in proctree.go stay platform-agnostic
// while the OS I/O they used to embed lives in one thin, filename-suffixed
// seam; see proctree_linux.go for the /proc-backed counterpart.
...
// computed in one Win32_Process pass through the configured pwsh (the same
// reliable Windows process-table probe serverProcessesOnSocket uses). It
// falls back to the bare roots if the pwsh probe fails (including
// non-Windows, where Win32_Process does not exist) and returns nil for no
// roots. Callers must pass only roots that are still running (a dead pane's
// recorded pid may already have been reused by an unrelated process, and a
// closure over a reused pid would mark innocent processes for force-kill).
...
// through the configured pwsh (Get-CimInstance Win32_Process), because that
// table is the ONLY reliable psmux-server liveness signal: every psmux CLI
// probe (list-sessions, kill-server, has-session) exits 0 or 1 identically
// with and without a server on the socket. Returns nil on any query failure
