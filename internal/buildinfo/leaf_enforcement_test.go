...
// It is empty: internal/buildinfo has no permitted non-stdlib import.
...
func TestLeafInvariant_AllowlistOnly(t *testing.T) {
	...
	if !ok {
		t.Fatal("could not determine buildinfo source directory location")
	}
	...
	if err != nil {
		t.Fatalf("failed to walk buildinfo directory: %v", err)
	}

	if len(failures) > 0 {
		t.Errorf("Buildinfo Leaf Invariant violated; imports outside the allowlist (empty -- no non-stdlib import permitted) found: %v", failures)
	}
}
