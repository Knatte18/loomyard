...
// host-checked-first contract fabric_test.go's TestNew_MissingWarpPath/TestNew_MissingWeftPath pin
...
// TestOpen_MissingHostWorktree asserts that a missing host worktree errors, naming the host path,
// and that the host side is checked first (ahead of the sibling).
func TestOpen_MissingHostWorktree(t *testing.T) {
	fixture := lyxtest.CopyPaired(t)
	...
	if err == nil {
		t.Fatal("Open() error = nil; want error naming the missing host worktree")
	}
