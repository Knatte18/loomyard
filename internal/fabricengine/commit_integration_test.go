...
// TestCommit_TwoSided_WarpFirstOrdering asserts that a two-sided Fabric.Commit's weft commit carries a Warp-SHA trailer naming the warp commit Fabric.Commit just made — never the prior warp HEAD.
...
// TestCommit_TwoSided_RecordsCorrespondence asserts that a two-sided Fabric.Commit's weft commit is recorded in the correspondence index against the warp SHA it just created.
...
// TestCommit_ResultFields asserts CommitResult field population across the two-sided, warp-only, and weft-only shapes — including, on the warp-only path, the plain-git property (no trailer, no correspondence entry) a bare warp commit must have.
...
// TestCommit_SnapshotTrailers asserts a Snapshot: trailer is present on the weft commit for each snapshotTags entry,
...
// TestCommit_InvokesPushRecorder asserts a successful two-sided Fabric.Commit invokes spawnDetachedPushFn exactly once with (warpPath, weftPath).
...
// TestCommit_NoOp_DoesNotInvokePushRecorder asserts a Fabric.Commit call that lands nothing on either side — an empty files list,
...
// TestCommit_WarpOnly_SnapshotTagsForceEmptyWeftCommit is the inverted form of the old TestCommit_WarpOnly_SnapshotTagsDropped: since the tags-force-a-weft-commit Shared Decision, a warp-only Fabric.Commit call carrying a non-empty snapshotTags no longer drops the tags — it commits the warp side as before AND lands an empty weft commit carrying both a Warp-SHA trailer (naming the warp commit that just landed) and a Snapshot: trailer per tag.
// The old name asserted the opposite of what this batch makes true, so it is renamed rather than merely re-bodied: the name is the clearest single statement of the behaviour this batch reverses.
...
// TestCommit_UnchangedWeftContent_TagsStillAdvanceSnapshotBaseline is the correctness-hole regression this whole batch exists to close: two Fabric.Commit calls land the IDENTICAL weft content under the same snapshot tag, at two different warp SHAs.
// Before the empty-commit rule, the second call's weft-side StageAndCommit would report committed=false (nothing changed against HEAD), no weft commit would land, and snapshotWarpSHA would keep answering with the first call's now-stale warp SHA forever — despite the second regeneration having just confirmed itself current against a newer baseline.
...
// TestCommit_NestedRelPath_ClassifiesWeftFileUnderRelPath is the regression guard for the card-6 fix: Fabric.Commit must classify against the resolved worktree's l.RelPath, not a hardcoded ".".
// With a recorded two-segment anchor ("wts/some-task"), a file physically nested at <RelPath>/_lyx/...
// in the weft checkout must still route to the weft side and land in a real commit — proving the fix beyond the RelPath=="."
...
// TestCommit_TagsOnly_LandsEmptyWeftCommit pins the tags-only supported call shape (Commit(nil, msg, tags, opts)): a nil files list with a non-empty snapshotTags still takes the combined write lock (proven here by externally holding it and observing the call block until released), lands an empty weft commit, and snapshotWarpSHA resolves the tag to warp's current HEAD.
...
// TestCommit_PathspecFilteredToNothing_WithTags_LandsEmptyWeftCommit covers the `!positive` fall-through: a weft-side pathspec entry that weftPathspecFilter drops (it matches nothing in the worktree or index) still lands an empty weft commit when snapshotTags is non-empty.
...
// TestCommit_UnbornWeftHEAD_WithTags_LandsAsRootCommit covers the case that is NOT an exception to the empty-commit rule: an unborn weft HEAD still gets an empty commit as its own root commit, carrying its Warp-SHA and Snapshot trailers like any other — contrast with the unborn-WARP case below, which drops the tags entirely.
...
// TestCommit_UnbornWarpHEAD_WithTags_DropsTagsNoErrorNoCommit covers the empty-commit rule's one genuine exception: an unborn warp HEAD drops the tags exactly as before this batch — no commit, no trailer, no error — because a snapshot's whole content is a warp SHA and there is none to record yet.
...
// TestCommit_SkipGit_WithTags_NoWeftCommitNoError covers the second exception: opts.SkipGit still produces no weft commit and no trailer regardless of tags — it is an explicit "touch no git at all" opt-out for the weft side — while the warp commit proceeds regardless, since SkipGit is weft-scoped for Fabric.Commit specifically.
...
// TestCommit_NoTagsNothingToCommit_RuleDoesNotOverFire guards against the uniform empty-commit rule firing when it should not: unchanged content and zero tags must still be a clean no-op, with the weft HEAD left untouched.
...
// TestCommit_WarpOnlyTagged_InvokesPushRecorderOnce pins the CommitResult and push-seam shape on a warp-only tagged commit: WeftCommitted=true with a populated WeftSHA (the empty commit),
...
// TestCommit_InvalidTag_OtherwiseEmpty_NothingCommitted pins that appendSnapshotTrailers' validate-all-before-appending-any property survives the empty-commit path: an invalid tag on an otherwise-fully-empty call fails before anything is staged or committed, on either side.
...
// TestCommit_DirtyWeftIndex_UnchangedContentWithTags_SurfacesPartialCommitError covers the accepted no-op-becomes-error transition: an aborted earlier run leaves unrelated content staged in the weft index (something the combined write lock serializes fabric's own callers against, but does not itself clean up);
// a tagged commit whose own pathspec content is unchanged reaches the `!committed` fall-through, calls CommitEmpty, and CommitEmpty's own dirty-index pre-check refuses with gitrepo.ErrIndexNotEmpty.
// That refusal must surface as a *PartialCommitError with WeftCommitted=false — the same unlanded-weft outcome the mapping already models — while the warp commit that already landed stands and is still pushed.
...
// TestCommitWeft_PathspecMatchesNothing_WithTags_LandsEmptyCommit pins that CommitWeft — an exported entry point, not just an internal dispatch target — inherits commitWeftLocked's empty-commit rule as its own contract: called directly (not via Fabric.Commit) with a pathspec matching nothing and one snapshot tag, it lands the empty commit.
