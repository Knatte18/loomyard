// anchor.go — the pure nearest-older-reachable-anchor walk Fabric.Pull's warp-rebase reconcile path builds on: given the correspondence index's recorded entries and an injected reachability predicate, find the newest entry whose warp SHA still survives on the (possibly rewritten) upstream tip.
// This is the one genuinely unit-testable core of the reconcile flow — see the discussion's TDD-candidate decision — so it takes the predicate as a parameter rather than calling git itself, keeping it git-free like corrindex.go.
