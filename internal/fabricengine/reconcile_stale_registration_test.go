...
// TestPrune_ApplyRemovesPortalAndLaunchers is the R6 fix regression guard: when the host worktree
...
// TestPrune_StaleRegistrationReportedOnce is the F2/F3 regression guard: when a host worktree
// directory is deleted by a bare removal (leaving a stale git worktree registration) rather than
// `git worktree remove`, Pass 1 (missing registered host) and Pass 2 (weft with no host sibling)
...
// main-weft must still survive — protected by fabric's live-host-branch logic, not merely by git's
...
// TestCleanup_DetachedHostHeadProtectsCheckedOutWeftBranch is the R5 fix: a live pair whose host
// worktree sits on a detached HEAD is invisible to the live-host-branch liveness check (readBranch
...
// _lyx junction belongs must be reported as "host _lyx is not a junction" — the wording
