...
// TestReconcile_RecreatesHandDeletedWeftWorktree deletes a pair's weft worktree directory with plain os.RemoveAll (leaving the stale registration behind) and asserts one Reconcile run recreates it from the surviving branch with no per-pair error.
...
// TestPrune_ApplyRemovesPortalAndLaunchers is the R6 fix regression guard: when the host worktree is deleted by hand, apply Prune removes the orphaned weft — but before the fix it left the dead slug's portal junction dangling and its launcher directory behind forever, since no other verb ever revisits a pruned slug (Remove would have, but the pair is already gone).
...
// TestPrune_StaleRegistrationReportedOnce is the F2/F3 regression guard: when a host worktree directory is deleted by a bare removal (leaving a stale git worktree registration) rather than `git worktree remove`, Pass 1 (missing registered host) and Pass 2 (weft with no host sibling) both see the same orphaned weft.
...
// TestCleanup_PrimaryBranchSurvivesForceWhenNotCheckedOut is the F1 regression guard: it moves the fabric weft primary OFF main-weft (exactly what a coordinated checkout of the primary to another branch produces) so main-weft is no longer the checked-out branch, then runs apply+force Cleanup.
// main-weft must still survive — protected by fabric's live-host-branch logic, not merely by git's refusal to delete a checked-out branch.
...
// TestCleanup_NonSuffixedBranchNeverDeleted covers a weft branch with no "-weft" suffix: it is not fabric-managed, whatever its origin;
...
// TestCleanup_DetachedHostHeadProtectsCheckedOutWeftBranch is the R5 fix: a live pair whose host worktree sits on a detached HEAD is invisible to the live-host-branch liveness check (readBranch reports the literal "HEAD"), so before the fix Cleanup listed the pair's weft branch as a deletable orphan and apply+force attempted a git branch -D that only git's own checked-out refusal stopped.
...
// TestHealthy_RealDirNotAJunction is the R10 fix: a real (non-link) directory sitting where the _lyx junction belongs must be reported as "host _lyx is not a junction" — the wording checkJunctionHealth already uses for the same drift shape — not as "junction missing".
