...
// TestBolt_DirtyRepo_CommitsAndPushes asserts that Commit followed by Push lands a real commit and advances the bare origin to that same SHA.
...
// TestBolt_CleanRepo_CommitAndPushAreNoOps asserts that Commit reports committed=false on a repo with nothing new to stage,
// and a subsequent Push is a true no-op (never touches the network, never errors) once nothing is ahead of upstream.
...
// TestBolt_SkipGit_ShortCircuits asserts that opts.SkipGit stops Commit before any git is spawned, leaving a dirty untracked file untouched.
...
// TestBolt_Sync_HoldsSingleAbsorbingLockAcrossBurst asserts that Sync acquires its absorbing lock at the same board.push.lock path Bolt.Sync documents, held once for the whole loop: an externally-held lock at that exact path blocks a concurrent Sync call until released.
