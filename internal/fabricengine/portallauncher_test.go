// portallauncher_test.go covers the portal and launcher path accessors relocated from internal/lyxcwd in this batch — PortalsDir, PortalLink, portalTarget, launchersDir, LauncherDir, menuLauncherPath, launcherSpawnRel and menuLauncherRel — pinning the _portals/_launchers layout and the launcher relative-path math across root and subpath AnchorRel cases, the reason a wrong HubPath base or a broken filepath.Rel would be caught here rather than in a live hub.
...
// TestPortalsDirAndLaunchersDir verifies that PortalsDir and launchersDir join the hub with their respective directory names,
...
// TestMirroredPortalLauncherMethods tests the AnchorRel-mirrored geometry accessors: PortalLink, LauncherDir, menuLauncherPath, launcherSpawnRel and menuLauncherRel, both at the anchor root and at a nested subpath, plus a no-collision check between two distinct subpaths.
