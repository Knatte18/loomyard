// bolt.go defines Bolt, the handle over the unpaired weft:main area (today consumed only by _board): it names that area as a self-contained unit through Commit/Push/Sync, so a caller outside this package never has to spell "weft" to reach it.

package fabricengine

import "path/filepath"

// Bolt is a handle over a single weft:main-backed repo path with no paired warp side — the exception to fabric's one-repo illusion.
...
// Commit stages and commits every change in the Bolt's repo under message, with no warp trailer and no correspondence recording.
