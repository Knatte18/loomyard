// hostjunction_test.go covers the host-side junction primitives relocated
// from internal/lyxcwd in this batch — HostLyxLink, HostLyxLinkHere,
// HostJunctions and HostJunctionsHere, plus the HostJunction record shape —
// over synthetic *lyxcwd.Location literals rather than real fixtures, the
// same table shapes lyxcwd's own tests used. Every _pattern row asserts
// against the generic config-driven junction join
// (filepath.Join(WorktreePath(l, slug), l.AnchorRel, pattern.DirName)) rather
// than a pattern-specific accessor, so this file survives card 35's deletion
// of those accessors unchanged.
...
// TestHostLyxLinkMethods covers HostLyxLink(l, slug) and HostLyxLinkHere(l)
// with both AnchorRel "." (root) and subpath cases, verifying
// AnchorRel-mirroring and junction pairing against the weft-sibling
// worktree.
...
// TestHostJunctions verifies that HostJunctions(l, slug, names) returns one
// record per name in names, in names's own input order, with Link/Target
// correctly composed from l's WorktreePath/weft-sibling path and AnchorRel,
// at AnchorRel == "." and at a nested AnchorRel, for an empty names slice, a
// 3-name slice, and a reversed 2-name slice — and that no entry's Name
// equals _raddle for the default two-name pathspec. The _pattern row asserts
// against the generic join, not a pattern-specific accessor.
...
// TestHostJunctionsHere verifies the Here-anchored, slug-free
// junction-detection accessor: it must return the expected Name/Link/Target
// for both RelPath == "." and a nested RelPath, and it must agree
// entry-for-entry with HostJunctions(l, slug, names) when l's slug and
// current worktree coincide — the precondition every health-check call site
// relies on.
