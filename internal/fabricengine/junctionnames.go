// junctionnames.go implements the fabric-config-to-junction-name-set bridge: loading the repo-wide fabric.yaml pathspec and turning it into the wired name-set the junction primitives operate on, with the hub-reserved-name wiring guard applied.
// Every wiring caller sources names through junctionNames/WiredNames/RepoWiredNames — never applies filterHubReserved itself — so the guard cannot be forgotten at a call site.
// reconcile/status callers read the name-set from the repo-wide `weft:main` base (repoWideFabricBase), not each pair's own weft base.
...
// internal/lyxcwd retains a private second declarer (boardDirName in anchor.go) purely to find the recorded- anchor marker;
// see that const's comment for why the duplication is sanctioned rather than a leak.
const BoardDirName = "_board"

// HubSuffix is the suffix appended to a repo name to form the hub container directory (e.g. "loomyard" → "loomyard-HUB").
...
// HubReservedNames returns the hub-structural reserved name-set that fabricengine owns: _raddle, _board, _portals, _launchers.
// It deliberately excludes configengine.LyxDirName and pattern.DirName, which are config-migrated junction names folded into the reserved set by IsReservedHubName's junctionNames parameter instead.
func HubReservedNames() []string {
	return []string{BoardDirName, portalsDirName, launchersDirName, "_raddle"}
}

// IsReservedHubName reports whether name is one of the hub-level entry names a worktree slug must never claim: HubReservedNames UNION the caller-supplied junctionNames (the weft-backed junction name-set injected from fabric config).
...
// WiredNames loads the fabric config at baseDir and returns its wired name-set — the pathspec directory names with hub-reserved names filtered out — for callers outside this package.
// It is a thin wrapper over junctionNames so out-of-package callers (internal/fabriccli's clone and add handlers) obtain the same filtered name-set the in-package sites use, without duplicating the filterHubReserved guard themselves.
//
// The raw, unfiltered Config.Dirs() is used only by Topology.Add's reserved-name union (which must include every pathspec name, filtered or not, in the set a new slug cannot claim) — never by WiredNames.
...
// It is a Location-taking convenience for callers that want the repo-wide junction name-set without re-deriving the base.
