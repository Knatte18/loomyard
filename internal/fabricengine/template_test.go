...
//
// Covers: ConfigTemplate returns valid YAML with both expected keys and resolves to the correct defaults when the environment is empty.
...
// TestConfigTemplate_ValidYAML asserts that ConfigTemplate returns valid YAML that can be parsed without error.
...
// TestConfigTemplate_HasBothKeys asserts that the template contains both the branch_prefix and pathspec keys.
...
// TestConfigTemplate_ResolvesToEmptyBranchPrefix asserts that resolving the template against an empty environment yields an empty string for branch_prefix.
...
// TestConfigTemplate_PathspecResolvesToLyxAndPattern asserts that the template's pathspec default resolves to "_lyx" and "_pattern", in that order, regardless of environment.
// The resolved value is whitespace-split (mirroring Config.Dirs, the consumer that actually splits it) rather than compared as one whole string, since the value is whitespace-split at the consumer -- a splitting bug there would otherwise be silent and would simply drop "_pattern".
