// warpforward.go collects the warp-only forwarding methods on *Fabric: thin, one-line delegations to the paired gitrepo.Repo verb on f.Warp, added so an out-of-package caller can invoke a warp-mutating git verb through Fabric's public API — preserving the one-repo illusion — without ever touching f.Warp directly.
// Kept in its own file rather than folded into fabric.go to keep the delegation cluster isolated from Fabric's construction and cross-repo plumbing.

package fabricengine

// CheckoutDetached moves the warp checkout's HEAD to sha without updating any branch ref, leaving the warp working tree at that commit's contents.
...
// ResetHard resets the warp checkout's HEAD, index, and working tree to sha, discarding any local commits or uncommitted changes past that point.
