...
// lyxtest.HermeticGitEnv() runs once before any test, so lyxcwd's git-spawning fixtures never
// inherit the operator's global gitconfig (see CONSTRAINTS.md's Hermetic Git Test Environment
// Invariant).
// This file lives in the external package lyxcwd_test, not the internal lyxcwd package: lyxtest
// imports lyxcwd (the lyxtest Leaf Invariant's direction), so an internal test file importing
// lyxtest would close a test-build cycle.
