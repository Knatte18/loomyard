// enforcement_test.go is a repo-wide guard: it walks every package and fails the build if any file outside internal/lyxcwd reaches for raw cwd or top-level git geometry, keeping internal/lyxcwd the sole geometry owner.
...
// TestEnforcement walks the repo source tree and verifies that no source file outside internal/lyxcwd and cmd/lyx contains the raw cwd/root primitives os.Getwd or git rev-parse --show-toplevel.
...
// TestEnforcement_GeometryLiterals walks the repo source tree and verifies that no production file outside each token's registered owner directory (or directories, during a transitional co-ownership window) constructs a geometry path token as a string literal in a path-construction context: a filepath.Join argument, a binary + operand, or a string const declaration value.
// Whole-token matching (exact equality, not substring) avoids false positives on compound names such as "_boardroom" or "-weft-bare".
// Test files (*_test.go) are excluded because test geometry is a review rule, not a machine-enforced invariant.
