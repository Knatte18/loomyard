// cli_test.go covers the burlercli cobra seam through RunCLI: bare-group listing, the unknown-subcommand JSON envelope, the PersistentPreRunE group-command guard, run's required --profile flag, the help-tree Short completeness check, decodeProfile's strict YAML decode, and resultEnvelope's success-envelope shape (including its forkCount nil guard).
...
// TestRunCLI_NoArgs verifies that "lyx burler" with no subcommand lists the run verb and exits 0 — no git repo is needed, since the PersistentPreRunE guard skips layout/config/engine resolution for the group command itself.
...
// TestRunCLI_UnknownSubcommand verifies that an unknown subcommand exits 1 and emits a JSON error envelope with ok=false, without needing a git repo (the PersistentPreRunE guard for cmd.Name() == "burler" fires before layout resolution).
...
// TestRunCLI_GroupGuard_OutsideGitRepo asserts the PersistentPreRunE guard: bare "lyx burler" works outside a git repository, mirroring shuttlecli's guard rationale (neither the bare listing nor the unknown-subcommand path should require layout/config resolution).
...
// TestRunCLI_Run_MissingProfile verifies that "lyx burler run" without --profile fails with run's own manual flag-shape error (not cobra's MarkFlagRequired) before ever touching PersistentPreRunE's engine wiring.
// This case runs against an uninitialized (non-git) directory, so PersistentPreRunE's own abort error is also present in the captured output alongside the flag-specific error line — the same documented double-failure shape as shuttlecli's TestRunCLI_Run_FlagValidation.
...
// TestCommand_EveryCommandHasShort walks the full burler command tree and asserts that every command — the parent group and every subcommand — carries a non-empty Short, per the CLI/Cobra Invariant.
...
// TestDecodeProfile covers decodeProfile's strict YAML decode: a full valid profile (every field lands, including the boolean/zero-value edge cases tool-use: true and cluster-fan: ""), a minimal valid profile, an unknown key (rejected per the yaml-strictness-split decision's KnownFields(true)), the now-removed cluster-n key specifically (rejected the same way), and malformed YAML.
...
// TestDecodeProfile_FullValidFieldMapping asserts every field of a full valid profile YAML lands on the corresponding Profile field, including the boolean/zero-value edge cases (tool-use: true, cluster-fan: "standard") that a zero-value-blind mapping bug could silently drop.
...
// TestResultEnvelope_ForkCountNilGuard asserts resultEnvelope's forkCount guards a nil ForkAudit (the non-cluster or non-done case) to 0 rather than panicking,
