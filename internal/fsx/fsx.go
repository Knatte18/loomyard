...
//
// This package provides path guards to prevent escapes (empty, absolute, parent directory references) and atomic file writes via temp-file + rename, ensuring that concurrent readers are not caught mid-write.
...
// PathGuard validates a relative path, rejecting empty, absolute, and parent-directory-reference paths.
...
// AtomicWriteBytes writes data to an absolute file path atomically via a temp file and rename, ensuring concurrent readers never see partial writes.
...
// AtomicWrite writes content to dir/relPath atomically after validating the relative path via PathGuard.
