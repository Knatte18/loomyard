...
// It is deliberately empty: internal/discussionparser is a stdlib-only leaf.
...
func TestLeafInvariant_AllowlistOnly(t *testing.T) {
	...
	if !ok {
		t.Fatal("could not determine discussionparser source directory location")
	}
	...
	if err != nil {
		t.Fatalf("failed to walk discussionparser directory: %v", err)
	}

	if len(failures) > 0 {
		t.Errorf("Discussionparser Sole-Parser Invariant violated; imports outside stdlib found: %v", failures)
	}
}
