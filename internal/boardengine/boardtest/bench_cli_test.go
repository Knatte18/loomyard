...
// BenchmarkUpsert measures a full "upsert" command through the CLI entrypoint: JSON parse → dispatch → lock → load → mutate → render all tasks → write files.
...
// BenchmarkList measures a "list" command: load all tasks, compute layers and has_proposal, and serialise the brief view.
