...
// TestNoTempLeak verifies that after WriteJSON, the directory contains only the data file and <path>.lock, with no .tmp- entries.
...
// TestOverwrite writes a value, then writes a different value, and verifies ReadJSON returns the new value.
