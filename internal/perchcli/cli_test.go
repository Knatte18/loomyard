// cli_test.go covers the perchcli cobra seam through RunCLI: bare-group listing, the
// unknown-subcommand JSON envelope, the PersistentPreRunE group-command guard, and the help-tree
// Short completeness check.
// run and pause verb behavior (missing --profile, missing --run-id, strict profile decode,
// pause-flag mechanics) is covered by run_test.go (card 15) and the pause-verb tests appended to
// this file (card 16).
...
// The fixture-backed pause tests (a real hub, built by the hubforge package) live in cli_integration_test.go
...
// TestRunCLI_NoArgs verifies that "lyx perch" with no subcommand lists the run and pause verbs and
// exits 0 — no git repo is needed, since the PersistentPreRunE guard skips layout/config/engine
...
// "perch" fires before layout resolution).
...
// TestRunCLI_GroupGuard_OutsideGitRepo asserts the PersistentPreRunE guard: bare "lyx perch" works
// outside a git repository, mirroring burlercli's guard rationale (neither the bare listing nor the
// unknown-subcommand path should require layout/config resolution).
...
// TestCommand_EveryCommandHasShort walks the full perch command tree and asserts that every command
