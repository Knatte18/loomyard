...
// TestCLIPromoteNote seeds a note via "notes upsert", promotes it, and asserts the move landed: the
// promoted task's fields round-trip, the slug is gone from notes.json (notes get returns
// task:null), and the same slug is now present via the task-verb "get" (tasks.json).
