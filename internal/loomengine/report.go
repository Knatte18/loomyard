// report.go defines the CheckID/Failure/Report result types Preflight uses to communicate its determined verdict — which of the four preconditions failed,
...
// Each corresponds to one of the four checks described in Preflight's godoc (geometry/worktree-root fold into check 1, worktree-clean is check 2, weft-pairing/weft-sync/junction are check 3, and seed-missing/seed-unreadable/seed-incoherent/half-finished are check 4).
const (
	...
	// CheckWorktreeRoot fails when Preflight is invoked from a subdirectory of the worktree rather than its root.
	...
	// CheckJunction fails when the host _lyx junction is missing or points somewhere other than the paired weft's _lyx directory.
	CheckJunction CheckID = "junction"
	// CheckSeedMissing fails when _lyx/status.json does not exist and the weft pairing/junction are otherwise healthy.
	CheckSeedMissing CheckID = "seed-missing"
	// CheckSeedUnreadable fails when _lyx/status.json cannot be stat'd or read for a reason other than not-existing,
	// or when a stat failure (including not-exist) is attributable to an unhealthy weft pairing/junction rather than a genuinely missing seed.
	CheckSeedUnreadable CheckID = "seed-unreadable"
	// CheckSeedIncoherent fails when _lyx/status.json exists and decodes but violates the coherence validator's rules (see checkCoherence).
	CheckSeedIncoherent CheckID = "seed-incoherent"
	// CheckHalfFinished fails when _lyx/status.json is otherwise coherent but its fresh-start invariants are violated — the task has already advanced past the point Preflight is meant to gate.
