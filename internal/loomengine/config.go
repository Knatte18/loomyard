...
// loomDirName is the relative-path segment loomengine joins onto lyxdirs.DotLyxDirName to scope
// every loom-owned path under its own subdirectory, distinct from the other products (e.g. Someday
// Hardener) that also configure Shed.
// loomengine is this segment's sole declarer.
...
// Per the Durable-vs-Ephemeral State Invariant, this file is a never-tracked transient under
// lyxdirs.DotLyxDirName, sitting beside the LoomStatusLock that guards it.
// Scoped under a "loom" subdirectory, not bare .lyx, because Shed (see manifest/designs/shed.md) is
// a generic engine more than one product configures -- the Someday Hardener product will need its
// own status file too, and a bare .lyx/status.json could not serve both without colliding.
func LoomStatusFile(l *lyxcwd.Location) string {
	return filepath.Join(l.AnchorPath(), lyxdirs.DotLyxDirName, loomDirName, loomStatusFileName)
}

// LoomStatusLock returns the path to the advisory lock file guarding concurrent access to
// LoomStatusFile(l).
// It is AnchorPath-anchored like LoomStatusFile, and lives under the same lyxdirs.DotLyxDirName
// root: the lock is a never-tracked transient, exactly like the status file it guards.
...
// It is AnchorPath-anchored, living under the ephemeral tree beside LoomStatusFile and
// LoomStatusLock per the Durable-vs-Ephemeral State Invariant.
...
// directory those three -- and LoomStatusFile and LoomStatusLock -- already share:
// lyxdirs.DotLyxDirName joined with loomDirName.
