...
func TestLeafInvariant_AllowlistOnly(t *testing.T) {
	...
	if !ok {
		t.Fatal("could not determine friction source directory location")
	}
	...
	err := filepath.WalkDir(pkgDir, func(path string, d fs.DirEntry, err error) error {
		...
		for _, imp := range astFile.Imports {
			...
			// A stdlib import path has no '.' in its first path segment (e.g. "fmt", "os",
			// "go/parser") — a domain that would need a registered TLD (e.g. "github.com/...")
			// always contains one.
	...
	if err != nil {
		t.Fatalf("failed to walk friction directory: %v", err)
	}

	if len(failures) > 0 {
		t.Errorf("Friction Leaf Invariant violated; imports outside the allowlist (stdlib + logger + stencil + stencilstore) found: %v", failures)
	}
}
