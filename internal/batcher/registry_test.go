// registry_test.go covers the unexported register/lookup pair in isolation from package-level init() state: each subtest builds its own registry map so a probe batcher's registration can never leak into — or be masked by — the real identity registration identity.go's init() performs on package load.
