...
// Pull fast-forwards the current branch from upstream via `git pull --ff-only`,
// refusing a diverged branch. Errors name the repo path and git's exit code.
...
// Fetch refreshes remote-tracking refs via `git fetch` without moving the
// local branch, unlike Pull. Errors name the repo path and git's exit code.
