...
// TestPull_RemoteAdvanced_FastForwards asserts Pull() fast-forwards the local branch when the
// remote has commits this clone lacks.
...
// TestPull_LocalDiverged_ReturnsError asserts the fast-forward-only contract: a local branch with
// its own unpushed commit, pulling from a remote that has diverged from underneath it, must be
// refused with an error rather than folded into a merge commit — and local history must be left
// untouched.
...
// TestPull_NoRemoteConfigured_ErrorNamesRepoPath asserts that a repo with no remote at all still
// fails loudly,
// and that the error names the repo path (per Pull's documented error style) without leaking git's
// raw stderr.
