...
// TestResetHard_MovesToEarlierCommit asserts ResetHard restores an earlier commit's file state and
...
// TestResetHard_WellFormedSHANotInHistory_ReturnsError asserts that a well-formed but fabricated
// hex SHA — one that passes validSHA but names no commit in this repo's history — surfaces as a
// genuine git failure, not ErrInvalidSHA.
