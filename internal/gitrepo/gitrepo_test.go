...
// TestBisectPrimitives_DetachRestoreCycle exercises the full in-place bisect cycle the integration
// bisect drives: capture the current branch, detach to an older commit, assert HEAD landed there
// detached, restore the branch, and assert HEAD is back on it — the exact CurrentBranch ->
// CheckoutDetached -> RestoreBranch sequence the bisect's per-candidate loop performs.
...
// TestCheckoutDetached_RejectsNonHexSHA asserts CheckoutDetached validates its sha argument the
// same way ChangedFilesSince and SHAExists do, before ever spawning git.
