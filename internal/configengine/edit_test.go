...
// TestEdit_ScaffoldWhenMissing tests that Edit writes the template to _lyx/config/<module>.yaml
...
// TestEdit_ReEditLoop tests that when the fake editor writes invalid YAML on the first pass and
// valid YAML on the second pass, Edit loops and invokes the editor twice, returning nil on success.
...
// TestEdit_AbortOnUnchangedAfterFailure_Scaffolded tests that when the fake editor writes invalid
// YAML and leaves it unchanged, Edit returns ErrAborted and removes the scaffolded file (restoring
// the pre-call filesystem state).
...
// TestEdit_AbortOnUnchangedAfterFailure_PreExisting tests that when the file pre-existed and the
// editor writes invalid YAML then leaves it unchanged, Edit returns ErrAborted but leaves the file
// in place.
...
// TestEdit_AbortOnEditorError tests that when the fake editor returns an error, Edit returns
// ErrAborted (wrapping the editor error).
...
// TestEdit_NotInitialized tests that calling Edit with a baseDir lacking _lyx/ returns the
// FindBaseDir not-initialized error (not ErrAborted).
