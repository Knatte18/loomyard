// profile_test.go table-drives Profile.validate over the happy path and every fail-loud rule documented on validate: default resolution for RoundCaps/JudgeModel/JudgeEffort/Gate.Timeout, RoundCaps shape, Gate.Mode legality and its Command-emptiness pairing, and the two negative-duration rejections.
// It also separately exercises the three-level default resolution chain (profile > Config > built-in) for RoundCaps and JudgeModel.
...
// TestProfile_Validate_RoundCapsDefaultChain exercises the three-level default resolution chain (profile > Config > built-in) in isolation: a profile value always wins, a Config value is used only when the profile is empty, and the built-in default is used only when both are empty.
...
// TestProfile_Validate_JudgeModelDefaultChain exercises the three-level default resolution chain (profile > Config > built-in) for JudgeModel.
