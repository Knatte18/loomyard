...
// Role names one of webster.yaml's two model-spec roles. Unlike builder,
// only the cold recovery strand carries its own role; in-session forks always
// inherit Master's current model.
type Role string
...
// ResolveRoles parses and resolves cfg's two role model-spec strings against
// reg, failing before Master spawns on any unknown alias. It returns the
// resolved values keyed by Role, with each violation wrapped naming the
// offending role.
