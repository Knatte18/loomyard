// pause.go implements webster's pause-flag mechanics: a webster-local copy of builderengine's pause.go (RequestPause/PauseRequested/ClearPause), since builder's own versions have an in-tree builder caller (frozen, per the Shared Decision builder-is-frozen-copy-not-move) and cannot be moved.
// RequestPause writes a flag file that the batch boundary refuses against, PauseRequested observes it, and ClearPause removes it.
// The clearing rules mirror builder's exactly: ClearPause must be called once a run has passed its refusal gates and is committed to spawning (never instantly re-pause on the flag that requested the pause a resumed run is now resuming from — while a run that refuses on validation or a fingerprint mismatch leaves a pending pause intact) and again at every terminal outcome (a pause requested while the last batch was still in flight must not linger in a finished run's webster dir).
// Exported because internal/webstercli calls these directly (pause.go/status.go/weft.go retarget in batch 9).
...
// Exported so webstercli's pause verb can name the same file it writes without recomputing the join itself.
...
// Callers must invoke this once committed to spawning Master and again at terminal outcome (per double-clear pattern).
