...
// chainFakeReed is a minimal shuttleengine.ReedOps double for RestartChain's
...
type chainFakeReed struct {
	live    []string
	removed []string
}

func (m *chainFakeReed) Status() (reedengine.StatusResult, error) {
...
func (m *chainFakeReed) RemoveStrand(guid string, recursive bool) (reedengine.Removed, error) {
	m.removed = append(m.removed, guid)
	return reedengine.Removed{}, nil
}
func (m *chainFakeReed) AddStrand(spec reedengine.AddSpec) (reedengine.Strand, error) {
	return reedengine.Strand{}, nil
}
func (m *chainFakeReed) SendText(guid, text string, submit bool) error { return nil }
func (m *chainFakeReed) SendKey(guid, key string) error               { return nil }
func (m *chainFakeReed) CapturePane(guid string) (string, error)      { return "", nil }

var _ shuttleengine.ReedOps = (*chainFakeReed)(nil)
...
func TestRestartChain_ResetAndClearMembers(t *testing.T) {
	...
	lowest, err := websterengine.RestartChain(&chainFakeReed{}, worktree, st, chainTestPlan(), 4, reportsDir)
	if err != nil {
...
// step: a chain member whose recorded recovery strand the reed still reports
...
func TestRestartChain_StopsLiveMemberStrandsBeforeReset(t *testing.T) {
	...
	reed := &chainFakeReed{live: []string{"live-recovery-strand"}}
	if _, err := websterengine.RestartChain(reed, worktree, st, chainTestPlan(), 4, t.TempDir()); err != nil {
		t.Fatalf("RestartChain() error = %v; want nil", err)
	}

	if len(reed.removed) != 1 || reed.removed[0] != "live-recovery-strand" {
		t.Errorf("reed.removed = %v; want exactly [live-recovery-strand] stopped before the reset", reed.removed)
	}
}
...
func TestRestartChain_UnrecordedAnchorErrors(t *testing.T) {
	...
	_, err := websterengine.RestartChain(&chainFakeReed{}, worktree, st, chainTestPlan(), 4, reportsDir)
	if err == nil {
...
func TestRestartChain_ChainlessBatchErrors(t *testing.T) {
	...
	_, err := websterengine.RestartChain(&chainFakeReed{}, worktree, st, plan, 1, t.TempDir())
	if err == nil {
