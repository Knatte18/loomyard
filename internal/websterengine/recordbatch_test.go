...
// TestRecordBatch_NoBeginRecord proves the bracket-discipline check: a record call with no matching BatchState entry,
...
// TestRecordBatch_RecoveryBatchRefusedLoud proves the kind guard: a recovery batch's report is recover-batch's to classify, never record-batch's — the refusal names the correct verb instead of dying later in the fork audit with a misleading "never forked" error (round fable-r3 live).
...
// TestRecordBatch_AuditsBracketOpeningSession proves the crash/resume seam: the fork audit keys on the session recorded in the batch's begin record (bs.SessionID), never blindly on the CURRENT Master session — a resumed run's fresh Master must be able to consume a report whose fork transcript lives under the crashed session's own subagents directory (round fable-r3 live: auditing the current session instead wedged that resume across all three verbs).
...
// TestRecordBatch_ZeroNewTranscriptsHardErrorsEvenWithReport proves the unfakeable-report rule: zero new transcripts through the whole settle window is a hard error REGARDLESS of a batch-report file already sitting on disk — a report with no fork behind it means Master wrote it itself.
...
// TestRecordBatch_TranscriptAppearsOnLaterTick proves a fork transcript that only appears on the fetch AFTER the first miss still resolves clean — SettleRetry's own "first miss is inconclusive" de-risk applied through the whole RecordBatch call.
...
// TestRecordBatch_OneNewTranscriptWithReport_TerminalDigestPersisted proves the normal happy path: one new transcript plus a valid, matching report distills and persists the digest, marks the batch Terminal, clears CurrentBatch, and attributes the transcript to both SeenForkTranscripts and the batch's own ForkTranscripts.
...
// TestRecordBatch_OneNewTranscriptNoReport_RetrySeesExactlyOneNew proves the no_report ladder: a fork transcript with no report yet returns NoReport true, leaves the batch non-terminal, and STILL advances attribution — so a second record-batch call (after Master's re-fork) sees exactly its own new transcript and resolves clean, never re-counting the first one.
...
// TestRecordBatch_MultipleNewTranscriptsWarnsNeverErrors proves more than one new transcript in a single call is a warning only, never a hard error — legitimate retry behavior (a fork's Agent call errored mid-flight followed by a direct re-fork, with no intervening record-batch call).
...
// TestRecordBatch_ParentWriteOutsideContractFilesErrors proves CheckParent's write-policy violation is surfaced as a hard error naming the offending write, even when the fork-transcript count and the report itself are both otherwise clean.
...
// TestRecordBatch_HeadSHAMismatchErrors proves a batch-report whose own self-reported head_sha disagrees with the worktree's actual current HEAD is a hard error, naming both — the fork's report and the host repo it left behind must never be trusted to agree silently.
...
// TestRecordBatch_MalformedReportYAMLErrors proves an unparseable batch-report (here, an unrecognized status value) is a hard error, never a guessed digest.
...
// TestRecordBatch_MissingSessionTranscriptNamesRecourse proves the TRUE cross-machine resume failure — the bracket-opening session's transcript file does not exist on this machine at all, so the audit read itself fails with fs.ErrNotExist — is wrapped with the machine-local-transcripts explanation and the move-the-report-aside operator recourse, instead of surfacing a bare "no such file or directory" (found live in crucible round fable-r3).
