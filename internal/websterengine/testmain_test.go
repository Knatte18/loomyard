// testmain_test.go wires the package's test binary into the hermetic git test environment: lyxtest.HermeticGitEnv() runs once before any test, so websterengine's git-spawning fixtures (begin-batch's HeadSHA capture, record-batch's ChangedFiles/Dirty, chain rollback's ResetHard) never inherit the operator's global gitconfig (see CONSTRAINTS.md's Hermetic Git Test Environment Invariant).
