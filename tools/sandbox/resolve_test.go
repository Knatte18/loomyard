// resolve_test.go contains unit tests for resolveLyx (dev/prod binary resolution) and prependPath (child-process PATH construction).
...
// TestResolveLyx_DevBinaryExists verifies that resolveLyx returns the dev-binary path and sourceDev when devBinPath resolves to an existing file.
...
// TestResolveLyx_DevBinaryMissingFallsBackToProd verifies that resolveLyx falls back to lookPath and returns sourceProd when the dev binary is absent.
...
// TestResolveLyx_DevBinaryMissingAndLookPathFails verifies that resolveLyx propagates a lookPath error when both the dev binary is absent and PATH lookup fails.
...
// TestPrependPath_PrependsToExistingPath verifies that prependPath makes dir the first PATH segment.
...
// TestPrependPath_EmptyDirReturnsEnvironUnchanged verifies that prependPath returns environ unchanged when dir is empty.
