...
// TestRun_DefaultBuildRoutesToClone tests that a bare run with no subcommand routes to decideClone (the build path) and invokes cloneRun.
...
// TestRun_BuildSubcommandRoutesToClone tests that the explicit "build" token also routes to the clone path.
...
// TestRun_BuildResetRoutesToBuildWithReset tests that -reset passed after the build token removes the existing Hub and re-runs the clone.
...
// TestRun_BuildNoResetDoesNotRemove tests that a bare build (no -reset) over an existing Hub does not remove it -- reset must be explicit.
// Deliberately no devBinPath/lookPath stub is set up here: the no-op path must never resolve lyx at all.
...
// TestRun_SuiteRoutesSuiteToLaunch tests that the "suite" positional routes to the suite path and ultimately invokes launchAgent with the correct directory.
// The suite subcommand no longer fetches, so it needs neither -loomyard nor a report written by the launch stub.
...
// TestRun_ReedSuiteRoutesToLaunch tests that the "reed-suite" positional routes to the reed-suite path and ultimately invokes launchAgent with the correct host repo directory and the reed default instruction, mirroring TestRun_SuiteRoutesSuiteToLaunch for the "suite" dispatch.
...
// TestRun_ReedSuiteFlagsRoutedAfterToken tests that -claude/-prompt flags following the "reed-suite" positional are parsed and forwarded to launchAgent, mirroring the "suite" subcommand's flag handling.
...
// TestRun_ReedSuiteErrorPropagation tests that a runSuite error under the reed-suite dispatch (Hub absent) is propagated as a non-zero exit code, mirroring the error-propagation coverage the "suite" dispatch relies on via TestRunSuite_HubAbsent at the runSuite level.
...
// TestRun_ShuttleSuiteRoutesToLaunch tests that the "shuttle-suite" positional routes to the shuttle-suite path and ultimately invokes launchAgent with the correct host repo directory and the shuttle default instruction, mirroring TestRun_ReedSuiteRoutesToLaunch for the "shuttle-suite" dispatch.
...
// TestRun_ShuttleSuiteFlagsRoutedAfterToken tests that -claude/-prompt flags following the "shuttle-suite" positional are parsed and forwarded to launchAgent, mirroring the "reed-suite" subcommand's flag handling.
...
// TestRun_ShuttleSuiteErrorPropagation tests that a runSuite error under the shuttle-suite dispatch (Hub absent) is propagated as a non-zero exit code, mirroring TestRun_ReedSuiteErrorPropagation for the "shuttle-suite" dispatch.
...
// TestRun_BurlerSuiteRoutesToLaunch tests that the "burler-suite" positional routes to the burler-suite path and ultimately invokes launchAgent with the correct host repo directory and the burler default instruction, mirroring TestRun_ReedSuiteRoutesToLaunch for the "burler-suite" dispatch.
...
// TestRun_PerchSuiteRoutesToLaunch tests that the "perch-suite" positional routes to the perch-suite path and ultimately invokes launchAgent with the correct host repo directory and the perch default instruction, mirroring TestRun_ReedSuiteRoutesToLaunch for the "perch-suite" dispatch.
...
// TestRun_FabricSuiteRoutesToLaunch tests that the "fabric-suite" positional routes to the fabric-suite path and ultimately invokes launchAgent with the correct dedicated fabric host repo directory and the fabric default instruction, mirroring TestRun_MuxSuiteRoutesToLaunch for the "fabric-suite" dispatch.
// The dedicated fabric hub host repo dir is pre-created so decideFabricClone finds the hub already present and skips fabricCloneRun.
...
// TestRun_FetchReportRoutesToFetch verifies that the "fetch" positional routes to runFetch: with a built Hub, an on-PATH lyx, and a host report, the dispatch reaches fetchReport and run returns 0.
...
// TestRun_FetchReportRequiresLoomyard verifies that the fetch subcommand fails fast when -loomyard is not supplied, covering the required-flag guard.
...
// TestRun_UnknownSubcommandReturnsNonZero tests that an unrecognised positional argument causes run to return a non-zero code.
