...
A structured test-loop for exercising `lyx reed` against a **live tmux server** in the
...
3. **Live-tmux requirement.** tmux (or the Windows tmux port) on PATH and PowerShell 7 present. If tmux or pwsh is
   ...
   it as a reed defect** -- the `**Covers:** reed` tag on M2 satisfies the sandbox
...
Discovering the command surface is done via `lyx reed`, `lyx reed <subcommand>`, and
`lyx reed <subcommand> --help` alone -- not from documentation outside the Hub.
...
- **(a) Direct `tmux -L <socket>` verbs** (`kill-server`, `list-panes`, `ls`) are
  ...
  `<socket>` is taken from `lyx reed status` output (its JSON result carries `session`,
...
- Read the **Goal** -- it names the task, not the commands. Discover the commands via
  `lyx reed`, `lyx reed <subcommand>`, and `--help` flags (M0 ethos).
...
`lyx reed` can do and report the full command tree."

**Watch:** Does `lyx reed` list all subcommands (`up`, `add`, `remove`, `status`,
...
**Goal:** "From a fresh state (no reed session running), try to add a strand and remove
one. See how reed tells you what to do first."

**Watch:** `lyx reed add --cmd ...` and `lyx reed remove <guid>` must fail with the
friendly JSON-envelope error `no reed session; run "lyx reed up"` -- that message is the
...
**Watch:** `lyx reed up` boots the named server + this worktree's session; a second `up`
...
**Watch:** `lyx reed add --cmd <long-running command>` returns an assigned `guid` +
...
**Watch:** `lyx reed status` reports `session`, `socket`, and the strand from M3 with
`live: true`.
...
**Goal:** "Add a strand that should not show up as a pane, then confirm reed still knows
about it."

**Watch:** `lyx reed add --anchor hidden --cmd ...` creates **no pane**; `status` still
lists the strand; `lyx reed resume` **skips** it (hidden strands are pending, not dead --
still no pane after resume). Note explicitly: surfacing a hidden strand is
engine-API-only in v1 (there is no `lyx reed update` verb) -- do **not** attempt it, and
...
**Watch:** The agent pauses and instructs the operator to run `lyx reed attach` **in a
...
**Watch:** `lyx reed resume` with all strands live leaves them untouched (no double
...
crash; `lyx reed resume` boots a fresh server + session and replays every non-hidden
...
**Watch:** `lyx reed down` kills the server and clears the worktree's strand state;
`tmux -L <socket> ls` (controlled exception) confirms no server survives, and a
follow-up `lyx reed status` reports the friendly no-session error rather than stale
...
> `#{pane_pid}` descendant and reed does not reap it — an OS console host is not stray
> *agent* state (the smoke harness kills hub-holding conhosts itself; see
> `deferHubRelease`). A held worktree dir is therefore not by itself a reed leak. Only a
...
**Watch:** After all three `add`s, `lyx reed status` reports all three strands
...
`lyx reed add --cmd <long-running command>` returns a guid, and the strand reads
`live: true` in `status` **both immediately and again after one more verb** (e.g.
`lyx reed up`) -- a strand that reads live once and then flips to `live: false` with an
...
**Goal:** "Confirm `lyx reed attach` actually renders the session's panes, not just
that the command exits."

**Watch:** With the overlay up and at least one strand added, `lyx reed attach` from a
**second terminal** shows the strands' panes and the tmux status bar; `Ctrl+b d`
detaches cleanly and the session keeps running (`lyx reed status` still lists it).
...
**Watch:** `lyx reed add --cmd "claude '...codeword...'" --resume-cmd "claude
--continue"`; after claude answers, `tmux -L <socket> kill-server` (controlled
exception) simulates a crash; `lyx reed resume` replays `claude --continue` and the
...
**Goal:** "With the overlay up and **no strands added**, create a pane in the reed session
behind reed's back, then run `lyx reed up` again and prove the session is still usable."

**Watch:** `tmux -L <socket> split-window -t <session>` (controlled exception) simulates
an operator-split/foreign pane. The follow-up `lyx reed up` must **not** destroy the
...
`lyx reed add --cmd <long-running command>` must succeed and read `live: true` in `status`
...
reconcile (the documented "reed owns the session window" policy, not a finding) — what
...
worktree on the same hub** can run its own reed session at the same time, and that tearing
...
basename). So a second worktree on the same hub that runs `lyx reed up` + `lyx reed add`
...
sessions, and `#{pid}` from each is identical. The proof is what happens on `lyx reed down`
...
the reed surface, so the authoritative headless coverage is
...
**Watch:** With only the mother strand live, `lyx reed status` reports it `live: true`
...
bug to file). After `lyx reed add --parent <mother-guid> --cmd claude ...` adds the
...
**Watch:** After `lyx reed up`, the session holds one extra pane beyond any strands: the
...
*alive* is not enough). `lyx reed status` must never list the header as a strand, and
`up`'s `strands` count must exclude it. Then: add a strand, remove it — the session
**survives** on the header alone (pre-header reed destroyed the session with its last
...
means a stale header cell scrambled the layout: `FAIL`), and the next `lyx reed up` must
...
- Host/weft scenarios stay in `SANDBOX-CORE-SUITE.md`; this suite grows with reed (windows
