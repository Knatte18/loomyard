// pathresolve_guard_test.go enforces the Dev/Prod Binary Separation Invariant: no non-test *.go file in the tools/sandbox package, other than resolve.go, may perform a bare-PATH "lyx" lookup or spawn -- lookPath("lyx"), or an exec.Command/ exec.CommandContext call whose line also names "lyx".
// The exec forms are matched line-based rather than as a whole-file substring, because exec.CommandContext takes its context.Context argument first, so the literal exec.CommandContext("lyx" never appears in compilable Go -- a substring scan for it can never match.
// resolve.go's resolveLyx is the single allowlisted resolution site;
// every other call site must route through it instead, so the dev/prod distinction can never silently regress to a bare PATH fallback.
...
// TestPathResolveGuard_NoBarePathLyxOutsideResolve fails if any non-test *.go file contains a banned bare-PATH lyx literal.
