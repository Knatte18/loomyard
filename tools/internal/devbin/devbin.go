// devbin.go derives the single, non-hardcoded location where dev/test builds of the lyx binary are installed and resolved from: a `.dev-bin` directory sitting at the repository root.
// Every consumer (the deploy tool, the sandbox launcher) imports this package instead of deriving the repo root or the `.dev-bin` convention itself, so they can never disagree on where the dev binary lives.
...
// RepoRoot returns the repository root directory, derived from this source file's location via runtime.Caller.
